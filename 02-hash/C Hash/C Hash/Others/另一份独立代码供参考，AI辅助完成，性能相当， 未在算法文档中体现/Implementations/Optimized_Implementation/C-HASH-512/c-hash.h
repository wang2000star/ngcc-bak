...
int c_hash_512(const uint8_t *msg, size_t msg_len, uint8_t digest[64]);

/* C-Hash-512: bit-length API (msg_len_bits is BITS, supports non-byte-aligned) */
int c_hash_512_bits(const uint8_t *msg, unsigned long long msg_len_bits, uint8_t digest[64]);
