/*
 * C-Hash-512 x86 AVX2 Optimized Implementation 
 *
 * Architecture: C-Engine (pure permutation P) + unified VIL/FIL mode.
 * C-Engine optimizations: AVX2 shuffle S-box, fused S-box+SubConst,
 * 28-round loop, fully inlined, AVX2 bulk XOR.
 */
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
...
/* ═══════════════════════════════════════════════════════════════════ */
/* C-Hash-512: Unified VIL/FIL mode with C-Engine (pure P)           */
/* ═══════════════════════════════════════════════════════════════════ */
#define NB      184
#define BB      92
#define N_BITS  1472
...
#define HASHLEN     512
#define LANE_VIL    0x01
#define LANE_FIL    0x02
#define LANE_LDRH   0x03
...
static inline void ldrh_512(const uint8_t *msg,size_t msg_len,uint8_t digest[64]){
...
static inline int vil_512(const uint8_t *msg,size_t msg_len,uint8_t digest[64]){
    ...
    {uint8_t in[192],o[192];memcpy(in,h1,BB);memcpy(in+BB,h2,BB);
     ...
     c_engine_permute(in,o);lmb(512,o,digest,64);}
    return 0;
}
int c_hash_512(const uint8_t *msg,size_t len,uint8_t digest[64]){
    init_cengine();
    if(len<(size_t)NB){ldrh_512(msg,len,digest);return 0;}
    return vil_512(msg,len,digest);
}

static inline void ldrh_bits_512(const uint8_t *msg,unsigned long long mlen_bits,uint8_t digest[64]){
...
static inline int vil_fil_bits_512(const uint8_t *msg,unsigned long long mlen_bits,uint8_t digest[64]){
    uint8_t iv[BB];build_iv(iv);
    uint8_t h1[BB],h2[BB];
    {uint8_t in[192],o[192];memset(in,0,192);memcpy(in,iv,BB);
     set_counter(in,build_counter(LANE_VIL,0));
...
int c_hash_512_bits(const uint8_t *msg,unsigned long long mlen_bits,uint8_t digest[64]){
    init_cengine();
    if(mlen_bits<(unsigned long long)N_BITS){ldrh_bits_512(msg,mlen_bits,digest);return 0;}
    return vil_fil_bits_512(msg,mlen_bits,digest);
}
