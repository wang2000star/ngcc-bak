...
1.  This optimized implementation keeps the ICCS interface unchanged and uses 
    the same digest construction as the reference implementation.
2.  Full-byte prefixes are processed in 8-byte chunks by precomputing the 
    exact signed 2x2 matrix for each byte and combining eight byte matrices 
    before each modular state update.
3.  Remaining full bytes fall back to the scalar byte path, and any final 
    partial byte remains MSB-first.