...
#(
    ...
    input  logic                   rst_i,
    input  logic                   start_i,
    input  logic [BLOCK_BITS-1:0]  block_i,
    input  logic                   last_i,
    input  logic [7:0]             valid_bytes_i,
    output logic                   done_o,
    output logic [DIGEST_BITS-1:0] digest_o,
    output logic                   busy_o
);
    ...
    logic [DIGEST_BITS-1:0]            digest_reg;
    logic [DIGEST_BITS-1:0]            digest_nxt;
    ...
    function automatic logic [255:0] make_domain(
        ...
        make_domain[2*WORD_BITS +: WORD_BITS] = hash_bits[63:0];
        make_domain[3*WORD_BITS +: WORD_BITS] = hash_bits[127:64];
    endfunction
    ...
    assign busy_o         = (state == WAIT_CF);
    assign cf_block       = mask_last_block(block_i, valid_bytes_i, last_i);
    assign cf_domain      = make_domain(last_i, hashed_bits_reg);
    assign cf_start    = (state == IDLE) && start_i;
    ...
    always_comb begin
        ...
        digest_nxt             = digest_reg;

        case (state)
            IDLE: begin
                if (start_i) begin
                    active_last_nxt        = last_i;
                    active_valid_bytes_nxt = valid_bytes_i;
                    state_nxt              = WAIT_CF;
                end
            end
            WAIT_CF: begin
                if (cf_done) begin
                    ...
                    if (active_last_reg) begin
                        digest_nxt = format_digest(cf_h_out);
                        state_nxt  = DONE;
                    end else begin
                        state_nxt = IDLE;
                    end
