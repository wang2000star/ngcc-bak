/*
 * This file is the test file for the optimized CHIME-512 implementation.
 * It reads the test vectors generated by the reference implementation from
 * KAT_2_12_CHIME-512.txt and verifies the correctness of the optimized implementation.
 *
 * 本文件为 CHIME-512 的优化实现的测试文件，将读取参考实现生成的测试向量 KAT_2_12_CHIME-512.txt，
 * 并核对优化实现的正确性。
 */
...
static bool run_test_vectors(const std::vector<TestVector>& vectors,
    ...
    for (size_t i = 0; i < vectors.size(); ++i) {
        ...
        if (tv.dst_len_bits != expected_digest_bytes * 8ull) {
            std::printf("[FAIL] vector %zu: only 512-bit outputs are supported now\n", i);
            return false;
        }
...
/*
 * Runs the CHIME-512 optimized test harness over all configured algorithms.
 * Inputs:
 *     none
 * Outputs:
 *     return value: 0 on success, 1 on parse or test failure
 *
 * 运行 CHIME-512 优化实现的测试驱动，覆盖所有配置的算法。
 * 输入：
 *     无
 * 输出：
...
int main() {
    static const AlgoEntry algos[] = {
        {
            "CHIME-512 with bit padding",
            chime_512_bit_padding,
            true,
            "../../../Test_Vector/KAT_2_12_CHIME-512.txt",
            64
        },
        {
            "CHIME-512 with byte padding",
            chime_512_byte_padding,
            false,
            "../../../Test_Vector/KAT_2_12_CHIME-512.txt",
            64
        }
