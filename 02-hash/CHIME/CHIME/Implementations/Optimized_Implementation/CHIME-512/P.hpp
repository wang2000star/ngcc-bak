...
/*
 * Provides aligned allocation helpers used by the optimized CHIME-512 implementation.
 * 为 CHIME-512 优化实现提供 64 字节对齐的内存分配辅助函数。
 */
#ifndef _MSC_VER
#include <mm_malloc.h>
...
/*
 * Stores the six-lane permutation state used by CHIME-512.
 * 存储 CHIME-512 使用的六个 lane 置换状态。
 */
struct alignas(64) S {
...
/*
 * Applies a 32-bit lane shuffle within each 128-bit half.
 * 输入：
 *     x：输入向量
 *     i：shuffle 立即数
 * 输出：
...
/*
 * Exposes the global round-constant vector table used by the permutation layer.
 * 暴露置换层使用的全局轮常量向量表。
 */
