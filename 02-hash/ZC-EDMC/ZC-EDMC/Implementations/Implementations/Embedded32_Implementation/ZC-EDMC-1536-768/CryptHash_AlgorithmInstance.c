...
#include "../../../lib/low/ZuD-1536/32bit/ZuD1536-32bit.h"

#define WIDTH_BITS 1536u
#define WIDTH_BYTES (WIDTH_BITS / 8u)

static void permute(ZuD1536_32_state *s)
{
    ZuD1536_32_Permute_12rounds(s);
}
static void permute_half(ZuD1536_32_state *s)
{
    ZuD1536_32_Permute_6rounds(s);
}
/*
...
static void absorb_block_custom(ZuD1536_32_state *state, const uint8_t *block, unsigned int rateBytes);
...
static void absorb_block_custom(ZuD1536_32_state *state, const uint8_t *block, unsigned int rateBytes)
{
    ZuD1536_32_AddBytes(state, block, 0, rateBytes);
    uint8_t oldState[WIDTH_BYTES];
    memcpy(oldState, state, WIDTH_BYTES);

    //ZuD1536_32_AddBytes(state, block, 0, rateBytes);
    permute_half(state);
    ZuD1536_32_AddBytes(state, rateBytes + oldState, rateBytes, WIDTH_BYTES - rateBytes);

    permute_half(state);
}

static int squeeze_bytes(ZuD1536_32_state *state, unsigned int rateBytes, uint8_t *out, unsigned int outBytes)
{
    unsigned int produced = 0;
    while (produced < outBytes) {
        unsigned int take = outBytes - produced;
        if (take > rateBytes) take = rateBytes;
        ZuD1536_32_ExtractBytes(state, out + produced, 0, take);
        produced += take;
