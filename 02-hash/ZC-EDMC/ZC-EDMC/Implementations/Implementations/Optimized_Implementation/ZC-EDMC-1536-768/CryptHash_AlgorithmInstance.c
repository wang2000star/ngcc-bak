...
Optimized instance: ZC1536 permutation (AVX2 hybrid).
*/
...
#include "../../../lib/low/ZuD-1536/AVX2/ZuD1536-AVX2.h"

/* Width of ZuD1536 state in bits */
#define WIDTH_BITS 1536u
#define WIDTH_BYTES (WIDTH_BITS/8u)

static void permute(ZuD1536_avx2_state *s)
{
    ZuD1536_avx2_Permute_12rounds(s);
}
static void permute_half(ZuD1536_avx2_state *s)
{
    ZuD1536_avx2_Permute_6rounds(s);
}
...
static void absorb_block_custom(ZuD1536_avx2_state *state, const uint8_t *block, unsigned int rateBytes);

static void append_bit_msb_custom(
    ZuD1536_avx2_state *st,
    uint8_t *block,
    unsigned int *bitpos,
...
static void absorb_block_custom(ZuD1536_avx2_state *state, const uint8_t *block, unsigned int rateBytes)
{
    ZuD1536_avx2_AddBytes(state, block, 0, rateBytes);
    ...
    ZuD1536_avx2_AddBytes(state, rateBytes + oldState, rateBytes, WIDTH_BYTES - rateBytes);

    permute_half(state);
}

/* Squeeze digest_len_bits (must be byte-aligned for this harness). */
static int squeeze_bytes(ZuD1536_avx2_state *state, unsigned int rateBytes, uint8_t *out, unsigned int outBytes)
{
    unsigned int produced = 0;
    while (produced < outBytes) {
        unsigned int take = outBytes - produced;
        if (take > rateBytes) take = rateBytes;
        ZuD1536_avx2_ExtractBytes(state, out + produced, 0, take);
        produced += take;
