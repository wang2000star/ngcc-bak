...
void Laurus_512_SelfTest()
{
	...
	laurus_512(h0, message, 6400);
	laurus_512_init(&state);
	laurus_512_absorb(&state, message, 37);
	laurus_512_absorb(&state, message + 37, 640);
	laurus_512_absorb(&state, message + 677, 5723);
	laurus_512_finalize(&state);
	laurus_512_squeeze(h1, 64, &state);
	for (i = 0; i < 64; i++)
	{
		if (h0[i] != h1[i])
		{
			printf(" Laurus_512 test is error!\n");
		}
	}

	printf("\n Laurus_512 test is right!\n");
}

void Laurus_512_SpeedTest(FILE *fp)
{
	uint8_t hash[65536];
	...
	{
		SPEED_START(start - i);
		laurus_512(hash, hash, value[i]);
		SPEED_FINISH(fp, "laurus_512", value[i], 3.7);
	}
}
