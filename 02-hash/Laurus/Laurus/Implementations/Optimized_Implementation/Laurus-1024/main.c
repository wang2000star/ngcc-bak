...
void Laurus_1024_SelfTest()
{
	...
	laurus_1024(h0, message, 6400);
	laurus_1024_init(&state);
	laurus_1024_absorb(&state, message, 37);
	laurus_1024_absorb(&state, message + 37, 640);
	laurus_1024_absorb(&state, message + 677, 5723);
	laurus_1024_finalize(&state);
	laurus_1024_squeeze(h1, 128, &state);
	for (i = 0; i < 128; i++)
	{
		if (h0[i] != h1[i])
		{
			printf(" Laurus_1024 test is error!\n");
		}
	}

	printf("\n Laurus_1024 test is right!\n");
}

void Laurus_1024_SpeedTest(FILE *fp)
{
	uint8_t hash[65536];
	...
	{
		SPEED_START(start - i);
		laurus_1024(hash, hash, value[i]);
		SPEED_FINISH(fp, "laurus_1024", value[i], 3.7);
	}
}
...
void main()
{
	...
	Laurus_1024_SelfTest();
	Laurus_1024_SpeedTest(stdout);
}