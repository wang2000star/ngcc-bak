...
/* Derive the fixed instance layout at compile time: the capacity holds the
 * digest plus one 64-bit word, and the rest of the 32-word state is the rate. */
#define OUT_WORDS  (DIGEST_BIT_LENGTH / 64U)
#define CAP_WORDS  (OUT_WORDS + 1U)
#define RATE_WORDS (32U - CAP_WORDS)
#define RATE_BITS  (64U * RATE_WORDS)

/* AC constants are kept in one compact table shared by every permutation step. */
static const word_t C[26] = {
    ...
    UINT64_C(0x6c9e0e8bb01e8a3e), UINT64_C(0xd71577c1bd314b27)
};

static word_t rotl64(word_t x, unsigned n) { return (x << n) | (x >> (64U - n)); }

/* Recursive alternating add/XOR tree sum avoids separate temporary reduction
 * arrays, favoring a small code and data footprint. */
...
/*
 * Apply all five rounds of AC, GM, WR, and WP in place.
 *
 * The step-local y array exists only while one group size is processed, so its
 * lifetime is explicit and reusable. Destination indices are derived instead
 * of stored in an additional table, reducing static data at the cost of a small
 * amount of control work.
 */
static void CryptHash_AlgorithmInstance_permutation(word_t x[32])
{
    int r, i, j, k, l, g;
    ...
    for (r = 0; r < 5; r++) {
        for (i = 0; i < 5; i++) {
            ...
            /* AC injects group, step, and round constants. */
            for (j = 0; j < 32; j += g)
                x[j] ^= C[j / g] ^ C[16 + i] ^ C[21 + r];

            /* GM uses the tree sum and computes each destination group in place. */
            for (j = 0; j < 32; j += g) {
                word_t s = S(x + j, g);
                l = j;
                for (k = 32; k > (l | g); ) {
                ...
                for (k = 0; k < g; k++)
                    y[l + k] = k ? x[l + k] ^ (s + (s << (k + 1))) : x[l] + 3U * s;
            }

            /* WR rotates words; WP then maps even/odd positions to state halves. */
            for (j = 0; j < 32; j++)
...
/* Reverse API byte bit order without a 256-byte lookup table. */
...
static word_t load_message_word(const unsigned char *msg, unsigned long long base)
{
    ...
    for (bit = 0; bit < 8U; bit++)
        w |= (word_t)reverse8(msg[base / 8U + bit]) << (8U * bit);
    return w;
}
                              ...
                              unsigned final_bits, unsigned local_base)
{
    ...
    for (bit = 0; bit < 64U; bit++) {
        ...
        unsigned long long pos = final_base + local_pos;
        if ((local_pos < remaining_bits &&
             ((msg[pos / 8U] >> (7U - (unsigned)(pos % 8U))) & 1U)) ||
              ...
              unsigned long long msg_len_bits, unsigned char *digest)
{
    ...
    unsigned remaining_bits, padding_blocks, final_bits, padding_block, i, j;
    ...
    for (block_number = 0; block_number < full_blocks; block_number++) {
        word_t feed_forward[DIGEST_BIT_LENGTH / 64 + 1];
    ...
    for (i = 0; i < out_words; i++)
        for (j = 0; j < 8U; j++)
            digest[8U * i + j] = (unsigned char)(state[rate_words + i] >> (8U * j));
