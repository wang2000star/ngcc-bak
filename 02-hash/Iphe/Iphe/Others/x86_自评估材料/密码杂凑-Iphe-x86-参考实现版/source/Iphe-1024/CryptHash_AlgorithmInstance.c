...
/* The 2048-bit permutation state is represented as 32 little-endian 64-bit
 * words. These constants provide the AC domain separation for groups, steps,
 * and rounds. */
...
/* Compute the alternating add/XOR tree sum used by GM. The recursion mirrors
 * the specification directly, which keeps this reference version easy to
 * compare with the algorithm description. */
...
/*
 * Apply the complete 25-step Iphe permutation in place.
 *
 * x is both the 32-word input state and the output state. Each of five rounds
 * processes group sizes 2, 4, 8, 16, and 32. AC injects constants, GM mixes
 * each group using its tree sum, WR rotates every word, and WP permutes word
 * positions. The temporary y array preserves the preceding step until WP has
 * committed the complete next state.
 */
void CryptHash_AlgorithmInstance_permutation(word_t x[32])
{
    ...
    for (r = 0; r < 5; r++) {
        for (i = 0; i < 5; i++) {
            ...
            /* AC: distinguish the group, step, and round before nonlinear
             * group mixing. */
            for (j = 0; j < 32; j += g)
            ...
            for (j = 0; j < 32; j += g) {
                word_t s = S(x + j, g);
                /* Derive the GM destination group without a lookup table. */
                l = j;
                for (k = 32; k > (l | g); ) {
                    k >>= 1;
                    l ^= k;
            ...
            /* WR assigns a position-dependent rotation to every mixed word. */
            for (j = 0; j < 32; j++)
                y[j] = rotl64(y[j], (unsigned)((7 * j % 32) + 16));

            /* WP separates even and odd source positions into state halves. */
            for (j = 0; j < 32; j++)
                x[j / 2 + (j % 2) * 16] = y[j];
        }
    }
...
/* The public interface numbers message bits most-significant-bit first within
 * each byte, including a partially used final byte. */
...
/*
 * Validate an official one-shot request, absorb the padded message, and write
 * the fixed-size digest. Returns 0 on success and -1 on invalid arguments.
 * The zero-initialized state and all feed-forward storage are local, so the
 * only externally visible side effect is the digest output.
 */
