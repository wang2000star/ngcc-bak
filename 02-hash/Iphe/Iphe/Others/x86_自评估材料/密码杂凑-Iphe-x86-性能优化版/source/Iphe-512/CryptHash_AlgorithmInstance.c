...
/* AC constants are shared by all unrolled group-size kernels. */
...
/* Precomputed GM destinations replace the reference implementation's
 * per-group index derivation in the permutation's hot path. */
...
/* Byte bit-reversal is table driven for callers that need a single-byte
 * conversion; full aligned words use the parallel mask implementation below. */
...
/* These fixed-depth helpers spell out the alternating add/XOR tree sum. They
 * remove recursion and expose instruction-level parallelism to the compiler. */
...
/* Combine WR and WP in one fully unrolled assignment schedule. Writing the
 * permuted destinations directly avoids separate rotation and copy loops. */
...
/* Specialized GM/AC kernels remove group-size branches and fixed-bound inner
 * loops. Each kernel injects constants, computes its tree sum, mixes the group,
 * then completes the fused WR/WP stage. */
...
/* Apply five rounds, each containing the specialized g=2,4,8,16,32 steps.
 * The state is updated in place and no data persists after the call. */
...
/* API message bits are most-significant-bit first within each byte. */
