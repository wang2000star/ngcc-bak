...
static inline int arm_cycle_counter_fd(void) {
    static int perf_fd = -2;
    ...
    if(perf_fd < 0) {
        fprintf(stderr,
                "Failed to open ARM CPU cycle counter via perf_event_open: %s\n",
                strerror(errno));
        fprintf(stderr,
...
static inline uint64_t read_cycle_counter(void) {
    uint64_t cycles = 0;
    int perf_fd = arm_cycle_counter_fd();

    if(perf_fd < 0) {
        return 0;
    }

    if(read(perf_fd, &cycles, sizeof(cycles)) != (ssize_t)sizeof(cycles)) {
        return 0;
    }
