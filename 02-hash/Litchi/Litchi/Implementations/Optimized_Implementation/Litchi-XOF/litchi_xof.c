...
/*************************************************
* Name:        litchi_xof_init
*
* Description: xof Initilizes
*
* Arguments:   - litchi_state *state: pointer to (uninitialized) state
**************************************************/
void litchi_xof_init(litchi_state* state)
{
    litchi_init(state, 1024, 1);
}

/*************************************************
* Name:        litchi_xof_absorb
*
* Description: Absorb step of the xof.
*
...
/*************************************************
* Name:        litchi_xof_finalize
*
* Description: Finalize absorb step of the xof.
*
* Arguments:   - litchi_state *state: pointer to state
**************************************************/
...
/*************************************************
* Name:        litchi_xof_squeezeblocks
*
* Description: Squeeze step of xof. Squeezes full blocks of
*              136 bytes each. Can be called multiple times
*              to keep squeezing. Assumes zero bytes of current block
*              have already been squeezed (state->pos = 0).
*
* Arguments:   - uint8_t *out:        pointer to output blocks
*              - int nblocks:         number of blocks to be squeezed
*                                     (written to output)
*              - litchi_state *state: pointer to input/output  state
**************************************************/
void litchi_xof_squeezeblocks(uint8_t* out, int nblocks, litchi_state* state)
{
    litchi_squeezeblocks(out, nblocks, state);
}

/*************************************************
* Name:        litchi_xof_squeeze
*
* Description: Squeeze step of xof xof. Squeezes arbitraily many
*              bytes. Can be called multiple times to keep squeezing.
*
* Arguments:   - uint8_t *out:     pointer to output blocks
*              - uint64_t outlen : number of bytes to be squeezed
*                                 (written to output)
*              - litchi_state *s:  pointer to input/output  state
**************************************************/
...
* Name:        litchi_xof
*
* Description: xof with non-incremental API
*
* Arguments:   - uint8_t *out:        pointer to output
*              - uint64_t outlen:     length of output in bytes
...
void litchi_xof(uint8_t* out, uint64_t outlen, const uint8_t* in, uint64_t inlen)
{
    ...
    litchi_xof_init(&state);
    litchi_xof_absorb(&state, in, inlen);
    litchi_xof_finalize(&state);
    litchi_xof_squeeze(out, outlen, &state);
}