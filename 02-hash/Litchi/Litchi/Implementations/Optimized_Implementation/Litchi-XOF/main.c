...
void Litchi_xof_SelfTest()
{
	...
	randombytes(message, 6400);
        ...
        litchi_xof(h0, 1024, message, 6400);
	litchi_xof_init(&state);
	litchi_xof_absorb(&state, message, 37);
	litchi_xof_absorb(&state, message + 37, 640);
	litchi_xof_absorb(&state, message + 677, 5723);
	litchi_xof_finalize(&state);
	litchi_xof_squeeze(h1, 100, &state);
	litchi_xof_squeeze(h1 + 100, 337, &state);
	litchi_xof_squeeze(h1 + 437, 587, &state);
	litchi_xof_init(&state);
	litchi_xof_absorb(&state, message, 6400);
	litchi_xof_finalize(&state);
	litchi_xof_squeezeblocks(h2, 8, &state);
	for (i = 0; i < 1000; i++)
	{
		if ((h0[i] != h1[i]) || (h0[i] != h2[i]))
		{
			printf(" Litchi_xof test is error!\n");
		}
	}

	printf(" Litchi_xof test is right!\n");
}
