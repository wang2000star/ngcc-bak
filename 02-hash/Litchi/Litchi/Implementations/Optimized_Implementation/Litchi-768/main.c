...
void Litchi_768_SelfTest()
{
	...
	litchi_768(h0, message, 6400);
	litchi_768_init(&state);
	litchi_768_absorb(&state, message, 37);
	litchi_768_absorb(&state, message + 37, 640);
	litchi_768_absorb(&state, message + 677, 5723);
	litchi_768_finalize(&state);
	litchi_768_squeeze(h1, 96, &state);
	
	for (i = 0; i < 96; i++)
	{
		if (h0[i] != h1[i])
		{
			printf(" Litchi_768 test is error!\n");
		}
	}

	printf("\n Litchi_768 test is right!\n");
}

void Litchi_768_SpeedTest(FILE *fp)
{
	uint8_t hash[65536];
	...
	{
		SPEED_START(start - i);
		litchi_768(hash, hash, value[i]);
		SPEED_FINISH(fp, "litchi_768", value[i], 3.7);
	}
}
