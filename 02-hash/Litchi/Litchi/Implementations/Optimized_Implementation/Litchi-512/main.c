...
void Litchi_512_SelfTest()
{
	...
	litchi_512(h0, message, 6400);
	litchi_512_init(&state);
	litchi_512_absorb(&state, message, 37);
	litchi_512_absorb(&state, message + 37, 640);
	litchi_512_absorb(&state, message + 677, 5723);
	litchi_512_finalize(&state);
	litchi_512_squeeze(h1, 64, &state);
	for (i = 0; i < 64; i++)
	{
		if (h0[i] != h1[i])
		{
			printf(" Litchi_512 test is error!\n");
		}
	}

	
	printf("\n Litchi_512 test is right!\n");
}

void Litchi_512_SpeedTest(FILE *fp)

{
	...
	{
		SPEED_START(start - i);
		litchi_512(hash, hash, value[i]);
		SPEED_FINISH(fp, "litchi_512", value[i], 3.7);
	}
}
