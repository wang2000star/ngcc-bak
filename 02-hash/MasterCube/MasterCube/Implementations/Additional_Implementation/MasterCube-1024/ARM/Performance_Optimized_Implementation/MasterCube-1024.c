...
#if defined(MCUBE_USE_SSE2NEON)
static inline int MasterCube1024_sse_neon_256bit_absorb(const unsigned char *message, unsigned char *digest)
{
    __m128i state[12];
    ...
    state[3] = vreinterpretq_m128i_u8(vsetq_lane_u8(0x01u, zero_u8, 7));
    state[4] = zero;
    state[5] = zero;
    state[6] = zero;
    state[7] = zero;
    ...
    zip_prf(state);
    _sponge_squeeze_helper(mc_transform, state, 448, digest, 1024);
...
{
    if (message_bit_len == 256ULL)
        return MasterCube1024_sse_neon_256bit_absorb(message, digest);
