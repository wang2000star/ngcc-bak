...
static inline void _sponge_absorb_768_avx2(__m256i state[6], const unsigned char *message, const uint64_t message_bits)
{
    for (int row = 0; row < 6; ++row)
        state[row] = _mm256_setzero_si256();

    enum { block_bits = 704, block_bytes = 88 };
    ...
    for (uint64_t i = 0; i < n_block; ++i) {
        ...
        state[2] = _mm256_xor_si256(state[2], loadu_tail24_avx2(p + 64));
    ...
    state[2] = _mm256_xor_si256(state[2], loadu_tail24_avx2(last_block + 64));
    mc_transform_avx2(state);
}
...
int MasterCube768_avx2(const unsigned char *message, unsigned long long message_bit_len, unsigned char *digest)
{
    ...
    _sponge_absorb_768_avx2(state, message, message_bit_len);
