...
{
    memcpy(digest, state->raw_data, 88);
    transform(state);
    memcpy(digest + 88, state->raw_data, 8);
}
...
{
    return MasterCube768_plain(message, message_bit_len, digest);
}
#endif
...
{
    const __m128i x0 = state[0];
    const __m128i x1 = state[1];
    const __m128i x2 = state[2];
    const __m128i x3 = state[3];
    const __m128i x4 = state[4];
    const __m128i x5 = state[5];
    const __m128i x6 = state[6];
    const __m128i x7 = state[7];
    const __m128i x8 = state[8];
    const __m128i x9 = state[9];
    const __m128i x10 = state[10];
    const __m128i x11 = state[11];

    state[0] = _mm_xor_si128(_mm_xor_si128(x0, x2), x8);
    state[2] = _mm_xor_si128(_mm_xor_si128(x2, x4), x10);
    state[4] = _mm_xor_si128(_mm_xor_si128(x4, x6), x0);
    state[6] = _mm_xor_si128(_mm_xor_si128(x6, x8), x2);
    state[8] = _mm_xor_si128(_mm_xor_si128(x8, x10), x4);
    state[10] = _mm_xor_si128(_mm_xor_si128(x10, x0), x6);

    state[1] = _mm_xor_si128(_mm_xor_si128(x1, x3), x9);
    state[3] = _mm_xor_si128(_mm_xor_si128(x3, x5), x11);
    state[5] = _mm_xor_si128(_mm_xor_si128(x5, x7), x1);
    state[7] = _mm_xor_si128(_mm_xor_si128(x7, x9), x3);
    state[9] = _mm_xor_si128(_mm_xor_si128(x9, x11), x5);
    state[11] = _mm_xor_si128(_mm_xor_si128(x11, x1), x7);
}
...
static inline void _sponge_squeeze_helper(void (*transform)(__m128i *), __m128i state[12], const int block_bits, unsigned char *digest, const int digest_bits)
{
    ...
    _mm_storeu_si128((__m128i *)(digest + 64), state[4]);
    _mm_storel_epi64((__m128i *)(digest + 80), state[5]);

    transform(state);

    _mm_storel_epi64((__m128i *)(digest + 88), state[0]);
...
{
    return MasterCube768_sse(message, message_bit_len, digest);
}
#endif
...
static inline void _sponge_squeeze_short_a512(void (*transform)(__m256i *), __m256i state[6], unsigned char *digest)
{
    ...
    _mm256_storeu_si256((__m256i *)(digest + 32), state[1]);
    _mm_storeu_si128((__m128i *)(digest + 64), _mm256_castsi256_si128(state[2]));
    _mm_storel_epi64((__m128i *)(digest + 80), _mm256_extracti128_si256(state[2], 1));
    transform(state);
    _mm_storel_epi64((__m128i *)(digest + 88), _mm256_castsi256_si128(state[0]));
