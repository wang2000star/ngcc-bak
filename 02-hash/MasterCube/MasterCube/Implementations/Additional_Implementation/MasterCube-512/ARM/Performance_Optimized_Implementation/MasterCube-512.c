...
#pragma GCC target ("cpu=cortex-a76")
...
#define MC_SSE_TRANSFORM_DECL static inline __attribute__((always_inline))
#define MC_SSE_ABSORB_DECL static inline __attribute__((always_inline))
#define MC_SSE_ENTRY_ATTR __attribute__((flatten))
#else
...
static inline void _sponge_squeeze_512_direct(struct mc_state *state, unsigned char *digest)
{
    memcpy(digest, state->raw_data, 64);
}
...
int MasterCube512_plain(const unsigned char *message, unsigned long long message_bit_len, unsigned char *digest)
{
    ...
    _sponge_squeeze_512_direct(&state, digest);
...
{
    return MasterCube512_plain(message, message_bit_len, digest);
}
#endif
...
static inline __m128i neon_rotl_bytes(__m128i x, const int n)
{
    ...
    switch (n) {
        ...
        default: return vreinterpretq_m128i_u8(vextq_u8(v, v, n & 15));
    }
}
...
static inline __m128i ror128_optimized(const __m128i x, const int n_ror) {
    switch (n_ror) {
        ...
        default:
            return neon_rotl_bytes(x, n_ror);
            break;
    }
}
...
static inline void AddConstant(__m128i state[12], const int rn)
{
    ...
    state[0] = _mm_xor_si128(state[0], c);
#endif
...
#if defined(MCUBE_USE_SSE2NEON)
static inline __m128i ror16_1_neon_short(const __m128i x)
{
    const uint16x8_t v = vreinterpretq_u16_m128i(x);
    return vreinterpretq_m128i_u16(vsriq_n_u16(vshlq_n_u16(v, 15), v, 1));
}

static inline __m128i ror16_5_neon_short(const __m128i x)
{
    const uint16x8_t v = vreinterpretq_u16_m128i(x);
    return vreinterpretq_m128i_u16(vsriq_n_u16(vshlq_n_u16(v, 11), v, 5));
}

static inline __m128i ror16_8_neon_short(const __m128i x)
{
    return vreinterpretq_m128i_u8(vrev16q_u8(vreinterpretq_u8_m128i(x)));
}

static inline __m128i ror16_9_neon_short(const __m128i x)
{
    const uint16x8_t v = vreinterpretq_u16_m128i(x);
    return vreinterpretq_m128i_u16(vsriq_n_u16(vshlq_n_u16(v, 7), v, 9));
}

static inline __m128i ror16_12_neon_short(const __m128i x)
{
    const uint16x8_t v = vreinterpretq_u16_m128i(x);
    return vreinterpretq_m128i_u16(vsriq_n_u16(vshlq_n_u16(v, 4), v, 12));
}

static inline __m128i ror16_14_neon_short(const __m128i x)
{
    const uint16x8_t v = vreinterpretq_u16_m128i(x);
    return vreinterpretq_m128i_u16(vsriq_n_u16(vshlq_n_u16(v, 2), v, 14));
}

static inline void MAndRXs_0_1_8_neon_short(__m128i state_l[6], __m128i state_r[6])
{
#pragma GCC unroll 6
    for (int i = 0; i < 6; ++i) {
        const __m128i x = state_l[i];
        const __m128i y = state_r[i];
        const __m128i r = _mm_xor_si128(_mm_and_si128(x, ror16_1_neon_short(x)), ror16_8_neon_short(x));
        state_r[i] = _mm_xor_si128(r, y);
    }
}

static inline void MAndRXs_14_5_0_neon_short(__m128i state_l[6], __m128i state_r[6])
{
#pragma GCC unroll 6
    for (int i = 0; i < 6; ++i) {
        const __m128i x = state_l[i];
        const __m128i y = state_r[i];
        const __m128i r = _mm_xor_si128(_mm_and_si128(ror16_14_neon_short(x), ror16_5_neon_short(x)), x);
        state_r[i] = _mm_xor_si128(r, y);
    }
}

static inline void MAndRXs_9_12_8_neon_short(__m128i state_l[6], __m128i state_r[6])
{
#pragma GCC unroll 6
    for (int i = 0; i < 6; ++i) {
        const __m128i x = state_l[i];
        const __m128i y = state_r[i];
        const __m128i r = _mm_xor_si128(_mm_and_si128(ror16_9_neon_short(x), ror16_12_neon_short(x)), ror16_8_neon_short(x));
        state_r[i] = _mm_xor_si128(r, y);
    }
}

static inline void MAndRXs_8_9_0_neon_short(__m128i state_l[6], __m128i state_r[6])
{
#pragma GCC unroll 6
    for (int i = 0; i < 6; ++i) {
        const __m128i x = state_l[i];
        const __m128i y = state_r[i];
        const __m128i r = _mm_xor_si128(_mm_and_si128(ror16_8_neon_short(x), ror16_9_neon_short(x)), x);
        state_r[i] = _mm_xor_si128(r, y);
    }
}

static inline void _core_neon_short_mand(__m128i state[12], const int n_rounds)
{
    for (int i = 0; i < n_rounds; ++i) {
        CrossMixs(state);
        SwapRows(state);

        MAndRXs_0_1_8_neon_short(state, state + 6);
        MAndRXs_14_5_0_neon_short(state + 6, state);
        MAndRXs_9_12_8_neon_short(state, state + 6);
        MAndRXs_14_5_0_neon_short(state + 6, state);
        MAndRXs_0_1_8_neon_short(state, state + 6);
        MAndRXs_8_9_0_neon_short(state + 6, state);
...
static inline void _core_inverse_neon_short_mand(__m128i state[12], const int n_rounds)
{
    for (int i = n_rounds - 1; i >= 0; --i) {
        AddConstant(state, i + 9);
...
static inline void zip_prf_neon_short(__m128i state[12])
{
    __m128i state_copy[12];
    state_copy[0] = state[0];
    state_copy[1] = state[1];
    state_copy[2] = state[2];
    state_copy[3] = state[3];
    state_copy[4] = state[4];
    state_copy[5] = state[5];
    state_copy[6] = state[6];
    state_copy[7] = state[7];
    state_copy[8] = state[8];
    state_copy[9] = state[9];
    state_copy[10] = state[10];
    state_copy[11] = state[11];

    _core_neon_short_mand(state, N_ROUNDS / 2);
    _core_inverse_neon_short_mand(state_copy, N_ROUNDS / 2);

    state[0] = _mm_xor_si128(state[0], state_copy[0]);
    state[1] = _mm_xor_si128(state[1], state_copy[1]);
    state[2] = _mm_xor_si128(state[2], state_copy[2]);
    state[3] = _mm_xor_si128(state[3], state_copy[3]);
    state[4] = _mm_xor_si128(state[4], state_copy[4]);
    state[5] = _mm_xor_si128(state[5], state_copy[5]);
    state[6] = _mm_xor_si128(state[6], state_copy[6]);
    state[7] = _mm_xor_si128(state[7], state_copy[7]);
    state[8] = _mm_xor_si128(state[8], state_copy[8]);
    state[9] = _mm_xor_si128(state[9], state_copy[9]);
    state[10] = _mm_xor_si128(state[10], state_copy[10]);
    state[11] = _mm_xor_si128(state[11], state_copy[11]);
}

static inline int MasterCube512_sse_neon_short(const unsigned char *message, unsigned long long message_bit_len, unsigned char *digest)
{
    __m128i state[12];
    const uint8x16_t zero_u8 = vdupq_n_u8(0);
    const __m128i zero = vreinterpretq_m128i_u8(zero_u8);

    if (message_bit_len == 128ULL) {
        state[0] = _mm_loadu_si128((const __m128i *)message);
        state[1] = vreinterpretq_m128i_u8(vsetq_lane_u8(0x80u, zero_u8, 0));
        state[2] = zero;
        state[3] = zero;
        state[4] = zero;
        state[5] = zero;
        state[6] = zero;
        state[7] = vreinterpretq_m128i_u8(vsetq_lane_u8(0x01u, zero_u8, 7));
        state[8] = zero;
        state[9] = zero;
        state[10] = zero;
        state[11] = zero;
    } else if (message_bit_len == 256ULL) {
        state[0] = _mm_loadu_si128((const __m128i *)(message + 0));
        state[1] = _mm_loadu_si128((const __m128i *)(message + 16));
        state[2] = vreinterpretq_m128i_u8(vsetq_lane_u8(0x80u, zero_u8, 0));
        state[3] = zero;
        state[4] = zero;
        state[5] = zero;
        state[6] = zero;
        ...
        state[10] = zero;
        state[11] = zero;
    } else if (message_bit_len == 512ULL) {
        state[0] = _mm_loadu_si128((const __m128i *)(message + 0));
        state[1] = _mm_loadu_si128((const __m128i *)(message + 16));
        state[2] = _mm_loadu_si128((const __m128i *)(message + 32));
        state[3] = _mm_loadu_si128((const __m128i *)(message + 48));
        state[4] = vreinterpretq_m128i_u8(vsetq_lane_u8(0x80u, zero_u8, 0));
        state[5] = zero;
        state[6] = zero;
        state[7] = vreinterpretq_m128i_u8(vsetq_lane_u8(0x01u, zero_u8, 7));
        state[8] = zero;
        ...
        const uint64_t message_bytes = (message_bit_len + 7ULL) / 8ULL;
        memcpy(state_bytes, message, message_bytes);
        pad10star1(state_bytes, (int)message_bit_len, 960);
    }

    zip_prf_neon_short(state);

    _mm_storeu_si128((__m128i *)(digest +  0), state[0]);
    _mm_storeu_si128((__m128i *)(digest + 16), state[1]);
    _mm_storeu_si128((__m128i *)(digest + 32), state[2]);
    _mm_storeu_si128((__m128i *)(digest + 48), state[3]);
...
MC_SSE_TRANSFORM_DECL void mc_transform(__m128i state[12])
{
    zip_prf(state);
    return;
}
...
MC_SSE_ABSORB_DECL void _sponge_absorb_helper(void (*transform)(__m128i *), __m128i state[12], const int block_bits, const unsigned char *message, const uint64_t message_bits)
{
...
MC_SSE_ENTRY_ATTR int MasterCube512_sse(const unsigned char *message, unsigned long long message_bit_len, unsigned char *digest)
{
    __m128i state[12] = {0};
...
static inline void ShiftRows_neon_b003(__m128i state_l[6], __m128i state_r[6])
{
    state_l[0] = ror128_optimized(state_l[0], 0);
    state_l[1] = ror128_optimized(state_l[1], 2);
    state_l[2] = ror128_optimized(state_l[2], 4);
    state_l[3] = ror128_optimized(state_l[3], 6);
    state_l[4] = ror128_optimized(state_l[4], 8);
    state_l[5] = ror128_optimized(state_l[5], 10);

    state_r[0] = ror128_optimized(state_r[0], 16);
    state_r[1] = ror128_optimized(state_r[1], 14);
    state_r[2] = ror128_optimized(state_r[2], 12);
    state_r[3] = ror128_optimized(state_r[3], 10);
    state_r[4] = ror128_optimized(state_r[4], 8);
    state_r[5] = ror128_optimized(state_r[5], 6);
}

static inline void _core_neon_b003(__m128i state[12], const int n_rounds)
{
    for (int i = 0; i < n_rounds; ++i) {
        CrossMixs(state);
        SwapRows(state);
        ...
        MixColumns(state);
        ShiftRows_neon_b003(state, state + 6);
        MixRows(state);
...
static inline void _core_inverse_neon_b003(__m128i state[12], const int n_rounds)
{
    for (int i = n_rounds - 1; i >= 0; --i) {
        AddConstant(state, i + 9);

        MixRows(state);
        ShiftRows_neon_b003(state + 6, state);
        MixColumns(state);
...
static inline void zip_prf_neon_b003(__m128i state[12])
{
    __m128i state_copy[12];
    memcpy(state_copy, state, 12 * sizeof(__m128i));

    _core_neon_b003(state, N_ROUNDS / 2);
    _core_inverse_neon_b003(state_copy, N_ROUNDS / 2);

    for (int i = 0; i < 12; ++i)
        state[i] = _mm_xor_si128(state[i], state_copy[i]);
}

static void mc_transform_neon_b003(__m128i state[12])
{
    zip_prf_neon_b003(state);
}
...
int MasterCube512_neon_b003(const unsigned char *message, unsigned long long message_bit_len, unsigned char *digest)
{
    ...
    _sponge_absorb_helper(mc_transform_neon_b003, state, block_bits, message, message_bit_len);
    _sponge_squeeze_helper(mc_transform_neon_b003, state, block_bits, digest, digest_bits);
...
{
    if (message_bit_len <= 512ULL)
        return MasterCube512_sse_neon_short(message, message_bit_len, digest);
    return MasterCube512_sse(message, message_bit_len, digest);
}
#endif
