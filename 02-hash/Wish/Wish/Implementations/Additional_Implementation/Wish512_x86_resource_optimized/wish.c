...
int Wish512Absorb(u128 *state, u128 counter, int last_block, const unsigned char *block) {
    ...
    for (size_t j = 0; j < WISH_512_NLANES / 2; ++j) {
        state[j] = XOR(state[j], LOAD(block + (16 * j)));
    }

    // add block separation const (to the last lane) if last block
    if (last_block == 1){
        state[WISH_512_NLANES - 1] = XOR(state[WISH_512_NLANES-1], LOAD(theta));
    }

    // save second half for feedforward (post-theta, matching the reference)
    u128 feedforward[WISH_512_NLANES / 2];
    memcpy(feedforward, &state[WISH_512_NLANES / 2], (WISH_512_NLANES / 2) * sizeof(u128));

    // #pragma GCC unroll 9
    for (int i = 0; i < WISH_512_NSTEPS; ++i) {
        // add tweak to the first capacity lane
        state[WISH_512_NLANES / 2] = XOR(state[WISH_512_NLANES / 2], counter);

        // 1st AES round, constant only add to lane 0
        state[0] =  _mm_aesenc_si128(state[0], LOAD(rcon[2 * i]));
        // #pragma GCC unroll 7
        for (int j = 1; j < WISH_512_NLANES; ++j) {
            state[j] = _mm_aesenc_si128(state[j], U128_ZEROES);
        }
        // 2nd AES round, constant only add to lane 0
        state[0] =  _mm_aesenc_si128(state[0], LOAD(rcon[2 * i + 1]));
        // #pragma GCC unroll 7
        for (int j = 1; j < WISH_512_NLANES; ++j) {
            state[j] = _mm_aesenc_si128(state[j], U128_ZEROES);
        }
        ...
        WISH_MIX(64, state[0], state[4]);
        WISH_MIX(64, state[1], state[5]);
        WISH_MIX(64, state[2], state[6]);
        WISH_MIX(64, state[3], state[7]);
    ...
    for (int i = WISH_512_NLANES / 2; i < WISH_512_NLANES; ++i) {
        state[i] = XOR(state[i], feedforward[i - (WISH_512_NLANES / 2)]);
    }
