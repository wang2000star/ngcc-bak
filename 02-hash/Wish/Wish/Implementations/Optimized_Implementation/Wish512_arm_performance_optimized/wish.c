...
int Wish512Absorb(u128 *state, u128 counter, int last_block, const unsigned char *block) {
    // lstate: local shadow of state[] - keeps all lanes in registers, avoids
    ...
    for (int i = 0; i < WISH_512_NSTEPS; ++i) {
        ...
        lstate[0] = XOR(vaesmcq_u8(vaeseq_u8(lstate[0],
                        (i == 0) ? LOAD(block) : U128_ZEROES)),
                        LOAD(rcon[2 * i]));
        // 1st AES round: rate lanes 1..NLANES/2-1, embed msg for i==0
        for (int j = 1; j < WISH_512_NLANES / 2; ++j) {
            lstate[j] = vaesmcq_u8(vaeseq_u8(lstate[j],
                        (i == 0) ? LOAD(block + (16 * j)) : U128_ZEROES));
        }
        // 1st AES round: first capacity lane, embed counter every step
        lstate[WISH_512_NLANES / 2] = vaesmcq_u8(vaeseq_u8(lstate[WISH_512_NLANES / 2], counter));
        // 1st AES round: capacity lanes NLANES/2+1..NLANES-2
        for (int j = WISH_512_NLANES / 2 + 1; j < WISH_512_NLANES - 1; ++j) {
            lstate[j] = vaesmcq_u8(vaeseq_u8(lstate[j], U128_ZEROES));
        }
        // 1st AES round: last lane, embed theta for i==0 && last_block
        lstate[WISH_512_NLANES - 1] = vaesmcq_u8(vaeseq_u8(lstate[WISH_512_NLANES - 1],
                        (i == 0 && last_block == 1) ? LOAD(theta) : U128_ZEROES));

        // 2nd AES round: lane 0, rcon[2*i+1] XOR after MC
        lstate[0] = XOR(vaesmcq_u8(vaeseq_u8(lstate[0], U128_ZEROES)), LOAD(rcon[2 * i + 1]));
