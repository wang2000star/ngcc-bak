...
#define U128_ZEROES _mm_setzero_si128()
#define U128_EXTEND_FROM_U64(x) _mm_set_epi64x(0LL, (long long)(x))

#define XOR(a, b) _mm_xor_si128(a, b)
#define LOAD(p) _mm_load_si128((__m128i *)(p))
// #define STORE(p, v) _mm_store_si128((__m128i *)(p), v)
// #define AESR_AC(p) _mm_aesenc_si128(p)

// M1: interleave bytes
#define UNPACKLO8(a, b) _mm_unpacklo_epi8(a, b)
#define UNPACKHI8(a, b) _mm_unpackhi_epi8(a, b)
// M2: interleave 16-bit words
#define UNPACKLO16(a, b) _mm_unpacklo_epi16(a, b)
#define UNPACKHI16(a, b) _mm_unpackhi_epi16(a, b)
// M3: interleave 32-bit dwords
#define UNPACKLO32(a, b) _mm_unpacklo_epi32(a, b)
#define UNPACKHI32(a, b) _mm_unpackhi_epi32(a, b)
// M4: interleave 64-bit qwords
#define UNPACKLO64(a, b) _mm_unpacklo_epi64(a, b)
#define UNPACKHI64(a, b) _mm_unpackhi_epi64(a, b)
...
int Wish512Absorb(u128 *state, u128 counter, int last_block, const unsigned char *block) {
    // lstate: local shadow of state[] â keeps all 16 lanes in registers, avoids
    // premature writebacks to *state that the pointer assignment pattern forces
    u128 lstate[WISH_512_NLANES];
    // load all lanes at once
    memcpy(lstate, state, WISH_512_NLANES * sizeof(u128));


    // absorb msg
    #pragma GCC unroll 4
    for (size_t j = 0; j < WISH_512_NLANES / 2; ++j) {
        lstate[j] = XOR(lstate[j], LOAD(block + (16 * j)));
    }

    // add block separation const (to the last lane) if last block
    if (last_block == 1){
        lstate[WISH_512_NLANES - 1] = XOR(lstate[WISH_512_NLANES-1], LOAD(theta));
    }

    // save second half for feedforward (post-theta, matching the reference)
    u128 feedforward[WISH_512_NLANES / 2];
    memcpy(feedforward, &lstate[WISH_512_NLANES / 2], (WISH_512_NLANES / 2) * sizeof(u128));

    #pragma GCC unroll 9
    for (int i = 0; i < WISH_512_NSTEPS; ++i) {
        // add tweak to the first capacity lane
        lstate[WISH_512_NLANES / 2] = XOR(lstate[WISH_512_NLANES / 2], counter);

        // 1st AES round, constant only add to lane 0
        lstate[0] =  _mm_aesenc_si128(lstate[0], LOAD(rcon[2 * i]));
        #pragma GCC unroll 7
        for (int j = 1; j < WISH_512_NLANES; ++j) {
            lstate[j] = _mm_aesenc_si128(lstate[j], U128_ZEROES);
        }
        // 2nd AES round, constant only add to lane 0
        lstate[0] =  _mm_aesenc_si128(lstate[0], LOAD(rcon[2 * i + 1]));
        #pragma GCC unroll 7
        for (int j = 1; j < WISH_512_NLANES; ++j) {
            lstate[j] = _mm_aesenc_si128(lstate[j], U128_ZEROES);
        }
        ...
        WISH_MIX(8, lstate[0], lstate[1]);
        WISH_MIX(8, lstate[2], lstate[3]);
        WISH_MIX(8, lstate[4], lstate[5]);
        WISH_MIX(8, lstate[6], lstate[7]);

        // mixing M2: 16-bit granularity
        WISH_MIX(16, lstate[0], lstate[2]);
        WISH_MIX(16, lstate[1], lstate[3]);
        WISH_MIX(16, lstate[4], lstate[6]);
        WISH_MIX(16, lstate[5], lstate[7]);

        // mixing M3: 64-bit granularity
        WISH_MIX(64, lstate[0], lstate[4]);
        WISH_MIX(64, lstate[1], lstate[5]);
        WISH_MIX(64, lstate[2], lstate[6]);
        WISH_MIX(64, lstate[3], lstate[7]);

    }

    // feedforward the second half
    #pragma GCC unroll 4
    for (int i = WISH_512_NLANES / 2; i < WISH_512_NLANES; ++i) {
        lstate[i] = XOR(lstate[i], feedforward[i - (WISH_512_NLANES / 2)]);
    }


    // single writeback: all lanes stored in one pass at the very end
    memcpy(state, lstate, WISH_512_NLANES * sizeof(u128));
    return 0;
}

int Wish512(const unsigned char *msg, unsigned long long msg_len_bits, unsigned char *digest) {
    u128 state[WISH_512_NLANES];

    // state initialized to all 0s
    memset(state, 0, (WISH_512_NLANES) * sizeof(u128));


    // number of 64-byte blocks, rounded up, without final partial block
    // (which is padded and processed separately)
    unsigned long long n_full_blocks = msg_len_bits / WISH_512_BLOCK_SIZE_BITS;
    ...
    for (unsigned long long i = 0; i < n_full_blocks; ++i) {
        ...
        Wish512Absorb(state, counter, 0, msg + (WISH_512_BLOCK_SIZE_BYTES * i));
    }

    // final block: copy any partial tail, then pad with a 1-bit followed by zeros
    unsigned char padded_block[WISH_512_BLOCK_SIZE_BYTES];
    const unsigned long long full_bytes = msg_len_bits / 8;
    const unsigned long long partial_bits = msg_len_bits % 8;
    const unsigned long long tail = full_bytes % WISH_512_BLOCK_SIZE_BYTES;

    memset(padded_block, 0, WISH_512_BLOCK_SIZE_BYTES);
    if (tail > 0) {
    ...
    Wish512Absorb(state, pad_counter, 1, padded_block);


    memcpy(digest, &state[WISH_512_NLANES / 2], (WISH_512_NLANES / 2) * sizeof(u128));
    return 0;
}
