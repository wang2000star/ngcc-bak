...
int Wish1024Absorb(u128 *state, u128 counter, int last_block, const unsigned char *block) {
    ...
    u128 lstate[WISH_1024_NLANES];
    // load all lanes at once
    memcpy(lstate, state, WISH_1024_NLANES * sizeof(u128));


    // absorb msg
    #pragma GCC unroll 8
    for (size_t j = 0; j < WISH_1024_NLANES / 2; ++j) {
        lstate[j] = XOR(lstate[j], LOAD(block + (16 * j)));
    }

    // add block separation const (to the last lane) if last block
    if (last_block == 1){
        lstate[WISH_1024_NLANES - 1] = XOR(lstate[WISH_1024_NLANES-1], LOAD(theta));
    }

    // save second half for feedforward (post-theta, matching the reference)
    u128 feedforward[WISH_1024_NLANES / 2];
    memcpy(feedforward, &lstate[WISH_1024_NLANES / 2], (WISH_1024_NLANES / 2) * sizeof(u128));

    #pragma GCC unroll 12
    for (int i = 0; i < WISH_1024_NSTEPS; ++i) {

        // add tweak to the first capacity lane
        lstate[WISH_1024_NLANES / 2] = XOR(lstate[WISH_1024_NLANES / 2], counter);
        ...
        WISH_MIX(8, lstate[8], lstate[9]);
        WISH_MIX(8, lstate[10], lstate[11]);
        WISH_MIX(8, lstate[12], lstate[13]);
        WISH_MIX(8, lstate[14], lstate[15]);
        ...
        WISH_MIX(16, lstate[8], lstate[10]);
        WISH_MIX(16, lstate[9], lstate[11]);
        WISH_MIX(16, lstate[12], lstate[14]);
        WISH_MIX(16, lstate[13], lstate[15]);

        // mixing M3: 32-bit granularity
        WISH_MIX(32, lstate[0], lstate[4]);
        WISH_MIX(32, lstate[1], lstate[5]);
        WISH_MIX(32, lstate[2], lstate[6]);
        WISH_MIX(32, lstate[3], lstate[7]);
        WISH_MIX(32, lstate[8], lstate[12]);
        WISH_MIX(32, lstate[9], lstate[13]);
        WISH_MIX(32, lstate[10], lstate[14]);
        WISH_MIX(32, lstate[11], lstate[15]);

        // mixing M4: 64-bit granularity
        WISH_MIX(64, lstate[0], lstate[8]);
        WISH_MIX(64, lstate[1], lstate[9]);
        WISH_MIX(64, lstate[2], lstate[10]);
        WISH_MIX(64, lstate[3], lstate[11]);
        WISH_MIX(64, lstate[4], lstate[12]);
        WISH_MIX(64, lstate[5], lstate[13]);
        WISH_MIX(64, lstate[6], lstate[14]);
        WISH_MIX(64, lstate[7], lstate[15]);
    ...
    for (int i = WISH_1024_NLANES / 2; i < WISH_1024_NLANES; ++i) {
        lstate[i] = XOR(lstate[i], feedforward[i - (WISH_1024_NLANES / 2)]);
    }


    // single writeback: all lanes stored in one pass at the very end
    memcpy(state, lstate, WISH_1024_NLANES * sizeof(u128));
    return 0;
}
