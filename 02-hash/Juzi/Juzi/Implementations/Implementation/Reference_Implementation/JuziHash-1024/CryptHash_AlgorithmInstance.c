...
void juzihash1024_ref(const uint8_t *msg, uint64_t msg_bitlen,
                      uint8_t digest[JUZI1024_DIGEST_BYTES])
{
    ...
    /* Output the 1024-bit capacity branch S2 || S3 || S6 || S7. */
    memcpy(digest +  0u, state + 2u * 32u, 32u);
    memcpy(digest + 32u, state + 3u * 32u, 32u);
    memcpy(digest + 64u, state + 6u * 32u, 32u);
    memcpy(digest + 96u, state + 7u * 32u, 32u);
}
              ...
              unsigned long long msg_len_bits, unsigned char *digest)
{
    ...
    juzihash1024_ref((const uint8_t *)msg, (uint64_t)msg_len_bits, (uint8_t *)digest);
    return 0;
}
