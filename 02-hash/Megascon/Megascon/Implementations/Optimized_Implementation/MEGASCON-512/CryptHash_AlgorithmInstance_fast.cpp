...
// -----------------------------------------------------------------------------
// Fast absorb loop for MEGASCON-512, rate = 128 bytes
// -----------------------------------------------------------------------------
...
static inline void absorb_block128_scalar(State2048& s, const unsigned char *block) {
    for (int r = 0; r < 4; r++) {
...
static inline void fast_absorb_blocks128_scalar(State2048& state,
                                                const unsigned char *blocks,
                                                ULL block_count,
                                                unsigned rounds) {
    for (ULL i = 0; i < block_count; i++) {
        absorb_block128_scalar(state, blocks + i * 128ULL);
...
static inline void absorb_block128_sse_regs(Vec256_SSE row[8], const unsigned char *block) {
...
static inline void fast_absorb_blocks128_sse(State2048& state,
                                             const unsigned char *blocks,
                                             ULL block_count,
                                             unsigned rounds) {
    Vec256_SSE row[8];
    ...
    for (ULL i = 0; i < block_count; i++) {
        absorb_block128_sse_regs(row, blocks + i * 128ULL);
        permute2048_sse_regs(row, rounds);
    }
...
static inline void absorb_block128_avx2_regs(__m256i row[8], const unsigned char *block) {
...
static inline void fast_absorb_blocks128_avx2(State2048& state,
                                              const unsigned char *blocks,
                                              ULL block_count,
                                              unsigned rounds) {
    __m256i row[8];
    ...
    for (ULL i = 0; i < block_count; i++) {
        absorb_block128_avx2_regs(row, blocks + i * 128ULL);
        permute2048_avx2_regs(row, rounds);
    }
...
static inline void absorb_block128_avx512_regs(__m256i row[8], const unsigned char *block) {
    row[0] = _mm256_xor_si256(row[0], _mm256_loadu_si256(reinterpret_cast<const __m256i *>(block +   0)));
    row[1] = _mm256_xor_si256(row[1], _mm256_loadu_si256(reinterpret_cast<const __m256i *>(block +  32)));
    row[2] = _mm256_xor_si256(row[2], _mm256_loadu_si256(reinterpret_cast<const __m256i *>(block +  64)));
    row[3] = _mm256_xor_si256(row[3], _mm256_loadu_si256(reinterpret_cast<const __m256i *>(block +  96)));
...
static inline void fast_absorb_blocks128_avx512(State2048& state,
                                                const unsigned char *blocks,
                                                ULL block_count,
                                                unsigned rounds) {
    __m256i row[8];
    ...
    for (ULL i = 0; i < block_count; i++) {
        absorb_block128_avx512_regs(row, blocks + i * 128ULL);
        permute2048_avx512_regs(row, rounds);
    }
...
static inline void fast_absorb_blocks128_dispatch(State2048& state,
                                                  const unsigned char *blocks,
                                                  ULL block_count,
                                                  unsigned rounds) {
#if defined(__AVX512F__) && defined(__AVX512VL__)
    fast_absorb_blocks128_avx512(state, blocks, block_count, rounds);
#elif defined(__AVX2__)
    fast_absorb_blocks128_avx2(state, blocks, block_count, rounds);
#elif defined(__SSSE3__)
    fast_absorb_blocks128_sse(state, blocks, block_count, rounds);
#else
    fast_absorb_blocks128_scalar(state, blocks, block_count, rounds);
#endif
}
...
{
    if (rate != 128 || hash != 64 || output_len_bits != hash * 8) {
    ...
    const ULL rate_bits = 128ULL * 8ULL;
    const ULL full_blocks = input_len_bits / rate_bits;
    const ULL rem_bits = input_len_bits % rate_bits;

    if (full_blocks > 0) {
        fast_absorb_blocks128_dispatch(state, input, full_blocks, 15);
    }

    unsigned char last_block[128];
    std::memset(last_block, 0, sizeof(last_block));

    const unsigned char *tail = input + full_blocks * 128ULL;
    ...
    fast_absorb_blocks128_dispatch(state, last_block, 1, 15);
...
{
    return hash_sponge(digest_len_bits / 8, 128,
                       msg, msg_len_bits,
