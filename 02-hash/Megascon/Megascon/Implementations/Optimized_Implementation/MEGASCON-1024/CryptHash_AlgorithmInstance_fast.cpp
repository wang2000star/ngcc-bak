...
// -----------------------------------------------------------------------------
// Fast F-sponge absorb loop for MEGASCON-1024, rate = 120 bytes
// -----------------------------------------------------------------------------
...
static inline void absorb_block120_scalar(State2048& s, const unsigned char *block) {
...
static inline void fast_absorb_one120_scalar(State2048& state,
                                             const unsigned char *block,
                                             unsigned rounds,
                                             bool final_block) {
    absorb_block120_scalar(state, block);

    if (final_block) {
        state.w[3][3] ^= 0x80ULL;
    }

    uint64_t f33 = state.w[3][3];
    uint64_t f4[4], f5[4], f6[4], f7[4];
    std::memcpy(f4, state.w[4], sizeof(f4));
...
static inline void fast_absorb_blocks120_scalar(State2048& state,
    ...
    for (ULL i = 0; i < block_count; i++) {
        ...
        fast_absorb_one120_scalar(state, blocks + i * 120ULL, rounds, final_block);
    }
}
...
static inline void absorb_block120_sse_regs(Vec256_SSE row[8], const unsigned char *block) {
    ...
    row[3].hi = _mm_xor_si128(row[3].hi, _mm_loadl_epi64(reinterpret_cast<const __m128i *>(block + 112)));
}

static inline void domain_sep120_sse_regs(Vec256_SSE row[8]) {
    row[3].hi = _mm_xor_si128(row[3].hi, _mm_set_epi64x(0x80LL, 0LL));
}

static inline void feedforward120_sse_regs(Vec256_SSE row[8], unsigned rounds) {
    __m128i f3hi = mask_high64_sse(row[3].hi);
    Vec256_SSE f4 = row[4];
    Vec256_SSE f5 = row[5];
    Vec256_SSE f6 = row[6];
    Vec256_SSE f7 = row[7];
    ...
    row[3].hi = _mm_xor_si128(row[3].hi, f3hi);
    row[4] = xor256(row[4], f4);
    row[5] = xor256(row[5], f5);
    row[6] = xor256(row[6], f6);
    row[7] = xor256(row[7], f7);
}

static inline void fast_absorb_blocks120_sse(State2048& state,
                                             const unsigned char *blocks,
                                             ULL block_count,
                                             unsigned rounds,
                                             bool last_block_is_final) {
    ...
    for (ULL i = 0; i < block_count; i++) {
        absorb_block120_sse_regs(row, blocks + i * 120ULL);
        if (last_block_is_final && (i + 1 == block_count)) {
            domain_sep120_sse_regs(row);
        }
        feedforward120_sse_regs(row, rounds);
    }
...
static inline __m256i load_absorb_row3_tail120_avx2(const unsigned char *block) {
    __m128i lo = _mm_loadu_si128(reinterpret_cast<const __m128i *>(block + 96));
    __m128i hi = _mm_cvtsi64_si128(static_cast<long long>(load_u64_unaligned(block + 112)));
    __m256i v = _mm256_castsi128_si256(lo);
    return _mm256_inserti128_si256(v, hi, 1);
}

static inline void absorb_block120_avx2_regs(__m256i row[8], const unsigned char *block) {
    ...
    row[3] = _mm256_xor_si256(row[3], load_absorb_row3_tail120_avx2(block));
}

static inline void domain_sep120_avx2_regs(__m256i row[8]) {
    row[3] = _mm256_xor_si256(row[3], _mm256_set_epi64x(0x80LL, 0LL, 0LL, 0LL));
}

static inline void feedforward120_avx2_regs(__m256i row[8], unsigned rounds) {
    const __m256i cap3_mask = _mm256_set_epi64x(-1LL, 0LL, 0LL, 0LL);
    __m256i f3 = _mm256_and_si256(row[3], cap3_mask);
    __m256i f4 = row[4];
    __m256i f5 = row[5];
    __m256i f6 = row[6];
    __m256i f7 = row[7];
    ...
    row[3] = _mm256_xor_si256(row[3], f3);
...
static inline void fast_absorb_blocks120_avx2(State2048& state,
                                              const unsigned char *blocks,
                                              ULL block_count,
                                              unsigned rounds,
                                              bool last_block_is_final) {
    ...
    for (ULL i = 0; i < block_count; i++) {
        absorb_block120_avx2_regs(row, blocks + i * 120ULL);
        if (last_block_is_final && (i + 1 == block_count)) {
            domain_sep120_avx2_regs(row);
        }
        feedforward120_avx2_regs(row, rounds);
    }
...
static inline __m256i load_absorb_row3_tail120_avx512(const unsigned char *block) {
    __m128i lo = _mm_loadu_si128(reinterpret_cast<const __m128i *>(block + 96));
    __m128i hi = _mm_cvtsi64_si128(static_cast<long long>(load_u64_unaligned(block + 112)));
    __m256i v = _mm256_castsi128_si256(lo);
    return _mm256_inserti128_si256(v, hi, 1);
...
static inline void absorb_block120_avx512_regs(__m256i row[8], const unsigned char *block) {
    row[0] = _mm256_xor_si256(row[0], _mm256_loadu_si256(reinterpret_cast<const __m256i *>(block +   0)));
    row[1] = _mm256_xor_si256(row[1], _mm256_loadu_si256(reinterpret_cast<const __m256i *>(block +  32)));
    row[2] = _mm256_xor_si256(row[2], _mm256_loadu_si256(reinterpret_cast<const __m256i *>(block +  64)));
    row[3] = _mm256_xor_si256(row[3], load_absorb_row3_tail120_avx512(block));
}

static inline void domain_sep120_avx512_regs(__m256i row[8]) {
    row[3] = _mm256_xor_si256(row[3], _mm256_set_epi64x(0x80LL, 0LL, 0LL, 0LL));
}

static inline void feedforward120_avx512_regs(__m256i row[8], unsigned rounds) {
    const __m256i cap3_mask = _mm256_set_epi64x(-1LL, 0LL, 0LL, 0LL);
    __m256i f3 = _mm256_and_si256(row[3], cap3_mask);
    __m256i f4 = row[4];
    __m256i f5 = row[5];
...
static inline void fast_absorb_blocks120_avx512(State2048& state,
                                                const unsigned char *blocks,
                                                ULL block_count,
                                                unsigned rounds,
                                                bool last_block_is_final) {
    ...
    for (ULL i = 0; i < block_count; i++) {
        absorb_block120_avx512_regs(row, blocks + i * 120ULL);
        if (last_block_is_final && (i + 1 == block_count)) {
            domain_sep120_avx512_regs(row);
        }
        feedforward120_avx512_regs(row, rounds);
    }
...
static inline void fast_absorb_blocks120_dispatch(State2048& state,
                                                  const unsigned char *blocks,
                                                  ULL block_count,
                                                  unsigned rounds,
                                                  bool last_block_is_final) {
#if defined(__AVX512F__) && defined(__AVX512VL__)
    fast_absorb_blocks120_avx512(state, blocks, block_count, rounds, last_block_is_final);
#elif defined(__AVX2__)
    fast_absorb_blocks120_avx2(state, blocks, block_count, rounds, last_block_is_final);
#elif defined(__SSSE3__)
    fast_absorb_blocks120_sse(state, blocks, block_count, rounds, last_block_is_final);
#else
    fast_absorb_blocks120_scalar(state, blocks, block_count, rounds, last_block_is_final);
#endif
}
                             ...
                             unsigned char *output)
{
    ...
    if (full_blocks > 0) {
        fast_absorb_blocks120_dispatch(state, input, full_blocks, 16, false);
    }

    unsigned char last_block[120];
    std::memset(last_block, 0, sizeof(last_block));

    const unsigned char *tail = input + full_blocks * 120ULL;
    ...
    fast_absorb_blocks120_dispatch(state, last_block, 1, 16, true);

    std::memcpy(output, reinterpret_cast<const unsigned char *>(state.w) + (256 - 128), 128);
    return 0;
}
...
{
    return hash_Fsponge(digest_len_bits / 8, 120,
                        msg, msg_len_bits,
