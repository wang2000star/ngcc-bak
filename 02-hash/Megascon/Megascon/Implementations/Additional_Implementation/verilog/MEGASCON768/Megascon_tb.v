...
module tb_hash_top;
    ...
    initial begin
        ...
        /*
            ...
            RATE = 1216 bits = 152 bytes.

            If the C reference has:
                padded_input[0] = 8'h80;
                padded_input[1..151] = 0;
        ...
        */
        ...
        $display("time = %0t", $time);
        $display("block_in = 0x%0304h", block_in);
        $display("====================================");
    ...
    always @(posedge clk) begin
        if (rst_n) begin
            ...
            if (block_valid && ready) begin
                ...
                print_rows_64({
                    uut.state_reg[2047:1216],
                    uut.state_reg[1215:0] ^ block_in
                });
            ...
            if (done) begin
                ...
                $display("digest = 0x%0192h", digest);
