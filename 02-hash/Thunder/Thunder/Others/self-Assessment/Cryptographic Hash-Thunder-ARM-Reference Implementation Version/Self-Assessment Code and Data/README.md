...
This package is named for the reference version, so this example uses `Reference_Implementation`:

~~~~bash
cd Thunder_ARM64/API_CryptHash/Implementations/Reference_Implementation/Thunder512
sh build.sh
~~~~

The expected output is `libThunder512ref.so`. The sibling `Thunder768` and `Thunder1024` directories are built the same way.
...
~~~~bash
sudo ./ngcc_bench \
  --lib ../../Thunder_ARM64/API_CryptHash/Implementations/Reference_Implementation/Thunder512/libThunder512ref.so \
  --test hash \
  --mode all \
  --digest-len-bits 512 \
  --kat ../../Thunder_ARM64/API_CryptHash/Test_Vector/Thunder512 \
  --json-out Thunder512ref_A64_report.json
~~~~
