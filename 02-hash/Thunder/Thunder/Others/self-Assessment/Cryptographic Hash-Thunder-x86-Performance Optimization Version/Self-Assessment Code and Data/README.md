...
This directory contains the complete self-assessment harness and data for Thunder on x86.
...
This package is named for the performance-optimized version, so this example uses `Optimized_Implementation`:

~~~~bash
cd Thunder_X86/API_CryptHash/Implementations/Optimized_Implementation/Thunder512
sh build.sh
~~~~

The expected output is `libThunder512Opt.so`. The sibling `Thunder768` and `Thunder1024` directories are built the same way.
...
The provided x86 script runs without `sudo`. It tests Thunder-512, Thunder-768, and Thunder-1024 for all three implementation versions with `--test hash --mode all`.
...
~~~~bash
./ngcc_bench \
  --lib ../../Thunder_X86/API_CryptHash/Implementations/Optimized_Implementation/Thunder512/libThunder512Opt.so \
  --test hash \
  --mode all \
  --digest-len-bits 512 \
  --kat ../../Thunder_X86/API_CryptHash/Test_Vector/Thunder512 \
  --json-out Thunder512opt_X86_report.json
~~~~
...
Raw JSON result files are stored in `result/Thunder/`. New runs write `.json.en` and `.json.zh` files in `ngcc_bench-main/build` unless the script or command is changed.
