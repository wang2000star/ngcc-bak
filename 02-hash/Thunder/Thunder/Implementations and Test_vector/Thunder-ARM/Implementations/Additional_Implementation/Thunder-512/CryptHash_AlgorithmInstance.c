...
{   
    ...
    /* --- 1. Initialize Phase --- */ 
  	...
  	s00=IV[8]; 	s10=IV[3];	s20=0;	s30=0;	s40=0;
	s01=IV[7];  s11=IV[2];	s21=0;	s31=0;	s41=0;
	s02=IV[6];  s12=IV[1];	s22=0;	s32=0;	s42=0;
	s03=IV[5];  s13=IV[0];	s23=0;	s33=0;	s43=0;
	s04=IV[4];  s14=0;	s24=0;	s34=0;	s44=0;  

    /* --- 2. Absorbing Phase --- */
   
    while (rem_bits >= RATE_BITS) {
    	 
 S[8]=s00; S[7]=s01;S[6]=s02; S[5]=s03; S[4]=s04;
        S[3]=s10; S[2]=s11; S[1]=s12; S[0]=s13;
        ...
        s21^=SWAP64((*(uint64_t * )(msg +104)));
        s20^=SWAP64((*(uint64_t * )(msg +112)));
        s14^=SWAP64((*(uint64_t * )(msg +120)));
    ...
    if (partial_bits > 0) {
        ...
        s21^=SWAP64((*(uint64_t * )(pad_block +104)));
        s20^=SWAP64((*(uint64_t * )(pad_block +112)));
        s14^=SWAP64((*(uint64_t * )(pad_block +120))); 
    ...
    *((uint64_t *)(digest+16 ))= SWAP64(s10);
    *((uint64_t *)(digest+24))= SWAP64(s04);
    *((uint64_t *)(digest+32 ))= SWAP64(s03);
