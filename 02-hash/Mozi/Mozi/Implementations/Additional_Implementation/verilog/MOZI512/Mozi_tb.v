...
module tb_hash_top;
    ...
    initial begin
        ...
        // 单块输入，假设该 block 已经 padding 完成
        // 若 state[j] <=> state_reg[8*j +: 8]，则空消息 padding 可写成 1024'h80
        block_in = 1024'h80;

        #30;
        rst_n = 1'b1;

        // 避免 testbench 和 DUT 在同一个 posedge 竞争，建议在 negedge 改输入
        @(negedge clk);
        start = 1'b1;
    ...
    always @(posedge clk) begin
        if (rst_n) begin
            ...
            if (start && !busy) begin
                $display("====================================");
                $display("Start hash");
                $display("block_in = 0x%0256h", block_in);

                $display("Initial state lanes:");
                print_lanes_128({1024'd0, block_in});
            ...
            if (block_valid && ready) begin
                ...
                print_lanes_128({
                    uut.state_reg[2047:1024],
                    uut.state_reg[1023:0] ^ block_in
                });
            ...
            if (done) begin
                ...
                $display("digest = 0x%0128h", digest);
