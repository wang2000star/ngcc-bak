...
MOZI-512 implementation with named-register fast absorb loop.
Build selection:
  - AVX-512F + AVX-512VL: named-register AVX512VL S-box using VPTERNLOGQ.
  - AVX2 or SSE2: previous named-register SSE2 implementation.
...
static inline void absorb_block128_regs128(Regs128& x, const unsigned char *block) {
...
static inline void fast_absorb_blocks128_dispatch(State2048& state,
    ...
    for (ULL i = 0; i < block_count; i++) {
        absorb_block128_regs128(x, blocks + i * 128ULL);
#if defined(__AVX512F__) && defined(__AVX512VL__)
        permute2048_regs128_avx512vl(x, rounds);
#elif defined(__AVX2__)
        permute2048_regs128_sse(x, rounds);
...
extern "C" int hash_sponge(int hash,
    ...
    if (full_blocks != 0) {
        fast_absorb_blocks128_dispatch(state, input, full_blocks, 20);
    }

    alignas(64) unsigned char last_block[128];
    std::memset(last_block, 0, sizeof(last_block));

    const unsigned char *tail = (input != nullptr) ? (input + full_blocks * 128ULL) : nullptr;
    ...
    fast_absorb_blocks128_dispatch(state, last_block, 1, 20);
