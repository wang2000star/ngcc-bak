...
MOZI-768 feedforward-sponge implementation with named-register fast absorb loop.
Build selection:
...
static inline __m128i domain_sep_152_mask128() {
    return _mm_set_epi64x(0x80LL, 0LL);
}

static inline void absorb_block152_regs128(Regs128& x, const unsigned char *block) {
    ...
    x.x12 = _mm_xor_si128(x.x12, _mm_loadl_epi64(reinterpret_cast<const __m128i *>(block + 144)));
}
...
static inline void fsponge_permute_feedforward_regs128(Regs128& x, unsigned rounds, bool last_block) {
    ...
    if (last_block) {
        x.x12 = _mm_xor_si128(x.x12, domain_sep_152_mask128());
    }

    __m128i f12 = _mm_and_si128(x.x12, hi_mask);
    __m128i f22 = x.x22;
    __m128i f32 = x.x32;
    __m128i f03 = x.x03;
    __m128i f13 = x.x13;
...
static inline void fast_absorb_fblocks152_dispatch(State2048& state,
    ...
    for (ULL i = 0; i < block_count; i++) {
        absorb_block152_regs128(x, blocks + i * 152ULL);
...
static inline void fast_absorb_fblock152_last_dispatch(State2048& state,
                                                       const unsigned char block[152],
                                                       unsigned rounds) {
    Regs128 x;
    load_regs128(state, x);
    absorb_block152_regs128(x, block);
...
extern "C" int hash_Fsponge(int hash,
    ...
    if (full_blocks != 0) {
        fast_absorb_fblocks152_dispatch(state, input, full_blocks, 24);
    }

    alignas(64) unsigned char last_block[152];
    std::memset(last_block, 0, sizeof(last_block));

    const unsigned char *tail = (input != nullptr) ? (input + full_blocks * 152ULL) : nullptr;
    ...
    fast_absorb_fblock152_last_dispatch(state, last_block, 24);

    std::memcpy(output, state.b + (256 - 96), 96);
    return 0;
}
