...
struct garnet_u128_t gf128_mul_alpha_aa_MC4_st(struct garnet_u128_t a)
{
...
struct  garnet_u128_t gf128_mul_alpha_aa_MD4_st(struct  garnet_u128_t a)
{
    struct  garnet_u128_t mask_low = { 0x0, 0xFFFFFFFFFFFFFFFF };//高64位全为1
    struct  garnet_u128_t result = { 0,0 }, reducer = { 0,0 }, carry = { 0,0 };
    u64 tmp = 0, msb[2] = { 0,0 }, bits = 0;
...
struct garnet_u128_t xor128(struct  garnet_u128_t x, struct  garnet_u128_t y)
{
...
// AbsorbMessage步骤, 一次吸入896比特消息
static void absorb_message5_4x4_st(struct  garnet_u128_t state[STATE_4x4], struct  garnet_u128_t msg[7])
{
...
// KeepCapacity步骤, 保留更新前的capacity部分
static void keep_capacity5_4x4_st(struct  garnet_u128_t state[STATE_4x4], struct  garnet_u128_t cap[9])
{
    cap[0] = state[1]; cap[1] = state[2]; cap[2] = state[4]; cap[3] = state[7];
    cap[4] = state[8]; cap[5] = state[11]; cap[6] = state[12]; cap[7] = state[13]; cap[8] = state[14];
}

// AbsorbCapacity步骤, 吸入更新前的capacity部分
static void absorb_capacity5_4x4_st(struct  garnet_u128_t state[STATE_4x4], struct  garnet_u128_t cap[9])
{
    state[1] = xor128(state[1], cap[0]);
    state[2] = xor128(state[2], cap[1]);
    state[4] = xor128(state[4], cap[2]);
    state[7] = xor128(state[7], cap[3]);
    state[8] = xor128(state[8], cap[4]);
    state[11] = xor128(state[11], cap[5]);
    state[12] = xor128(state[12], cap[6]);
    state[13] = xor128(state[13], cap[7]);
    state[14] = xor128(state[14], cap[8]);
}
...
// 主哈希函数（4x4状态, rate=896比特, 512位摘要-）
 void Garnet_1024(const u8* message, u64 mlength, u8* digest, struct garnet_u128_t state[STATE_4x4])
 {
     int i, j, k;
     struct  garnet_u128_t msg[7] = { 0 }, cap[9] = {0}, msglen;
     u8 lastblock[16];
     struct  garnet_u128_t zero_key = { 0, 0 }, one_key = { 7,0 }, two_key = { 8,0 };
     u64 byte_num;

    state[0].v[0] = 0x6d6794b2b44ce079; state[0].v[1] = 0xae3e63c412c399c7; // ae3e63c412c399c7  6d6794b2b44ce079
    state[1].v[0] = 0x3313b546b6b97c6a; state[1].v[1] = 0x4ba73a9694578b7b; // 4ba73a9694578b7b  3313b546b6b97c6a
    state[2].v[0] = 0x1ffe7cde7b97f757; state[2].v[1] = 0xf1732c187b567448; // f1732c187b567448  1ffe7cde7b97f757
    state[3].v[0] = 0x3ca91ae242531b1b; state[3].v[1] = 0xf306671bb4c3825d; // f306671bb4c3825d  3ca91ae242531b1b

    state[4].v[0] = 0x7f93197cdf02b501; state[4].v[1] = 0xfb30f2348484e130; // fb30f2348484e130   7f93197cdf02b501
    state[5].v[0] = 0xea2ca2425a036ee4; state[5].v[1] = 0xfabc1c4d4fc43e64; // fabc1c4d4fc43e64   ea2ca2425a036ee4
    state[6].v[0] = 0x11d429de5a6f596c; state[6].v[1] = 0x706de8c85e5c85ef; // 706de8c85e5c85ef  11d429de5a6f596c
    state[7].v[0] = 0x0c62de986f621216; state[7].v[1] = 0xf768093617b362bd; // f768093617b362bd   0c62de986f621216

    state[8].v[0] = 0xe9bb5cabecfdc033; state[8].v[1] = 0xcb8d620f0de9ab02; // cb8d620f0de9ab02   e9bb5cabecfdc033
    state[9].v[0] = 0xf3786fd62accae7f; state[9].v[1] = 0xda3f7bac0b8a4288; // da3f7bac0b8a4288  f3786fd62accae7f
    state[10].v[0] = 0x5025d51190f4aef1; state[10].v[1] = 0x29c38914c6d95522; // 29c38914c6d95522  5025d51190f4aef1
    state[11].v[0] = 0xfa559a587be6b290; state[11].v[1] = 0xd35f234f7bb89d5c; // d35f234f7bb89d5c   fa559a587be6b290

    state[12].v[0] = 0x0e7454120469cf3c; state[12].v[1] = 0x9b5e142fb1b033fb; // 9b5e142fb1b033fb   0e7454120469cf3c
    state[13].v[0] = 0xe259137329632770; state[13].v[1] = 0xc5b11c6410d4ed09; // c5b11c6410d4ed09  e259137329632770
    state[14].v[0] = 0x33616a45fa80ee0b; state[14].v[1] = 0x9cf6fefd3c9cf33b; // 9cf6fefd3c9cf33b   33616a45fa80ee0b
    state[15].v[0] = 0x194ea6997ef891f9; state[15].v[1] = 0xf90e95fde810f135; // f90e95fde810f135   194ea6997ef891f9
    ...
    // 吸收阶段
     for (i = 0; (i + 112) <= byte_num; i += 112)
     {
         ...
         memcpy(&msg[4], (u8*)message + i + 64, 16);
         memcpy(&msg[5], (u8*)message + i + 80, 16);
         memcpy(&msg[6], (u8*)message + i + 96, 16);

         keep_capacity5_4x4_st(state, cap);
         
         absorb_message5_4x4_st(state, msg);
         for (j = 0; j < 12; j++)permutation_p_4x4_st(state, zero_key, j);
         absorb_message5_4x4_st(state, msg);
         absorb_capacity5_4x4_st(state, cap);
     }
...
// In this function, we assume that the message length is multiple of bytes after padding.
     ...
     absorb_message5_4x4_st(state, msg);//sponge-dm
     absorb_capacity5_4x4_st(state, cap);
     ...
     memcpy(digest + 16, &state[2], sizeof(struct garnet_u128_t));
     memcpy(digest + 32, &state[5], sizeof(struct garnet_u128_t));
     memcpy(digest + 48, &state[7], sizeof(struct garnet_u128_t));
     memcpy(digest + 64, &state[9], sizeof(struct garnet_u128_t));
     memcpy(digest + 80, &state[11], sizeof(struct garnet_u128_t));
     memcpy(digest + 96, &state[12], sizeof(struct garnet_u128_t));
     memcpy(digest + 112, &state[14], sizeof(struct garnet_u128_t));

 }

int CryptHash_1024(const unsigned char *msg, unsigned long long msg_len_bits, unsigned char *digest) {
    ...
    Garnet_1024(temp_msg, msg_len_bits, digest, state_c);
