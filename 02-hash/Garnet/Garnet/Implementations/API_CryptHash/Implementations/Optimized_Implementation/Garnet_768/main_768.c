...
// 声明外部汇编或 C 实现的 Garnet_768 算法和填充函数
extern void Garnet_768(const uint8_t* message, uint64_t mlength, uint8_t* digest, garnet_u128_t state[STATE_4x4]);
extern void pad_message(unsigned char* msg, uint64_t bit_len);
...
int main(void)
{
    ...
    uint8_t digest_c[96];       /* Garnet_768 的摘要长度为 768 比特 (96 字节) */
    ...
    memset(digest_c, 0, 96);

    /* 调用适配后的 Garnet_768，可无缝调用 C 语言或优化后的汇编版本 */
    Garnet_768(message, bitlength, digest_c, state_c);

    printf("\n-------------------------------------------------------------\n");
    printf("Garnet-768 result: \n");
    for (i = 0; i < 96; i++) {
