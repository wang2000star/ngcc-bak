...
static const uint64_t DUET_AVX2_6_L64_COUNTS[5][2][4] = {
    {{0, 5, 10, 15}, {20, 25, 0, 0}},
    {{1, 6, 11, 16}, {21, 26, 0, 0}},
    {{2, 7, 12, 17}, {22, 27, 0, 0}},
    {{3, 8, 13, 18}, {23, 28, 0, 0}},
...
DUET_FORCE_INLINE __m256i duet_avx2_l64_part(__m256i x, unsigned int row,
  ...
  __m256i n0 = _mm256_loadu_si256(
      (const __m256i *)(const void *)DUET_AVX2_6_L64_COUNTS[row][part]);
  __m256i n1 = _mm256_and_si256(_mm256_add_epi64(n0, _mm256_set1_epi64x(21)),
                                _mm256_set1_epi64x(63));
  __m256i n2 = _mm256_and_si256(_mm256_add_epi64(n0, _mm256_set1_epi64x(43)),
                                _mm256_set1_epi64x(63));
