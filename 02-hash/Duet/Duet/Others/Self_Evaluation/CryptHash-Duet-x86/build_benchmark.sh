...
Builds and runs the Duet x86-64 AVX2 self-evaluation benchmarks.
...
build_one() {
  ...
  local main_src="$pkg/CryptHash_Duet-${bits}-c-avx2.c"

  [[ -f "$main_src" ]] || die "Missing source file: $main_src"
  mkdir -p "$(dirname "$exe")"

  "$cc" -O3 -std=c99 -Wpedantic -Wall -Wextra \
    -mavx2 -mbmi -mbmi2 -madx -mtune=native "-DNGCC_HASH_BITS=$bits" \
...
run_one() {
  ...
  printf '\n== Duet-%s x86 AVX2 benchmark ==\n' "$bits"
  "$exe" "${bench_args[@]}"
}

for bits in "${bits_list[@]}"; do
  exe="${out:-$bin_dir/ngcc_duet_${bits}_x86_avx2_benchmark}"
  printf '\n== Build Duet-%s x86 AVX2 benchmark ==\n' "$bits"
  build_one "$bits" "$exe"
