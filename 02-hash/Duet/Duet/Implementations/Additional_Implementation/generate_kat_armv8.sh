...
Duet-*/ARMv8/output/. Use --bits to run a single algorithm instance.
USAGE
}
...
run_one() {
  local bits="$1"
  local pkg="$SCRIPT_DIR/Duet-$bits/ARMv8"
  local main_src="$pkg/CryptHash_Duet-${bits}-c-neon.c"
  local exe="$out"
  ...
  if [[ -z "$exe" ]]; then
    exe="/tmp/kat_duet_${bits}_armv8_neon"
  fi

  printf '\n== Duet-%s ARMv8 NEON additional KAT ==\n' "$bits"
...
printf '\nARMv8 NEON additional implementation KAT task finished.\n'
