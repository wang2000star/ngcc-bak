...
// duet_768.v
// -----------
// Top level wrapper for the Duet-768 hash function providing a simple
...
//   0x10..0x31      BLOCK   (34 x 32-bit = 1088-bit message block, MSB word first)
//   0x40..0x57      DIGEST  (24 x 32-bit = 768-bit digest, ro)
//
...
// writing it through BLOCK0..BLOCK33, and pulsing CTRL.absorb; this
// mirrors how the Duet-512 wrapper expects a pre-padded tail block
// from the host rather than padding in hardware.
//
//======================================================================
...
module duet_768(
  ...
  localparam ADDR_BLOCK_LAST = 8'h31;

  localparam ADDR_DIGEST0  = 8'h40;
  localparam ADDR_DIGEST_LAST = 8'h57;

  localparam CORE_NAME0   = 32'h64756574; // "duet"
  localparam CORE_NAME1   = 32'h2d373638; // "-768"
  ...
  reg [31 : 0] block_reg [0 : 33];
  reg          block_we;

  reg [767 : 0] digest_reg;
  ...
  //----------------------------------------------------------------
  wire           core_ready;
  wire [1087 : 0] core_block;
  wire [767 : 0] core_digest;
          ...
          if (we)
            ...
            begin
              if ((address >= ADDR_DIGEST0) && (address <= ADDR_DIGEST_LAST))
                tmp_read_data = digest_reg[(23 - (address - ADDR_DIGEST0)) * 32 +: 32];
