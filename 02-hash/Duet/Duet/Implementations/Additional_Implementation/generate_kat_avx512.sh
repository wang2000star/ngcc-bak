...
and Duet-1024 in sequence. Generated files are written to
Duet-*/AVX512/output/. Use --bits to run a single algorithm instance.
USAGE
}
...
run_one() {
  local bits="$1"
  local pkg="$SCRIPT_DIR/Duet-$bits/AVX512"
  ...
  if [[ -z "$exe" ]]; then
    exe="/tmp/kat_duet_${bits}_x86_avx512"
  fi

  printf '\n== Duet-%s x86 AVX512 additional KAT ==\n' "$bits"
