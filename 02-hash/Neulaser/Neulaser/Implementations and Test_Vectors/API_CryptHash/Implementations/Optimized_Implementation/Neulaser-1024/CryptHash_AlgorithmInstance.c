...
#if defined(_MSC_VER)
#define NL_INLINE static __forceinline
#define NL_RESTRICT __restrict
#else
#define NL_INLINE static inline __attribute__((always_inline))
#define NL_RESTRICT __restrict__
#endif
...
NL_INLINE uint32_t nl_rotl32(uint32_t x, unsigned int r)
{
    r &= 31U;
    return r ? ((x << r) | (x >> (32U - r))) : x;
}

NL_INLINE uint32_t nl_redp32(uint32_t x)
{
    return x >= NL_P32 ? x - NL_P32 : x;
}

/* p = 2^32 - 5, hence 2^32 == 5 (mod p).  The largest value passed by
 * Neulaser's linear core needs two folds and at most one subtraction.  This
 * replaces the particularly expensive 64-bit integer division generated for
 * x % p.  Montgomery reduction is useful for chains of products in a generic
 * modulus; for these fixed small coefficients the pseudo-Mersenne fold is
 * both simpler and faster. */
NL_INLINE uint32_t nl_redp64(uint64_t x)
{
    uint64_t t = (uint64_t)(uint32_t)x + 5ULL * (x >> 32);
    t = (uint64_t)(uint32_t)t + 5ULL * (t >> 32);
    return (uint32_t)(t >= NL_P ? t - NL_P : t);
}

NL_INLINE uint32_t nl_load32_be(const uint8_t x[4])
{
    return ((uint32_t)x[0] << 24) | ((uint32_t)x[1] << 16) | ((uint32_t)x[2] << 8) | (uint32_t)x[3];
}

NL_INLINE void nl_store32_be(uint8_t out[4], uint32_t x)
{
    out[0] = (uint8_t)(x >> 24);
    out[1] = (uint8_t)(x >> 16);
    out[2] = (uint8_t)(x >> 8);
...
NL_INLINE uint32_t nl_linear_layer(uint32_t x, int layer)
{
    if (layer == 0)
...
NL_INLINE uint32_t nl_sbox_word(uint32_t x)
{
...
NL_INLINE uint32_t nl_F(uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3)
{
    uint32_t a = r0 + r1;
    uint32_t b = r2 ^ r3;
...
NL_INLINE uint32_t nl_linear_core(const uint32_t L[NL_WORDS_PER_MODULE])
{
    uint64_t x = 0;
...
NL_INLINE int nl_mod_index(int x, int mu)
{
    x %= mu;
    return x < 0 ? x + mu : x;
}
...
static void nl_update(uint32_t S[NL_MAX_MU][NL_WORDS_PER_MODULE], const NL_Params *p, uint32_t round)
{
    ...
    /* Every module is independent in this phase.  Keeping the input/output
     * arrays disjoint exposes instruction-level parallelism to -O3. */
    for (int i = 0; i < mu; i++)
    {
    ...
    for (int i = 0; i < mu; i++)
    {
        memcpy(T[i], S[i] + 1, (NL_WORDS_PER_MODULE - 1U) * sizeof(uint32_t));
                          ...
                          const uint8_t K[NL_MAX_K_BYTES])
{
    ...
    {
        size_t off = (size_t)w * 4U;
        const uint8_t *src = off < v_bytes ? V + off : K + off - v_bytes;
        S[w / NL_WORDS_PER_MODULE][w % NL_WORDS_PER_MODULE] = nl_load32_be(src);
    }
    (void)k_bytes;
}
                                  ...
                                  uint8_t block[NL_MAX_K_BYTES])
{
    ...
    /* The API benchmark and virtually all callers use byte-sized messages.
     * In that common case the length field is independent of the chaining
     * value and can be written as eight bytes instead of testing 64 bits. */
    if ((length_start & 7ULL) == 0ULL && length_start >= block_start &&
        length_start + NL_LENGTH_FIELD_BITS <= block_end)
    {
        size_t off = (size_t)((length_start - block_start) >> 3);
        nl_store32_be(block + off, (uint32_t)(msg_len_bits >> 32));
        nl_store32_be(block + off + 4U, (uint32_t)msg_len_bits);
    }
    else if (length_start < block_end && length_start + NL_LENGTH_FIELD_BITS > block_start)
    {
...
int CryptHash(int digest_len_bits, const unsigned char *NL_RESTRICT msg,
              unsigned long long msg_len_bits, unsigned char *NL_RESTRICT digest)
{
    NL_Params p;
