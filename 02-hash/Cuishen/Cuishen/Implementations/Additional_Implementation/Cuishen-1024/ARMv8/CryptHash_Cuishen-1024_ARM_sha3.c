...
ARM NEON/SHA3-XAR optimized Cuishen-1024 implementation.
*/
...
#error "CryptHash_Cuishen-1024-c-neon.c requires ARM SHA3/XAR support"
#endif
 ...
 */
#define CUISHEN1024_PAIR_ROUND_NEON()                                        \
  do {                                                                       \
...
#undef CUISHEN1024_PAIR_ROUND_NEON
...
static inline void store_pair_digest_be(const uint64x2_t state[8],
  ...
  store_u64x2_be(digest + 96, vzip2q_u64(state[4], state[5]));
  store_u64x2_be(digest + 112, vzip2q_u64(state[6], state[7]));
}
...
int cuishen1024_arm_sha3_impl(int digest_len_bits, const unsigned char *msg,
