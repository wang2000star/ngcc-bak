...
Cuishen-1024 implementation for ARM targets without SHA3/XAR.
...
#error "CryptHash_Cuishen-1024-c-neon-nosha3.c must be built without SHA3"
#endif
 ...
 */
#define CUISHEN1024_PAIR_ROUND(I)                                           \
  do {                                                                       \
...
#pragma clang loop unroll(disable)
  ...
  for (round = 0; round < ROUNDS; round += 8) {
    CUISHEN1024_PAIR_ROUND(round);
    CUISHEN1024_PAIR_ROUND(round + 1);
    CUISHEN1024_PAIR_ROUND(round + 2);
    CUISHEN1024_PAIR_ROUND(round + 3);
    CUISHEN1024_PAIR_ROUND(round + 4);
    CUISHEN1024_PAIR_ROUND(round + 5);
    CUISHEN1024_PAIR_ROUND(round + 6);
    CUISHEN1024_PAIR_ROUND(round + 7);
  }
...
#undef CUISHEN1024_PAIR_ROUND

static inline void store_digest_be(unsigned char digest[128],
  ...
  store_u64x2_be(digest + 96, make_u64x2(bottom[4], bottom[5]));
  store_u64x2_be(digest + 112, make_u64x2(bottom[6], bottom[7]));
}
...
int cuishen1024_arm_nosha3_impl(int digest_len_bits, const unsigned char *msg,
