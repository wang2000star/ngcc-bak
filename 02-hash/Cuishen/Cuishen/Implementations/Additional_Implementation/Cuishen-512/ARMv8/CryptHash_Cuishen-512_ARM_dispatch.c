...
ARM optimized Cuishen-512 implementation dispatcher.
*/
...
static int cuishen512_arm_detect_sha3(void) {
...
static int cuishen512_arm_have_sha3(void) {
#if defined(__GNUC__)
  static int cached = -1;
  int have_sha3 = __atomic_load_n(&cached, __ATOMIC_RELAXED);

  if (have_sha3 < 0) {
    have_sha3 = cuishen512_arm_detect_sha3();
    __atomic_store_n(&cached, have_sha3, __ATOMIC_RELAXED);
  }

  return have_sha3;
#else
  return cuishen512_arm_detect_sha3();
#endif
}

int CryptHash(int digest_len_bits, const unsigned char *msg,
              unsigned long long msg_len_bits, unsigned char *digest) {
  if (cuishen512_arm_have_sha3()) {
    return cuishen512_arm_sha3_impl(digest_len_bits, msg, msg_len_bits,
                                    digest);
  }

  return cuishen512_arm_nosha3_impl(digest_len_bits, msg, msg_len_bits,
                                    digest);
}
