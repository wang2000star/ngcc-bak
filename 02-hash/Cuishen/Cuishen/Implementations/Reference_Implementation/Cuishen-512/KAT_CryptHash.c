...
static int
validate_algorithm_instance_name(const char *algorithm_instance_name);
static int create_directory(const char *path);
static int progress_bar(int total, const char *test_name);
static void fprint_message_full(FILE *fp, const unsigned char *msg,
                                unsigned long long msg_len_bits);
static void fprint_message_partial(FILE *fp, const unsigned char *msg,
                                   unsigned long long msg_len_bits,
                                   const unsigned char *seed);
static void fprint_digest(FILE *fp, unsigned char *digest, int digest_len_bits);
static int gen_KAT_2_12(const char *algorithm_instance_name,
                        const int digest_len_bits);
static int gen_KAT_2_23(const char *algorithm_instance_name,
                        const int digest_len_bits);
static int gen_KAT_2_33(const char *algorithm_instance_name,
                        const int digest_len_bits);
static int gen_KAT_Loop(const char *algorithm_instance_name,
                        const int digest_len_bits);
...
int main() {
  ...
  if (validate_algorithm_instance_name(ALGORITHM_INSTANCE)) {
    fprintf(stderr, "ERROR: Invalid algorithm instance name. Only letters, "
                    "numbers, '-' or '_' are permitted.\n");
    rt = KAT_ALGORITHM_INSTANCE_NAME_INVALID;
    goto end;
  }

  // Generate the message digests for messages of lengths ranging from 0 to 2^12
  // bits
  ...
  // Generate the message digest for a message of length 2^13 bits by running
  // Loop Test
  //***********************Loop Test***********************//
  // a. h_0 = CryptHash(m_0);
  ...
  progress_bar(4, "");
  printf("\nFiles have been saved in the 'output' folder within the working "
         "directory.");
...
static void fprint_message_full(FILE *fp, const unsigned char *msg,
  ...
  if (msg_len_bits % 8) {
    fprintf(fp, "%02X",
            (HIGH_N_BIT_MASK(msg_len_bits % 8)) & (msg[msg_len_bits / 8]));
  }
  fprintf(fp, "\n");
}
...
static void fprint_digest(FILE *fp, unsigned char *digest,
                          int digest_len_bits) {
...
static int gen_KAT_2_12(const char *algorithm_instance_name,
  ...
  for (msg_len_bits = 0ULL; msg_len_bits <= MSG_LEN_BITS_2_12; msg_len_bits++) {
    ...
    if (CryptHash(digest_len_bits, msg, msg_len_bits, digest)) {
      fprintf(stderr,
              "\nERROR: \"CryptHash()\" returns a non-zero value when "
              "generating \"%s\" at %s, line %d. \n",
              filename, __FILE__, __LINE__);
      rt = KAT_CRYPTHASH_FAILED;
      goto cleanup;
    } else {
      fprint_digest(fp, OUTPUT_BLANK_TEST_VECTORS ? NULL : digest,
                    digest_len_bits);
    }
    fprintf(fp, "\n");
  }
...
static int gen_KAT_2_23(const char *algorithm_instance_name,
  ...
  if (CryptHash(digest_len_bits, msg, msg_len_bits, digest)) {
    fprintf(stderr,
            "\nERROR: \"CryptHash()\" returns a non-zero value when generating "
            "\"%s\" at %s, line %d. \n",
            filename, __FILE__, __LINE__);
    rt = KAT_CRYPTHASH_FAILED;
    goto cleanup;
...
static int gen_KAT_Loop(const char *algorithm_instance_name,
  ...
  if (CryptHash(digest_len_bits, msg, msg_len_bits, digest)) {
    ...
    for (int i = 0; i < 1000000; i++) {
      ...
      memmove(msg, msg + digest_len_bits / 8,
              (msg_len_bits - digest_len_bits) / 8);
      memcpy(msg + (msg_len_bits - digest_len_bits) / 8, buffer,
             digest_len_bits / 8);
