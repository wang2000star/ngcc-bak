...
and Cuishen-1024 in sequence. Generated files are written to Cuishen-*/output/.
...
run_one() {
  ...
  if [[ -z "$exe" ]]; then
    exe="/tmp/kat_cuishen_${bits}_x86_avx2_bmi2"
  fi

  printf '\n== Cuishen-%s x86 AVX2/BMI2 optimized KAT ==\n' "$bits"
...
printf '\nx86 AVX2/BMI2 optimized implementation KAT task finished.\n'
