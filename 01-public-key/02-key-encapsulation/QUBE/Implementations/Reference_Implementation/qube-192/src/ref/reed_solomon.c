...
void reed_solomon_encode(uint8_t *cdw, const uint8_t *msg) {
    ...
    uint16_t rs_poly[PARAM_G] = {0};

    compute_generator_poly(rs_poly);
    for (i = 0; i < PARAM_K; ++i) {
        gate_value = msg[PARAM_K - 1 - i] ^ cdw_bytes[PARAM_N1 - PARAM_K - 1];
...
static void compute_syndromes(uint16_t *syndromes, const uint8_t *cdw) {
    for (size_t i = 0; i < 2 * PARAM_DELTA; ++i) {
        for (size_t j = 1; j < PARAM_N1; ++j) {
            const uint16_t exponent = (uint16_t)(((i + 1U) * j) % PARAM_GF_MUL_ORDER);
            syndromes[i] ^= gf_mul(cdw[j], gf_exp[exponent]);
        }
        syndromes[i] ^= cdw[0];
    }
}
...
void reed_solomon_decode(uint8_t *msg, const uint8_t *cdw) {
    ...
    memset_zero(error, sizeof error);
    memset_zero(z, sizeof z);
    memset_zero(error_values, sizeof error_values);
}
