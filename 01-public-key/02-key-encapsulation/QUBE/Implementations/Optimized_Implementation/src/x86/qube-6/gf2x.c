...
#include "dencoder.h"
#include "bc_1.h"
//42283 == PARAM_N  49152 == 32768 + 16384 == 2*16384 +4*4096
#define R_FFTFORM_BYTES  (16384 + 4096) 
...
static inline void polymul_768U64_input( uint64_t * a_fft , const uint64_t * a )
{
	uint64_t fft_end[512];            // 512xu64 = 32768 bits
    memcpy( fft_end , a , 512*8 );    // 512xu64 = 32768 bits
    bc_1( fft_end , 512*8 );          // 512xu64 = 32768 bits
    encode_64(a_fft,1024,fft_end,32); // 1024 = 2*512
    btfy_64(a_fft,9+1,1ULL<<(32+9+1));// 9=log512
}

///
/// @brief input transform for multiplying two bit-polynomials of size < 49152 bits
///
/// @param a_fft [out] size : 65536 bits + 768 u64 = 1024 u64 + 768 u64
/// @param a [in] a bit-polynomial of size < 32768 + 16384 = 768 u64
static inline void polymul_49152_input( uint64_t * a_fft , const uint64_t * a )
{
    polymul_768U64_input( a_fft , a );
    uint64_t * fft_end = a_fft + 1024;  // offset 1024 u64
    memcpy( fft_end , a , 768*8 ); // remain 768u64
}
...
static inline void polymul_768U64_mul( uint64_t * c , const uint64_t * a , const uint64_t * b , const uint64_t * a_fft , const uint64_t * b_fft )
{
	uint64_t temp[1024];                       //lenth 1024 u64
    ...
    __m256i _a[192]; memcpy( _a , a , 192*32 ); //192 u256 = 768 u64
    __m256i _b[192]; memcpy( _b , b , 192*32 );
    
    const __m256i * a_poly = _a;
    const __m256i * b_poly = _b;
    const __m256i * a_rem = a_poly + 128;  //split 192 to 128 + 64
    const __m256i * b_rem = b_poly + 128; 
...

    {
        ...
        for (int j=0;j<128;j++) { _mm256_storeu_si256( cc+256+j , tmp0[j] ); } //hightest 128-degree
    }
    for( int i=0;i<128;i+=64) { //a_poly is a 128-degree polynomal 
        karat_mult_64( tmp0 , a_poly + i , b_rem );//lenth 64 u256
        karat_mult_64( tmp1 , b_poly + i , a_rem );
        for (int j=0;j<128;j++) {
            __m256i tmp = _mm256_loadu_si256( cc+128+i+j ) ^ tmp0[j]^tmp1[j]; //128 is a block = 2 64-degree multiplication
...
static inline void polymul_49152_mul( uint64_t * c , const uint64_t * a_fft , const uint64_t * b_fft )
{
	polymul_768U64_mul( c , a_fft+1024 , b_fft+1024 , a_fft , b_fft );//offset 1024u64
}
...
void ring_to_fftform( uint8_t *v_fft , const uint8_t *v )
{
    ...
    polymul_49152_input((uint64_t *)v_fft , tmp1 );
}
...
void ring_mul_fftformx2( uint8_t *c, const uint8_t *a_fft, const uint8_t *b_fft )
{
    ...
    polymul_49152_mul( temp_c ,(uint64_t *)a_fft ,(uint64_t *)b_fft );
