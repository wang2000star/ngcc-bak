...
#include "dencoder.h"
#include "bc_1.h"
//86027 == PARAM_N  98304 == 65536 + 32768 == 2*32768 +4*8192
#define R_FFTFORM_BYTES  (32768 + 8192) 
 ...
 * @param[in] B Pointer to the polynomial B(x)
 */
static inline void karat_mult_128(__m256i *C, __m256i *A, __m256i *B) {
...
static inline void polymul_1536U64_input( uint64_t * a_fft , const uint64_t * a )
{
	uint64_t fft_end[1024];             // 1024 u64
    memcpy( fft_end , a , 1024*8 );     // 1024 u64
    bc_1( fft_end , 1024*8 );           // 1024 u64
    encode_64(a_fft,2048,fft_end,32);   // 2048 = 2*1024
    btfy_64(a_fft,10+1,1ULL<<(32+10+1));// 10=log1024
}

///
/// @brief input transform for multiplying two bit-polynomials of size < 49152 bits
///
/// @param a_fft [out] size : 131072 bits + 1536 u64 = 2048 u64 + 1536 u64
/// @param a [in] a bit-polynomial of size < 65536 + 32768 = 1536 u64
static inline void polymul_98304_input( uint64_t * a_fft , const uint64_t * a )
{
    polymul_1536U64_input( a_fft , a );
    uint64_t * fft_end = a_fft + 2048;  // offset 2048 u64
    memcpy( fft_end , a , 1536*8 ); // remain 2304u64
}
...
static inline void polymul_1536U64_mul( uint64_t * c , const uint64_t * a , const uint64_t * b , const uint64_t * a_fft , const uint64_t * b_fft )
{
	uint64_t temp[2048];                       //lenth 2048 u64
    gf264v_mul( temp , a_fft , b_fft , 2048 ); 
    // output transform
    ibtfy_64( temp,10+1,1ULL<<(32+10+1));
    decode_64( c , temp , 2048 );
    ibc_1( c , 2048*8 );

    __m256i _a[384]; memcpy( _a , a , 384*32 ); //384 u64
    __m256i _b[384]; memcpy( _b , b , 384*32 );
    
    const __m256i * a_poly = _a;
    const __m256i * b_poly = _b;
    const __m256i * a_rem = a_poly + 256;  //split 384 to 256 + 128
    const __m256i * b_rem = b_poly + 256; 

	__m256i tmp0[256]; // 256 for 2 128-degree multiplication
    __m256i tmp1[256];
    __m256i * cc = (__m256i*) (c); // c=a*b 

    {
        karat_mult_128( tmp0 , a_rem , b_rem ); //lenth 128 u256
        for (int j=0;j<256;j++) { _mm256_storeu_si256( cc+512+j , tmp0[j] ); } //hightest 512-degree
    }
    for( int i=0;i<256;i+=128) { //a_poly is a 256-degree polynomal 
        karat_mult_128( tmp0 , a_poly + i , b_rem );//lenth 128 u256
        karat_mult_128( tmp1 , b_poly + i , a_rem );
        for (int j=0;j<256;j++) {
            __m256i tmp = _mm256_loadu_si256( cc+256+i+j ) ^ tmp0[j]^tmp1[j]; //128 is a block = 2 64-degree multiplication
            _mm256_storeu_si256( cc+256+i+j , tmp );
        }
    }
...
static inline void polymul_98304_mul( uint64_t * c , const uint64_t * a_fft , const uint64_t * b_fft )
{
	polymul_1536U64_mul( c , a_fft+2048 , b_fft+2048 , a_fft , b_fft );//offset 1024u64
}
...
void ring_to_fftform( uint8_t *v_fft , const uint8_t *v )
{
    ...
    polymul_98304_input((uint64_t *)v_fft , tmp1 );
}
...
void ring_mul_fftformx2( uint8_t *c, const uint8_t *a_fft, const uint8_t *b_fft )
{
    ...
    polymul_98304_mul( temp_c ,(uint64_t *)a_fft ,(uint64_t *)b_fft );
