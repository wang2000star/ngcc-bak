/**
 * @file parsing.c
 * @brief Functions to parse secret key, public key and ciphertext of the QUBE scheme
 */
...
void qube_dk_pke_from_string(uint64_t *y, const uint8_t *dk_pke) {
    ...
    vect_sample_fixed_weight1(&dk_xof_ctx, (__m256i *)y, PARAM_OMEGA_Y1);
...
void qube_ek_pke_from_string(uint64_t *h1, uint64_t *h2, uint64_t *s1, uint64_t *s2, const uint8_t *ek_pke) {
    ...
    memcpy(s1, ek_pke + SEED_BYTES, VEC_N_SIZE_BYTES);
    memcpy(s2, ek_pke + SEED_BYTES +VEC_N_SIZE_BYTES, VEC_N_SIZE_BYTES);
}
