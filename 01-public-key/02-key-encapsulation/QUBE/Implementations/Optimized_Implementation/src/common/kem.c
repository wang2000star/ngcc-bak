 ...
 * @note An example of correct seeding is provided in `main_qube.c` (see `init_randomness()`), which
 *       seeds the PRNG using `syscall(SYS_getrandom, ...)` (32 bytes) by default..
 * @see prng_init, prng_get_bytes, main_qube.c
 */
int crypto_kem_keypair(uint8_t *ek_kem, uint8_t *dk_kem) {
    ...
    prng_get_bytes(seed_pke, SEED_BYTES);
    prng_get_bytes(sigma, PARAM_SECURITY_BYTES);

    // DRNG_ctx ctx_kem;
    // init_random_number(&ctx_kem,seed_kem,SEED_BYTES*8);
    // get_random_number(&ctx_kem,seed_kem,SEED_BYTES*8);
    // get_random_number(&ctx_kem,seed_pke,SEED_BYTES*8);
    // get_random_number(&ctx_kem,sigma,PARAM_SECURITY_BYTES*8);
    ...
    // 	qube_pke_keygen(ek_pke, dk_pke, seed_pke);
    //     });
    // }
    // report(mesg,sizeof(mesg),rec_mul,len_mul[0]);
    // printf("mul: %s\n", mesg);    
    ////////////////////////////////////////////////////////////////
    // Compute QUBE-PKE keypair
    qube_pke_keygen(ek_pke, dk_pke, seed_pke);

    // Compute QUBE-KEM keypair
...
int crypto_kem_enc(uint8_t *c_kem, uint8_t *K, const uint8_t *ek_kem) {
    ...
    // Compute shared key K and ciphertext c_kem
    sm3hash(256,ek_kem,8*kem_get_pk_len_bytes(),hash_ek_kem);

    uint8_t randomness[PARAM_SECURITY_BYTES+32+SALT_BYTES];
    memcpy(randomness, m, PARAM_SECURITY_BYTES);
    memcpy(randomness+PARAM_SECURITY_BYTES, hash_ek_kem, 32);
    memcpy(randomness+PARAM_SECURITY_BYTES+32, c_kem_t.salt, SALT_BYTES);

    pseudohash((SHARED_SECRET_BYTES+SEED_BYTES)*8,randomness,8*(PARAM_SECURITY_BYTES+32+SALT_BYTES),K_theta);
    memcpy(theta, K_theta + SHARED_SECRET_BYTES, SEED_BYTES);

    qube_pke_encrypt(&c_kem_t.c_pke, ek_kem, (uint64_t *)m, theta);

    qube_c_kem_to_string(c_kem, &c_kem_t);
...
int crypto_kem_dec(uint8_t *K_prime, const uint8_t *c_kem, const uint8_t *dk_kem) {
    ...
    qube_c_kem_from_string(&c_kem_t.c_pke, c_kem_t.salt, c_kem);

    // Compute message m_prime
    result = qube_pke_decrypt((uint64_t *)m_prime, dk_pke, &c_kem_t.c_pke);
    ...
    // Compute shared key K_prime and ciphertext c_kem_prime
    sm3hash(256,ek_pke,8*kem_get_pk_len_bytes(),hash_ek_kem);
    

    uint8_t randomness[PARAM_SECURITY_BYTES+32+SALT_BYTES];
    memcpy(randomness, m_prime, PARAM_SECURITY_BYTES);
    memcpy(randomness+PARAM_SECURITY_BYTES, hash_ek_kem, 32);
    memcpy(randomness+PARAM_SECURITY_BYTES+32, c_kem_t.salt, SALT_BYTES);
    pseudohash((SHARED_SECRET_BYTES+SEED_BYTES)*8,randomness,8*(PARAM_SECURITY_BYTES+32+SALT_BYTES),K_theta_prime);
    memcpy(K_prime, K_theta_prime, SHARED_SECRET_BYTES);
    memcpy(theta_prime, K_theta_prime + SHARED_SECRET_BYTES, SEED_BYTES);
    
    qube_pke_encrypt(&c_kem_prime_t.c_pke, ek_pke, (uint64_t *)m_prime, theta_prime);
    memcpy(c_kem_prime_t.salt, c_kem_t.salt, SALT_BYTES);

    // Compute rejection key K_bar
    uint8_t Randomness[PARAM_SECURITY_BYTES+32+CRYPTO_CIPHERTEXTBYTES];
    memcpy(Randomness, sigma, PARAM_SECURITY_BYTES);
    memcpy(Randomness+PARAM_SECURITY_BYTES, hash_ek_kem, 32);
    memcpy(Randomness+PARAM_SECURITY_BYTES+32, c_kem_t.c_pke.u, VEC_N_SIZE_BYTES);
    memcpy(Randomness+PARAM_SECURITY_BYTES+32+VEC_N_SIZE_BYTES, c_kem_t.c_pke.v, VEC_N1N2_SIZE_BYTES);
    memcpy(Randomness+PARAM_SECURITY_BYTES+32+VEC_N_SIZE_BYTES+VEC_N1N2_SIZE_BYTES, c_kem_t.salt, SALT_BYTES);
    if(SHARED_SECRET_BYTES==32) sm3hash(256,Randomness,8*(PARAM_SECURITY_BYTES+32+CRYPTO_CIPHERTEXTBYTES),K_bar);
    else pseudohash(SHARED_SECRET_BYTES*8,Randomness,8*(PARAM_SECURITY_BYTES+32+CRYPTO_CIPHERTEXTBYTES),K_bar);
    
    result |= vect_compare((uint8_t *)c_kem_t.c_pke.u, (uint8_t *)c_kem_prime_t.c_pke.u, VEC_N_256_SIZE_64);
    result |= vect_compare((uint8_t *)c_kem_t.c_pke.v, (uint8_t *)c_kem_prime_t.c_pke.v, VEC_N1N2_256_SIZE_64);
