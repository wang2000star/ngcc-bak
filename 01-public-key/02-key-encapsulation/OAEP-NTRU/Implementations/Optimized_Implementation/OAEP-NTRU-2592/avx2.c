...
static inline __attribute__((always_inline))
uint32_t load32(const uint8_t *r)
{
	uint32_t v;
...
void poly_cbd1_avx2(int16_t r[NTRUOAEP_N], const uint8_t buf[NTRUOAEP_N / 4])
{
	...
	{
		uint32_t t1 = load32(buf + 32 * CBD1_GROUPS);
		uint32_t t2 = load32(buf + CBD1_HALF_BYTES + 32 * CBD1_GROUPS);

		for(int k = 0; k < 2; k++)
		{
			for(int l = 0; l < 16; l++)
			{
				r[256 * CBD1_GROUPS + 2 * l + k] = (int16_t)((t1 & 1) - (t2 & 1));
				t1 >>= 1;
				t2 >>= 1;
			}
		}
                       ...
                       const uint8_t buf[NTRUOAEP_N / 8])
{
	...
	{
		uint32_t t2 = load32(buf + 32 * CBD1_GROUPS);
		uint32_t t3 = 0;
...

	{
		...
		for(int k = 0; k < 2; k++)
		{
			for(int l = 0; l < 16; l++)
			{
				...
				t4 = (uint32_t)a[256 * CBD1_GROUPS + 2 * l + k] + t4;
				r |= t4;
				t3 ^= ((t4 ^ t1) & 1) << (l + 16 * k);
				t1 >>= 1;
				t2 >>= 1;
...
static inline __m128i trunc_8x32_to_8x16(__m256i a)
{
	const __m128i shuf = _mm_setr_epi8(
		0, 1, 4, 5, 8, 9, 12, 13,
		(char)0x80, (char)0x80, (char)0x80, (char)0x80,
		(char)0x80, (char)0x80, (char)0x80, (char)0x80);
	__m128i lo = _mm_shuffle_epi8(_mm256_castsi256_si128(a), shuf);
	__m128i hi = _mm_shuffle_epi8(_mm256_extracti128_si256(a, 1), shuf);

	return _mm_unpacklo_epi64(lo, hi);
}

static inline __m256i onetime_reduce_8x32(__m256i a)
{
	const __m256i q = _mm256_set1_epi32(NTRUOAEP_Q);
	const __m256i upper = _mm256_set1_epi32(NTRUOAEP_Q / 2 - 1);
	const __m256i lower = _mm256_set1_epi32(NTRUOAEP_Q / 2);
	const __m256i zero = _mm256_setzero_si256();
	__m256i mask;

	mask = _mm256_srli_epi32(_mm256_sub_epi32(upper, a), 31);
	mask = _mm256_sub_epi32(zero, mask);
	a = _mm256_sub_epi32(a, _mm256_and_si256(mask, q));

	mask = _mm256_srli_epi32(_mm256_add_epi32(a, lower), 31);
	mask = _mm256_sub_epi32(zero, mask);
	a = _mm256_add_epi32(a, _mm256_and_si256(mask, q));

	return a;
}

static inline __m128i onetime_reduce_8x32_to_8x16(__m256i a)
{
	return trunc_8x32_to_8x16(onetime_reduce_8x32(a));
}

static inline __m256i poly_sub_reduce_16x16(__m256i a, __m256i b)
{
	const __m256i q = _mm256_set1_epi16(NTRUOAEP_Q);
	const __m256i halfq = _mm256_set1_epi16(NTRUOAEP_Q / 2);
	const __m256i high = _mm256_set1_epi16(32767 - NTRUOAEP_Q / 2 + 1);
	const __m256i low = _mm256_set1_epi16(-32768 + NTRUOAEP_Q / 2 - 1);
	const __m256i allones = _mm256_cmpeq_epi16(q, q);
	__m256i diff = _mm256_sub_epi16(a, b);
	__m256i gt_upper;
	__m256i lt_lower;

	gt_upper = _mm256_and_si256(_mm256_cmpgt_epi16(high, b),
	                            _mm256_xor_si256(
	                                _mm256_cmpgt_epi16(_mm256_add_epi16(b, halfq), a),
	                                allones));
	lt_lower = _mm256_and_si256(_mm256_cmpgt_epi16(b, low),
	                            _mm256_cmpgt_epi16(_mm256_sub_epi16(b, halfq), a));

	diff = _mm256_sub_epi16(diff, _mm256_and_si256(gt_upper, q));
	diff = _mm256_add_epi16(diff, _mm256_and_si256(lt_lower, q));
...
static inline __m256i centered_reduce_8x32(__m256i a)
{
	const __m256i q = _mm256_set1_epi32(NTRUOAEP_Q);
	const __m256i upper = _mm256_set1_epi32(NTRUOAEP_Q / 2 - 1);
	const __m256i lower = _mm256_set1_epi32(-(NTRUOAEP_Q / 2));
	...
	for(int i = 0; i < 2; i++)
	{
		__m256i ge_upper = _mm256_cmpgt_epi32(a, upper);
		__m256i lt_lower = _mm256_cmpgt_epi32(lower, a);

		a = _mm256_sub_epi32(a, _mm256_and_si256(ge_upper, q));
		a = _mm256_add_epi32(a, _mm256_and_si256(lt_lower, q));
	}
                   ...
                   const int16_t b[NTRUOAEP_N])
{
	...
	for(i = 0; i <= NTRUOAEP_N - 64; i += 64)
	{
		...
		_mm256_storeu_si256((__m256i *)(r + i), poly_sub_reduce_16x16(av0, bv0));
		_mm256_storeu_si256((__m256i *)(r + i + 16), poly_sub_reduce_16x16(av1, bv1));
		_mm256_storeu_si256((__m256i *)(r + i + 32), poly_sub_reduce_16x16(av2, bv2));
		_mm256_storeu_si256((__m256i *)(r + i + 48), poly_sub_reduce_16x16(av3, bv3));
	}
	for(; i <= NTRUOAEP_N - 16; i += 16)
	{
		__m256i av = _mm256_loadu_si256((const __m256i *)(a + i));
		__m256i bv = _mm256_loadu_si256((const __m256i *)(b + i));
		_mm256_storeu_si256((__m256i *)(r + i), poly_sub_reduce_16x16(av, bv));
	}
	for(; i < NTRUOAEP_N; i++)
		r[i] = onetime_reduce((int32_t)a[i] - b[i]);
}
...
void poly_tomontgomery_avx2(int16_t r[NTRUOAEP_N], const int16_t a[NTRUOAEP_N])
{
	...
	{
		__m256i av = _mm256_loadu_si256((const __m256i *)(a + i));
		_mm256_storeu_si256((__m256i *)(r + i), fqmul_const_16x16(av, -2920));
	}
	for(; i < NTRUOAEP_N; i++)
		r[i] = montgomery_reduce((int32_t)a[i] * -2920);
}
