...
void poly_cbd1(poly *r, const uint8_t buf[NTRUOAEP_N/4])
{
	...
	for(int i = 0; i < 5; i++)
	{
		for(int j = 0; j < 8; j++)
		{
			...
			t2 = load32_littleendian(buf + 32*i + 4*j + 162);
	...
	t1 = (uint32_t)buf[160] | ((uint32_t)buf[161] << 8);
	t2 = (uint32_t)buf[322] | ((uint32_t)buf[323] << 8);

	for (int j = 0; j < 16; j++) {
		r->coeffs[1280 + j] = (t1 & 0x1) - (t2 & 0x1);
		t1 >>= 1;
		t2 >>= 1;
	}
}
...
int poly_cbd1_inv(uint8_t *w, const poly *a, const uint8_t buf[NTRUOAEP_N/8]) {
	...
	t2 = (uint32_t)buf[160] | ((uint32_t)buf[161] << 8);
	t3 = 0;

	for (int j = 0; j < 16; j++) {
		t4 = t2 & 0x1;
		t4 = t4 - a->coeffs[1280 + j];
		r |= t4;
		t4 = t4 & 0x1;
		t3 ^= t4 << j;
		t2 >>= 1;
...
int poly_baseinv(poly *r, const poly *a)
{
	...
	{
		result = baseinv(r->coeffs + 4*i, a->coeffs + 4*i, zetas[324 + i]);
		if(result) return 1;
		result = baseinv(r->coeffs + 4*i + 2, a->coeffs + 4*i + 2, -zetas[324 + i]);
		if(result) return 1;
	 }
...
void poly_basemul(poly *r, const poly *a, const poly *b)
{
	for(int i = 0; i < NTRUOAEP_N/4; ++i)
	{
		basemul(r->coeffs + 4*i, a->coeffs + 4*i, b->coeffs + 4*i, zetas[324 + i]);
		basemul(r->coeffs + 4*i + 2, a->coeffs + 4*i + 2, b->coeffs + 4*i + 2, -zetas[324 + i]);
	}
...
void poly_basemul_add(poly *r, const poly *a, const poly *b, const poly *c)
{
	for(int i = 0; i < NTRUOAEP_N/4; ++i)
	{
		basemul_add(r->coeffs + 4*i, a->coeffs + 4*i, b->coeffs + 4*i, c->coeffs + 4*i, zetas[324 + i]);
		basemul_add(r->coeffs + 4*i + 2, a->coeffs + 4*i + 2, b->coeffs + 4*i + 2, c->coeffs + 4*i + 2, -zetas[324 + i]);
	}
}
...
void poly_tomontgomery(poly *r, const poly *a)
{
	for (int i = 0; i < NTRUOAEP_N; i++)
	{
		r->coeffs[i] = montgomery_reduce(a->coeffs[i]*-3797);
	}
}
