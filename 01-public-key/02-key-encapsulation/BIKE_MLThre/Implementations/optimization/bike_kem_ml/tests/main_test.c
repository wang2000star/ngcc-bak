...
int main(void)
{
  ...
  printf("Security target: %d-bit", SECURITY_BITS);
  printf(" (internal BIKE level %d)\n", LEVEL);

  if(mlthre_sampling_init_from_env() != 0) {
    printf("Failed to initialize ML threshold sampling.\n");
    return EXIT_FAILURE;
  }
  ...
  for(size_t i = 1; i <= NUM_OF_TESTS; ++i) {
    ...
    printf("Code test: %lu\n", i);
    mlthre_sampling_set_sample((uint32_t)i);
    ...
    print("Initiator's generated key (K) = ", (uint64_t *)k_enc.val,
          SIZEOF_BITS(k_enc.val));
    print("Responder's computed key (K)  = ", (uint64_t *)k_dec.val,
          SIZEOF_BITS(k_enc.val));
  }

  mlthre_sampling_close();
