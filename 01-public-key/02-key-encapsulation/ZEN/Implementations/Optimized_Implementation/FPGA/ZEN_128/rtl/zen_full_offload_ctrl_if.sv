module zen_full_offload_ctrl_if (
  ...
  function automatic logic [31:0] task_len_default(
    ...
    begin
      if (is_terminal_task_cmd(cmd_word)) begin
        task_len_default = zen_accel_pkg::ZEN_N;
      end else begin
  ...
  function automatic logic [31:0] task_out_bytes0_default(
    ...
    begin
      unique case (cmd_word)
        CMD_RUN_PKE_KEYGEN: task_out_bytes0_default = zen_swift_profile_pkg::SWIFT_INDCPA_PUBLICKEY_LEN_BYTES;
        CMD_RUN_PKE_ENC:    task_out_bytes0_default = zen_swift_profile_pkg::SWIFT_INDCPA_CIPHERTEXT_LEN_BYTES;
        CMD_RUN_PKE_DEC:    task_out_bytes0_default = zen_swift_profile_pkg::SWIFT_INDCPA_MSG_LEN_BYTES;
  ...
  function automatic logic [31:0] task_out_bytes1_default(
    ...
    begin
      unique case (cmd_word)
        CMD_RUN_PKE_KEYGEN: task_out_bytes1_default = zen_swift_profile_pkg::SWIFT_F_NTT_PACK +
                                                      zen_swift_profile_pkg::SWIFT_F2_PACK;
          ...
          16'h0000: begin
            ...
            if (wr_data[0]) begin
              ...
              if (is_terminal_task_cmd(cmd_raw_reg)) begin
                if (!terminal_slots_ok_i) begin
                  status_reg[2] <= 1'b1;
                  task_errcode_reg <= ERR_SLOT_SET_MISMATCH;
                  task_out_bytes0_reg <= 32'h0;
                  task_out_bytes1_reg <= 32'h0;
                end else begin
                  task_out_bytes0_reg <= task_out_bytes0_default(cmd_raw_reg);
                  task_out_bytes1_reg <= task_out_bytes1_default(cmd_raw_reg);
                end
              end else begin
                task_out_bytes0_reg <= 32'h0;
                task_out_bytes1_reg <= 32'h0;
  ...
  always_comb begin
    host_buf_limit = ZEN_N;
    host_seed_limit = SEEDBUF_BYTES;
  ...
  always_comb begin
    ...
    start_pulse = start_req && (!terminal_task_cmd || terminal_slots_ok_i);
    ...
    len = terminal_task_cmd ? task_len_default(cmd_raw_reg) : len_raw_reg;
