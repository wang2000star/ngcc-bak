module zen_buf_mgr (
  ...
  input  zen_accel_pkg::zen_row_addr_t wr_addr,
  input  logic [zen_accel_pkg::COEFF_W-1:0] wr_data,
  input  logic [2:0]                rd0_buf_sel,
  input  zen_accel_pkg::zen_row_addr_t rd0_addr,
  output logic [zen_accel_pkg::COEFF_W-1:0] rd0_data,
  input  logic [2:0]                rd1_buf_sel,
  input  zen_accel_pkg::zen_row_addr_t rd1_addr,
  output logic [zen_accel_pkg::COEFF_W-1:0] rd1_data,
  input  logic [2:0]                vec_rd0_buf_sel,
  output zen_accel_pkg::zen_buf_row_t vec_rd0_row_data,
  input  logic [2:0]                vec_rd1_buf_sel,
  output zen_accel_pkg::zen_buf_row_t vec_rd1_row_data,
  input  zen_accel_pkg::zen_row_addr_t vec_rd_row,
  input  logic                      vec_wr_en,
  input  logic [2:0]                vec_wr_buf_sel,
  input  zen_accel_pkg::zen_row_addr_t vec_wr_row,
  input  zen_accel_pkg::zen_buf_row_t vec_wr_data
);

  import zen_accel_pkg::*;
  localparam int DEPTH = ZEN_N;
...
`ifdef SYNTHESIS
  generate
    ...
    for (g_bank = 0; g_bank < BUF_BANKS; g_bank++) begin : g_bank_store
      zen_buf_mgr_bank_store u_bank_store (
        .clk(clk),
...
`else
  always_ff @(posedge clk) begin
    ...
    for (i = 0; i < BUF_BANKS; i++) begin
      vec_rd0_row_data[i*COEFF_W +: COEFF_W] <= mem[vec_rd0_buf_sel][i][vec_rd_index];
      vec_rd1_row_data[i*COEFF_W +: COEFF_W] <= mem[vec_rd1_buf_sel][i][vec_rd_index];
    end
  end
`endif
...
module zen_buf_mgr_sdp_ram (
  input  logic                    clk,
  input  logic                    wr_en,
  input  logic [($clog2(zen_accel_pkg::BANK_DEPTH) > 0 ? $clog2(zen_accel_pkg::BANK_DEPTH) : 1)-1:0] wr_addr,
  input  logic [zen_accel_pkg::COEFF_W-1:0] wr_data,
  input  logic [($clog2(zen_accel_pkg::BANK_DEPTH) > 0 ? $clog2(zen_accel_pkg::BANK_DEPTH) : 1)-1:0] rd_addr,
  output logic [zen_accel_pkg::COEFF_W-1:0] rd_data
);
...
module zen_buf_mgr_bank_store (
  ...
  input  logic [2:0]              scalar_wr_buf_sel,
  input  logic [($clog2(zen_accel_pkg::BANK_DEPTH) > 0 ? $clog2(zen_accel_pkg::BANK_DEPTH) : 1)-1:0] scalar_wr_index,
  input  logic [zen_accel_pkg::COEFF_W-1:0] scalar_wr_data,
  input  logic                    vec_wr_en,
  input  logic [2:0]              vec_wr_buf_sel,
  input  logic [($clog2(zen_accel_pkg::BANK_DEPTH) > 0 ? $clog2(zen_accel_pkg::BANK_DEPTH) : 1)-1:0] vec_wr_index,
  input  logic [zen_accel_pkg::COEFF_W-1:0] vec_wr_data,
  input  logic [2:0]              rd0_buf_sel,
  input  logic [($clog2(zen_accel_pkg::BANK_DEPTH) > 0 ? $clog2(zen_accel_pkg::BANK_DEPTH) : 1)-1:0] rd0_index,
  output logic [zen_accel_pkg::COEFF_W-1:0] rd0_data,
  input  logic [2:0]              rd1_buf_sel,
  input  logic [($clog2(zen_accel_pkg::BANK_DEPTH) > 0 ? $clog2(zen_accel_pkg::BANK_DEPTH) : 1)-1:0] rd1_index,
  output logic [zen_accel_pkg::COEFF_W-1:0] rd1_data,
  input  logic [2:0]              vec_rd0_buf_sel,
  input  logic [($clog2(zen_accel_pkg::BANK_DEPTH) > 0 ? $clog2(zen_accel_pkg::BANK_DEPTH) : 1)-1:0] vec_rd0_index,
  output logic [zen_accel_pkg::COEFF_W-1:0] vec_rd0_data,
  input  logic [2:0]              vec_rd1_buf_sel,
  input  logic [($clog2(zen_accel_pkg::BANK_DEPTH) > 0 ? $clog2(zen_accel_pkg::BANK_DEPTH) : 1)-1:0] vec_rd1_index,
  output logic [zen_accel_pkg::COEFF_W-1:0] vec_rd1_data
);

  import zen_accel_pkg::*;
  localparam int BUF_SEL_W = 3;
  localparam int BANK_INDEX_W = (BANK_DEPTH <= 1) ? 1 : $clog2(BANK_DEPTH);
  ...
  generate
    for (g_buf = 0; g_buf < BUF_COUNT; g_buf++) begin : g_buf_ram
      ...
      zen_buf_mgr_sdp_ram u_vec0_ram (
        .clk(clk),
        .wr_en(bank_wr_en && (bank_wr_buf_sel == BUF_SEL_W'(g_buf))),
        .wr_addr(bank_wr_index),
        .wr_data(bank_wr_data),
      ...
      zen_buf_mgr_sdp_ram u_vec1_ram (
        .clk(clk),
        .wr_en(bank_wr_en && (bank_wr_buf_sel == BUF_SEL_W'(g_buf))),
        .wr_addr(bank_wr_index),
        .wr_data(bank_wr_data),
