        ...
        PH_INIT_F_LOAD: begin
          if (idx_local < active_n4_reg) begin
            init_f_chunk[lane_idx] =
              vec_a_data[idx_local*COEFF_W] ^
              vec_a_data[(idx_local + active_n4_reg)*COEFF_W] ^
              vec_a_data[(idx_local + (2 * active_n4_reg))*COEFF_W] ^
              vec_a_data[(idx_local + (3 * active_n4_reg))*COEFF_W];
          end
        end
        PH_INIT_PFX1: begin
          if (idx_local < active_n4_reg) begin
