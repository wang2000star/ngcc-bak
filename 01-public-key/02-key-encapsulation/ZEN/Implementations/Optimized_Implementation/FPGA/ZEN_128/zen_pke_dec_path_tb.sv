...
module zen_pke_dec_path_tb;
  ...
  localparam int NTT_N2 = 256;
  localparam int NTT_N4 = 128;
...
`endif
  ...
  task automatic poll_done;
    ...
    begin : wait_done_block
      for (tries = 0; tries < 800; tries++) begin
...
`endif
      poll_done();
      if (!prep_checked || !ct_decomp_checked || !ntt_checked ||
          !basemul_checked || !intt_checked || !t0_checked ||
          !a_mod2_checked || !mp0_checked || !t2_checked ||
          !mp1_checked) begin
        $display("TB ERROR: %0s stage checks incomplete prep=%0b ct_decomp=%0b ntt=%0b basemul=%0b intt=%0b t0=%0b a_mod2=%0b mp0=%0b t2=%0b mp1=%0b",
                 name, prep_checked, ct_decomp_checked, ntt_checked,
                 basemul_checked, intt_checked, t0_checked,
...
`else
    for (i = 0; i < NTT_N; i++) begin
      host_buf_write(3'd0, i[8:0], {8'h00, ct_packed_mem[i]});
      host_buf_write(3'd1, i[8:0], 16'd0);
      host_buf_write(3'd5, i[8:0], 16'd0);
    end
    for (i = 0; i < SK_PACK_BYTES; i++) begin
      if (i < NTT_N) begin
        host_buf_write(3'd1, i[8:0], {8'h00, sk_packed_mem[i]});
      end else begin
...
`endif
    ...
    launch_cmd("PKE_DEC", 8'h90, 32'h8000_0005, 32'd512, 3'd0, 3'd1, 3'd2);
