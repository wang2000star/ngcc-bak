proc usage {} {
  puts "usage: vivado -mode batch -source vivado_xcvu37p_impl.tcl -tclargs <top> <clock_port> <part> <period_ns> <out_dir> <run_name> <stage> ?<max_threads>?"
...
lassign $argv top_name clock_port part_name clk_period_ns out_dir run_name stage max_threads profile
...
zen_read_verilog_with_defines $rtl_files $profile xcvu37p
read_xdc $xdc_path

set synth_args [list -top $top_name -part $part_name -flatten_hierarchy none]
if {$stage eq "ooc"} {
...
puts $metrics_fh "profile=${profile}"
