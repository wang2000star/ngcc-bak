...
void pke_dec(unsigned char *sk, unsigned char *ct, unsigned char *m)
{
    ...
    for(i = 0; i < ZEN_SWIFT_N4; i++)
    {
        ...
        mask1 = (c1 >= 0);
        c1 = ((ZEN_SWIFT_Q2 - c1) & (-mask1)) | ((ZEN_SWIFT_Q2 + c1) & (~(-mask1)));
        mask1 = (c2 >= 0);
        c2 = ((ZEN_SWIFT_Q2 - c2) & (-mask1)) | ((ZEN_SWIFT_Q2 + c2) & (~(-mask1)));
        mask1 = (c3 >= 0);
        c3 = ((ZEN_SWIFT_Q2 - c3) & (-mask1)) | ((ZEN_SWIFT_Q2 + c3) & (~(-mask1)));
        ...
        mask1 = (c0 <= c1);
        idx = (i & (-mask1)) | ((i + ZEN_SWIFT_N4) & (~(-mask1)));
        t2[idx] = t1[i] & 1;
    }
    mul_in_R2_256(t2, f2, t1);
