...
Options:
  --board <xcvu37p|xcv80>  select board family, default xcvu37p
  --swift <128>          only swift128 is supported in this workspace
...
Examples:
  run_swift_profile_path_cycles.sh --swift 128 balanced
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --swift)
      ...
      if [ "$2" != "128" ]; then
        printf 'This workspace only supports --swift 128 (got %s)\n' "$2" >&2
        exit 1
      fi
      shift 2
      ;;
