...
{
	return ZEN_SWIFT_PUBLICKEY_LEN_BYTES;
}

unsigned long long kem_get_sk_len_bytes()
{
	return ZEN_SWIFT_SECREKEY_LEN_BYTES;
}

unsigned long long kem_get_ss_len_bytes()
{
	return ZEN_SWIFT_SHAREDKEY_LEN_BYTES;
}

unsigned long long kem_get_ct_len_bytes()
{
	return ZEN_SWIFT_CIPHERTEXT_LEN_BYTES;
}
	...
	const unsigned char *seed)
{
	...
	sm3hash(ZEN_SWIFT_SYM_LEN_BYTES*8, pk, ZEN_SWIFT_INDCPA_PUBLICKEY_LEN_BYTES*8, sk+ZEN_SWIFT_INDCPA_SECREKEY_LEN_BYTES+ZEN_SWIFT_INDCPA_PUBLICKEY_LEN_BYTES);
	...
	unsigned char *ct, unsigned long long *ct_len_bytes)
{
	...
	uint8_t kr[ZEN_SWIFT_SYM_LEN_BYTES+SEED_LEN_BYTES];

	//m
	get_random_number(&drng_algorithm, buf, ZEN_SWIFT_INDCPA_MSG_LEN_BYTES*8);
	//hash(pk)
	sm3hash(ZEN_SWIFT_SYM_LEN_BYTES*8, pk, ZEN_SWIFT_INDCPA_PUBLICKEY_LEN_BYTES*8, buf+ZEN_SWIFT_INDCPA_MSG_LEN_BYTES);
	//hash(m||hash(pk))
	pseudohash((ZEN_SWIFT_SYM_LEN_BYTES+SEED_LEN_BYTES)*8, buf, (ZEN_SWIFT_INDCPA_MSG_LEN_BYTES+ZEN_SWIFT_SYM_LEN_BYTES)*8, kr);
	...
	unsigned char *ss, unsigned long long *ss_len_bytes)
{
	...
	sm3hash(ZEN_SWIFT_SYM_LEN_BYTES*8, zc, (ZEN_SWIFT_SHAREDKEY_LEN_BYTES+ZEN_SWIFT_CIPHERTEXT_LEN_BYTES)*8, ssp);
