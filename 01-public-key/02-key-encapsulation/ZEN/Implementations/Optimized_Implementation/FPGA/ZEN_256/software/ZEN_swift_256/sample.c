...
static inline uint32_t load32_littleendian(const uint8_t *a)
{
    uint32_t r;
    r  = (uint32_t)a[0];
    r |= (uint32_t)a[1] << 8;
...
{
    unsigned int i, j;
    int16_t t[ZEN_SWIFT_N * 4];

    poly_byte2bit_unpack(t, buf, ZEN_SWIFT_N * 4);

    for(i = 0; i < ZEN_SWIFT_N; i++)
    {
        r[i] = (t[i] & t[i + ZEN_SWIFT_N]) - (t[i + 2 * ZEN_SWIFT_N] & t[i + 3 * ZEN_SWIFT_N]);
    }
}
