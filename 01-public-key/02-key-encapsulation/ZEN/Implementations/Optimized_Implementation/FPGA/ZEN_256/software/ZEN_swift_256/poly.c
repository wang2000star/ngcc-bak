...
{
    unsigned int i, j;
    int16_t k[ZEN_SWIFT_N4];
    int16_t tmp[ZEN_SWIFT_N4];
    int16_t acc;
    int16_t bit;
    ...
    /*
     * Pack f[0..255] once.
     * The original code copied f into tmp_f and then mul_in_R2_256
    ...
    /*
     * Level n = 128.
     * This level needs two 64-bit words for the packed product.
     */
    for (i = 0; i < 128; i++) {
        acc = 0;
        for (j = i; j < ZEN_SWIFT_N4; j += 128) {
            acc ^= k[j];
        }
        tmp[i] = acc;
    }
...
void poly_generate_g(int16_t *a, const uint8_t *seed, uint8_t nonce)
{
    ...
    zen_pseudoXOF(ZEN_SWIFT_N*4, seed, SEED_LEN_BYTES*8, buf, nonce);
    tenary3_16(a, buf);
}
...
void poly_generate_f(int16_t *a, const uint8_t *seed, uint8_t nonce)
{
    ...
    zen_pseudoXOF(ZEN_SWIFT_N*3, seed, SEED_LEN_BYTES*8, buf, nonce);
    tenary1_8(a, buf);
}
...
void poly_generate_se(int16_t *a, const uint8_t *seed, uint8_t nonce)
{
    ...
    zen_pseudoXOF(ZEN_SWIFT_N*2, seed, SEED_LEN_BYTES*8, buf, nonce);
    cbd1(a, buf);
}
