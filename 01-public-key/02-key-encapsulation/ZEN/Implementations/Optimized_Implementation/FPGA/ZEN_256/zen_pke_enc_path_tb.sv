...
module zen_pke_enc_path_tb;
  ...
  localparam int ENC_STALL_GUARD_CYCLES = 50000;
  ...
  logic [7:0] pk_packed_mem [0:1228];
...
`endif
  ...
  task automatic poll_done;
    ...
    begin : wait_done_block
      for (tries = 0; tries < 2000; tries++) begin
  ...
  task automatic launch_cmd(
    ...
    begin
      ...
      while (!(done || error) && (done_wait < 400000)) begin
        @(posedge clk);
  ...
  initial begin
    ...
    launch_cmd("PKE_ENC", 8'h91, 32'h8000_0000 | RAW_SEED_BASE, 32'd1024, 3'd0, 3'd4, 3'd5);
