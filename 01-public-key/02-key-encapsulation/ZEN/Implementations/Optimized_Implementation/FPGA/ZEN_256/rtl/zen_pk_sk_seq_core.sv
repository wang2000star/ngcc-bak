...
module zen_pk_unpack_seq_core #(
  ...
  localparam int ACTIVE_N_CONST = zen_swift_profile_pkg::SWIFT_N;
  ...
  function automatic logic [63:0] get_pk_packed_block(
    ...
    begin
      ...
      if (block_idx < 204) begin
        ...
        if ((active_n_i == ACTIVE_N_CONST) && (active_pk_pack_bytes_i == PK_PACK_BYTES_CONST)) begin
          get_pk_packed_block = 64'd0;
        end
      end
    end
      ...
      if (start_i && !busy_o) begin
        ...
        if (!block_active_r) begin
          if (block_idx_r < total_blocks_r) begin
            ...
            coeff_idx_r <= 32'd0;
            packed_block_r <= get_pk_packed_block(PK_PACK_BYTES_CONST, ACTIVE_N_CONST, pk_bytes_vec_i, block_idx_r);
            block_active_r <= 1'b1;
          end else begin
            busy_o <= 1'b0;
            done_o <= 1'b1;
...
module zen_sk_unpack_seq_core #(
  ...
  import zen_accel_pkg::*;
  localparam int ACTIVE_N_CONST = zen_swift_profile_pkg::SWIFT_N;
  localparam int ACTIVE_N4_CONST = zen_swift_profile_pkg::SWIFT_N4;
  localparam int ACTIVE_SK_FNTT_PACK_BYTES_CONST = zen_swift_profile_pkg::SWIFT_F_NTT_PACK;
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (start_i && !busy_o) begin
        ...
        active_sk_fntt_pack_bytes_r <= ACTIVE_SK_FNTT_PACK_BYTES_CONST;
...
module zen_keypair_pack_seq_core #(
  ...
  localparam int ACTIVE_N4_CONST = zen_swift_profile_pkg::SWIFT_N4;
  localparam int PK_PACK_BYTES_CONST = zen_swift_profile_pkg::SWIFT_INDCPA_PUBLICKEY_LEN_BYTES;
  localparam int SK_PACK_BYTES_CONST =
      zen_swift_profile_pkg::SWIFT_F_NTT_PACK + zen_swift_profile_pkg::SWIFT_F2_PACK;
  localparam int PK_TAIL_BASE_WORD_IDX_CONST = 32'd612;
  localparam int PK_TAIL_TOTAL_HALFWORDS_CONST = 32'd4;
  ...
  function automatic logic [63:0] get_pk_tail_word(
    ...
    begin
      if (active_n_i == ACTIVE_N_CONST) begin
        get_pk_tail_word = x0;
      end else begin
    ...
    if (!rst_n) begin
      ...
      if (start_i && !busy_o) begin
        ...
        pk_tail_block_count_r <=
          ...
          ((ACTIVE_N_CONST / 20) * 4);
        pk_tail_base_word_idx_r <= PK_TAIL_BASE_WORD_IDX_CONST;
        pk_tail_total_halfwords_r <= PK_TAIL_TOTAL_HALFWORDS_CONST;
        ...
        case (phase_r)
          ...
          KP_PHASE_SK_F2_HI: begin
            ...
            if ((sk_f2_hi_byte_idx_r < (active_n_r / 32)) &&
                (sk_f2_ready_i ||
                 sk_f2_byte_ready(active_n4_r, sk_f2_ready_coeffs_i,
                                  sk_f2_hi_byte_idx_r))) begin
