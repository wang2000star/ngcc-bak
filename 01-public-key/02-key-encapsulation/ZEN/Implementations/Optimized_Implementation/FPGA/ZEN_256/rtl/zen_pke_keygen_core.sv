module zen_pke_keygen_core #(
  ...
  input  logic [ADDR_W-1:0] baseinv_row_idx_i,
  input  logic signed [(16*COEFF_W)-1:0] baseinv_row_data_i,
  ...
  logic [31:0] active_stream_rows;
  logic [31:0] active_f2_stream_rows;
  logic [31:0] active_n;
  ...
  assign active_stream_rows = active_n / KEYGEN_STREAM_ROW_COEFFS;
  assign active_f2_stream_rows = active_n4 / KEYGEN_STREAM_ROW_COEFFS;
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (baseinv_row_valid_i && ($unsigned(baseinv_row_idx_i) < active_stream_rows)) begin
        f_inv_ntt_row_ready_r[baseinv_row_idx_i] <= 1'b1;
        for (keygen_coeff_idx = 0;
             keygen_coeff_idx < KEYGEN_STREAM_ROW_COEFFS;
             keygen_coeff_idx = keygen_coeff_idx + 1) begin
          coeff_idx_local =
            ($unsigned(baseinv_row_idx_i) * KEYGEN_STREAM_ROW_COEFFS) + keygen_coeff_idx;
      ...
      if (!baseinv_ready && baseinv_done_i && baseinv_vec_out_valid_i) begin
        baseinv_ready <= 1'b1;
        for (keygen_row_idx = 0; keygen_row_idx < KEYGEN_VEC_ROW_COUNT; keygen_row_idx = keygen_row_idx + 1) begin
          if (keygen_row_idx < active_stream_rows) begin
            f_inv_ntt_row_ready_r[keygen_row_idx] <= 1'b1;
          end
        end
      end

      if (binary_row_valid_i && ($unsigned(binary_row_idx_i) < active_f2_stream_rows)) begin
        ...
        for (keygen_coeff_idx = 0;
             ...
             keygen_coeff_idx = keygen_coeff_idx + 1) begin
          coeff_idx_local =
            ($unsigned(binary_row_idx_i) * KEYGEN_STREAM_ROW_COEFFS) + keygen_coeff_idx;
      ...
      if (!fast_inv_ready && binary_done_i && binary_vec_out_valid_i) begin
        ...
        for (keygen_row_idx = 0; keygen_row_idx < KEYGEN_F2_ROW_COUNT; keygen_row_idx = keygen_row_idx + 1) begin
          if (keygen_row_idx < active_f2_stream_rows) begin
      ...
      if (basemul_row_valid_i && ($unsigned(basemul_row_idx_i) < active_stream_rows)) begin
        ...
        for (keygen_coeff_idx = 0;
             ...
             keygen_coeff_idx = keygen_coeff_idx + 1) begin
          coeff_idx_local =
            ($unsigned(basemul_row_idx_i) * KEYGEN_STREAM_ROW_COEFFS) + keygen_coeff_idx;
          if (coeff_idx_local < active_n) begin
  ...
  always_comb begin
    ...
    unique case (pke_keygen_state)
      ...
      PKE_KEYGEN_WRITEBACK_PK: begin
        ...
        vec_wr_row_o = pk_wr_row;
        for (wr_bank_idx = 0; wr_bank_idx < BUF_BANKS; wr_bank_idx = wr_bank_idx + 1) begin
