module zen_pke_dec_core #(
  ...
  input  logic [BUF_BANKS*COEFF_W-1:0] vec_rd0_row_data_i,
  input  logic [BUF_BANKS*COEFF_W-1:0] vec_rd1_row_data_i,
  ...
  output logic [ADDR_W-1:0] vec_rd_row_o,
  ...
  localparam int SK_UNPACK_GROUPS_PER_CYCLE = 1;
  localparam int SK_UNPACK_F2_COEFFS_PER_CYCLE = 8;
  ...
  logic msg_codec_busy;
  logic msg_codec_done;
  ...
  logic sk_unpack_f_ntt_ready;
  logic sk_unpack_f2_ready;
  logic f_ntt_ready;
  logic f2_ready;
  logic f2_prefetch_active;
  ...
  zen_msg_seq_core #(
    ...
    .clk(clk),
    .rst_n(rst_n),
    .start_i(pke_dec_state == PKE_DEC_LAUNCH_PACK),
    .op_i(2'd1),
    .profile_id_i(profile_id_i),
    .msg_bytes_vec_i('0),
    .msg_poly_vec_i(stage_vec),
    .busy_o(msg_codec_busy),
    .done_o(msg_codec_done),
    .vec_out_o(msg_codec_vec_out),
    .bytes_consumed_o(),
    .bytes_produced_o()
  );

  zen_sk_unpack_seq_core #(
    .NTT_N(NTT_N),
    .COEFF_W(COEFF_W),
    .GROUPS_PER_CYCLE(SK_UNPACK_GROUPS_PER_CYCLE),
    .F2_COEFFS_PER_CYCLE(SK_UNPACK_F2_COEFFS_PER_CYCLE)
  ) u_sk_unpack_seq_core (
    .clk(clk),
    .rst_n(rst_n),
    ...
    .sk_f_ntt_ready_o(sk_unpack_f_ntt_ready),
    .sk_f2_ready_o(sk_unpack_f2_ready),
  ...
  assign out_bytes0_o = profile_msg_bytes(profile_id_i);
    ...
    if (!rst_n) begin
      ...
      if (dec_prefetch_load) begin
        for (prefetch_bank_idx = 0; prefetch_bank_idx < BUF_BANKS; prefetch_bank_idx++) begin
          ...
          end else if ((pke_dec_state == PKE_DEC_PREFETCH_F2) || f2_prefetch_active) begin
            if (raw_sk_mode) begin
      ...
      end else if (msg_codec_done) begin
        stage_vec <= msg_codec_vec_out;
        wr_row <= '0;
      end

      if (dec_prefetch_done && (pke_dec_state == PKE_DEC_PREFETCH_AB) && !raw_sk_mode) begin
        f_ntt_ready <= 1'b1;
      end

      if (dec_prefetch_done && f2_prefetch_active) begin
        f2_ready <= 1'b1;
        f2_prefetch_active <= 1'b0;
      end

      if (sk_unpack_f_ntt_ready) begin
        f_ntt_vec <= sk_unpack_sk_f_ntt_poly_vec_out;
        f_ntt_ready <= 1'b1;
      end

      if (sk_unpack_f2_ready) begin
      ...
      if ((pke_dec_state == PKE_DEC_WAIT_CT_DECOMP) && binary_done_i && binary_vec_out_valid_i) begin
        ...
        if (wr_row + 1 < active_writeback_rows) begin
          wr_row <= wr_row + 1'b1;
          stage_vec <= stage_vec >> (BUF_BANKS * COEFF_W);
        end else begin
          wr_row <= '0;
          stage_vec <= '0;
        end
  ...
  always_comb begin
    ...
    unique case (pke_dec_state)
      ...
      PKE_DEC_WAIT_INTT: begin
        ...
        if (ntt_error_i) begin
          ...
          if (f2_ready) begin
            ...
            pke_dec_state_n = PKE_DEC_WAIT_F2_READY;
          end
        end
      end

      PKE_DEC_WAIT_F2_READY: begin
        if (f2_ready) begin
          pke_dec_state_n = PKE_DEC_LAUNCH_POST;
        end
      end
      ...
      PKE_DEC_WRITEBACK: begin
        ...
        vec_wr_buf_sel_o = buf_r_sel_i;
        vec_wr_row_o = wr_row;
        vec_wr_data_o = stage_vec[0 +: BUF_BANKS*COEFF_W];
        if (wr_row + 1 >= active_writeback_rows) begin
          busy_o = 1'b0;
          done_o = 1'b1;
          pke_dec_state_n = PKE_DEC_IDLE;
