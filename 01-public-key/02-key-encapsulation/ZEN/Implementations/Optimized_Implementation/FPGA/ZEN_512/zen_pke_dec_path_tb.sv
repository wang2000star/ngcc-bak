...
module zen_pke_dec_path_tb;
  ...
  localparam int NTT_N2 = 1024;
  localparam int NTT_N4 = 512;
  ...
  localparam int BIN_ST_RUN_DEC_POST      = 12;
...
`endif
  ...
  task automatic poll_done;
    ...
    begin : wait_done_block
      for (tries = 0; tries < 7000; tries++) begin
  ...
  task automatic launch_cmd(
    ...
    begin
      ...
      while (!(done || error)) begin
        ...
        if ((cur_bin_state == BIN_ST_RUN_DEC_POST) &&
            !t0_checked) begin
...
`endif
  ...
  task automatic read_buffer_to_mem(input [2:0] buf_sel);
    ...
    begin
      for (i = 0; i < NTT_N; i++) begin
        host_buf_read(buf_sel, i[10:0], got);
        buf_read_mem[i] = got;
      end
    end
  endtask
  ...
  task automatic compare_dec_t2(input [255:0] name);
    ...
    begin
      mismatch_count = 0;
      for (i = 0; i < NTT_N; i++) begin
        got_coeff = dut.u_dut.u_binary_core.dec_t2_vec[i*COEFF_W +: COEFF_W];
...
`endif
  ...
  initial begin
    $readmemh("testdata/swift512/dec_ct_packed.mem", ct_packed_mem);
    $readmemh("testdata/swift512/dec_sk_packed.mem", sk_packed_mem);
    $readmemh("testdata/swift512/dec_ct_decomp.mem", ct_decomp_mem);
    $readmemh("testdata/swift512/dec_f_ntt.mem", f_ntt_mem);
    $readmemh("testdata/swift512/dec_f2_vec.mem", f2_vec_mem);
    $readmemh("testdata/swift512/dec_ct_ntt.mem", ct_ntt_mem);
    $readmemh("testdata/swift512/dec_cf_ntt.mem", cf_ntt_mem);
    $readmemh("testdata/swift512/dec_cf_intt.mem", cf_intt_mem);
    $readmemh("testdata/swift512/dec_t0_transform.mem", t0_transform_mem);
    $readmemh("testdata/swift512/dec_a_mod2_vec.mem", a_mod2_mem);
    $readmemh("testdata/swift512/dec_mp0.mem", mp0_mem);
    $readmemh("testdata/swift512/dec_t2.mem", t2_mem);
    $readmemh("testdata/swift512/dec_mp1_bits.mem", mp1_bits_mem);
    $readmemh("testdata/swift512/dec_msg.mem", msg_mem);
...
`else
    for (i = 0; i < NTT_N; i++) begin
      host_buf_write(3'd0, i[10:0], {8'h00, ct_packed_mem[i]});
      host_buf_write(3'd1, i[10:0], 16'd0);
      host_buf_write(3'd5, i[10:0], 16'd0);
    end
    for (i = 0; i < SK_PACK_BYTES; i++) begin
      if (i < NTT_N) begin
        host_buf_write(3'd1, i[10:0], {8'h00, sk_packed_mem[i]});
      end else begin
...
`endif
    ...
    launch_cmd("PKE_DEC", 8'h90, 32'h8000_0005, 32'd2048, 3'd0, 3'd1, 3'd2);
