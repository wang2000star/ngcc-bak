...
int main()
{
    ...
    m = (unsigned char *)calloc(ZEN_SWIFT_INDCPA_MSG_LEN_BYTES, sizeof(unsigned char));
    pk = (unsigned char *)calloc(ZEN_SWIFT_PUBLICKEY_LEN_BYTES, sizeof(unsigned char));
    sk = (unsigned char *)calloc(ZEN_SWIFT_SECREKEY_LEN_BYTES, sizeof(unsigned char));
    ss = (unsigned char *)calloc(ZEN_SWIFT_SHAREDKEY_LEN_BYTES, sizeof(unsigned char));
    ss1 = (unsigned char *)calloc(ZEN_SWIFT_SHAREDKEY_LEN_BYTES, sizeof(unsigned char));
    ct = (unsigned char *)calloc(ZEN_SWIFT_CIPHERTEXT_LEN_BYTES, sizeof(unsigned char));
    seed = (unsigned char *)calloc(SEED_LEN_BYTES, sizeof(unsigned char));
    ap = (int16_t *)calloc(ZEN_SWIFT_N, sizeof(int16_t));
    bp = (int16_t *)calloc(ZEN_SWIFT_N, sizeof(int16_t));
    cp = (int16_t *)calloc(ZEN_SWIFT_N, sizeof(int16_t));
    ...
    alg_nonce = 0;
    poly_generate_gf(ap, seed, alg_nonce++);
    poly_ntt_mq(ap);
    for(i = 0; i < NTESTS; i++) 
    {
        t[i] = cpucycles();
    ...
    {
        ap[i] = (ap[i] & 1) ^ (ap[i + ZEN_SWIFT_N4] & 1) ^ (ap[i + 2*ZEN_SWIFT_N4] & 1) ^ (ap[i + 3*ZEN_SWIFT_N4] & 1);
    }
    for(i = 0; i < NTESTS; i++) 
    {
        t[i] = cpucycles();
    ...
    print_results("poly_generate_gf: ", t, NTESTS);

    for(i = 0; i < NTESTS; i++) 
    {
        t[i] = cpucycles();
        poly_generate_se(ap, seed, alg_nonce++);
    }
    print_results("poly_generate_se: ", t, NTESTS);
    ...
    poly_generate_gf(ap, seed, alg_nonce++);
    poly_generate_gf(bp, seed, alg_nonce++);
    poly_ntt(ap);
    poly_ntt(bp);
    poly_baseinv_ntt(cp, ap);
    poly_basemul_ntt_mq(ap, bp, cp);
    for(i = 0; i < NTESTS; i++) 
    {
        t[i] = cpucycles();
    ...
    pke_keygen(pk, sk);
    get_random_number(&drng_seed, seed, SEED_LEN_BYTES*8);
    get_random_number(&drng_seed, m, ZEN_SWIFT_INDCPA_MSG_LEN_BYTES*8);
    for(i = 0; i < NTESTS; i++) 
    {
        t[i] = cpucycles();
