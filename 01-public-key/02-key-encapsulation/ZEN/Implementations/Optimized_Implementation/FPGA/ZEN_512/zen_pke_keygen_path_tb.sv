...
module zen_pke_keygen_path_tb;
  ...
  localparam int NTT_N = 2048;
  localparam int COEFF_W = 16;
  localparam int RAW_SEED_BASE = 0;
  localparam int PK_PACK_BYTES = 2458;
  localparam int SK_PACK_BYTES = 2624;
`ifdef ZEN_BOARD_XCV80
...
`endif
  ...
  task automatic poll_done;
    ...
    begin : wait_done_block
      for (tries = 0; tries < 40000; tries++) begin
  ...
  task automatic launch_cmd(
    ...
    integer state_cycles [0:PKE_KEYGEN_WRITEBACK_SK_HI];
    integer sampler_busy_cycles;
    integer ntt_busy_cycles;
    integer baseinv_busy_cycles;
    integer binary_busy_cycles;
    integer basemul_busy_cycles;
    integer overlap_sample_baseinv_cycles;
    integer overlap_ntt_baseinv_cycles;
    integer overlap_ntt_binary_cycles;
    integer overlap_basemul_binary_cycles;
    bit f_checked;
    bit f_ntt_checked;
    bit f_inv_checked;
    bit f2_checked;
    ...
    begin
      ...
      g_ntt_checked = 1'b0;
      pkpoly_checked = 1'b0;
      sampler_busy_cycles = 0;
      ntt_busy_cycles = 0;
      baseinv_busy_cycles = 0;
      binary_busy_cycles = 0;
      basemul_busy_cycles = 0;
      overlap_sample_baseinv_cycles = 0;
      overlap_ntt_baseinv_cycles = 0;
      overlap_ntt_binary_cycles = 0;
      overlap_basemul_binary_cycles = 0;
      for (i = 0; i <= PKE_KEYGEN_WRITEBACK_SK_HI; i++) begin
        state_cycles[i] = 0;
      end
      @(posedge clk);
      while (!(busy || done || error)) begin
        ...
        if (wait_cycles > 500000) begin
          $fatal(1,
                 "TB ERROR: %0s launch timeout keygen_state=%0d sampler_state=%0d xof_state=%0d nonce_cur=%0d",
                 name,
      ...
      while (!(done || error)) begin
        ...
        if ((cur_keygen_state >= 0) && (cur_keygen_state <= PKE_KEYGEN_WRITEBACK_SK_HI)) begin
          state_cycles[cur_keygen_state] = state_cycles[cur_keygen_state] + 1;
        end
        if (dut.u_dut.u_sampler_core.busy) sampler_busy_cycles = sampler_busy_cycles + 1;
        if (dut.u_dut.u_ntt_core.busy) ntt_busy_cycles = ntt_busy_cycles + 1;
        if (dut.u_dut.u_baseinv_core.busy) baseinv_busy_cycles = baseinv_busy_cycles + 1;
        if (dut.u_dut.u_binary_core.busy) binary_busy_cycles = binary_busy_cycles + 1;
        if (dut.u_dut.u_basemul_core.busy) basemul_busy_cycles = basemul_busy_cycles + 1;
        if (dut.u_dut.u_sampler_core.busy && dut.u_dut.u_baseinv_core.busy) begin
          overlap_sample_baseinv_cycles = overlap_sample_baseinv_cycles + 1;
        end
        if (dut.u_dut.u_ntt_core.busy && dut.u_dut.u_baseinv_core.busy) begin
          overlap_ntt_baseinv_cycles = overlap_ntt_baseinv_cycles + 1;
        end
        if (dut.u_dut.u_ntt_core.busy && dut.u_dut.u_binary_core.busy) begin
          overlap_ntt_binary_cycles = overlap_ntt_binary_cycles + 1;
        end
        if (dut.u_dut.u_basemul_core.busy && dut.u_dut.u_binary_core.busy) begin
          overlap_basemul_binary_cycles = overlap_basemul_binary_cycles + 1;
        end
        ...
        if (cmd_cycles > 500000) begin
          $fatal(1,
                 "TB ERROR: %0s completion timeout keygen_state=%0d sampler_state=%0d xof_state=%0d nonce_cur=%0d",
                 name,
                 dut.u_dut.u_pke_keygen_core.pke_keygen_state,
                 dut.u_dut.u_sampler_core.state,
                 dut.u_dut.u_sampler_core.u_seed_xof_core.state,
      ...
      if (!f_checked || !f_ntt_checked || !f_inv_checked || !f2_checked ||
        ...
        $fatal(1,
               "TB ERROR: %0s stage checks incomplete f=%0b f_ntt=%0b f_inv=%0b f2=%0b g=%0b g_ntt=%0b pkpoly=%0b",
               name, f_checked, f_ntt_checked, f_inv_checked, f2_checked,
               g_checked, g_ntt_checked, pkpoly_checked);
      end
      check_keygen_stage_counters();
      $display("TB INFO: %0s state_cycles WAIT_SAMPLE_F=%0d CHECK_F_Z2=%0d WAIT_NTT_F=%0d CHECK_F_ZQ=%0d WAIT_BASEINV_F=%0d WAIT_FAST_INV_F=%0d WAIT_SAMPLE_G=%0d WAIT_NTT_G=%0d CHECK_G_ZQ=%0d WAIT_BASEMUL=%0d WAIT_PREP_OUT=%0d WRITEBACK_PK=%0d WRITEBACK_SK_LO=%0d WRITEBACK_SK_HI=%0d",
               name,
               state_cycles[PKE_KEYGEN_WAIT_SAMPLE_F],
               state_cycles[PKE_KEYGEN_CHECK_F_Z2],
               state_cycles[PKE_KEYGEN_WAIT_NTT_F],
               state_cycles[PKE_KEYGEN_CHECK_F_ZQ],
               state_cycles[PKE_KEYGEN_WAIT_BASEINV_F],
               state_cycles[PKE_KEYGEN_WAIT_FAST_INV_F],
               state_cycles[PKE_KEYGEN_WAIT_SAMPLE_G],
               state_cycles[PKE_KEYGEN_WAIT_NTT_G],
               state_cycles[PKE_KEYGEN_CHECK_G_ZQ],
               state_cycles[PKE_KEYGEN_WAIT_BASEMUL],
               state_cycles[PKE_KEYGEN_WAIT_PREP_OUT],
               state_cycles[PKE_KEYGEN_WRITEBACK_PK],
               state_cycles[PKE_KEYGEN_WRITEBACK_SK_LO],
               state_cycles[PKE_KEYGEN_WRITEBACK_SK_HI]);
      $display("TB INFO: %0s core_busy sampler=%0d ntt=%0d baseinv=%0d binary=%0d basemul=%0d overlap(sample,baseinv)=%0d overlap(ntt,baseinv)=%0d overlap(ntt,binary)=%0d overlap(basemul,binary)=%0d",
               name,
               sampler_busy_cycles,
               ntt_busy_cycles,
               baseinv_busy_cycles,
               binary_busy_cycles,
               basemul_busy_cycles,
               overlap_sample_baseinv_cycles,
               overlap_ntt_baseinv_cycles,
               overlap_ntt_binary_cycles,
               overlap_basemul_binary_cycles);
  ...
  task automatic compare_keygen_vec16(
    ...
    begin
      ...
      if (mismatch_count != 0) begin
        $fatal(1, "TB ERROR: %0s mismatch count=%0d", name, mismatch_count);
      end
    end
  endtask
  ...
  initial begin
    $readmemh("testdata/swift512/keygen_seed.mem", keygen_seed_mem);
    $readmemh("testdata/swift512/keygen_f_nonce.mem", keygen_f_nonce_mem);
    $readmemh("testdata/swift512/keygen_g_nonce.mem", keygen_g_nonce_mem);
    $readmemh("testdata/swift512/keygen_f.mem", f_mem);
    $readmemh("testdata/swift512/keygen_f_nttmq.mem", f_nttmq_mem);
    $readmemh("testdata/swift512/keygen_f_inv_ntt.mem", f_inv_ntt_mem);
    $readmemh("testdata/swift512/keygen_f2_vec.mem", f2_vec_mem);
    $readmemh("testdata/swift512/keygen_g.mem", g_mem);
    $readmemh("testdata/swift512/keygen_g_nttmq.mem", g_nttmq_mem);
    $readmemh("testdata/swift512/keygen_pkpoly.mem", pkpoly_mem);
    $readmemh("testdata/swift512/keygen_pk_packed.mem", pk_packed_mem);
    $readmemh("testdata/swift512/keygen_sk_packed.mem", sk_packed_mem);
    ...
    launch_cmd("PKE_KEYGEN", 8'h92, 32'h8000_0000 | RAW_SEED_BASE, 32'd2048, 3'd0, 3'd1, 3'd5);
