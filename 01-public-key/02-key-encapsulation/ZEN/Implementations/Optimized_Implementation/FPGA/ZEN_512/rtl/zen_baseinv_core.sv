      ...
      case (state)
        ...
        ST_BLOCK16_STORE: begin
          if (lane_active_reg[0] && (active_block_coeffs == 16)) begin
            ...
            poly_r_flat[(base + 0)*COEFF_W +: COEFF_W] <= block16_get(block_res16, chunk_base + 0);
            poly_r_flat[(base + 1)*COEFF_W +: COEFF_W] <= block16_get(block_res16, chunk_base + 1);
            poly_r_flat[(base + 2)*COEFF_W +: COEFF_W] <= block16_get(block_res16, chunk_base + 2);
            poly_r_flat[(base + 3)*COEFF_W +: COEFF_W] <= block16_get(block_res16, chunk_base + 3);
          end
          if (block16_chunk_idx == BLOCK16_SCRATCH_CHUNKS - 1) begin
            block16_chunk_idx <= '0;
          end else begin
          ...
          if (active_block_coeffs != 16) begin
            for (i = 0; i < MAX_EXEC_SUBBLOCK_LANES; i++) begin
              if (lane_active_reg[i]) begin
                ...
                if (active_block_coeffs == 4) begin
                  poly_r_flat[(base + 0)*COEFF_W +: COEFF_W] <= fqmul(fqmul(r0pre_reg[i], lane_qinv[i]), 16'sd19);
                  poly_r_flat[(base + 1)*COEFF_W +: COEFF_W] <= fqmul(fqmul(r1pre_reg[i], lane_qinv[i]), 16'sd19);
                  poly_r_flat[(base + 2)*COEFF_W +: COEFF_W] <= fqmul(fqmul(r2pre_reg[i], lane_qinv[i]), 16'sd19);
                  poly_r_flat[(base + 3)*COEFF_W +: COEFF_W] <= fqmul(fqmul(r3pre_reg[i], lane_qinv[i]), 16'sd19);
                end else begin
                  logic signed [127:0] block_res;
                  block_res = lane8_block_res_flat[i*BLOCK8_BITS +: BLOCK8_BITS];
                  poly_r_flat[(base + 0)*COEFF_W +: COEFF_W] <= block_get(block_res, 0);
                  poly_r_flat[(base + 1)*COEFF_W +: COEFF_W] <= block_get(block_res, 1);
                  poly_r_flat[(base + 2)*COEFF_W +: COEFF_W] <= block_get(block_res, 2);
                  poly_r_flat[(base + 3)*COEFF_W +: COEFF_W] <= block_get(block_res, 3);
                  poly_r_flat[(base + 4)*COEFF_W +: COEFF_W] <= block_get(block_res, 4);
                  poly_r_flat[(base + 5)*COEFF_W +: COEFF_W] <= block_get(block_res, 5);
                  poly_r_flat[(base + 6)*COEFF_W +: COEFF_W] <= block_get(block_res, 6);
                  poly_r_flat[(base + 7)*COEFF_W +: COEFF_W] <= block_get(block_res, 7);
                end
              end
            end
          end
