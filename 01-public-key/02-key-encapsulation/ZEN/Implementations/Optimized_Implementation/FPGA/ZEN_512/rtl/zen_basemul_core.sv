module zen_basemul_core #(
  parameter int BLOCK_LANES = zen_accel_pkg::BASEMUL_LANES,
  ...
  logic mq_mode;
  logic active_profile_supported;
...
`endif
  ...
  logic signed [NTT_N*COEFF_W-1:0] poly_r_flat;
  ...
  logic [31:0] active_pack_rows;
  logic [PACK_ROW_W-1:0] active_pack_last_row;
  genvar g;
  integer i;
  ...
  assign mq_mode = (cmd == CMD_BASEMUL_MQ);

  always_comb begin
    active_block4 = (profile_id_i == PROFILE_SWIFT128);
    active_block8 = (profile_id_i == PROFILE_SWIFT256);
    active_block16 = (profile_id_i == PROFILE_SWIFT512);
    active_block_coeffs = 0;
    if (active_block4) active_block_coeffs = 4;
    else if (active_block8) active_block_coeffs = 8;
    else if (active_block16) active_block_coeffs = 16;
    active_block_supported = active_block4 ||
                             (COMPILE_BLOCK8 && active_block8) ||
                             (COMPILE_BLOCK16 && active_block16);
    active_profile_supported = profile_valid(profile_id_i) &&
                               (profile_n(profile_id_i) <= NTT_N) &&
                               active_block_supported;
    ...
    active_pack_rows = profile_n(profile_id_i) / PACK_ROW_COEFFS;
    if (active_pack_rows == 0) begin
      active_pack_rows = 1;
    end else if (active_pack_rows > PACK_ROW_COUNT) begin
      active_pack_rows = PACK_ROW_COUNT;
    end
    active_pack_last_row = active_pack_rows - 1;
  end
  ...
  generate
    for (g = 0; g < EXEC_SUBBLOCK_LANES; g++) begin : gen_basemul_lane_blocks
      ...
      zen_basemul4_post_block #(
        ...
        .block_i(lane4_block_res_flat[g*BLOCK4_BITS +: BLOCK4_BITS]),
        .mq_mode_i(mq_mode),
          ...
          for (i = 0; i < EXEC_SUBBLOCK_LANES; i++) begin
            ...
            if (i < active_lane_count) begin
              ...
              if (active_block4) begin
                ...
                c0_reg[i] <= fqmul(a0, b0);
                c1_reg[i] <= fqmul(a1, b1);
                c2_reg[i] <= fqmul(a2, b2);
                c3_reg[i] <= fqmul(a3, b3);
                p13_reg[i] <= fqmul(a1 + a3, b1 + b3);
                p23_reg[i] <= fqmul(a2 + a3, b2 + b3);
                p01_reg[i] <= fqmul(a0 + a1, b0 + b1);
                p02_reg[i] <= fqmul(a0 + a2, b0 + b2);
                p03_reg[i] <= fqmul(a0 + a3, b0 + b3);
                p12_reg[i] <= fqmul(a1 + a2, b1 + b2);
          ...
          for (i = 0; i < EXEC_SUBBLOCK_LANES; i++) begin
            if (lane_active_reg[i]) begin
              if (active_block4) begin
                ...
                block_res4 = lane4_post_block_flat[i*BLOCK4_BITS +: BLOCK4_BITS];
                poly_r_flat[(base4 + 0)*COEFF_W +: COEFF_W] <= block4_get(block_res4, 0);
                poly_r_flat[(base4 + 1)*COEFF_W +: COEFF_W] <= block4_get(block_res4, 1);
                poly_r_flat[(base4 + 2)*COEFF_W +: COEFF_W] <= block4_get(block_res4, 2);
                poly_r_flat[(base4 + 3)*COEFF_W +: COEFF_W] <= block4_get(block_res4, 3);
              end else if (active_block8) begin
                integer base8;
                logic signed [127:0] block_res;
                base8 = lane_subidx_reg[i] * 8;
                block_res = lane8_post_block_flat[i*BLOCK8_BITS +: BLOCK8_BITS];
                poly_r_flat[(base8 + 0)*COEFF_W +: COEFF_W] <= block_get(block_res, 0);
                poly_r_flat[(base8 + 1)*COEFF_W +: COEFF_W] <= block_get(block_res, 1);
                poly_r_flat[(base8 + 2)*COEFF_W +: COEFF_W] <= block_get(block_res, 2);
                poly_r_flat[(base8 + 3)*COEFF_W +: COEFF_W] <= block_get(block_res, 3);
                poly_r_flat[(base8 + 4)*COEFF_W +: COEFF_W] <= block_get(block_res, 4);
                poly_r_flat[(base8 + 5)*COEFF_W +: COEFF_W] <= block_get(block_res, 5);
                poly_r_flat[(base8 + 6)*COEFF_W +: COEFF_W] <= block_get(block_res, 6);
                poly_r_flat[(base8 + 7)*COEFF_W +: COEFF_W] <= block_get(block_res, 7);
              end
            end
          end
        ...
        ST_BLOCK16_REDUCE: begin
          for (i = 0; i < EXEC_SUBBLOCK_LANES; i++) begin
            if (lane_active_reg[i] && active_block16) begin
              ...
              poly_r_flat[(base16 + block16_reduce_coeff_base + 0)*COEFF_W +: COEFF_W] <= coeff0;
              poly_r_flat[(base16 + block16_reduce_coeff_base + 1)*COEFF_W +: COEFF_W] <= coeff1;
            end
          end
        ...
        ST_PACK_ROWS: begin
          ...
          for (pack_coeff_idx = 0; pack_coeff_idx < PACK_ROW_COEFFS; pack_coeff_idx++) begin
            row_out_data[pack_coeff_idx*COEFF_W +: COEFF_W] <=
              poly_r_flat[((pack_row_idx * PACK_ROW_COEFFS) + pack_coeff_idx)*COEFF_W +: COEFF_W];
          end
          if (pack_row_idx >= active_pack_last_row) begin
            pack_row_idx <= '0;
          end else begin
    ...
    unique case (state)
      ST_IDLE: begin
        ...
        if (start) begin
          ...
          else if (!active_profile_supported) state_n = ST_ERROR;
      ...
      ST_C_STAGE: begin
        if (active_block16) state_n = ST_MUL_PHASE0;
      ...
      ST_TWIDDLE: begin
        if (({{8-$clog2(EXEC_SUBBLOCK_LANES){1'b0}}, twiddle_lookup_idx} + 8'd1) >= active_lane_count) state_n = ST_ZETA_WRITE;
      end

      ST_ZETA_WRITE: begin
        if (active_block16) state_n = ST_BLOCK16_REDUCE;
        else if (batch_last) state_n = mq_mode ? ST_PACK_ROWS : ST_DONE;
        else state_n = ST_C_STAGE;
      end

      ST_BLOCK16_REDUCE: begin
        if (block16_reduce_last_group) begin
          if (batch_last) state_n = mq_mode ? ST_PACK_ROWS : ST_DONE;
          else state_n = ST_C_STAGE;
        end else begin
      ...
      ST_PACK_ROWS: begin
        if (pack_row_idx >= active_pack_last_row) state_n = ST_DONE;
      end
