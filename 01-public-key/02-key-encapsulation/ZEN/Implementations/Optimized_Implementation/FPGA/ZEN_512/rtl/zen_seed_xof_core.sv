module zen_seed_xof_core #(
  parameter int MAX_BYTES = zen_accel_pkg::SAMPLE_MAX_BYTES
) (
  input  logic clk,
  input  logic rst_n,
  input  logic start,
  ...
  always_comb begin
    seed_cache_hit = prefix_cache_valid;
    for (cmp_idx = 0; cmp_idx < RAW_SEED_BYTES; cmp_idx = cmp_idx + 1) begin
      if (raw_seed_cache[cmp_idx] != prefix_seed_cache[cmp_idx]) begin
        seed_cache_hit = 1'b0;
      end
    end
  end
...
`ifdef VERILATOR
        raw_seed_cache[idx] = 8'h00;
        prefix_seed_cache[idx] = 8'h00;
`else
        raw_seed_cache[idx] <= 8'h00;
        prefix_seed_cache[idx] <= 8'h00;
`endif
...
`ifdef VERILATOR
        digest_bytes[idx] = 8'h00;
`else
        digest_bytes[idx] <= 8'h00;
`endif
      end
      for (idx = 0; idx < 68; idx = idx + 1) begin
`ifdef VERILATOR
        w_mem[idx] = 32'h0000_0000;
`else
        w_mem[idx] <= 32'h0000_0000;
`endif
      end
