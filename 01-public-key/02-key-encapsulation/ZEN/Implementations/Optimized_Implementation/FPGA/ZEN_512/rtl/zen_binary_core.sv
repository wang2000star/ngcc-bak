module zen_binary_core #(
  ...
  localparam int FIXED_COL_FACTOR = COL_FACTOR;
  ...
  // The common 16/32-lane builds consume scratch rows in full-line strides,
  // so decode/A_MOD2 never need the cross-line fallback mux tree.
  localparam bit SCRATCH_DIRECT_LANE_MAP =
      (BINARY_XOR_PHASE_LANES == SCRATCH_LINE_COEFFS) &&
      ((LOCAL_N2 % SCRATCH_LINE_COEFFS) == 0) &&
      ((LOCAL_N4 % SCRATCH_LINE_COEFFS) == 0);
  localparam bit SCRATCH_PAIR_DIRECT_MAP =
      ((LOCAL_N2 % SCRATCH_LINE_COEFFS) == 0);
  localparam int FAST_INV_CORE_CHUNK_LANES = (LOCAL_N4 >= 32) ? 32 : LOCAL_N4;
  ...
  (* ram_style = "block" *) logic [SCRATCH_LINE_W-1:0] vec_a_line_reg0 [0:SCRATCH_LINE_COUNT-1];
  (* ram_style = "block" *) logic [SCRATCH_LINE_W-1:0] vec_a_line_reg1 [0:SCRATCH_LINE_COUNT-1];
  (* ram_style = "block" *) logic [SCRATCH_LINE_W-1:0] vec_b_line_reg [0:SCRATCH_LINE_COUNT-1];
  (* ram_style = "block" *) logic [SCRATCH_LINE_W-1:0] vec_r_line_reg [0:SCRATCH_LINE_COUNT-1];
  ...
  logic [31:0] fast_inv_init_f_chunk;
  logic [ADDR_W:0] fast_inv_init_f_work_idx;
  ...
  logic [SCRATCH_LINE_ADDR_W-1:0]       unary_a_rd0_addr;
  logic [SCRATCH_LINE_ADDR_W-1:0]       unary_a_rd1_addr;
  logic [SCRATCH_LINE_ADDR_W-1:0]       unary_a_rd1_next_addr;
  logic [SCRATCH_LINE_ADDR_W-1:0]       unary_b_rd_addr;
  logic [SCRATCH_LINE_W-1:0]            unary_a_rd0_data_reg;
  logic [SCRATCH_LINE_W-1:0]            unary_a_rd1_data_reg;
  logic [SCRATCH_LINE_W-1:0]            unary_a_rd1_next_data_reg;
  logic [SCRATCH_LINE_W-1:0]            unary_b_rd_data_reg;
  logic                                 msg_pack_prep_pending;
  logic [ADDR_W:0]                      msg_pack_prep_idx;
  logic [7:0]                           msg_pack_byte_cache [0:ZEN_MSG_BYTES-1];
...
`endif
  ...
  genvar line_g;
  generate
    for (line_g = 0; line_g < SCRATCH_LINE_COUNT; line_g++) begin : gen_vec_r_line_pack
      assign vec_r_data[line_g*SCRATCH_LINE_W +: SCRATCH_LINE_W] = vec_r_line_reg[line_g];
    end
  endgenerate
  ...
  always_comb begin
    ...
    fast_inv_init_f_chunk = '0;

    for (lane_idx = 0; lane_idx < FAST_INV_CORE_CHUNK_LANES; lane_idx++) begin
      coeff_idx0 = $unsigned(fast_inv_init_f_work_idx) + lane_idx;
      coeff_idx1 = coeff_idx0 + LOCAL_N4;
      coeff_idx2 = coeff_idx0 + (2 * LOCAL_N4);
      coeff_idx3 = coeff_idx0 + (3 * LOCAL_N4);
      line_idx0 = 0;
      line_idx1 = 0;
      line_idx2 = 0;
      line_idx3 = 0;
      ...
      if (coeff_idx0 < LOCAL_N4) begin
        line_idx0 = coeff_idx0 / SCRATCH_LINE_COEFFS;
        line_idx1 = coeff_idx1 / SCRATCH_LINE_COEFFS;
        line_idx2 = coeff_idx2 / SCRATCH_LINE_COEFFS;
        line_idx3 = coeff_idx3 / SCRATCH_LINE_COEFFS;
        line_off0 = coeff_idx0 % SCRATCH_LINE_COEFFS;
        line_off1 = coeff_idx1 % SCRATCH_LINE_COEFFS;
        line_off2 = coeff_idx2 % SCRATCH_LINE_COEFFS;
        line_off3 = coeff_idx3 % SCRATCH_LINE_COEFFS;
        fast_inv_init_f_chunk[lane_idx] =
          vec_a_line_reg0[line_idx0][line_off0*COEFF_W] ^
          vec_a_line_reg0[line_idx1][line_off1*COEFF_W] ^
          vec_a_line_reg0[line_idx2][line_off2*COEFF_W] ^
          vec_a_line_reg0[line_idx3][line_off3*COEFF_W];
      end
    end
  end
  ...
  zen_binary_fast_inv_core #(
    ...
    .init_f_chunk_i (fast_inv_init_f_chunk),
    .done_o         (fast_inv_done),
    .init_f_work_idx_o(fast_inv_init_f_work_idx),
...
`endif
  ...
  always_comb begin
    integer lane_idx;
    integer row_idx;
    integer row_line_idx0;
    integer row_line_off0;
    integer row_line_idx1;
    integer row_line_off1;
        ...
        unique case (binary_chunk_op)
          BINARY_CHUNK_OP_R2: begin
            if (state == ST_RUN_DEC_POST_R2) begin
              ...
              row_line_idx0 = row_idx / SCRATCH_LINE_COEFFS;
              row_line_off0 = row_idx % SCRATCH_LINE_COEFFS;
              binary_chunk_row_lo_en_cache[lane_idx] =
                vec_a_line_reg0[row_line_idx0][row_line_off0*COEFF_W];
            end
          end
          BINARY_CHUNK_OP_R2_FROM_T0: begin
            row_line_idx0 = row_idx / SCRATCH_LINE_COEFFS;
            row_line_off0 = row_idx % SCRATCH_LINE_COEFFS;
            row_line_idx1 = (row_idx + binary_chunk_active_ring_n) / SCRATCH_LINE_COEFFS;
            row_line_off1 = (row_idx + binary_chunk_active_ring_n) % SCRATCH_LINE_COEFFS;
            binary_chunk_row_lo_en_cache[lane_idx] =
              vec_a_line_reg0[row_line_idx0][row_line_off0*COEFF_W] ^
              vec_a_line_reg1[row_line_idx1][row_line_off1*COEFF_W];
          end
  ...
  always_comb begin
    integer lane_idx;
    integer coeff_idx;
    integer pair_coeff_idx;
    integer byte_idx;
    integer bit_idx;
    integer src_idx;
    integer base_idx;
    integer coeff_line_idx0;
    integer coeff_line_off0;
    integer coeff_line_idx1;
    integer coeff_line_off1;
    logic signed [15:0] coeff_word;
    logic [7:0] packed_byte;
    logic t0_pair_cross_half_chunk;
    ...
    for (lane_idx = 0; lane_idx < XOR_LANES; lane_idx++) begin
      ...
      byte_idx = 0;
      bit_idx = 0;
      t0_pair_cross_half_chunk = 1'b0;

      unique case (unary_stage_op)
        STAGE_OP_CT_DECOMP: begin
          if (coeff_idx < active_n) begin
            coeff_line_off0 = coeff_idx % SCRATCH_LINE_COEFFS;
            coeff_word = unary_a_rd0_data_reg[coeff_line_off0*COEFF_W +: COEFF_W];
        ...
        STAGE_OP_MSG_UNPACK: begin
          if (coeff_idx < active_n) begin
            ...
            byte_idx = src_idx >>> 3;
            bit_idx = src_idx & 7;
            coeff_line_off0 = byte_idx % SCRATCH_LINE_COEFFS;
            unary_src0_capture[lane_idx*COEFF_W +: COEFF_W] =
              unary_a_rd0_data_reg[coeff_line_off0*COEFF_W +: COEFF_W];
            unary_byte_capture[lane_idx*8 +: 8] = {5'd0, bit_idx[2:0]};
          end
        end
        STAGE_OP_MSG_PACK: begin
          if (coeff_idx < active_msg_bytes) begin
            unary_byte_capture[lane_idx*8 +: 8] = msg_pack_byte_cache[coeff_idx];
          end
        end
        STAGE_OP_T0: begin
          if (coeff_idx < active_n) begin
            if (SCRATCH_PAIR_DIRECT_MAP) begin
              if (coeff_idx < active_n2) begin
                coeff_line_off0 = coeff_idx % SCRATCH_LINE_COEFFS;
                coeff_line_off1 = (coeff_idx + active_n2) % SCRATCH_LINE_COEFFS;
                unary_src0_capture[lane_idx*COEFF_W +: COEFF_W] =
                  unary_a_rd1_data_reg[coeff_line_off1*COEFF_W +: COEFF_W];
              ...
              t0_pair_cross_half_chunk =
                ($unsigned(col_ptr) < active_n2) &&
                (($unsigned(col_ptr) + XOR_LANES) > active_n2);
              if (t0_pair_cross_half_chunk) begin
                if (coeff_idx < active_n2) begin
                  coeff_line_idx0 = coeff_idx / SCRATCH_LINE_COEFFS;
                  coeff_line_off0 = coeff_idx % SCRATCH_LINE_COEFFS;
                  coeff_line_idx1 = (coeff_idx + active_n2) / SCRATCH_LINE_COEFFS;
                  coeff_line_off1 = (coeff_idx + active_n2) % SCRATCH_LINE_COEFFS;
                  unary_src0_capture[lane_idx*COEFF_W +: COEFF_W] =
                    vec_a_line_reg1[coeff_line_idx1][coeff_line_off1*COEFF_W +: COEFF_W];
                  unary_src1_capture[lane_idx*COEFF_W +: COEFF_W] =
                    vec_a_line_reg0[coeff_line_idx0][coeff_line_off0*COEFF_W +: COEFF_W];
                end else begin
                  coeff_line_idx0 = coeff_idx / SCRATCH_LINE_COEFFS;
                  coeff_line_off0 = coeff_idx % SCRATCH_LINE_COEFFS;
                  coeff_line_idx1 = (coeff_idx - active_n2) / SCRATCH_LINE_COEFFS;
                ...
                coeff_line_off0 = coeff_idx % SCRATCH_LINE_COEFFS;
                if (coeff_idx < active_n2) begin
                  pair_coeff_idx = coeff_idx + active_n2;
                  coeff_line_off1 = pair_coeff_idx % SCRATCH_LINE_COEFFS;
                  coeff_line_idx1 = pair_coeff_idx / SCRATCH_LINE_COEFFS;
                  unary_src0_capture[lane_idx*COEFF_W +: COEFF_W] =
                    ((coeff_line_idx1 == $unsigned(unary_a_rd1_addr)) ?
                      unary_a_rd1_data_reg[coeff_line_off1*COEFF_W +: COEFF_W] :
                      unary_a_rd1_next_data_reg[coeff_line_off1*COEFF_W +: COEFF_W]);
                  unary_src1_capture[lane_idx*COEFF_W +: COEFF_W] =
                    unary_a_rd0_data_reg[coeff_line_off0*COEFF_W +: COEFF_W];
                end else begin
                  pair_coeff_idx = coeff_idx - active_n2;
        ...
        STAGE_OP_POLY_ADD: begin
          if (coeff_idx < active_n) begin
            ...
            unary_src0_capture[lane_idx*COEFF_W +: COEFF_W] =
              unary_a_rd0_data_reg[coeff_line_off0*COEFF_W +: COEFF_W];
            unary_src1_capture[lane_idx*COEFF_W +: COEFF_W] =
              unary_b_rd_data_reg[coeff_line_off0*COEFF_W +: COEFF_W];
          end
        end
  ...
  always_comb begin
    ...
    for (lane_idx = 0; lane_idx < XOR_LANES; lane_idx++) begin
      ...
      unique case (unary_stage_op)
        ...
        STAGE_OP_DEC_A_MOD2: begin
          if (coeff_idx < active_n2) begin
            if (SCRATCH_DIRECT_LANE_MAP) begin
              src0_word = dec_post_t0_rd0_data_reg[lane_idx*COEFF_W +: COEFF_W];
              src1_word = dec_post_t0_rd2_data_reg[lane_idx*COEFF_W +: COEFF_W];
            end else begin
              ...
              if (src0_off < SCRATCH_LINE_COEFFS) begin
                ...
                src0_word =
                  dec_post_t0_rd1_data_reg[(src0_off - SCRATCH_LINE_COEFFS)*COEFF_W +:
                                           COEFF_W];
              end
              ...
              if (src1_off < SCRATCH_LINE_COEFFS) begin
                ...
                src1_word =
                  dec_post_t0_rd3_data_reg[(src1_off - SCRATCH_LINE_COEFFS)*COEFF_W +:
                                           COEFF_W];
              end
            end
  ...
  always_comb begin
    int unsigned prefetch_col_base_u;
    int unsigned msg_unpack_coeff_base_u;
    int unsigned msg_unpack_byte_base_u;
    int unsigned pair_base_u;
    int unsigned pair_line_u;
    int unsigned pair_next_line_u;
    logic pair_cross_half_chunk;

    prefetch_col_base_u = $unsigned(col_ptr);
    msg_unpack_coeff_base_u = 0;
    msg_unpack_byte_base_u = 0;
    pair_base_u = 0;
    pair_line_u = 0;
    pair_next_line_u = 0;
    pair_cross_half_chunk = 1'b0;

    unary_a_rd0_addr = '0;
    unary_a_rd1_addr = '0;
    unary_a_rd1_next_addr = '0;
    unary_b_rd_addr = '0;
    ...
    if (state == ST_PREP) begin
      unique case (cmd_reg)
        ...
        CMD_MSG_UNPACK: begin
          unary_a_rd0_addr = '0;
        end
        CMD_T0_TRANSFORM,
        CMD_DEC_POSTPROC: begin
          unary_a_rd0_addr = '0;
          pair_base_u = active_n2;
          pair_line_u = pair_base_u / SCRATCH_LINE_COEFFS;
          pair_next_line_u = (((pair_line_u + 1) < SCRATCH_LINE_COUNT) ?
                              (pair_line_u + 1) : pair_line_u);
          unary_a_rd1_addr = SCRATCH_LINE_ADDR_W'(pair_line_u);
          unary_a_rd1_next_addr = SCRATCH_LINE_ADDR_W'(pair_next_line_u);
        end
        CMD_A_MOD2: begin
          unary_a_rd0_addr = '0;
          pair_base_u = active_n2;
      ...
      endcase
    end else if (unary_state_active) begin
      if (binary_write_pending && (($unsigned(col_ptr) + XOR_LANES) < unary_write_limit)) begin
        prefetch_col_base_u = $unsigned(col_ptr) + XOR_LANES;
      end

      unique case (unary_stage_op)
        STAGE_OP_CT_DECOMP: begin
          unary_a_rd0_addr = SCRATCH_LINE_ADDR_W'(prefetch_col_base_u / SCRATCH_LINE_COEFFS);
        end
        STAGE_OP_MSG_UNPACK: begin
          msg_unpack_coeff_base_u = prefetch_col_base_u & (active_n4 - 1);
          msg_unpack_byte_base_u = msg_unpack_coeff_base_u >>> 3;
          unary_a_rd0_addr = SCRATCH_LINE_ADDR_W'(msg_unpack_byte_base_u / SCRATCH_LINE_COEFFS);
        end
        STAGE_OP_T0: begin
          unary_a_rd0_addr = SCRATCH_LINE_ADDR_W'(prefetch_col_base_u / SCRATCH_LINE_COEFFS);
          pair_cross_half_chunk =
            !SCRATCH_PAIR_DIRECT_MAP &&
            (prefetch_col_base_u < active_n2) &&
            ((prefetch_col_base_u + XOR_LANES) > active_n2);
          if (pair_cross_half_chunk) begin
            unary_a_rd1_addr = '0;
            unary_a_rd1_next_addr = '0;
          end else begin
            if (prefetch_col_base_u < active_n2) begin
              pair_base_u = prefetch_col_base_u + active_n2;
            end else begin
              pair_base_u = prefetch_col_base_u - active_n2;
            end
            pair_line_u = pair_base_u / SCRATCH_LINE_COEFFS;
        ...
        STAGE_OP_A_MOD2: begin
          ...
          if (prefetch_col_base_u < active_n2) begin
            ...
            unary_a_rd1_addr = unary_a_rd0_addr;
            unary_a_rd1_next_addr = unary_a_rd0_addr;
          end
        end
        STAGE_OP_POLY_ADD,
        STAGE_OP_ENC_POST: begin
          unary_a_rd0_addr = SCRATCH_LINE_ADDR_W'(prefetch_col_base_u / SCRATCH_LINE_COEFFS);
          unary_b_rd_addr = unary_a_rd0_addr;
        end
        default: begin
        end
      endcase
  ...
  always_comb begin
    ...
    if (binary_src_load_pending) begin
      ...
      binary_src_wr_data = vec_a_line_reg0[$unsigned(binary_src_load_addr)];
    end
    ...
    if ((state == ST_RUN_DECODE) &&
      ...
      if (!binary_chunk_cache_pending &&
        ...
        if (SCRATCH_DIRECT_LANE_MAP) begin
          binary_src_rd1_addr = SCRATCH_LINE_ADDR_W'(cur_line_u);
        end else begin
        ...
        if (SCRATCH_DIRECT_LANE_MAP) begin
          binary_src_rd3_addr = SCRATCH_LINE_ADDR_W'(cur_line_u);
        end else begin
          next_line_u = ((cur_line_u + 1) < SCRATCH_LINE_COUNT) ? (cur_line_u + 1) : cur_line_u;
  ...
  always_comb begin
    ...
    if ((state == ST_RUN_DEC_POST_A_MOD2) &&
      ...
      if (SCRATCH_DIRECT_LANE_MAP) begin
        dec_post_t0_rd1_addr = dec_post_t0_rd0_addr;
        dec_post_t0_rd3_addr = dec_post_t0_rd2_addr;
      end else begin
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      unary_a_rd0_data_reg <= '0;
      unary_a_rd1_data_reg <= '0;
      unary_a_rd1_next_data_reg <= '0;
      unary_b_rd_data_reg <= '0;
    end else begin
      unary_a_rd0_data_reg <= vec_a_line_reg0[unary_a_rd0_addr];
      unary_a_rd1_data_reg <= vec_a_line_reg1[unary_a_rd1_addr];
      unary_a_rd1_next_data_reg <= vec_a_line_reg1[unary_a_rd1_next_addr];
      unary_b_rd_data_reg <= vec_b_line_reg[unary_b_rd_addr];
    end
  end
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      row_out_idx <= '0;
      row_out_data <= '0;
      for (idx = 0; idx < SCRATCH_LINE_COUNT; idx++) begin
        vec_a_line_reg0[idx] = '0;
        vec_a_line_reg1[idx] = '0;
        vec_b_line_reg[idx] = '0;
        vec_r_line_reg[idx] = '0;
      end
      for (idx = 0; idx < ZEN_MSG_BYTES; idx++) begin
        msg_pack_byte_cache[idx] <= '0;
      end
...
`endif
      ...
      unique case (state)
        ST_IDLE: begin
          ...
          for (idx = 0; idx < SCRATCH_LINE_COUNT; idx++) begin
            vec_a_line_reg0[idx] <= vec_a_data[idx*SCRATCH_LINE_W +: SCRATCH_LINE_W];
            vec_a_line_reg1[idx] <= vec_a_data[idx*SCRATCH_LINE_W +: SCRATCH_LINE_W];
            vec_b_line_reg[idx] <= vec_b_data[idx*SCRATCH_LINE_W +: SCRATCH_LINE_W];
          end
          binary_src_load_pending <= (cmd == CMD_DECODE_HELPER);
          binary_src_load_addr <= '0;
          msg_pack_prep_pending <= (cmd == CMD_MSG_PACK);
          msg_pack_prep_idx <= '0;
            binary_chunk_cache_pending <= 1'b0;
            binary_chunk_decode_c23_pending <= 1'b0;
            binary_chunk_decode_compare_pending <= 1'b0;
          ...
          for (idx = 0; idx < LOCAL_N2; idx++) begin
            int line_idx_local;
            int line_off_local;
            line_idx_local = idx / SCRATCH_LINE_COEFFS;
            line_off_local = idx % SCRATCH_LINE_COEFFS;
            binary_chunk_b_lsb_reg[idx] <= vec_b_data[line_idx_local*SCRATCH_LINE_W +
                                                      line_off_local*COEFF_W];
          end
            binary_chunk_acc_seed_reg <= '0;
            binary_chunk_partial_lsb_reg <= '0;
            binary_chunk_partial_mid_lsb_reg <= '0;
        ...
        ST_PREP: begin
          ...
          if (msg_pack_prep_pending) begin
            int base_idx_local;
            int bit_idx_local;
            int line_idx_local;
            int line_off_local;
            logic [7:0] packed_byte_local;

            base_idx_local = $unsigned(msg_pack_prep_idx) << 3;
            packed_byte_local = '0;
            for (bit_idx_local = 0; bit_idx_local < 8; bit_idx_local++) begin
              line_idx_local = (base_idx_local + bit_idx_local) / SCRATCH_LINE_COEFFS;
              line_off_local = (base_idx_local + bit_idx_local) % SCRATCH_LINE_COEFFS;
              packed_byte_local[bit_idx_local] =
                vec_a_line_reg0[line_idx_local][line_off_local*COEFF_W];
            end
            msg_pack_byte_cache[$unsigned(msg_pack_prep_idx)] <= packed_byte_local;
            if (($unsigned(msg_pack_prep_idx) + 1) < active_msg_bytes) begin
              msg_pack_prep_idx <= msg_pack_prep_idx + 1'b1;
            end else begin
              msg_pack_prep_pending <= 1'b0;
            end
          end else begin
          ...
          for (idx = 0; idx < LOCAL_N2; idx++) begin
            ...
            line_idx_local = idx / SCRATCH_LINE_COEFFS;
            line_off_local = idx % SCRATCH_LINE_COEFFS;
            binary_chunk_b_lsb_reg[idx] <= vec_b_line_reg[line_idx_local][line_off_local*COEFF_W];
          end
          binary_chunk_acc_seed_reg <= '0;
          binary_chunk_partial_lsb_reg <= '0;
          binary_chunk_partial_mid_lsb_reg <= '0;
            ...
            if (!binary_chunk_cache_pending &&
              ...
              for (idx = 0; idx < BINARY_CHUNK_COL_LANES; idx++) begin
                if (($unsigned(binary_phase_col_base) + idx) < binary_chunk_active_acc_n) begin
                  ...
                  int line_idx_local;
                  int line_off_local;
                  coeff_idx_local = $unsigned(binary_phase_col_base) + idx;
                  line_idx_local = coeff_idx_local / SCRATCH_LINE_COEFFS;
                  line_off_local = coeff_idx_local % SCRATCH_LINE_COEFFS;
                  binary_chunk_acc_seed_reg[idx] <=
                    vec_r_line_reg[line_idx_local][line_off_local*COEFF_W];
                end else begin
                  ...
                  if (SCRATCH_DIRECT_LANE_MAP) begin
                    if (state == ST_RUN_DEC_POST_DECODE) begin
                      binary_chunk_dec_c0_reg[idx] <=
                        dec_post_t0_rd0_data_reg[idx*COEFF_W +: COEFF_W];
                      binary_chunk_dec_c1_reg[idx] <=
                        dec_post_t0_rd2_data_reg[idx*COEFF_W +: COEFF_W];
                    end else begin
                      binary_chunk_dec_c0_reg[idx] <=
                        binary_src_rd0_data_reg[idx*COEFF_W +: COEFF_W];
                      binary_chunk_dec_c1_reg[idx] <=
                        binary_src_rd2_data_reg[idx*COEFF_W +: COEFF_W];
                    end
                  end else begin
                    int unsigned c0_off;
                    int unsigned c1_off;
              ...
              for (idx = 0; idx < BINARY_CHUNK_COL_LANES; idx++) begin
                if ((((col_phase * BINARY_CHUNK_COL_LANES) + idx) < active_bin_col_lanes) &&
                  ...
                  line_idx_local = coeff_idx_local / SCRATCH_LINE_COEFFS;
                  line_off_local = coeff_idx_local % SCRATCH_LINE_COEFFS;
                  vec_r_line_reg[line_idx_local][line_off_local*COEFF_W +: COEFF_W] =
                    binary_stage_data_reg[idx*COEFF_W +: COEFF_W];
                end
              end
              binary_chunk_cache_pending <= 1'b0;
          ...
          if (fast_inv_row_out_valid) begin
            ...
            for (row_coeff_idx = 0; row_coeff_idx < 16; row_coeff_idx++) begin
              ...
              if (coeff_idx < NTT_N) begin
                int line_idx_local;
                int line_off_local;
                line_idx_local = coeff_idx / SCRATCH_LINE_COEFFS;
                line_off_local = coeff_idx % SCRATCH_LINE_COEFFS;
                vec_r_line_reg[line_idx_local][line_off_local*COEFF_W +: COEFF_W] =
              ...
              for (idx = 0; idx < XOR_LANES; idx++) begin
                if ((col_ptr + idx) < unary_write_limit) begin
                  if (unary_write_dec_a_mod2) begin
                    ...
                    int line_idx_local;
                    int line_off_local;
                    coeff_idx_local = $unsigned(col_ptr) + idx;
                    line_idx_local = coeff_idx_local / SCRATCH_LINE_COEFFS;
                    line_off_local = coeff_idx_local % SCRATCH_LINE_COEFFS;
                    vec_r_line_reg[line_idx_local][line_off_local*COEFF_W +: COEFF_W] =
                      binary_stage_data_reg[idx*COEFF_W +: COEFF_W];
    ...
    unique case (state)
      ...
      ST_PREP: begin
        if ((cmd_reg == CMD_DECODE_HELPER) && binary_src_load_pending) begin
          state_n = ST_PREP;
        end else if ((cmd_reg == CMD_MSG_PACK) && msg_pack_prep_pending) begin
          state_n = ST_PREP;
