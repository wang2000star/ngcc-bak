module zen_pke_enc_core #(
  ...
  zen_pk_unpack_seq_core #(
    .NTT_N(NTT_N),
    .COEFF_W(COEFF_W)
  ) u_pk_unpack_seq_core (
    .clk(clk),
    .rst_n(rst_n),
    .start_i(pke_enc_state == PKE_ENC_PREP),
    .profile_id_i(profile_id_i),
  ...
  assign out_bytes0_o = profile_ct_pack_bytes(profile_id_i);
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (pk_unpack_done) begin
        h_vec <= pk_unpack_pk_poly_vec_out;
        h_ready <= 1'b1;
      end

      if ((pke_enc_state == PKE_ENC_WAIT_SAMPLE_S) && sampler_done_i && sampler_vec_out_valid_i) begin
      ...
      if (((pke_enc_state == PKE_ENC_WAIT_SAMPLE_E) ||
        ...
        if (wr_row + 1 < active_writeback_rows) begin
          ...
          e_vec <= e_vec >> (BUF_BANKS * COEFF_W);
          msg_poly_vec <= msg_poly_vec >> (BUF_BANKS * COEFF_W);
        end else begin
          wr_row <= '0;
          stage_vec <= '0;
          e_vec <= '0;
    ...
    unique case (pke_enc_state)
      ...
      PKE_ENC_WAIT_SAMPLE_E: begin
        ...
        if (sampler_error_i) begin
          ...
          errcode_o = ERR_NTT;
          pke_enc_state_n = PKE_ENC_IDLE;
        end else if (sample_e_ready && s_ntt_ready && h_ready) begin
