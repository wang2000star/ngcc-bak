...
File Description: NGCC ARM self-evaluation benchmark for the optimized ZEN-256 KEM instance.
*/
...
static int prepare_fixed_kem(bench_ctx *ctx)
{
    ...
    fill_seed(ctx->derand_seed, sizeof(ctx->derand_seed), 0x256001u);
    if (reset_drng(ctx, 0x256101u) != 0) {
        return -1;
    }
...
static int functional_tests(bench_ctx *ctx, functional_summary *summary)
{
    ...
    memset(ctx, 0, sizeof(*ctx));
    if (reset_drng(ctx, 0x256301u) != 0) {
        return -1;
    }
    if (kem_keygen(ctx->pk, &pk_len, ctx->sk, &sk_len) != 0) {
        return -1;
    }
    if (reset_drng(ctx, 0x256401u) != 0) {
        return -1;
    }
    if (kem_enc(ctx->pk, pk_len, ctx->ss, &ss_len, ctx->ct, &ct_len) != 0) {
        return -1;
...
int main(int argc, char **argv)
{
    ...
    if (run_operation("kem_keygen", op_kem_keygen, 0x256501u, ctx, iterations,
                      timer_overhead, cpu_info.hz, csv, &summaries[0]) != 0) {
        status = 1;
    }
    if (run_operation("kem_keygen_derand", op_kem_keygen_derand, 0x256601u, ctx,
                      iterations, timer_overhead, cpu_info.hz, csv,
                      &summaries[1]) != 0) {
        status = 1;
    }
    if (run_operation("kem_enc", op_kem_enc, 0x256701u, ctx, iterations,
                      timer_overhead, cpu_info.hz, csv, &summaries[2]) != 0) {
        status = 1;
    }
    if (run_operation("kem_dec", op_kem_dec, 0x256801u, ctx, iterations,
