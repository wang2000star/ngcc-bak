...
#ifdef USE_KECCAK
	sha3_512(sk + ZEN_INDCPA_SECREKEY_LEN_BYTES + ZEN_INDCPA_PUBLICKEY_LEN_BYTES, pk, ZEN_INDCPA_PUBLICKEY_LEN_BYTES);
	randombytes(sk+ZEN_INDCPA_SECREKEY_LEN_BYTES+ZEN_INDCPA_PUBLICKEY_LEN_BYTES+ZEN_SYM_LEN_BYTES, ZEN_SHAREDKEY_LEN_BYTES);
#else
	pseudohash(ZEN_SYM_LEN_BYTES*8, pk, ZEN_INDCPA_PUBLICKEY_LEN_BYTES*8, sk+ZEN_INDCPA_SECREKEY_LEN_BYTES+ZEN_INDCPA_PUBLICKEY_LEN_BYTES);
	...
	unsigned char *ct, unsigned long long *ct_len_bytes)
{
	...
	uint8_t kr[2*ZEN_SYM_LEN_BYTES];
#ifdef USE_KECCAK
	randombytes(buf, ZEN_INDCPA_MSG_LEN_BYTES);
	sha3_512(buf + ZEN_INDCPA_MSG_LEN_BYTES, pk, ZEN_INDCPA_PUBLICKEY_LEN_BYTES);
	shake256(kr, 2 * ZEN_SYM_LEN_BYTES, buf, (ZEN_INDCPA_MSG_LEN_BYTES + ZEN_SYM_LEN_BYTES));
#else
	//m
	get_random_number(&drng_algorithm, buf, ZEN_INDCPA_MSG_LEN_BYTES*8);
	//hash(pk)
	pseudohash(ZEN_SYM_LEN_BYTES*8, pk, ZEN_INDCPA_PUBLICKEY_LEN_BYTES*8, buf+ZEN_INDCPA_MSG_LEN_BYTES);
	//hash(m||hash(pk))
	pseudohash(2*ZEN_SYM_LEN_BYTES*8, buf, (ZEN_INDCPA_MSG_LEN_BYTES+ZEN_SYM_LEN_BYTES)*8, kr);
#endif
	...
	unsigned char *ss, unsigned long long *ss_len_bytes)
{
	...
	uint8_t ssp[ZEN_SHAREDKEY_LEN_BYTES];
...
#ifdef USE_KECCAK
	shake256(ssp, ZEN_SHAREDKEY_LEN_BYTES, zc, (ZEN_SHAREDKEY_LEN_BYTES+ZEN_CIPHERTEXT_LEN_BYTES));
#else
	pseudoXOF(ZEN_SYM_LEN_BYTES*8, zc, (ZEN_SHAREDKEY_LEN_BYTES+ZEN_CIPHERTEXT_LEN_BYTES)*8, ssp);
#endif
