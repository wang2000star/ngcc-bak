 ...
 * 
 * 关键修正: neon_mont_reduce_4way 使用 vmovn_u32 + 掩码方式提取低16位
 * 避免 vmovn_s32 的饱和截断问题 (saturation vs truncation)
 * ======================================================================== */
#ifdef __ARM_NEON__

/// @brief NEON 4路并行 Montgomery 约简 (int32x4_t → int32x4_t)
/// @note 使用无符号窄化+掩码避免 vmovn_s32 饱和溢出
...
/// @brief NEON 8路前向 NTT 蝶形 (Cooley-Tukey): t=fqmul(zeta,a[j+len]); a[j+len]=a[j]-t; a[j]=a[j]+t
...
/* ========================================================================
 * poly_ntt — 前向 NTT (ZEN_N=2048, 所有 len 均被8整除, 无需标量尾部)
 * ======================================================================== */
#ifdef __ARM_NEON__
...
/* ========================================================================
 * poly_ntt_mq — 前向 NTT + Montgomery缩放 (ZEN_N=2048)
 * ======================================================================== */
#ifdef __ARM_NEON__
void poly_ntt_mq(int16_t *a)
{
    ...
    /* NEON 8路并行 MONT 缩放: a[j]=fqmul(a[j],171); a[j]+=(a[j]>>15)&ZEN_Q */
    {
        ...
        for(j = 0; j < ZEN_N; j += 8)
        {
            ...
            int16x8_t sign = vshrq_n_s16(a_vec, 15);
            a_vec = vaddq_s16(a_vec, vandq_s16(sign, q16_vec));
            vst1q_s16(&a[j], a_vec);
        }
    }
}
...
/* ========================================================================
 * poly_intt — 逆向 NTT + 最终缩放 (ZEN_N=2048)
 * ======================================================================== */
#ifdef __ARM_NEON__
