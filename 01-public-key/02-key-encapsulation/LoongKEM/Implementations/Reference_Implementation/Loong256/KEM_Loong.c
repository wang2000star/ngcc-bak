                    ...
                    uint8_t sk[NOISE_POLY_BYTES])
{
    ...
    encode_vector(b, pk+SEED_BYTES, K1*N, DB);
    encode_vector(B, pk+SEED_BYTES+(K1*N*DB/8), K2*N*N, DB);

    encode_noise_vector(s, sk, K1*N+K2*N, 4);
}
...
static void indcpa_encrypt(const uint8_t m[MSG_BYTES],
    ...
    decode_vector(pk+SEED_BYTES+(K1*N*DB/8), B, K2*N*N, DB);
    ...
    encode_vector(u, c, K1*N, DU);
    encode_vector(U, c+K1*N*DU/8, K2*N*N, DU);
    encode_vector(V, c+(K1*N+K2*N*N)*DU/8, N*N, DV);
}
                    ...
                    uint8_t m[MSG_BYTES]) 
{
    ...
    int16_t u[K1*N], U[K2*N*N], V[N*N];
    decode_vector(c, u, K1*N, DU);
    decode_vector(c+K1*N*DU/8, U, K2*N*N, DU);
    decode_vector(c+(K1*N+K2*N*N)*DU/8, V, N*N, DV);
