...
void encode_6bit_lsb(const int16_t* input, size_t n, uint8_t* output) {
    ...
    // Process 8 int16_t to 6 bytes per iteration (2 groups of 4)
    for (; i + 8 <= n; i += 8) {
        // Load 8 int16_t
        __m128i v = _mm_loadu_si128((const __m128i*)(input + i));
        // Mask low 6 bits
        __m128i masked = _mm_and_si128(v, _mm_set1_epi16(0x3F));

        // Store to array
        uint16_t x[8];
        _mm_storeu_si128((__m128i*)x, masked);

        uint8_t* out = output + (i / 4) * 3; // each 4 inputs to 3 bytes

        // Group 0: x[0..3]
        out[0] = (uint8_t)(x[0] | (x[1] << 6));
        out[1] = (uint8_t)((x[1] >> 2) | (x[2] << 4));
        out[2] = (uint8_t)((x[2] >> 4) | (x[3] << 2));

        // Group 1: x[4..7]
        out[3] = (uint8_t)(x[4] | (x[5] << 6));
        out[4] = (uint8_t)((x[5] >> 2) | (x[6] << 4));
        out[5] = (uint8_t)((x[6] >> 4) | (x[7] << 2));
    }

    // No tail: n is multiple of 8
}

void decode_6bit_lsb(const uint8_t* input, size_t n, int16_t* output) {
    size_t i = 0;

    // Process 8 outputs to 6 input bytes per iteration
    for (; i + 8 <= n; i += 8) {
        const uint8_t* in = input + (i / 4) * 3; // 6-byte block

        // Group 0: bytes 0-2 to output[i..i+3]
        output[i+0] = (int16_t)(in[0] & 0x3F);
        output[i+1] = (int16_t)(((in[0] >> 6) & 0x03) | ((in[1] & 0x0F) << 2));
        output[i+2] = (int16_t)(((in[1] >> 4) & 0x0F) | ((in[2] & 0x03) << 4));
        output[i+3] = (int16_t)((in[2] >> 2) & 0x3F);

        // Group 1: bytes 3-5 to output[i+4..i+7]
        output[i+4] = (int16_t)(in[3] & 0x3F);
        output[i+5] = (int16_t)(((in[3] >> 6) & 0x03) | ((in[4] & 0x0F) << 2));
        output[i+6] = (int16_t)(((in[4] >> 4) & 0x0F) | ((in[5] & 0x03) << 4));
        output[i+7] = (int16_t)((in[5] >> 2) & 0x3F);
    }
...
void encode_10bit_lsb(const int16_t *data, size_t n, uint8_t *out) {
    ...
    // Process 8 int16_t to 10 bytes per iteration (2 groups of 4)
    for (; i + 8 <= n; i += 8) {
        // Load 8 int16_t
        __m128i v = _mm_loadu_si128((const __m128i*)(data + i));
        // Mask low 10 bits
        __m128i masked = _mm_and_si128(v, _mm_set1_epi16(0x3FF));

        // Store to array
        uint16_t x[8];
        _mm_storeu_si128((__m128i*)x, masked);

        uint8_t* dst = out + (i / 4) * 5; // each 4 inputs → 5 bytes

        // Group 0: x[0..3]
        dst[0] = (uint8_t)(x[0]);
        dst[1] = (uint8_t)((x[0] >> 8) | ((x[1] & 0x3F) << 2));
        dst[2] = (uint8_t)((x[1] >> 6) | ((x[2] & 0x0F) << 4));
        dst[3] = (uint8_t)((x[2] >> 4) | ((x[3] & 0x03) << 6));
        dst[4] = (uint8_t)(x[3] >> 2);

        // Group 1: x[4..7]
        dst[5] = (uint8_t)(x[4]);
        dst[6] = (uint8_t)((x[4] >> 8) | ((x[5] & 0x3F) << 2));
        dst[7] = (uint8_t)((x[5] >> 6) | ((x[6] & 0x0F) << 4));
        dst[8] = (uint8_t)((x[6] >> 4) | ((x[7] & 0x03) << 6));
        dst[9] = (uint8_t)(x[7] >> 2);
    }

    // No scalar tail needed because n is multiple of 8
}

void decode_10bit_lsb(const uint8_t *in, size_t n, int16_t *out) {
    size_t i = 0;

    // Process 8 outputs to 10 input bytes per iteration
    for (; i + 8 <= n; i += 8) {
        const uint8_t* src = in + (i / 4) * 5; // 10-byte block

        // Load 10 bytes safely (use two 8-byte loads or just read as array)
        // Modern CPUs handle unaligned well; we use simple array access
        uint8_t b0 = src[0], b1 = src[1], b2 = src[2], b3 = src[3], b4 = src[4];
        uint8_t b5 = src[5], b6 = src[6], b7 = src[7], b8 = src[8], b9 = src[9];

        // Group 0: bytes 0-4 to out[i..i+3]
        out[i+0] = (int16_t)( ((uint16_t)b0)        | ((uint16_t)b1 << 8) ) & 0x3FF;
        out[i+1] = (int16_t)( ((uint16_t)(b1 >> 2)) | ((uint16_t)b2 << 6) ) & 0x3FF;
        out[i+2] = (int16_t)( ((uint16_t)(b2 >> 4)) | ((uint16_t)b3 << 4) ) & 0x3FF;
        out[i+3] = (int16_t)( ((uint16_t)(b3 >> 6)) | ((uint16_t)b4 << 2) ) & 0x3FF;

        // Group 1: bytes 5-9 to out[i+4..i+7]
        out[i+4] = (int16_t)( ((uint16_t)b5)        | ((uint16_t)b6 << 8) ) & 0x3FF;
        out[i+5] = (int16_t)( ((uint16_t)(b6 >> 2)) | ((uint16_t)b7 << 6) ) & 0x3FF;
        out[i+6] = (int16_t)( ((uint16_t)(b7 >> 4)) | ((uint16_t)b8 << 4) ) & 0x3FF;
        out[i+7] = (int16_t)( ((uint16_t)(b8 >> 6)) | ((uint16_t)b9 << 2) ) & 0x3FF;
    }
