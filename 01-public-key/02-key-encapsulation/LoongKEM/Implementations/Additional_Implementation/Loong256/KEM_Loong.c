                    ...
                    uint8_t sk[NOISE_POLY_BYTES])
{
    ...
    encode_10bit_lsb(b,  K1*N, pk+SEED_BYTES);
    encode_10bit_lsb(B, K2*N*N, pk+SEED_BYTES+(K1*N*DB/8));
...
static void indcpa_encrypt(const uint8_t m[MSG_BYTES],
    ...
    int16_t b[K1*N], B[K2*N*N];
    decode_10bit_lsb(pk+SEED_BYTES,  K1*N, b);
    decode_10bit_lsb(pk+SEED_BYTES+(K1*N*DB/8),  K2*N*N, B);
    ...
    encode_10bit_lsb(u, K1*N, c);
    encode_10bit_lsb(U, K2*N*N, c+K1*N*DU/8);
    encode_10bit_lsb(V, N*N, c+(K1*N+K2*N*N)*DU/8);
}
                    ...
                    uint8_t m[MSG_BYTES]) 
{
    ...
    int16_t u[K1*N], U[K2*N*N], V[N*N];
    decode_10bit_lsb(c,  K1*N, u);
    decode_10bit_lsb(c+K1*N*DU/8, K2*N*N, U);
    decode_10bit_lsb(c+(K1*N+K2*N*N)*DU/8, N*N, V);
