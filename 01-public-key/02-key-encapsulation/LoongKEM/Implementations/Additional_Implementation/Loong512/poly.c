...
void sample_vector(const uint8_t *buf, int16_t *vec, size_t len) {
    ...
    // Process input buffer in 2-byte chunks until reaching target length or processing 3x length of data
...
void encode_11bit_lsb(const int16_t* input, size_t n, uint8_t* output) {
    for (size_t i = 0; i < n; i += 8) {
        ...
        __m128i v = _mm_loadu_si128((const __m128i*)(input + i));
        // Mask to 11 bits
        v = _mm_and_si128(v, _mm_set1_epi16(0x7FF));

        // Extract to scalar (SSE2+)
        uint16_t x0 = (uint16_t)_mm_extract_epi16(v, 0);
        uint16_t x1 = (uint16_t)_mm_extract_epi16(v, 1);
        uint16_t x2 = (uint16_t)_mm_extract_epi16(v, 2);
        uint16_t x3 = (uint16_t)_mm_extract_epi16(v, 3);
        uint16_t x4 = (uint16_t)_mm_extract_epi16(v, 4);
        uint16_t x5 = (uint16_t)_mm_extract_epi16(v, 5);
        uint16_t x6 = (uint16_t)_mm_extract_epi16(v, 6);
        uint16_t x7 = (uint16_t)_mm_extract_epi16(v, 7);

        uint8_t* out = output + (i / 8) * 11;

        // Pack into 11 bytes as a continuous 88-bit stream
        uint64_t w0 = ((uint64_t)x0)      |
                      ((uint64_t)x1 << 11) |
                      ((uint64_t)x2 << 22) |
                      ((uint64_t)x3 << 33) |
                      ((uint64_t)x4 << 44) |
        ...
        uint32_t w1 = (uint32_t)(x5 >> 9) |
                      ((uint32_t)x6 << 2)  |
                      ((uint32_t)x7 << 13);

        // Store low 8 bytes
        *(uint64_t*)(out) = w0;
        // Store high 3 bytes (bytes 8,9,10)
        out[8] = (uint8_t)(w1);
        out[9] = (uint8_t)(w1 >> 8);
...
void decode_11bit_lsb(const uint8_t* input, size_t n, int16_t* output) {
    for (size_t i = 0; i < n; i += 8) {
        ...
        uint64_t w0 = *(const uint64_t*)(src);     // bits 0–63
        uint32_t w1 = *(const uint32_t*)(src + 8); // bits 64–95 (we use 64–87)

        uint16_t x0 = (w0 >> 0) & 0x7FF;
        uint16_t x1 = (w0 >> 11) & 0x7FF;
        uint16_t x2 = (w0 >> 22) & 0x7FF;
        uint16_t x3 = (w0 >> 33) & 0x7FF;
        uint16_t x4 = (w0 >> 44) & 0x7FF;
        uint16_t x5 = ((w0 >> 55) | (w1 << 9)) & 0x7FF;   // x5: bits 55–65
        uint16_t x6 = (w1 >> 2) & 0x7FF;                  // x6: bits 66–76 → w1 bit 2–12
        uint16_t x7 = (w1 >> 13) & 0x7FF;                 // x7: bits 77–87 → w1 bit 13–23

        __m128i out_vec = _mm_setr_epi16(x0, x1, x2, x3, x4, x5, x6, x7);
        _mm_storeu_si128((__m128i*)(output + i), out_vec);
    }
}
