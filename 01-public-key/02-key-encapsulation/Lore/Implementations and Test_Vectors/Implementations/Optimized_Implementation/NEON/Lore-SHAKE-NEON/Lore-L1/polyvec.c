...
* - q-part uses NTT.
* - t-part uses sparse multiplication in Z_t[X]/(X^N+1).
*
* Arguments:   - poly_crt *r:                         pointer to the output CRT polynomial
* - const poly_crt_vec *a_row_dense:       pointer to the dense matrix row
...
void gen_matrix_ntt(poly_crt_vec a[LORE_K], const unsigned char *seed, int transposed)
{
  ...
  for (unsigned int base = 0; base < total; base += 2) {
    uint8_t extseed[2][LORE_SYMBYTES + 2];
    uint8_t buf2[2][5 * SHAKE128_RATE];
    unsigned int row[2];
    unsigned int col[2];
    ...
    lore_shake128x2_absorb_once_squeezeblocks(buf2[0], buf2[1],
                                              NBLOCKS,
                                              extseed[0], extseed[1],
                                              sizeof(extseed[0]));

    for (unsigned int lane = 0; lane < 2 && base + lane < total; lane++) {
      unsigned int ctr_q = rej_uniform_q(a[row[lane]].vec[col[lane]].q_poly.coeffs,
                                         LORE_N, buf2[lane], BUF_SIZE);
