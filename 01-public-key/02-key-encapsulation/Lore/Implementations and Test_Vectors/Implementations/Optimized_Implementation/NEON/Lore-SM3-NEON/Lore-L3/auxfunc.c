...
#define L_SHIFT(a, n) ((n) ? (((a) << (n)) | (((a) & 0xFFFFFFFFU) >> (32 - (n)))) : (a))
...
int sm3hash(int digest_len_bits, const unsigned char *msg, unsigned long long msg_len_bits, unsigned char *digest)
{
	...
	{
#ifdef LORE_USE_NEON_SM3
		if (lore_sm3_hash_neon(digest, msg, msg_len_bits) == 0)
		{
			return HASH_SUCCESS;
		}
#endif
...
{
#ifdef LORE_USE_NEON_SM3
	if (lore_sm3_pseudoXOF_neon(output_len_bits, msg, msg_len_bits, output) == 0)
	{
		return XOF_SUCCESS;
	}
#endif
