...
#if LORE_R_BITS == 1
    /* Fast path: pack 8 single-bit indices into one byte */
    for (size_t i = 0; i < LORE_N; i += 8) {
        r[i / 8] = (unsigned char)(
              (t_indices[i]     & 1u)
            | ((t_indices[i+1] & 1u) << 1)
            | ((t_indices[i+2] & 1u) << 2)
...
#if LORE_R_BITS == 1
    /* Fast path: unpack one byte into 8 single-bit indices */
    for (size_t i = 0; i < LORE_N; i += 8) {
        unsigned char byte = r[i / 8];
        t_indices[i]   = (byte     ) & 1u;
        t_indices[i+1] = (byte >> 1) & 1u;
        t_indices[i+2] = (byte >> 2) & 1u;
        t_indices[i+3] = (byte >> 3) & 1u;
        t_indices[i+4] = (byte >> 4) & 1u;
        t_indices[i+5] = (byte >> 5) & 1u;
        t_indices[i+6] = (byte >> 6) & 1u;
        t_indices[i+7] = (byte >> 7) & 1u;
    }
#else
