...
#if LORE_L == 1
    /* Fast path: each coefficient contributes 1 bit; pack 8 per byte. */
    for (int j = 0; j < LORE_N; j += 8) {
        ptr[j / 8] = (unsigned char)(
              ((unsigned)cv_crt->q_poly.coeffs[j]   & 1u)
            | (((unsigned)cv_crt->q_poly.coeffs[j+1] & 1u) << 1)
            | (((unsigned)cv_crt->q_poly.coeffs[j+2] & 1u) << 2)
            | (((unsigned)cv_crt->q_poly.coeffs[j+3] & 1u) << 3)
            | (((unsigned)cv_crt->q_poly.coeffs[j+4] & 1u) << 4)
            | (((unsigned)cv_crt->q_poly.coeffs[j+5] & 1u) << 5)
            | (((unsigned)cv_crt->q_poly.coeffs[j+6] & 1u) << 6)
            | (((unsigned)cv_crt->q_poly.coeffs[j+7] & 1u) << 7));
    }
#else
...
#if LORE_T_BITS == 1
    /* Fast path: 8 single-bit t-coefficients → 1 byte */
    for (int j = 0; j < LORE_N; j += 8) {
        ptr[j / 8] = (unsigned char)(
              ((unsigned)cv_crt->t_poly.coeffs[j]   & 1u)
            | (((unsigned)cv_crt->t_poly.coeffs[j+1] & 1u) << 1)
            | (((unsigned)cv_crt->t_poly.coeffs[j+2] & 1u) << 2)
            | (((unsigned)cv_crt->t_poly.coeffs[j+3] & 1u) << 3)
            | (((unsigned)cv_crt->t_poly.coeffs[j+4] & 1u) << 4)
            | (((unsigned)cv_crt->t_poly.coeffs[j+5] & 1u) << 5)
            | (((unsigned)cv_crt->t_poly.coeffs[j+6] & 1u) << 6)
            | (((unsigned)cv_crt->t_poly.coeffs[j+7] & 1u) << 7));
    }
#elif LORE_T_BITS == 2
    /* Fast path: 4 two-bit t-coefficients → 1 byte */
    for (int j = 0; j < LORE_N; j += 4) {
        ptr[j / 4] = (unsigned char)(
              ((unsigned)cv_crt->t_poly.coeffs[j]   & 3u)
            | (((unsigned)cv_crt->t_poly.coeffs[j+1] & 3u) << 2)
            | (((unsigned)cv_crt->t_poly.coeffs[j+2] & 3u) << 4)
            | (((unsigned)cv_crt->t_poly.coeffs[j+3] & 3u) << 6));
    }
#else
    memset(ptr, 0, (LORE_N * LORE_T_BITS + 7) / 8);
    for (int j = 0; j < LORE_N; ++j) {
        uint16_t val = (uint16_t)(cv_crt->t_poly.coeffs[j] & ((1 << LORE_T_BITS) - 1));
        size_t bit_idx = (size_t)j * LORE_T_BITS;
        ptr[bit_idx / 8] |= (unsigned char)(val << (bit_idx % 8));
    }
#endif
...
static void unpack_ciphertext_ntt(poly_crt_vec *cu_ntt, poly_crt *cv_crt, const unsigned char *c) {
...
#if LORE_L == 1
    /* Fast path: each byte holds 8 bits; bit=1 → 129 (= round(257/2)), bit=0 → 0 */
    {
        const int16_t one_cv = barrett_reduce((int16_t)((LORE_Q + 1) >> 1)); /* 129 */
        for (int j = 0; j < LORE_N; j += 8) {
            unsigned char byte = ptr[j / 8];
            for (int k = 0; k < 8; k++) {
                cv_crt->q_poly.coeffs[j + k] = ((byte >> k) & 1) ? one_cv : 0;
            }
        }
    }
#else
    for (int j = 0; j < LORE_N; ++j) {
        size_t bit_idx = (size_t)j * LORE_L;
    ...
    for (int j = 0; j < LORE_N; j += 8) {
        unsigned char byte = ptr[j / 8];
        for (int k = 0; k < 8; k++) {
            cv_crt->t_poly.coeffs[j + k] = (int16_t)((byte >> k) & 1);
        }
    }
#elif LORE_T_BITS == 2
    for (int j = 0; j < LORE_N; j += 4) {
        unsigned char byte = ptr[j / 4];
        cv_crt->t_poly.coeffs[j]   = (int16_t)(byte & 3u);
        cv_crt->t_poly.coeffs[j+1] = (int16_t)((byte >> 2) & 3u);
        cv_crt->t_poly.coeffs[j+2] = (int16_t)((byte >> 4) & 3u);
        cv_crt->t_poly.coeffs[j+3] = (int16_t)((byte >> 6) & 3u);
    }
#else
    for (int j = 0; j < LORE_N; ++j) {
        size_t bit_idx = (size_t)j * LORE_T_BITS;
