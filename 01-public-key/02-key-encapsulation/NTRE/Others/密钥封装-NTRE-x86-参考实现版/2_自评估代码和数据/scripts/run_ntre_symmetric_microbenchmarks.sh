...
cc="${CC:-gcc}"
tmp_dir="${TMPDIR:-/tmp}/ntre_symmetric_microbench_$$"

bench_src="$root_dir/Benchmark_Results/ntre_symmetric_microbench.c"
csv="$out_dir/ntre_symmetric_microbench.csv"
log="$out_dir/ntre_symmetric_microbench.log"
...
run_one()
{
    ...
    local exe="$tmp_dir/ntre_symmetric_microbench_$instance"
    local sources
    ...
    "$cc" $flags -DSYMMETRIC_BENCH_DEFAULT_LOOP_COUNT="$loops" \
        -I "$dir" -I "$dir/src" \
