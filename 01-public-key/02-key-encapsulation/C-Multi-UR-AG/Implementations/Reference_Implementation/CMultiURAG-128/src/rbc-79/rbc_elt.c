...
void rbc_elt_to_string(uint8_t* str, const rbc_elt e) {
  ...
  for(size_t j = 1 ; j <= bytes2 ; j++) {
    uint8_t bit = rbc_elt_get_coefficient(e, RBC_79_FIELD_M - j);
    *(str + bytes1) |= (bit << k % 8);
    k++;
    if(k % 8 == 0) bytes1++;
  }
...
void rbc_elt_from_string(rbc_elt e, const uint8_t* str) {
  ...
  for(size_t j = 1 ; j <= bytes2 ; j++) {
    ...
    rbc_elt_set_coefficient_vartime(e, RBC_79_FIELD_M - j, bit);
    k++;
    if(k % 8 == 0) bytes1++;
  }
}
