...
void rbc_tr_mat_fq_vec_mul(rbc_vec o, const rbc_mat_fq m, const rbc_vec v, uint32_t size) {
  ...
  rbc_mat_fq_init(&mv, size, RBC_181_FIELD_M);
  rbc_mat_fq_init(&tv, RBC_181_FIELD_M, size);
  rbc_mat_fq_set_transpose(tm, m, size, size);
  for(size_t i = 0 ; i < size ; ++i) { memcpy(mv[i], v[i], (RBC_181_FIELD_M + 7) / 8); }
  rbc_mat_fq_set_transpose(tv, mv, size, RBC_181_FIELD_M);
 ...
 * \param[in] v rbc_vec
 * \param[in] m rbc_mat_fq square matrix of dimension RBC_181_FIELD_M
...
void rbc_vec_mat_fq_compress(rbc_vec basis, rbc_vec lcomb, const rbc_vec v, uint32_t size, uint32_t w) {
  ...
  rbc_vec_init(&lc, RBC_181_FIELD_M);
  rbc_vec_set_zero(lc, RBC_181_FIELD_M);
  rbc_vec_init(&bs, RBC_181_FIELD_M);
  rbc_vec_set_zero(bs, RBC_181_FIELD_M);
  for(size_t i = 0 ; i < RBC_181_FIELD_M ; ++i) {
    lc[i][i / 64] = (uint64_t)1 << (i % 64);
  }
  rbc_mat_fq v_mat, lc_mat, bs_mat, v_out;
  rbc_mat_fq_init(&v_mat, size, RBC_181_FIELD_M);
  rbc_mat_fq_set_zero(v_mat, size, RBC_181_FIELD_M);
  rbc_mat_fq_init(&lc_mat, RBC_181_FIELD_M, RBC_181_FIELD_M);
  rbc_mat_fq_set_zero(lc_mat, RBC_181_FIELD_M, RBC_181_FIELD_M);
  rbc_mat_fq_init(&bs_mat, RBC_181_FIELD_M, size);
  rbc_mat_fq_set_zero(bs_mat, RBC_181_FIELD_M, size);
  rbc_mat_fq_init(&v_out, RBC_181_FIELD_M, RBC_181_FIELD_M);
  rbc_mat_fq_set_zero(v_out, RBC_181_FIELD_M, RBC_181_FIELD_M);
  for(size_t i = 0 ; i < size ; ++i) {
    for(size_t j = 0 ; j < (RBC_181_FIELD_M+63)/64; ++j) {
      v_mat[i][j] = v[i][j];
    }
  }
  rbc_mat_fq_set_transpose(bs_mat, v_mat, size, RBC_181_FIELD_M);
  for(size_t i = 0 ; i < RBC_181_FIELD_M ; ++i) {
    for(size_t j = 0 ; j < (size+63)/64; ++j) {
      bs[i][j] = bs_mat[i][j];
    }
  }
  rbc_vec_gauss(bs, RBC_181_FIELD_M, 0, &lc, 1);
  for(size_t i = 0 ; i < RBC_181_FIELD_M ; ++i) {
    for(size_t j = 0 ; j < (size+63)/64; ++j) {
      lc_mat[i][j] = lc[i][j];
    }
  }
  rbc_mat_fq_set_inverse(lc_mat, lc_mat, RBC_181_FIELD_M);
  rbc_mat_fq_set_transpose(v_out, lc_mat, RBC_181_FIELD_M, RBC_181_FIELD_M);
  rbc_vec_set_zero(lc, RBC_181_FIELD_M);
...
void rbc_vec_mat_fq_decompress(rbc_vec v, const rbc_vec basis, const rbc_vec lcomb, uint32_t size, uint32_t w) {
  ...
  rbc_mat_fq v_mat, lc_mat, bs_mat, v_out;
  rbc_mat_fq_init(&v_mat, RBC_181_FIELD_M, size);
  rbc_mat_fq_set_zero(v_mat, RBC_181_FIELD_M, size);
  rbc_mat_fq_init(&lc_mat, RBC_181_FIELD_M, RBC_181_FIELD_M);
  rbc_mat_fq_set_zero(lc_mat, RBC_181_FIELD_M, RBC_181_FIELD_M);
  rbc_mat_fq_init(&bs_mat, RBC_181_FIELD_M, size);
  rbc_mat_fq_set_zero(bs_mat, RBC_181_FIELD_M, size);
  ...
  rbc_mat_fq_set_transpose(v_mat, bs_mat, size, RBC_181_FIELD_M);
  rbc_mat_fq_mul(v_out, v_mat, lc_mat, size, RBC_181_FIELD_M, RBC_181_FIELD_M);
  for(size_t i = 0 ; i < size ; ++i) {
    for(size_t j = 0 ; j < (RBC_181_FIELD_M+63)/64; ++j) {
      v[i][j] = v_out[i][j];
    }
