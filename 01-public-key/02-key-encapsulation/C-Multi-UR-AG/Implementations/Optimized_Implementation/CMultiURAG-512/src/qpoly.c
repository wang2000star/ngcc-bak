 ...
 */
int8_t rbc_qpoly_set_interpolate_vect_and_zero(rbc_qpoly o1, rbc_qpoly o2, const rbc_vec p1, const rbc_vec p2, int32_t size) {
  ...
  for(int32_t i = 0 ; i < size ; ++i)  {
    rbc_qpoly_evaluate(tmp1, o1, p1[i]);
    rbc_qpoly_evaluate(tmp2, o2, p1[i]);

    rbc_elt_inv(tmp3, tmp1);
    rbc_elt_add(tmp2, tmp2, p2[i]);
...
/**
 * \fn int8_t rbc_qpoly_annulator(rbc_qpoly p, const rbc_vec v, uint32_t size)
 ...
 */
int8_t rbc_qpoly_annihilator(rbc_qpoly p, const rbc_vec v, int32_t size) {
  ...
  rbc_elt p_at_vi;
  rbc_qpoly t;
  rbc_qpoly_init(&t, size);
  rbc_qpoly_set_zero(t);
  ...
  int i;
  for(i = 0; i < size; ++i) {
    rbc_qpoly_evaluate(p_at_vi, p, v[i]);   // p_i(v_i)
    rbc_qpoly_qexp(t, p);                   // (p_i^q)(x)
    rbc_qpoly_scalar_mul(p, p, p_at_vi);    // (p_i(v_i) * p_i)(x)
    rbc_qpoly_add(p, p, t);                 // (p_i^q + p_i(v_i) * p_i)(x)
  }

  rbc_elt_set_zero(p_at_vi);
  rbc_qpoly_clear(t);
...
int8_t rbc_qpoly_qexp(rbc_qpoly o, const rbc_qpoly p) {
  ...
  for(int32_t i = 0 ; i <= p->degree ; ++i) {
    ...
    rbc_elt_set(o->values[(i + 1) % RBC_181_FIELD_M], tmp);
  }
...
int8_t rbc_qpoly_mul(rbc_qpoly o, const rbc_qpoly p1, const rbc_qpoly p2) {
  ...
  for(int32_t j = 0 ; j <= p2->degree ; ++j) {
    ...
    for(int32_t i = 1 ; i <= p1->degree ; ++i) {
      ...
      rbc_elt_add(o->values[(i + j) % RBC_181_FIELD_M], o->values[(i + j) % RBC_181_FIELD_M], tmp2); 
    }
  }
