/**
 * @file polarkem_ct.c
 * @brief Ciphertext codec for the portable optimized PolarKEM-256 profile.
 */
    ...
    const int8_t signs[POLARKEM_N])
{
    ...
    unsigned char error_bits[POLARKEM_ERROR_STREAM_BYTES];
    ...
    const unsigned char mu[POLARKEM_MU_BYTES])
{
    ...
    result = build_ciphertext_with_permutation(ct, pk, mu,
                                               permutation, signs);
    ...
    const unsigned char received[POLARKEM_CT_BYTES])
{
    ...
    result = build_ciphertext_with_permutation(rebuilt, pk, mu,
                                               permutation, signs);
    memset(permutation, 0, sizeof(permutation));
    memset(signs, 0, sizeof(signs));
    return result;
