 ...
 * @brief Portable 64-bit Polar transform for the PolarKEM-128 profile.
 */
 ...
 *
 * @param[in,out] words Eight little-bit-endian 64-bit words (512 bits).
 * @return Nothing.  The same function is the inverse over GF(2).
 */
void polarkem_polar_transform(uint64_t words[POLARKEM_WORDS]);

/**
 * Insert a 16-byte message at the fixed BEC information positions and encode.
 *
 ...
 *
 * @param[out] codeword Encoded 512-bit codeword in little-bit-endian words.
...
/**
 * Invert an encoded codeword and recover its 16-byte information payload.
 *
 * @param[out] mu Recovered message, exactly POLARKEM_MU_BYTES bytes.
 * @param[in] codeword Encoded 512-bit codeword in little-bit-endian words.
