...
uint64_t t[NTESTS];
uint8_t seed[COMPASS_KEM_SYMBYTES] = {0};
...
int main(void)
{
  ...
  uint8_t coins32[COMPASS_KEM_SYMBYTES];
  uint8_t coins64[2*COMPASS_KEM_SYMBYTES];
  polyvec matrix[COMPASS_KEM_K];
  poly ap;

  randombytes(coins32, COMPASS_KEM_SYMBYTES);
  randombytes(coins64, 2*COMPASS_KEM_SYMBYTES);
  ...
  print_results("COMPASS_KEM_keypair_derand: ", t, NTESTS);

  for(i=0;i<NTESTS;i++) {
    t[i] = cpucycles();
    crypto_kem_keypair(pk, sk);
  }
  print_results("COMPASS_KEM_keypair: ", t, NTESTS);

  for(i=0;i<NTESTS;i++) {
    t[i] = cpucycles();
    crypto_kem_enc_derand(ct, key, pk, coins32);
  }
  print_results("COMPASS_KEM_encaps_derand: ", t, NTESTS);

  for(i=0;i<NTESTS;i++) {
    t[i] = cpucycles();
    crypto_kem_enc(ct, key, pk);
  }
  print_results("COMPASS_KEM_encaps: ", t, NTESTS);

  for(i=0;i<NTESTS;i++) {
    t[i] = cpucycles();
    crypto_kem_dec(key, ct, sk);
  }
  print_results("COMPASS_KEM_decaps: ", t, NTESTS);
