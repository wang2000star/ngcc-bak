                           ...
                           const uint8_t coins[KYBER_SYMBYTES])
{
  ...
  /* skpv is reused across all K rows of the matrix-vector product -
   * cache it once into skpv_prime (Stage-1 asymmetric basemul). */
  polyvec skpv_prime;
  polyvec_cache_prime(&skpv_prime, &skpv);

  // matrix-vector multiplication: t = A*s + e
  // e is added in the coefficient domain (no poly_tomont in the Plant impl).
  for(i=0;i<KYBER_K;i++) {
  ...
  polyvec_reduce(&pkpv);   /* bring coeffs into packable range before pack_pk */
  polyvec_reduce(&skpv);
                ...
                const uint8_t coins[KYBER_SYMBYTES])
{
  ...
  /* sp is reused across K matrix-vector basemuls + the t.r inner product. */
  polyvec sp_prime;
                ...
                const uint8_t sk[KYBER_INDCPA_SECRETKEYBYTES])
{
  ...
  /* Fused sk byte-deserialize + basemul, accumulating across K=4 polys in
   * 32-bit r_tmp; final stage plant_red's to 16-bit mp. */
  polyvec_ntt(&b);
