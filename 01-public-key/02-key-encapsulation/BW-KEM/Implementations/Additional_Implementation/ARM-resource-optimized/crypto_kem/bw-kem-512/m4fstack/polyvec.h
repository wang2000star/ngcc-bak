...
/* Per-polynomial compress/decompress: operate on slot i of a full compressed
 * ciphertext vector, so the caller can stream b one poly at a time (on-the-fly)
 * instead of materializing the whole polyvec on the stack. */
