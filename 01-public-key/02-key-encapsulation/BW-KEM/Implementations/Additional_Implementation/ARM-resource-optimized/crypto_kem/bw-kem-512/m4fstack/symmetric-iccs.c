                               ...
                               size_t inlen)
{
  ...
  int rc = pseudoXOF((unsigned long long)outlen * 8ULL,
                     ...
                     out);
#ifdef PROFILE_FUNCTIONS
  pseudoxof_cycles += (hal_get_time() - t0);
#endif
  return rc;
}
...
void kyber_iccs_hash_g(uint8_t out[128], const uint8_t *in, size_t inlen)
{
#ifdef PROFILE_FUNCTIONS
  uint64_t t0 = hal_get_time();
#endif
  int rc = pseudohash(1024, in, (unsigned long long)inlen * 8ULL, out);
#ifdef PROFILE_FUNCTIONS
  pseudohash_cycles += (hal_get_time() - t0);
#endif
  if(rc != 0)
    kyber_zero_bytes(out, 128);
}
...
void kyber_iccs_xof_squeeze_full(uint8_t *out, size_t outlen, xof_state *state)
{
  ...
  /* SM3 pseudoXOF is prefix-consistent and non-incremental: regenerating the
   * first `outlen` bytes from extseed reproduces the same prefix every time.
   * Write straight into the caller buffer — no VLA, no redundant copy. */
  if(kyber_aux_pseudoxof(out, outlen, state->extseed, sizeof(state->extseed)) != 0)
    kyber_zero_bytes(out, outlen);

  state->squeezed_bytes = outlen;
}
