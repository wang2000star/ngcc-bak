...
static int32_t bwkem128_round_div_signed(int32_t num, int32_t den)
{
  if(num < 0) {
...
static void poly_modswitch_q_to_qhat(poly *r, const poly *a)
{
  ...
  for(i = 0; i < KYBER_N; i++) {
    ...
    scaled = bwkem128_round_div_signed((int32_t)a->coeffs[i] * BWKEM128_QHAT, KYBER_Q);
    r->coeffs[i] = (int16_t)((uint32_t)scaled & (BWKEM128_QHAT - 1u));
  }
}

static void poly_tomsg_qhat(uint8_t msg[KYBER_INDCPA_MSGBYTES], const poly *a)
{
#ifdef PROFILE_FUNCTIONS
  uint64_t _t0 = hal_get_time();
#endif
...
static void poly_getnoise_eta(poly *r,
                              ...
                              unsigned int eta)
{
  uint8_t buf[BWKEM128_MAX_SUPPORTED_ETA * KYBER_N / 4];
  size_t buflen;
  ...
  buflen = ((size_t)eta * KYBER_N) / 4;
  prf(buf, buflen, seed, nonce);
  if(eta == KYBER_ETA1)
    poly_cbd_eta1(r, buf);
  else if(eta == KYBER_ETA2)
    poly_cbd_eta2(r, buf);
  else
    poly_zero(r);
}
...
void poly_getnoise_eta1(poly *r, const uint8_t seed[KYBER_SYMBYTES], uint8_t nonce)
{
  ...
  uint64_t _t0 = hal_get_time();
#endif
  poly_getnoise_eta(r, seed, nonce, KYBER_ETA1);
#ifdef PROFILE_FUNCTIONS
...
void poly_getnoise_eta2(poly *r, const uint8_t seed[KYBER_SYMBYTES], uint8_t nonce)
{
  ...
  uint64_t _t0 = hal_get_time();
#endif
  poly_getnoise_eta(r, seed, nonce, KYBER_ETA2);
#ifdef PROFILE_FUNCTIONS
...
extern void basemul_asm_opt_16_16_noprime(int16_t *, const int16_t *, const int16_t *, const int32_t *);
...
extern void basemul_asm_acc_opt_16_16_noprime(int16_t *, const int16_t *, const int16_t *, const int32_t *);
