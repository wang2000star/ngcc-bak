...
- `crypto_kem/`: KEM schemes (this repository only ships bw-kem).

Each scheme is placed in a subfolder named after a scheme, with each
implementation in its own folder, i.e.,
`crypto_kem/<scheme>/<implementation>/<sourcefiles.{c,h,s}>`.
...
- A function profiling test `elf/<schemename>_profiling.elf`
