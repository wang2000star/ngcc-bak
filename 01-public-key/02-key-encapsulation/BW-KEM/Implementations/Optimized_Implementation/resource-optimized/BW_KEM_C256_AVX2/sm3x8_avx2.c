...
static void sm3_x8_digest_oneblock(uint8_t block[8][64], uint8_t dgst[8][32])
{
  __m256i W[68];
  ...
  IV[0] = _mm256_xor_si256(A, IV[0]);
  IV[1] = _mm256_xor_si256(B, IV[1]);
  IV[2] = _mm256_xor_si256(C, IV[2]);
  IV[3] = _mm256_xor_si256(D, IV[3]);
  IV[4] = _mm256_xor_si256(E, IV[4]);
  IV[5] = _mm256_xor_si256(F, IV[5]);
  IV[6] = _mm256_xor_si256(G, IV[6]);
  IV[7] = _mm256_xor_si256(H, IV[7]);

  for(j = 0; j < 8; j++) {
    int lane;
    _mm256_storeu_si256((__m256i *)lanes, IV[j]);
...
void sm3_x8_digest(const uint8_t *data, size_t datalen, uint8_t dgst[8][32])
{
  ...
  for(lane = 0; lane < 8; lane++) {
    memcpy(block[lane], data + (size_t)lane * datalen, datalen);
    block[lane][datalen] = 0x80;
    store_be32(block[lane] + 60, (uint32_t)datalen * 8U);
  }

  sm3_x8_digest_oneblock(block, dgst);
}
