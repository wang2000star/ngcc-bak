...
static void fill_polyvec(polyvec *v)
{
  ...
  for(i = 0; i < KYBER_K; i++) {
    for(j = 0; j < KYBER_N; j++) {
      v->vec[i].coeffs[j] = (int16_t)(next_u32() % (2u * KYBER_Q)) - (int16_t)KYBER_Q;
    }
  }
}
  ...
  for(i = 0; i < KYBER_K; i++) {
    for(j = 0; j < KYBER_N / 4; j++) {
      for(k = 0; k < 4; k++) {
        ...
        u += (u >> 15) & KYBER_Q;
        t[k] = (uint16_t)((((uint32_t)u << 10) + KYBER_Q / 2) / KYBER_Q) & 0x3ff;
      }
...
static int compare_bytes(const uint8_t *got, const uint8_t *want, unsigned int len, const char *name, unsigned int iter)
{
  ...
  for(i = 0; i < len; i++) {
    if(got[i] != want[i]) {
      fprintf(stderr,
              "%s byte mismatch iter=%u offset=%u got=%u want=%u\n",
              name,
              iter,
              i,
              (unsigned int)got[i],
  ...
  for(i = 0; i < KYBER_K; i++) {
    for(j = 0; j < KYBER_N; j++) {
      if(got->vec[i].coeffs[j] != want->vec[i].coeffs[j]) {
        fprintf(stderr,
                "%s coeff mismatch iter=%u poly=%u coeff=%u got=%d want=%d\n",
                name,
                iter,
                i,
                j,
