...
static void test_hashes(void)
{
  ...
  if(sm3hash(256, in, (unsigned long long)sizeof(in) * 8ULL, ref) != 0) {
    fprintf(stderr, "sm3hash reference failed\n");
    exit(1);
  }
  hash_h(got, in, sizeof(in));
  assert_equal("hash_h", got, ref, sizeof(ref));
  hash_g(got, in, sizeof(in));
  assert_equal("hash_g", got, ref, sizeof(ref));
}
