...
int uvw_pke_dec(const unsigned char *sk_bytes, ciphertext_t *ct, unsigned char *output_m)
{
    ...
    // int e_bar_weight = 0;
    // for (int i = 0; i < half_n; i++) {
    //     if (e_bar[i] != 0) { // 在有限域中，只要不是 0，就算作一个错误
    //         e_bar_weight++;
    //     }
    // }
    // printf(" e_bar 的实际汉明重量为: %d\n", e_bar_weight);
    // // ========================================================
...
static void compress_gf_array(unsigned char *out, const gf_elem_t *array, int total_elements)
{
    ...
    // 按 8 个元素为一个块 (8 × 10 = 80 bit = 10 bytes)
    int blocks = total_elements / 8;
    for (int b = 0; b < blocks; b++) {
        uint16_t e0 = array[i++] & MASK_Q_BITS;
        uint16_t e1 = array[i++] & MASK_Q_BITS;
        uint16_t e2 = array[i++] & MASK_Q_BITS;
        uint16_t e3 = array[i++] & MASK_Q_BITS;
        uint16_t e4 = array[i++] & MASK_Q_BITS;
        uint16_t e5 = array[i++] & MASK_Q_BITS;
        uint16_t e6 = array[i++] & MASK_Q_BITS;
        uint16_t e7 = array[i++] & MASK_Q_BITS;

        out[byte_idx++] = (unsigned char)((e0 >> 2) & 0xFF);
        out[byte_idx++] = (unsigned char)(((e0 << 6) | (e1 >> 4)) & 0xFF);
        out[byte_idx++] = (unsigned char)(((e1 << 4) | (e2 >> 6)) & 0xFF);
        out[byte_idx++] = (unsigned char)(((e2 << 2) | (e3 >> 8)) & 0xFF);
        out[byte_idx++] = (unsigned char)(e3 & 0xFF);
        out[byte_idx++] = (unsigned char)((e4 >> 2) & 0xFF);
        out[byte_idx++] = (unsigned char)(((e4 << 6) | (e5 >> 4)) & 0xFF);
        out[byte_idx++] = (unsigned char)(((e5 << 4) | (e6 >> 6)) & 0xFF);
        out[byte_idx++] = (unsigned char)(((e6 << 2) | (e7 >> 8)) & 0xFF);
    ...
    if (remaining > 0) {
        ...
        for (; i < total_elements; i++) {
            uint16_t val = array[i] & MASK_Q_BITS;
            buffer = (buffer << UVW_Q_BITS) | val;
...
static void decompress_gf_array(gf_elem_t *array, const unsigned char *in, int total_elements)
{
    ...
    for (int b = 0; b < blocks; b++) {
        ...
        uint8_t b9 = in[byte_idx++];

        array[i++] = (gf_elem_t)(((b0 << 2) | (b1 >> 6)) & MASK_Q_BITS);
        array[i++] = (gf_elem_t)((((b1 & 0x3F) << 4) | (b2 >> 4)) & MASK_Q_BITS);
        array[i++] = (gf_elem_t)((((b2 & 0x0F) << 6) | (b3 >> 2)) & MASK_Q_BITS);
        array[i++] = (gf_elem_t)((((b3 & 0x03) << 8) | b4) & MASK_Q_BITS);
        array[i++] = (gf_elem_t)(((b5 << 2) | (b6 >> 6)) & MASK_Q_BITS);
        array[i++] = (gf_elem_t)((((b6 & 0x3F) << 4) | (b7 >> 4)) & MASK_Q_BITS);
        array[i++] = (gf_elem_t)((((b7 & 0x0F) << 6) | (b8 >> 2)) & MASK_Q_BITS);
        array[i++] = (gf_elem_t)((((b8 & 0x03) << 8) | b9) & MASK_Q_BITS);
    }
    ...
    if (remaining > 0) {
        ...
        for (; i < total_elements; i++) {
            ...
            array[i] = (gf_elem_t)((buffer >> (bits_in_buffer - UVW_Q_BITS)) & MASK_Q_BITS);
            bits_in_buffer -= UVW_Q_BITS;
        }
    }
}