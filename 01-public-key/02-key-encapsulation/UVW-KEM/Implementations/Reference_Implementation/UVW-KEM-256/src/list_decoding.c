...
static gf_elem_t hasse_derivative_fast(
    ...
    int alpha, int beta,
    const gf_elem_t *x_pow, const gf_elem_t *r_pow)
{
    uint64_t accum = 0;
    for (int j = beta; j < POLY_Y_SIZE; j++) {
        ...
        if (y_comb == 0) continue;
        for (int i = alpha; i <= row_max; i++) {
            gf_elem_t coeff = poly->coeffs[j][i];
            if (coeff == 0) continue;
            uint64_t term = (uint64_t)coeff
                          * COMB_TABLE[i][alpha]
                          * y_comb
                          * x_pow[i - alpha]
                          * r_pow[j - beta];
        ...
        for (int alpha = 0; alpha < RS_M; alpha++) {
            for (int beta = 0; beta < (RS_M - alpha); beta++) {
                ...
                for (int h = 0; h < POLY_Y_SIZE; h++) {
                    ...
                    deltas[h] = hasse_derivative_fast(G[h], alpha, beta, x_pow, r_pow);
                    if (deltas[h] != 0) {
                    ...
                    if (h != min_lod_idx) {
                        ...
                        for (int j = 0; j < POLY_Y_SIZE; j++) {
                            ...
                            for (int i = 0; i <= max_x; i++) {
                                gf_elem_t coeff_h   = (i <= row_max_h)   ? G[h]->coeffs[j][i] : 0;
                                gf_elem_t coeff_min = (i <= row_max_min) ? G_min_copy->coeffs[j][i] : 0;
                                gf_elem_t term1 = gf_mul(deltas[min_lod_idx], coeff_h);
                                gf_elem_t term2 = gf_mul(deltas[h], coeff_min);
                                G[h]->coeffs[j][i] = gf_sub(term1, term2);
                            }
                        ...
                        for (int j = 0; j < POLY_Y_SIZE; j++) {
                            ...
                            for (int i = 0; i <= row_max; i++) {
                                ...
                                if (coeff == 0) continue;
                                gf_elem_t term_x = gf_mul(deltas[min_lod_idx], coeff);
                            ...
                            G[h]->row_max_x[j] = row_max + 1;
                        }
                        G[h]->cur_max_x = G_min_copy->cur_max_x + 1;
                    }
