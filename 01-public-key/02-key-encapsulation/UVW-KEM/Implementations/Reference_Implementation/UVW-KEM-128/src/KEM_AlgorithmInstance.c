...
static int gf_mat_inverse(gf_elem_t *inv, const gf_elem_t *A, int k)
{
    ...
    // 2. 初始化：拷贝 A，并将 inv 初始化为纯净的单位阵 I_k
    for (int i = 0; i < k; i++)
    {
...
int uvw_pke_dec(const unsigned char *sk_bytes, ciphertext_t *ct, unsigned char *output_m)
{
    ...
    gf_elem_t r2[UVW_K2];
    if (uvw_rs_list_decode(y_diff, support_set, r2) <= 0)
        return -1; // RS 译码失败
    ...
    for (int attempt = 0; attempt < max_attempts; attempt++)
    {
        ...
        // 求子矩阵的逆 (G_U)^-1_{I1}
        gf_elem_t GU_I1_inv[UVW_K1 * UVW_K1];
...
// -----------------------------------------------------------
// 密钥生成算法中由 seed, i1 扩展出 G_U
// 如果生成的 G_U 满秩返回 0，否则返回 -1 以触发重新生成
// -----------------------------------------------------------
static int expand_GU_KeyGen(const unsigned char seed[PRIVATE_KEY_SEED_BYTES], uint8_t i1,
                        private_key_t *sk_struct)
{
...
// -----------------------------------------------------------
// 密钥生成算法中由 seed, i2 扩展出 G_W
// 如果生成的 G_W 满秩返回 0，否则返回 -1 以触发重新生成
// -----------------------------------------------------------
static int expand_GW_KeyGen(const unsigned char seed[PRIVATE_KEY_SEED_BYTES], uint8_t i2,
                        private_key_t *sk_struct)
{
 ...
 * @param i1    私钥索引 1 (0-255)
 * @param i2    私钥索引 2 (0-255)  
 * @param i3    私钥索引 3 (0-255)
 * @param T      输出：公钥矩阵 (UVW_K × (UVW_N-UVW_K))，由调用者分配
 * @return 0 成功，非0 失败
 */
static int derive_pk_from_seeds(
    const unsigned char seed[PRIVATE_KEY_SEED_BYTES],
...
/**
 * 将 gf_elem_t 数组压缩为字节流 (MSB first, 每元素 9 比特)
 * @param out            输出字节数组 (长度至少为 ceil(total_elements * 9 / 8))
 * @param array          输入域元素数组
 * @param total_elements 元素个数
 */
static void compress_gf_array(unsigned char *out, const gf_elem_t *array, int total_elements)
{
    int byte_idx = 0;
    ...
    // 按 8 个元素为一个块 (8 * 9 = 72 bit = 9 byte)
    int blocks = total_elements / 8;
    for (int b = 0; b < blocks; b++)
    {
        uint16_t e0 = array[i++] & 0x01FF;
        uint16_t e1 = array[i++] & 0x01FF;
        uint16_t e2 = array[i++] & 0x01FF;
        uint16_t e3 = array[i++] & 0x01FF;
        uint16_t e4 = array[i++] & 0x01FF;
        uint16_t e5 = array[i++] & 0x01FF;
        uint16_t e6 = array[i++] & 0x01FF;
        uint16_t e7 = array[i++] & 0x01FF;

        out[byte_idx++] = (unsigned char)((e0 >> 1) & 0xFF);
        out[byte_idx++] = (unsigned char)(((e0 << 7) | (e1 >> 2)) & 0xFF);
        out[byte_idx++] = (unsigned char)(((e1 << 6) | (e2 >> 3)) & 0xFF);
        out[byte_idx++] = (unsigned char)(((e2 << 5) | (e3 >> 4)) & 0xFF);
        out[byte_idx++] = (unsigned char)(((e3 << 4) | (e4 >> 5)) & 0xFF);
        out[byte_idx++] = (unsigned char)(((e4 << 3) | (e5 >> 6)) & 0xFF);
        out[byte_idx++] = (unsigned char)(((e5 << 2) | (e6 >> 7)) & 0xFF);
        out[byte_idx++] = (unsigned char)(((e6 << 1) | (e7 >> 8)) & 0xFF);
        out[byte_idx++] = (unsigned char)(e7 & 0xFF);
    }

    // 处理剩余元素 (不足 8 个)
    int remaining = total_elements % 8;
    if (remaining > 0)
    {
        uint32_t buffer = 0;
        ...
        {
            uint16_t val = array[i] & 0x01FF;
            buffer = (buffer << 9) | val;
            bits_in_buffer += 9;
            while (bits_in_buffer >= 8)
            {
                out[byte_idx++] = (unsigned char)(buffer >> (bits_in_buffer - 8));
                bits_in_buffer -= 8;
            }
        }
        if (bits_in_buffer > 0)
        {
            out[byte_idx++] = (unsigned char)(buffer << (8 - bits_in_buffer));
        }
    }
}

/**
 * 从字节流解压缩为 gf_elem_t 数组
 * @param array          输出域元素数组 (长度 total_elements)
 * @param in             输入字节数组
 * @param total_elements 元素个数
 */
static void decompress_gf_array(gf_elem_t *array, const unsigned char *in, int total_elements)
{
    ...
    for (int b = 0; b < blocks; b++)
    {
        ...
        uint8_t b3 = in[byte_idx++];
        uint8_t b4 = in[byte_idx++];
        uint8_t b5 = in[byte_idx++];
        uint8_t b6 = in[byte_idx++];
        uint8_t b7 = in[byte_idx++];
        uint8_t b8 = in[byte_idx++];

        array[i++] = (gf_elem_t)(((b0 << 1) | (b1 >> 7)) & 0x01FF);
        array[i++] = (gf_elem_t)((((b1 & 0x7F) << 2) | (b2 >> 6)) & 0x01FF);
        array[i++] = (gf_elem_t)((((b2 & 0x3F) << 3) | (b3 >> 5)) & 0x01FF);
        array[i++] = (gf_elem_t)((((b3 & 0x1F) << 4) | (b4 >> 4)) & 0x01FF);
        array[i++] = (gf_elem_t)((((b4 & 0x0F) << 5) | (b5 >> 3)) & 0x01FF);
        array[i++] = (gf_elem_t)((((b5 & 0x07) << 6) | (b6 >> 2)) & 0x01FF);
        array[i++] = (gf_elem_t)((((b6 & 0x03) << 7) | (b7 >> 1)) & 0x01FF);
        array[i++] = (gf_elem_t)((((b7 & 0x01) << 8) | b8) & 0x01FF);
    }
    ...
    if (remaining > 0)
    {
        ...
        for (; i < total_elements; i++)
        {
            ...
            {
                buffer = (buffer << 8) | in[byte_idx++];
                bits_in_buffer += 8;
            }
            array[i] = (gf_elem_t)((buffer >> (bits_in_buffer - 9)) & 0x01FF);
            bits_in_buffer -= 9;
        }
    }
}