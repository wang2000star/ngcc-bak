...
//compute z = ((h1, h2), (h2, h1))*(y1, y2)
void vect_mul_2(uint64_t z[2][VEC_N_SIZE_64], const uint64_t h[2][VEC_N_SIZE_64], const uint64_t y[2][VEC_N_SIZE_64]) {
    uint64_t u[2][VEC_N_SIZE_64] = { 0 };
    uint64_t v[2][VEC_N_SIZE_64] = { 0 };

    vect_mul_1(z[0], y[0], h[0]);
    vect_mul_1(v[0], y[1], h[1]);
    vect_add(u[0], y[0], y[1], VEC_N_SIZE_64);
    vect_add(u[1], h[0], h[1], VEC_N_SIZE_64);
    vect_mul_1(v[1], u[0], u[1]);
    vect_add(z[0], z[0], v[0], VEC_N_SIZE_64);
    vect_add(z[1], z[0], v[1], VEC_N_SIZE_64);
}

//compute z = ((h1, h2, h3, h4), (h2, h1, h4, h3), (h3, h4, h1, h2), (h4, h3, h2, h1))*(y1, y2, y3, y4)
void vect_mul_4(uint64_t z[4][VEC_N_SIZE_64], const uint64_t h[4][VEC_N_SIZE_64], const uint64_t y[4][VEC_N_SIZE_64]) {
    int i;
    uint64_t u[4][VEC_N_SIZE_64] = { 0 };
    uint64_t v[4][VEC_N_SIZE_64] = { 0 };

    vect_mul_2(z, h, y);
    vect_mul_2(v, h + 2, y + 2);
    vect_add(u[0], h[0], h[2], VEC_N_SIZE_64);
    vect_add(u[1], h[1], h[3], VEC_N_SIZE_64);
    vect_add(u[2], y[0], y[2], VEC_N_SIZE_64);
    vect_add(u[3], y[1], y[3], VEC_N_SIZE_64);
    vect_mul_2(v + 2, u, u + 2);

    for (i = 0; i < 4; i++)
        vect_add(z[i], z[i & 1], v[i], VEC_N_SIZE_64);
}

/**
 * @brief Carry-less multiplication mod (X^PARAM_N - 1).
 *
 * Computes o = h * y, PARAM_L operands of VEC_N_SIZE_64 words, then reduces.
 *
 * @param[out] z   Result buffer, PARAM_L operands and size VEC_N_SIZE_64 words.
 * @param[in]  h  Operand vectors h(x).
 * @param[in]  y  Operand vectors y(x).
 */
