...
/**
 * @brief Encodes a message message of PARAM_K_BYTES bits to a Reed-Solomon codeword codeword of PARAM_N1 bytes
 *
 * Following @cite lin1983error (Chapter 4 - Cyclic Codes),
 * We perform a systematic encoding using a linear (PARAM_N1 - PARAM_K_BYTES)-stage shift register
...
void reed_solomon_encode(uint64_t* cdw, const uint64_t* msg) {
    ...
    uint8_t msg_bytes[PARAM_K_BYTES] = { 0 };

    memcpy(msg_bytes, msg, PARAM_K_BYTES);

    for (i = 0; i < PARAM_K_BYTES; ++i) {
        gate_value = msg_bytes[PARAM_K_BYTES - 1 - i] ^ cdw_bytes[PARAM_N1 - PARAM_K_BYTES - 1];

        for (j = 0; j < PARAM_G; ++j) {
            tmp[j] = gf_mul(gate_value, PARAM_RS_POLY[j]);
        }

        for (k = PARAM_N1 - PARAM_K_BYTES - 1; k; --k) {
    ...
    memcpy(cdw_bytes + PARAM_N1 - PARAM_K_BYTES, msg_bytes, PARAM_K_BYTES);
...
void reed_solomon_decode(uint64_t *msg, uint64_t *cdw) {
    ...
    memcpy(msg, cdw_bytes + (PARAM_G - 1), PARAM_K_BYTES);
}
