 ...
 */
#define MULTIPLICITY ((PARAM_N2 + 127) / 128)
 ...
 */
void find_peaks(int32_t* peak, rm_expanded_cdw* transform) {
    int32_t i, absolute, max = 0, pro = 0, pos[2] = { 0 };

    for (i = 0; i < 128; i++) {
        // get absolute value
        absolute = (*transform)[i];
        absolute -= 2 * ((absolute >> 31) & absolute);
    ...
    // set bit 7
    peak[0] = pos[0] | 128 * ((*transform)[pos[0]] > 0);
    peak[1] = pos[1] | 128 * ((*transform)[pos[1]] > 0);
}
...
void reed_muller_encode(uint64_t *cdw, const uint64_t *msg) {
    ...
    for (int32_t i = 0; i < VEC_N1_SIZE_BYTES; i++) {
...
static int swar_distance(uint64_t x) {
    static const uint64_t m1 = 0x5555555555555555;
    static const uint64_t m2 = 0x3333333333333333;
    static const uint64_t m3 = 0x0f0f0f0f0f0f0f0f;
    static const uint64_t m4 = 0x0101010101010101;
...
int reed_muller_decode(uint64_t *msg, uint64_t *pos, const uint64_t *cdw) {
    ...
    for (i = 0; i < VEC_N1_SIZE_BYTES; i++) {
        ...
        // finish the decoding
        find_peaks(peak, &transform);
#ifdef MITO_1
        message_array[i] = peak[0];
#else
        for (j = 0; j < 2; j++)
        {
            d[j] = 0;
            encode((rm_codeword_t*)&me, peak[j]);
            for (k = 0; k < MULTIPLICITY; k++) {
                d[j] += swar_distance(me[0] ^ cdw[2 * (i * MULTIPLICITY + k)]);
                d[j] += swar_distance(me[1] ^ cdw[2 * (i * MULTIPLICITY + k) + 1]);
            }
        }
        if (d[1] - d[0] <= PARAM_ALPHA) {
            message_array[i] = peak[0];
            pos_array[cnt++] = i;
        }
        else {
