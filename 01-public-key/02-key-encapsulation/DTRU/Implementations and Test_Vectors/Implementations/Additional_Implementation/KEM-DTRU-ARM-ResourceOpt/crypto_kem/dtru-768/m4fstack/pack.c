...
#include "params.h"
    ...
    static void Encode(unsigned char *out, const unsigned char *out_end, const uint16_t *R, const uint16_t *M, const long long len)
    {
        if (len == 1)
        {
            uint16_t r = R[0];
    ...
    static void Decode(uint16_t *out, const unsigned char *S, const uint16_t *M, const long long len)
    {
        if (len == 1)
        {
            if (M[0] == 1)
