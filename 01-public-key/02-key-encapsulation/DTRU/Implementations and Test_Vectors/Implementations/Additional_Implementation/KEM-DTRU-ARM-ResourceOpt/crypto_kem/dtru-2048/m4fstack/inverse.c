...
static int rq_inverse_recursive4(int16_t b[4], const int16_t a[4], int16_t zeta)
{
...
static int rq_inverse_recursive8(int16_t b[8], const int16_t a[8], const int16_t zeta)
{
    ...
    r = rq_inverse_recursive4(a1, a0, zeta); // a1 = a0^{-1} mod (x^4 - zeta) with mont
...
extern void doublebasemul_asm_8x8(int16_t *, const int16_t *, const int16_t *, const int16_t);
int rq_inverse_recursive16_asm(int16_t b[16], const int16_t a[16], const int16_t zeta)
{
    int16_t d[16], a0[8], a1[8], c0[8], c1[8], b0[8], b1[8];
    unsigned int i;
    int r;
    ...
    doublebasemul_asm_8x8(a0, d, d, zeta);
    doublebasemul_asm_8x8(a1, d + 8, d + 8, zeta);
    ...
    doublebasemul_asm_8x8(b0, c0, a1, zeta);
    doublebasemul_asm_8x8(b1, c1, a1, zeta);
    for (i = 0; i < 8; i++)
    {
        b[2 * i] = b0[i];
        b[2 * i + 1] = b1[i];
...
#define qR2inv 1242397 // round(2^32/q)
#define _2P15 (1 << 15)
extern int jump32divsteps(int minusdelta, int32_t *M, int32_t *f, int32_t *g);

int rq_inverse_asm(int16_t finv[16], const int16_t f[16], const int16_t zeta)
{
    int i;
    int16_t scale;
    int16_t Phi[64] = {0};
    ...
    Phi[0] = 1;
    Phi[16] = -zeta;
    for (i = 0; i < 16; ++i) F[16 - 1 - i] = f[i];
    F[16] = 0;

    int minusdelta = -1;
    minusdelta = jump32divsteps(minusdelta, (int32_t *)V, (int32_t *)Phi, (int32_t *)F);
    ...
    for (i = 0; i < 16; ++i)
        finv[i] = fq_freeze(scale * (int32_t)V[128-16 - i]);
    return minusdelta == -1 ? 0 : 1;
}
