...
int baseinv_opt(int16_t r[4], const int16_t a[4], int16_t zeta)
{
    ...
    t0 = montgomery_reduce((int32_t)a[2] * a[2] - 2 * (int32_t)a[1] * a[3]);
    t1 = montgomery_reduce((int32_t)a[3] * a[3]);
    t0 = montgomery_reduce((int32_t)a[0] * a[0] + (int32_t)t0 * zeta);
    t1 = montgomery_reduce((int32_t)a[1] * a[1] + (int32_t)t1 * zeta - 2 * (int32_t)a[0] * a[2]);
    t2 = montgomery_reduce((int32_t)t1 * zeta);

    t3 = montgomery_reduce((int32_t)t0 * t0 - (int32_t)t1 * t2);
    if (t3 == 0)
        return 1;

    r[0] = montgomery_reduce((int32_t)a[0] * t0 + (int32_t)a[2] * t2);
    r[1] = montgomery_reduce((int32_t)a[3] * t2 + (int32_t)a[1] * t0);
    r[2] = montgomery_reduce((int32_t)a[2] * t0 + (int32_t)a[0] * t1);
    r[3] = montgomery_reduce((int32_t)a[1] * t1 + (int32_t)a[3] * t0);

    t3 = fqinv(t3);

    r[0] = montgomery_reduce((int32_t)r[0] * t3);
    r[1] = -montgomery_reduce((int32_t)r[1] * t3);
    r[2] = montgomery_reduce((int32_t)r[2] * t3);
    r[3] = -montgomery_reduce((int32_t)r[3] * t3);
