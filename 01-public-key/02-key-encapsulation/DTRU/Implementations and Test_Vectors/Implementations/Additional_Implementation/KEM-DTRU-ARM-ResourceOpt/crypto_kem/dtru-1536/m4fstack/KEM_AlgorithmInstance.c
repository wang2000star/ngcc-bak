	...
	unsigned char *ct, unsigned long long *ct_len_bytes)
{
  ...
  for (i = 0; i < DTRU_PREFIXHASHBYTES && i < pk_len_bytes; ++i)
    ct[i] = pk[i];
  ct[DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES] = 0;
  pseudohash(512,ct,(DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES + 1) * 8 ,buf);
  ct[DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES] = 1;
  pseudohash(512,ct,(DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES + 1) * 8 ,buf + DTRU_SHAREDKEYBYTES);
	...
	unsigned char *ss, unsigned long long *ss_len_bytes)
{
  ...
  ct2[DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES] = 0;
  pseudohash(512,ct2,(DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES + 1) * 8 ,buf);
  ct2[DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES] = 1;
  pseudohash(512,ct2,(DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES + 1) * 8 ,buf + DTRU_SHAREDKEYBYTES);
