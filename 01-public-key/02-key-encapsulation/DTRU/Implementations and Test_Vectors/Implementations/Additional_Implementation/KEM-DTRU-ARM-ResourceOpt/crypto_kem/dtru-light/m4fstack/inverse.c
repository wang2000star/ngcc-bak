...
int16_t zetas_base_nega[128] = {
  762, 7, 335, 434, 489, 280, 327, 442, 183, 586, 580, 189, 399, 370, 130, 639, 437, 332, 179, 590, 562, 207, 239, 530, 550, 219, 264, 505, 468, 301, 563, 206, 111, 658, 730, 39, 595, 174, 747, 22, 284, 485, 690, 79, 594, 175, 685, 84, 321, 448, 677, 92, 536, 233, 165, 604, 177, 592, 208, 561, 159, 610, 630, 139, 426, 343, 266, 503, 122, 647, 643, 126, 508, 261, 736, 33, 326, 443, 218, 551, 650, 119, 312, 457, 623, 146, 176, 593, 35, 734, 632, 137, 631, 138, 672, 97, 56, 713, 396, 373, 702, 67, 460, 309, 74, 695, 743, 26, 653, 116, 498, 271, 349, 420, 106, 663, 118, 651, 395, 374, 214, 555, 195, 574, 101, 668, 110, 659
};
...
{
    int16_t Phi[ROOT_DIMENSION + 1] = {1, 0, 0, 0, zeta};
    int16_t V[ROOT_DIMENSION + 1] = {0};
    int16_t S[ROOT_DIMENSION + 1] = {1, 0, 0, 0, 0};
    int16_t F[ROOT_DIMENSION + 1] = {f[3], f[2], f[1], f[0], 0};
    int i, loop, swap, t;
    int Delta = 1;
    int32_t Phi0, F0;
    int16_t scale;
...
extern void fqinv_asm(int16_t *);
int rq_inverse_recursive_asm(int16_t b[8], const int16_t a[8], int16_t zeta)
{
    int16_t a0[4], a1[4], d[8], det[2];
    int r[2];

    a0[0] = CAL_S(a, 0, 0) + fqmul(CAL_S(a, 2, 2) - 2 * CAL_S(a, 1, 3), zeta); //a0*a0+(a2*a2-2*a1*a3)*zeta
    a0[1] = -CAL_S(a, 1, 1) - fqmul(CAL_S(a, 3, 3), zeta) + 2 * CAL_S(a, 0, 2); //-a1*a1-a3*a3*zeta+2*a0*a2
    a0[2] = CAL_S(a, 4, 4) + fqmul(CAL_S(a, 6, 6) - 2 * CAL_S(a, 5, 7), -zeta); 
    a0[3] = -CAL_S(a, 5, 5) - fqmul(CAL_S(a, 7, 7), -zeta) + 2 * CAL_S(a, 4, 6); 

    det[0] = CAL_S(a0, 1, 1);
    det[0] = fqmul(det[0], -zeta);
    det[0] += CAL_S(a0, 0, 0); // with mont^-2
    det[0] = barrett_reduce(det[0]);
    det[1] = CAL_S(a0, 3, 3);
    det[1] = fqmul(det[1], zeta);
    det[1] += CAL_S(a0, 2, 2); // with mont^-2
    det[1] = barrett_reduce(det[1]);

    // det[0] = fqinv(det[0]); // det^-1 with mont^2
    // det[1] = fqinv(det[1]); // det^-1 with mont^2
    fqinv_asm(det);
    
    a1[0] = fqmul(a0[0], det[0]);
    a1[1] = fqmul(-a0[1], det[0]);
    a1[2] = fqmul(a0[2], det[1]);
    a1[3] = fqmul(-a0[3], det[1]);
    ...
    b[2] = fqmul(a1[0] + a1[1], a[0] + a[2]) - d[0] - d[2];
    b[3] = fqmul(a1[0] + a1[1], -a[1] - a[3]) + d[1] + d[3];
    b[4] = d[4] + fqmul(d[6], -zeta);
    b[5] = -d[5] + fqmul(-d[7], -zeta);
    b[6] = fqmul(a1[2] + a1[3], a[4] + a[6]) - d[4] - d[6];
    b[7] = fqmul(a1[2] + a1[3], -a[5] - a[7]) + d[5] + d[7];
...
extern void fqinv_asm_single(int16_t *);
int rq_inverse_recursive_asm_single(int16_t b[4], const int16_t a[4], int16_t zeta)
{
    int16_t a0[2], a1[2], d[4], det;
    int r;
