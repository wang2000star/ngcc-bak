...
# makefile, that essentially just contains the variable KEM_SCHEMES,
# containing the list of paths of schemes.
obj/.schemes.mk:
	$(Q)[ -d $(@D) ] || mkdir -p $(@D); \
	touch $@; \
	printf "KEM_SCHEMES :=" > $@; \
	find $(KEM_SEARCH_PATHS) -mindepth 2 -maxdepth 2 -type d -print0 | xargs -0 printf " \\\\\\n\\t%s" >> $@;
...
# will be added to the KEM_SCHEMES list. Since the python scripts for
...
# since the code depends on the preprocessor definitions in the scheme headers.
