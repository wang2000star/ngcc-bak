...
extern void doublebasemul_asm(int16_t *, const int16_t *, const int16_t *, const int16_t *);
void poly_basemul(poly *c, const poly *a, const poly *b)
{
    int16_t zetas_ordered[DTRU_N / 2];

    for (int i = 0; i < DTRU_N / 6; ++i)
    {
        zetas_ordered[3 * i]     = zetas[128 + 2 * i];
        zetas_ordered[3 * i + 1] = zetas_base[i];
        zetas_ordered[3 * i + 2] = -zetas[128 + 2 * i] - zetas_base[i];
    }
    doublebasemul_asm(c->coeffs, a->coeffs, b->coeffs, zetas_ordered);
}
