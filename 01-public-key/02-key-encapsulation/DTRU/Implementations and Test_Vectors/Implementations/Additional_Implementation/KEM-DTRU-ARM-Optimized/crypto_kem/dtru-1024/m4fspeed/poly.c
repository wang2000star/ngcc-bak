...
extern void doublebasemul_asm_opt(int16_t *, const int16_t *, const int16_t *, const int16_t *);
void poly_basemul_asm(poly *c, const poly *a, const poly *b)
{
  int16_t zetas_reordered[DTRU_N/8];
  for (int i = 0; i < DTRU_N/16; i++) {
      zetas_reordered[2*i]   =  zetas[64 + i];
      zetas_reordered[2*i+1] = -zetas[64 + i];
  }
  doublebasemul_asm_opt(c->coeffs,a->coeffs,b->coeffs,zetas_reordered);
}
...
int poly_baseinv(poly *b, const poly *a)
{
  ...
  {
    if(r != 0)   return r;
    r += rq_inverse_recursive8_asm(b->coeffs + 16 * i,
                    a->coeffs + 16 * i,
                    zetas[64 + i]);                
    if(r != 0)   return r;
    r += rq_inverse_recursive8_asm(b->coeffs + 16 * i + 8,
                    a->coeffs + 16 * i + 8,
                    -zetas[64 + i]);             
  }
  return r;
