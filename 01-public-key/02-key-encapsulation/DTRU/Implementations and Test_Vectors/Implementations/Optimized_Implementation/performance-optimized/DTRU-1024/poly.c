...
{
   int r = 0; 
   r = rq_inverse_avx((int16_t*)b->coeffs,(int16_t*)a->coeffs,(const int16_t*)qdata.vec);
   return r;
}
...
{
   tomont_avx(a->coeffs,(const int16_t*)qdata.vec);
}

void poly_freeze_avx(poly *a)
{
  freeze_avx(a->coeffs,(const int16_t*)qdata.vec);
}
