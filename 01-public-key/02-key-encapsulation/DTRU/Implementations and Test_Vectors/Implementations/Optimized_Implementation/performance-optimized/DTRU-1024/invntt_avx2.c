...
void invntt_avx2(int16_t b[DTRU_N], const int16_t a[DTRU_N])
{
    ...
    for (j = 0; j < 8; j++)
    {
        ...
        __m128i lo = _mm_set1_epi16(zetas_inv_avx2[j * 8 + 0]);
        __m128i hi = _mm_set1_epi16(zetas_inv_avx2[j * 8 + 1]);
        zeta_vec = _mm256_setr_m128i(lo, hi);
        _mm256_GSbutterfly_epi16(&f0, &f1, zeta_vec, q_vec, qinv_vec);

        lo = _mm_set1_epi16(zetas_inv_avx2[j * 8 + 2]);
        hi = _mm_set1_epi16(zetas_inv_avx2[j * 8 + 3]);
        zeta_vec = _mm256_setr_m128i(lo, hi);
        _mm256_GSbutterfly_epi16(&f2, &f3, zeta_vec, q_vec, qinv_vec);

        lo = _mm_set1_epi16(zetas_inv_avx2[j * 8 + 4]);
        hi = _mm_set1_epi16(zetas_inv_avx2[j * 8 + 5]);
        zeta_vec = _mm256_setr_m128i(lo, hi);
        _mm256_GSbutterfly_epi16(&f4, &f5, zeta_vec, q_vec, qinv_vec);

        lo = _mm_set1_epi16(zetas_inv_avx2[j * 8 + 6]);
        hi = _mm_set1_epi16(zetas_inv_avx2[j * 8 + 7]);
        zeta_vec = _mm256_setr_m128i(lo, hi);
        _mm256_GSbutterfly_epi16(&f6, &f7, zeta_vec, q_vec, qinv_vec);
        ...
        // level 5 interval = 16
        zeta_vec = _mm256_set1_epi16(zetas_inv_avx2[64 + j * 4 + 0]);
        _mm256_GSbutterfly_epi16(&f0, &f1, zeta_vec, q_vec, qinv_vec);

        zeta_vec = _mm256_set1_epi16(zetas_inv_avx2[64 + j * 4 + 1]);
        _mm256_GSbutterfly_epi16(&f2, &f3, zeta_vec, q_vec, qinv_vec);

        zeta_vec = _mm256_set1_epi16(zetas_inv_avx2[64 + j * 4 + 2]);
        _mm256_GSbutterfly_epi16(&f4, &f5, zeta_vec, q_vec, qinv_vec);

        zeta_vec = _mm256_set1_epi16(zetas_inv_avx2[64 + j * 4 + 3]);
        _mm256_GSbutterfly_epi16(&f6, &f7, zeta_vec, q_vec, qinv_vec);
        ...
        // level 4 interval = 32
        zeta_vec = _mm256_set1_epi16(zetas_inv_avx2[96 + j * 2 + 0]);
        _mm256_GSbutterfly_epi16(&f0, &f2, zeta_vec, q_vec, qinv_vec);
        _mm256_GSbutterfly_epi16(&f1, &f3, zeta_vec, q_vec, qinv_vec);

        zeta_vec = _mm256_set1_epi16(zetas_inv_avx2[96 + j * 2 + 1]);
        _mm256_GSbutterfly_epi16(&f4, &f6, zeta_vec, q_vec, qinv_vec);
        _mm256_GSbutterfly_epi16(&f5, &f7, zeta_vec, q_vec, qinv_vec);

        // level 3 interval = 64
        zeta_vec = _mm256_set1_epi16(zetas_inv_avx2[112 + j]);
        _mm256_GSbutterfly_epi16(&f0, &f4, zeta_vec, q_vec, qinv_vec);
        _mm256_GSbutterfly_epi16(&f1, &f5, zeta_vec, q_vec, qinv_vec);
        _mm256_GSbutterfly_epi16(&f2, &f6, zeta_vec, q_vec, qinv_vec);
        _mm256_GSbutterfly_epi16(&f3, &f7, zeta_vec, q_vec, qinv_vec);
    ...
    for (j = 0; j < 8; j++)
    {
        ...
        // level 2
        zeta_vec = _mm256_set1_epi16(zetas_inv_avx2[120]);
        _mm256_GSbutterfly_epi16(&f0, &f1, zeta_vec, q_vec, qinv_vec);

        zeta_vec = _mm256_set1_epi16(zetas_inv_avx2[121]);
        _mm256_GSbutterfly_epi16(&f2, &f3, zeta_vec, q_vec, qinv_vec);

        zeta_vec = _mm256_set1_epi16(zetas_inv_avx2[122]);
        _mm256_GSbutterfly_epi16(&f4, &f5, zeta_vec, q_vec, qinv_vec);

        zeta_vec = _mm256_set1_epi16(zetas_inv_avx2[123]);
        _mm256_GSbutterfly_epi16(&f6, &f7, zeta_vec, q_vec, qinv_vec);

        // level 1
        zeta_vec = _mm256_set1_epi16(zetas_inv_avx2[124]);
        _mm256_GSbutterfly_epi16(&f0, &f2, zeta_vec, q_vec, qinv_vec);
        _mm256_GSbutterfly_epi16(&f1, &f3, zeta_vec, q_vec, qinv_vec);
        
        zeta_vec = _mm256_set1_epi16(zetas_inv_avx2[125]);
        _mm256_GSbutterfly_epi16(&f4, &f6, zeta_vec, q_vec, qinv_vec);
        _mm256_GSbutterfly_epi16(&f5, &f7, zeta_vec, q_vec, qinv_vec);

        // level 0
        zeta_vec = _mm256_set1_epi16(zetas_inv_avx2[126]);
        __m256i n1 = _mm256_set1_epi16(zetas_inv_avx2[127]);
        __m256i n2 = _mm256_set1_epi16(zetas_inv_avx2[128]);

        f8 = _mm256_sub_epi16(f0, f4);
        f8 = _mm256_fqmul_epi16(f8, zeta_vec, q_vec, qinv_vec);
