	...
	unsigned char *sk, unsigned long long *sk_len_bytes)
{
  ...
  } while (pke_keygen_avx2(pk, sk, coins));
	...
	unsigned char *ct, unsigned long long *ct_len_bytes)
{
  ...
  // crypto_hash_sha3_512(buf, buf2, DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES + 1);
  pseudohash(512,buf2,(DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES + 1) * 8 ,buf);
  buf2[DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES] = 1;
  // crypto_hash_sha3_512(buf + DTRU_SHAREDKEYBYTES, buf2, DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES + 1);
...
//   crypto_hash_shake256(buf + DTRU_SHAREDKEYBYTES, DTRU_COINBYTES_ENC, buf + DTRU_SHAREDKEYBYTES, DTRU_SHAREDKEYBYTES);
  ...
  pke_enc_avx2(ct, pk, m, buf + DTRU_SHAREDKEYBYTES);
	...
	unsigned char *ss, unsigned long long *ss_len_bytes)
{
  ...
  pke_dec_avx2(m + DTRU_PREFIXHASHBYTES, ct, sk);

  for (i = 0; i < DTRU_PREFIXHASHBYTES; ++i)
    m[i] = sk[i + DTRU_PKE_SECRETKEYBYTES];

  m[DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES] = 0;
  // crypto_hash_sha3_512(buf, m, DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES + 1);
  pseudohash(512,m,(DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES + 1) * 8 ,buf);
  m[DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES] = 1;
  // crypto_hash_sha3_512(buf + DTRU_SHAREDKEYBYTES, m, DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES + 1);
  pseudohash(512,m,(DTRU_PREFIXHASHBYTES + DTRU_MSGBYTES + 1) * 8 ,buf+ DTRU_SHAREDKEYBYTES);

//   crypto_hash_shake256(buf + DTRU_SHAREDKEYBYTES, DTRU_COINBYTES_ENC, buf + DTRU_SHAREDKEYBYTES, DTRU_SHAREDKEYBYTES);
  pseudoXOF(DTRU_COINBYTES_ENC * 8,buf + DTRU_SHAREDKEYBYTES,DTRU_SHAREDKEYBYTES *8,buf + DTRU_SHAREDKEYBYTES);

  pke_enc_avx2(ct2, sk + DTRU_PKE_SECRETKEYBYTES, m + DTRU_PREFIXHASHBYTES, buf + DTRU_SHAREDKEYBYTES);
