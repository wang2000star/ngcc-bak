#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "cpucycles.h"
#include "randombytes.h"
#include "dtru.h"
#include "params.h"
#include "speed.h"

#define NTESTS 1000
static uint64_t t[NTESTS];

void test_pke()
{
  unsigned int i;
  unsigned char ct[DTRU_PKE_CIPHERTEXTBYTES];
  unsigned char m[DTRU_MSGBYTES],  m2[DTRU_MSGBYTES];
  unsigned char coins[DTRU_COINBYTES_KEYGEN];
  unsigned char pk[DTRU_PKE_PUBLICKEYBYTES], sk[DTRU_PKE_SECRETKEYBYTES];
  randombytes(m, DTRU_MSGBYTES);

  for (i = 0; i < NTESTS; i++)
  {
    randombytes(coins, DTRU_COINBYTES_KEYGEN);
    pke_keygen(pk, sk, coins);
    pke_enc(ct, pk, m, coins);
    pke_dec(m2, ct, sk);
    for(int j = 0; j < DTRU_MSGBYTES; j ++)
    {
        if(m[j]!=m2[j])
        {
            printf("error key %d, m = %d, m2 = %d\n", j, m[j], m2[j]);
            return;
        }
    }
    if ((i + 1) % 1000 == 0) {
      printf("PKE test %d passed!\n", i + 1);
    }
  }

  for (i = 0; i < NTESTS; i++)
  {
      t[i] = cpucycles();
      pke_keygen(pk, sk, coins);
  }
  print_results("dtru_pke_keygen: ", t, NTESTS);

  for (i = 0; i < NTESTS; i++)
  {
      t[i] = cpucycles();
      pke_enc(ct, pk, m, coins);
  }
  print_results("dtru_pke_enc: ", t, NTESTS);

  for (i = 0; i < NTESTS; i++)
  {
      t[i] = cpucycles();
      pke_dec(m2, ct, sk);
  }
  print_results("dtru_pke_dec: ", t, NTESTS);

}

void test_pke_avx2()
{
  unsigned int i;
  unsigned char ct[DTRU_PKE_CIPHERTEXTBYTES];
  unsigned char m[DTRU_MSGBYTES],  m2[DTRU_MSGBYTES];
  unsigned char coins[DTRU_COINBYTES_KEYGEN];
  unsigned char pk[DTRU_PKE_PUBLICKEYBYTES], sk[DTRU_PKE_SECRETKEYBYTES];
  randombytes(m, DTRU_MSGBYTES);

  for (i = 0; i < NTESTS; i++)
  {
    randombytes(coins, DTRU_COINBYTES_KEYGEN);
    // pke_keygen(pk, sk, coins);
    pke_keygen_avx2(pk, sk, coins);

    // pke_enc(ct, pk, m, coins);
    pke_enc_avx2(ct, pk, m, coins);

    // pke_dec(m2, ct, sk);
    pke_dec_avx2(m2, ct, sk);
    for(int j = 0; j < DTRU_MSGBYTES; j ++)
    {
        if(m[j]!=m2[j])
        {
            printf("error key %d, m = %2x, m2 = %2x\n", j, m[j], m2[j]);
            return;
        }
    }
    if ((i + 1) % 1000 == 0) {
      printf("PKE test %d passed!\n", i + 1);
    }
  }

  for (i = 0; i < NTESTS; i++)
  {
      t[i] = cpucycles();
      pke_keygen_avx2(pk, sk, coins);
  }
  print_results("dtru_pke_keygen_avx2: ", t, NTESTS);

  for (i = 0; i < NTESTS; i++)
  {
      t[i] = cpucycles();
      pke_enc_avx2(ct, pk, m, coins);
  }
  print_results("dtru_pke_enc_avx2: ", t, NTESTS);

  for (i = 0; i < NTESTS; i++)
  {
      t[i] = cpucycles();
      pke_dec_avx2(m2, ct, sk);
  }
  print_results("dtru_pke_dec_avx2: ", t, NTESTS);

}

int main()
{
  // test_pke();
  test_pke_avx2();
  return 0;
}
