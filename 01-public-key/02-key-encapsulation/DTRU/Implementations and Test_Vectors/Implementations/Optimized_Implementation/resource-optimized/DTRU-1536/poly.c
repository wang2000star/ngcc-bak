                          ...
                          const unsigned char *msg)
{
  ...
  {
    bit_group = i % (DTRU_N / 16);
    tmp = (msg[bit_group / 2] >> ((bit_group % 2) * 4)) & 0xF;
    mh_byte = encode_e8(tmp);

    for (j = 0; j < 8; j++)
    {
      mask = -(int16_t)((mh_byte >> j) & 1);
