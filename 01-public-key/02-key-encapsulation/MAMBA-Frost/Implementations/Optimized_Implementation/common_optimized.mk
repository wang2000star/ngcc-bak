# Common build rules for the API_PKC AVX2 optimized Frost KEM instances.
#
# Each instance Makefile sets LEVEL and INSTANCE, then includes this file.  The
# KEM API remains the official API_PKC KEM_AlgorithmInstance interface.

CC ?= cc
ROOT := .
LEVEL ?= 128
INSTANCE ?= MAMBA-Frost-$(LEVEL)
CORE_API := $(ROOT)/Frost/src/api_frost$(LEVEL).h
ARCH_DEFINE ?= _AMD64_
...
CORE_SOURCES := \
	$(ROOT)/Frost/src/frost$(LEVEL).c \
	$(ROOT)/Frost/src/util.c \
...
kat-install: kat-generate
	...
	cp output/KAT_KEM_$(INSTANCE).txt ../../../Test_Vectors/KAT_KEM_$(INSTANCE).txt
