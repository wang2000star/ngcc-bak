...
FROST_STATIC_ASSERT(PARAMS_Q == 65536u || PARAMS_Q == 32768u, "Frost profiles require q = 2^15 or 2^16");
FROST_STATIC_ASSERT(PARAMS_P_PK == (1u << PARAMS_PK_LOGP), "p_pk must equal 2^t_pk");
FROST_STATIC_ASSERT(PARAMS_P_U == (1u << PARAMS_U_LOGP), "p_u must equal 2^t_u");
FROST_STATIC_ASSERT(PARAMS_P_V == (1u << PARAMS_V_LOGP), "p_v must equal 2^t_v");
FROST_STATIC_ASSERT(PARAMS_DELTA_PK == (PARAMS_Q / PARAMS_P_PK), "Delta_pk must equal q / p_pk");
FROST_STATIC_ASSERT(PARAMS_DELTA_U == (PARAMS_Q / PARAMS_P_U), "Delta_u must equal q / p_u");
FROST_STATIC_ASSERT(PARAMS_DELTA_V == (PARAMS_Q / PARAMS_P_V), "Delta_v must equal q / p_v");
FROST_STATIC_ASSERT(BYTES_SEED_A + PK_PACKED_BYTES == CRYPTO_PUBLICKEYBYTES, "public-key byte length mismatch");
FROST_STATIC_ASSERT(CT_C1_PACKED_BYTES + CT_C2_PACKED_BYTES + BYTES_SALT == CRYPTO_CIPHERTEXTBYTES, "ciphertext byte length mismatch");
FROST_STATIC_ASSERT(BYTES_PKHASH == 32u, "public-key hash byte length mismatch");
FROST_STATIC_ASSERT(SK_OFFSET_Z + CRYPTO_BYTES == CRYPTO_SECRETKEYBYTES, "secret-key byte length mismatch");
FROST_STATIC_ASSERT(CRYPTO_BYTES == (PARAMS_EXTRACTED_BITS * PARAMS_NBAR_R * PARAMS_NBAR_S) / 8u, "shared-secret byte length mismatch");
