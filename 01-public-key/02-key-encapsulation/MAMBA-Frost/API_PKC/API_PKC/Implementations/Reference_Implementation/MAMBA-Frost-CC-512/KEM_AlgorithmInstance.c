/* API_PKC adapter from the official KEM interface to the Frost-CC core API. */
#include <stddef.h>
#include "KEM_AlgorithmInstance.h"
#include "Frost-CC/src/api_frostcc512.h"
               ...
               unsigned char *sk, unsigned long long *sk_len_bytes)
{
    ...
    rc = crypto_kem_keypair_FrostCC512(pk, sk);
    if (rc != 0) {
            ...
            unsigned char *ct, unsigned long long *ct_len_bytes)
{
    ...
    rc = crypto_kem_enc_FrostCC512(ct, ss, pk);
    if (rc != 0) {
            ...
            unsigned char *ss, unsigned long long *ss_len_bytes)
{
    ...
    rc = crypto_kem_dec_FrostCC512(ss, ct, sk);
    if (rc != 0) {
        api_clear(ss, SHAREDSECRETBYTES);
        return KEM_ERR_INTERNAL;
    }
