...
#define TEST_B 2
#define TEST_U32 0
#define test_crypto_kem_keypair crypto_kem_keypair_FrostCC128
#define test_crypto_kem_enc crypto_kem_enc_FrostCC128
#define test_crypto_kem_dec crypto_kem_dec_FrostCC128
...
#define TEST_B 3
#define TEST_U32 0
#define test_crypto_kem_keypair crypto_kem_keypair_FrostCC192
#define test_crypto_kem_enc crypto_kem_enc_FrostCC192
#define test_crypto_kem_dec crypto_kem_dec_FrostCC192
...
#define TEST_B 4
#define TEST_U32 0
#define test_crypto_kem_keypair crypto_kem_keypair_FrostCC256
#define test_crypto_kem_enc crypto_kem_enc_FrostCC256
#define test_crypto_kem_dec crypto_kem_dec_FrostCC256
#elif FROST_TEST_LEVEL == 384
#include "../src/api_frostcc384.h"
#define TEST_BYTES_MU 48
#define TEST_NBAR 8
#define TEST_ELL_R 12
#define TEST_ELL_S 8
#define TEST_LOGQ 16
#define TEST_B 4
#define TEST_U32 0
#define test_crypto_kem_keypair crypto_kem_keypair_FrostCC384
#define test_crypto_kem_enc crypto_kem_enc_FrostCC384
#define test_crypto_kem_dec crypto_kem_dec_FrostCC384
#elif FROST_TEST_LEVEL == 512
#include "../src/api_frostcc512.h"
#define TEST_BYTES_MU 64
#define TEST_NBAR 8
#define TEST_ELL_R 16
#define TEST_ELL_S 8
#define TEST_LOGQ 16
#define TEST_B 4
#define TEST_U32 0
#define test_crypto_kem_keypair crypto_kem_keypair_FrostCC512
#define test_crypto_kem_enc crypto_kem_enc_FrostCC512
#define test_crypto_kem_dec crypto_kem_dec_FrostCC512
#else
...
static int check_row_wise_e8_blocks(void)
{
#ifdef FROST_USE_E8_CODE
    if (TEST_ELL_S != 8) return 0;
    uint8_t msg[TEST_BYTES_MU];
    memset(msg, 0, sizeof(msg));
    for (size_t block = 0; block < TEST_ELL_R; block++) {
        memset(msg, 0, sizeof(msg));
        for (size_t i = 0; i < TEST_B; i++) msg[block * TEST_B + i] = (uint8_t)(0xA5u ^ (uint8_t)(block + i));
#if TEST_U32
        uint32_t enc[TEST_ELL_R * TEST_ELL_S];
        frost_key_encode_u32(enc, msg);
#else
        uint16_t enc[TEST_ELL_R * TEST_ELL_S];
        frost_key_encode(enc, (const uint16_t *)msg);
#endif
        for (size_t row = 0; row < TEST_ELL_R; row++) {
            for (size_t col = 0; col < TEST_ELL_S; col++) {
                #if (TEST_ELL_S == 8) && (TEST_ELL_R != 8)
                const size_t idx = col * TEST_ELL_R + row;
#else
                const size_t idx = row * TEST_ELL_S + col;
#endif
                const int is_target_row = (row == block);
                if (!is_target_row && enc[idx] != 0) {
                    fprintf(stderr, "E8 row-wise block isolation failed at level %d block=%zu row=%zu col=%zu value=%u\n",
                            FROST_TEST_LEVEL, block, row, col, (unsigned)enc[idx]);
                    return 1;
                }
            }
        }
...
int main(void)
{
    ...
    if (check_row_wise_e8_blocks() != 0) return 1;
    if (check_kem() != 0) return 1;
    if (check_codec_trace_after_kem() != 0) return 1;
    printf("codec identity tests passed for Frost-CC-%d message_codec=%s\n", FROST_TEST_LEVEL, frost_message_codec_name());
    return 0;
}
