...
/* Global DRNG context — required by kem.c / indcpa.c extern */
...
int main(void)
{
    ...
    /* ================================================================
     *  Initialize DRNG, pre-generate ALL random data
     * ================================================================ */
    ...
    get_random_number(&drng_algorithm, m, (unsigned long long)sizeof(m) * 8);
    get_random_number(&drng_algorithm, coins, (unsigned long long)sizeof(coins) * 8);
    ...
    for (i = 0; i <= NTESTS; ++i) {
        get_random_number(&drng_algorithm, (unsigned char *)&rand_x[i],
                          (unsigned long long)sizeof(int16_t) * 8);
        get_random_number(&drng_algorithm, (unsigned char *)&rand_y[i],
                          (unsigned long long)sizeof(int16_t) * 8);
    }
    ...
    for (i = 0; i <= NTESTS; ++i) {
        t[i] = cpucycles();
        poly_r_ternary_p(&a, seed, nonce++);
    }
    print_results("poly_r_ternary_p:", t, NTESTS + 1);

    for (i = 0; i <= NTESTS; ++i) {
        t[i] = cpucycles();
        poly_e_ternary_p(&b, seed, nonce++);
    }
    ...
    get_random_number(&drng_algorithm, h_in, (unsigned long long)sizeof(h_in) * 8);
    get_random_number(&drng_algorithm, g_in, (unsigned long long)sizeof(g_in) * 8);
    ...
    /* KDF: pseudoXOF → SM3-based */
    {
        uint8_t kdf_in[2 * SEEDBYTES], kdf_out[SEEDBYTES];
        get_random_number(&drng_algorithm, kdf_in, (unsigned long long)sizeof(kdf_in) * 8);
        for (i = 0; i <= NTESTS; ++i) {
            t[i] = cpucycles();
            kdf(kdf_out, kdf_in, sizeof(kdf_in));
        }
        print_results("kdf:", t, NTESTS + 1);
    }
