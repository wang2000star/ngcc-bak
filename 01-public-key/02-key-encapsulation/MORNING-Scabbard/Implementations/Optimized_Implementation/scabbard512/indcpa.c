    ...
    uint8_t sk[SCABBARD_INDCPA_SECRETKEYBYTES])
{
    ...
    for (i = 0; i < SCABBARD_L; i++) {
        for(j = 0; j+16 <= SCABBARD_N; j+=16){
            v_coeffs = _mm256_loadu_si256((__m256i*)&pkpv.vec[i].coeffs[j]);
            ...
            _mm256_storeu_si256((__m256i*)&pkpv.vec[i].coeffs[j], v_coeffs);
        }
    }
    pack_sk(sk, &skpv);
    ...
    const uint8_t r[SCABBARD_SYMBYTES])
{
    ...
    const int shift_amt = SCABBARD_EQ - SCABBARD_EP;
    const int shift_amt_enc = SCABBARD_EQ - (SCABBARD_ET + SCABBARD_B);
    const __m256i v_h1 = _mm256_set1_epi16(h1);
    __m256i u_coeffs;
    for (i = 0; i < SCABBARD_L; i++) {
        for (j = 0; j+16 <= SCABBARD_N; j+=16) {
            u_coeffs = _mm256_loadu_si256((__m256i*)&u.vec[i].coeffs[j]);
            u_coeffs = _mm256_add_epi16(u_coeffs, v_h1);
            u_coeffs = _mm256_and_si256(u_coeffs, v_q);
            u_coeffs = _mm256_srli_epi16(u_coeffs, shift_amt);
            _mm256_storeu_si256((__m256i*)&u.vec[i].coeffs[j], u_coeffs);
        }
    }
    ...
    for (i = 0; i+16 <= SCABBARD_N; i+=16) {
        v_coeffs = _mm256_loadu_si256((__m256i*)&v.coeffs[i]);
