...
static void
printstr (char *identifier, unsigned char *msg, unsigned long long len)
{
	printf ("%s", identifier);
...
{
	int rtn = 0;
	long long pk_bytes = KEM_PUBLICKEYBYTES, sk_bytes = KEM_SECRETKEYBYTES,
	...
	init_random_number (&drng_algorithm, kat->seed, SEED_LEN_BYTES);
	...
	if (memcmp (ss0, kat->ss, ss_bytes) != 0)
		{
			...
			printstr ("\tsk =", kat->ss, ss_bytes);
			printstr ("\tss =", ss0, ss_bytes);
			rtn = 1;
		}
