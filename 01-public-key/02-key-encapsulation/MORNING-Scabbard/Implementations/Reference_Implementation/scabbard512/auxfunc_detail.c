...
static uint64_t pseudoXOF_cycles = 0;

void reset_pseudoXOF_cycles(void)
{
    pseudoXOF_cycles = 0;
}

uint64_t get_pseudoXOF_cycles(void)
{
    return pseudoXOF_cycles;
}
...
int pseudoXOF(unsigned long long output_len_bits, const unsigned char *msg, unsigned long long msg_len_bits, unsigned char *output)
{
	...
	#define PSEUDOXOF_RETURN(code) do { pseudoXOF_cycles += cpucycles() - start; return (code); } while (0)
	...
	{
		fprintf(stderr, "ERROR: Memory allocation failed at %s, line %d. \n", __FILE__, __LINE__);
		PSEUDOXOF_RETURN(MEMORY_ALLOCATION_FAILED);
	}
	unsigned char *K;
	...
	PSEUDOXOF_RETURN(XOF_SUCCESS);
	#undef PSEUDOXOF_RETURN
}