...
{
    PIKE_PROFILE_START(prof_start, PIKE_PROFILE_EC_ISOG_EVAL_ODD);
    ...
    // should we normalise it here, or do it later?
    image->is_A24_computed_and_normalized = 0;
    PIKE_PROFILE_STOP(prof_start, PIKE_PROFILE_EC_ISOG_EVAL_ODD);
}

void ec_isomorphism(ec_isom_t *isom, const ec_curve_t *from, const ec_curve_t *to)
{
    PIKE_PROFILE_START(prof_start, PIKE_PROFILE_EC_ISOMORPHISM);
    ...
    PIKE_PROFILE_STOP(prof_start, PIKE_PROFILE_EC_ISOMORPHISM);
}

void ec_iso_eval(ec_point_t *P, ec_isom_t *isom)
{
    PIKE_PROFILE_START(prof_start, PIKE_PROFILE_EC_ISO_EVAL);
    fp2_t tmp;
    fp2_mul(&P->x, &P->x, &isom->Nx);
    fp2_mul(&tmp, &P->z, &isom->Nz);
    fp2_sub(&P->x, &P->x, &tmp);
    fp2_mul(&P->z, &P->z, &isom->D);
    PIKE_PROFILE_STOP(prof_start, PIKE_PROFILE_EC_ISO_EVAL);
}
