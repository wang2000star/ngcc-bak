...
fp_run(void)
{
    ...
    fp_encode(tmp, &a);
    qsort(cycle_runs + 10, 10, sizeof cycle_runs[0], cmp_u64);
    printf("  GF(p) set_one runs in ........................................... %" PRIu64
           " cycles, (%u ignore me)\n",
           cycle_runs[14] / (16 * BENCH_LOOPS),
           tmp[0]);

    // GF(p) half
    for (i = 0; i < 20; i++) {
        fp_set_small(&a, (uint32_t)(12345 + i));
        cycles1 = cpucycles();
        for (n = 0; n < BENCH_LOOPS; n++) {
            fp_half(&a, &a);
            fp_half(&a, &a);
    ...
    fp_encode(tmp, &a);
    qsort(cycle_runs + 10, 10, sizeof cycle_runs[0], cmp_u64);
    printf("  GF(p) half runs in .............................................. %" PRIu64
           " cycles, (%u ignore me)\n",
           cycle_runs[14] / (6 * BENCH_LOOPS),
           tmp[0]);
