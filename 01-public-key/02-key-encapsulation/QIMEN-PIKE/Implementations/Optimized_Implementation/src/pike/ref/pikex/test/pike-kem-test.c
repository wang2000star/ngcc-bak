...
int test_pike_kem(int bench_loops) {
    ...
    for(int i = 0; i < bench_loops; i++) {
        ...
        PIKE_PROFILE_SET_PHASE(PIKE_PROFILE_PHASE_ENCAPS);
        cycles1 = cpucycles();
        ...
        PIKE_PROFILE_SET_PHASE(PIKE_PROFILE_PHASE_DECAPS);
        cycles1 = cpucycles();
...
int test_pike_encode_decode(int bench_loops) {
    ...
    for (int i = 0; i < bench_loops; i++) {
        ...
        PIKE_PROFILE_SET_PHASE(PIKE_PROFILE_PHASE_ENCODE_DECODE);
        sk_encode(sk_buf, &sk);
...
int main(int argc, char* argv[]) {
    ...
    int rc = test_pike_encode_decode(loops);
