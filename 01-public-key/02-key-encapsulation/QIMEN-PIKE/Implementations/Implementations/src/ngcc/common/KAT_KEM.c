...
int main()
{
	...
	const char *dir_name = getenv("NGCC_KAT_OUTPUT_DIR");
	if (NULL == dir_name || '\0' == dir_name[0])
	{
#ifdef NGCC_KAT_DEFAULT_OUTPUT_DIR
		dir_name = NGCC_KAT_DEFAULT_OUTPUT_DIR;
#else
		dir_name = "Test_Vector";
#endif
	}
	char file_path[512] = "";
	sprintf(algoname_output, "KAT_KEM_%s.txt", ALGORITHM_INSTANCE);
	path_len = snprintf(file_path, sizeof(file_path), "%s/%s", dir_name, algoname_output);
	...
	printf("\nFiles have been saved in the '%s' folder.\n", dir_name);
