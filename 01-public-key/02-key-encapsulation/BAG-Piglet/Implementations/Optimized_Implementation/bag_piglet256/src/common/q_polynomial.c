...
void q_polynomial_update_degree(q_polynomial* p, unsigned int position) {
  for(int i = (int) position ; i >= 0 ; --i) {
    if(ffi_elt_is_zero(&p->values.data[i]) == 0) {
      p->degree = i;
      return;
    }
  }
...
int q_polynomial_set(q_polynomial* o, const q_polynomial* p) {
  ...
  if(q_polynomial_is_zero(p) == 1) {
    ...
    ffi_elt zero = ffi_elt_get_zero();
    o->degree = p->degree;

    for(int i = 0 ; i <= p->degree ; ++i) {
      ffi_vec_set_coeff(&o->values, &p->values.data[i], i);
    }

    for(int i = p->degree + 1 ; i <= o->max_degree ; ++i) {
      ffi_vec_set_coeff(&o->values, &zero, i);
    }
  }
...
int q_polynomial_set_one(q_polynomial* o) {
  ...
  ffi_vec_set_coeff(&o->values, &one, 0);
...
int q_polynomial_set_interpolate(q_polynomial* o1, const ffi_vec* v1, int size) {
  if(o1->max_degree < size) {
    return INVALID_PARAMETERS;
  }

  ffi_elt tmp1 = {0}, tmp2 = {0}, tmp3 = {0};
  q_polynomial qtmp = {0};
  if(q_polynomial_init(&qtmp, (unsigned int) size) != 0) {
    return INVALID_PARAMETERS;
  }
  ...
  for(int i = 0 ; i < size ; ++i)  {
    q_polynomial_evaluate(&tmp1, o1, &v1->data[i]);
...
int q_polynomial_set_interpolate_vect_and_zero(q_polynomial* o1, q_polynomial* o2, const ffi_vec* v1, const ffi_vec* v2, int size) {
  ...
  for(int i = 0 ; i < size ; ++i)  {
    q_polynomial_evaluate(&tmp1, o1, &v1->data[i]);
    q_polynomial_evaluate(&tmp2, o2, &v1->data[i]);

    ffi_elt_inv(&tmp3, &tmp1);
    ffi_elt_add(&tmp2, &tmp2, &v2->data[i]);
    ffi_elt_mul(&tmp2, &tmp2, &tmp3);
...
void q_polynomial_evaluate(ffi_elt* o, const q_polynomial* p, const ffi_elt* e) {
  if(q_polynomial_is_zero(p) == 1) {
    ...
    ffi_elt tmp1 = {0}, tmp2 = {0};

    ffi_elt_set(&tmp1, e);
    ffi_elt_mul(o, &p->values.data[0], &tmp1);

    for(int i = 1 ; i <= p->degree ; ++i) {
      ffi_elt_sqr(&tmp1, &tmp1);
      ffi_elt_mul(&tmp2, &p->values.data[i], &tmp1);
...
int q_polynomial_scalar_mul(q_polynomial* o, const q_polynomial* p, const ffi_elt* e) {
  ...
  if(q_polynomial_is_zero(p) == 1 || ffi_elt_is_zero(e) == 1) {
    ...
    ffi_elt zero = ffi_elt_get_zero();
    for(int i = 0 ; i <= p->degree ; ++i) {
      ffi_elt_mul(&tmp, &p->values.data[i], e);
...
int q_polynomial_qexp(q_polynomial* o, const q_polynomial* p) {
  ...
  if(q_polynomial_is_zero(p) == 1) {
    ...
    ffi_elt tmp = {0};
    ffi_elt zero = ffi_elt_get_zero();
    ffi_vec_set_coeff(&o->values, &zero, 0);
    for(int i = 0 ; i <= p->degree ; ++i) {
      ffi_elt_sqr(&tmp, &p->values.data[i]);
      ffi_vec_set_coeff(&o->values, &tmp, (i + 1) % FIELD_M);
    }
...
int q_polynomial_add(q_polynomial* o, const q_polynomial* p, const q_polynomial* q) {
  ...
  if(degree == -1) {
    ...
    ffi_elt tmp = {0}, p_coeff = {0}, q_coeff = {0};
    ffi_elt zero = ffi_elt_get_zero();
    for(int i = 0 ; i <= o->degree ; ++i) {
      p_coeff = ffi_vec_get_coeff(&p->values, i);
      q_coeff = ffi_vec_get_coeff(&q->values, i);
      ffi_elt_add(&tmp, &p_coeff, &q_coeff);
      ffi_vec_set_coeff(&o->values, &tmp, i);
    }
...
int q_polynomial_mul(q_polynomial* o, const q_polynomial* p, const q_polynomial* q) {
  ...
  if(q_polynomial_is_zero(p) == 1 || q_polynomial_is_zero(q) == 1) {
    ...
    for(int j = 0 ; j <= q->degree ; ++j) {
      ffi_elt o_coeff = {0};
      ffi_elt_set(&tmp1, &q->values.data[j]);
      ffi_elt_mul(&tmp2, &p->values.data[0], &tmp1);
      o_coeff = ffi_vec_get_coeff(&o->values, j);
      ffi_elt_add(&tmp3, &o_coeff, &tmp2);
      ffi_vec_set_coeff(&o->values, &tmp3, j);
      for(int i = 1 ; i <= p->degree ; ++i) {
        unsigned int position = (unsigned int) ((i + j) % FIELD_M);
        ffi_elt_sqr(&tmp1, &tmp1);
        ffi_elt_mul(&tmp2, &p->values.data[i], &tmp1);
        o_coeff = ffi_vec_get_coeff(&o->values, position);
        ffi_elt_add(&tmp3, &o_coeff, &tmp2);
        ffi_vec_set_coeff(&o->values, &tmp3, position);
      }
    }
...
int q_polynomial_left_div(q_polynomial* q, q_polynomial* r, const q_polynomial* a, const q_polynomial* b) {
  ...
  q_polynomial s = {0};
  q_polynomial t = {0};
  if(q_polynomial_init(&rtmp, (unsigned int) a->degree) != 0 ||
     q_polynomial_init(&s, (unsigned int) (a->degree - b->degree)) != 0 ||
     q_polynomial_init(&t, (unsigned int) a->max_degree) != 0) {
    q_polynomial_clear(&rtmp);
  ...
  ffi_elt b_lead_coeff_inv = {0};
  ffi_elt r_lead_coeff = {0};
  ffi_elt s_lead_coeff = {0};
  
  ffi_elt_set(&b_lead_coeff_inv, &b->values.data[b->degree]);
  ffi_elt_inv(&b_lead_coeff_inv, &b_lead_coeff_inv);

  while(rtmp.degree >= b->degree) {
    // Compute monial s
    ffi_elt_set(&r_lead_coeff, &rtmp.values.data[rtmp.degree]);
...
void q_polynomial_print(const q_polynomial* p) {
  if(q_polynomial_is_zero(p) == 1) {
    ...
    for(int i = p->degree; i > 0 ; --i) {
      ffi_elt_print(&p->values.data[i]);
      printf("x^[%i] + ", i);
    }
    ffi_elt_print(&p->values.data[0]);
    printf("x^[0]");
  }
  printf("\n");
}
