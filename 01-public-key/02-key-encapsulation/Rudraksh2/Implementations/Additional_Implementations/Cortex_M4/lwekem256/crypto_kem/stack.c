...
static int test_keys(void) {
  ...
  FILL_STACK()
  MUPQ_crypto_kem_keypair(pk, &pk_len_bytes, sk_a, &sk_len_bytes);
  CHECK_STACK()
  ...
  FILL_STACK()
  MUPQ_crypto_kem_enc(pk, pk_len_bytes, key_a, &ss_len_bytes, sendb, &ct_len_bytes);
  CHECK_STACK()
  if(c >= canary_size) return -1;
  stack_encaps = c;
  ...
  FILL_STACK()
  MUPQ_crypto_kem_dec(sk_a, sk_len_bytes, sendb, ct_len_bytes, key_b, &ss_len_bytes);
  CHECK_STACK()
  if(c >= canary_size) return -1;
  stack_decaps = c;
