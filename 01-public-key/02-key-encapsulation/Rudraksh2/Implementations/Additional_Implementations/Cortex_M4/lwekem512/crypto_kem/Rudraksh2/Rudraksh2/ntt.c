...
const uint16_t omegas_inv_bitrev_montgomery[KEM_N / 2] = OMEGAS_INV_BITREV_MONTGOMERY;
					...
					for (j = start; j < start + (1 << level); ++j)
						{
							...
							// Explanations:
							// - (KEM_Q << 1): 2 * KEM_Q to avoid underflow, 0 < t < 2 * KEM_Q
					...
					for (j = start; j < KEM_N - 1; j += 2 * (1 << level))
						{
							...
							// Explanations:
							// - (KEM_Q << 1): 2 * KEM_Q to avoid underflow, 0 < a[...] < 2 * KEM_Q
