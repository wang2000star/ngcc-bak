...
// #define MINAL_BETA 270

static int KEM_Q = 0;
static int B2_MINAL_Q = 0;
static int B2_MINAL_HALF_Q = 0;
static int MINAL_Q = 0;
static int MINAL_ALPHA = 0;
static int MINAL_BETA = 0;

#define MINAL_DIM 2
#define N_CODE_VALS 4

// static int16_t CODE_VALS[4] = {0, MINAL_BETA, MINAL_ALPHA, MINAL_BETA + MINAL_ALPHA - MINAL_Q};
static int16_t CODE_VALS[4] = {0};
...
void minal_b2_init(int kem_q, int beta) {
    KEM_Q = kem_q;
    B2_MINAL_Q = KEM_Q;
    B2_MINAL_HALF_Q = (B2_MINAL_Q / 2 + 1);
    MINAL_Q = (B2_MINAL_Q / 2);
    MINAL_ALPHA = (MINAL_Q / 2);
    MINAL_BETA = beta;

    CODE_VALS[0] = 0;
    CODE_VALS[1] = MINAL_BETA;
    CODE_VALS[2] = MINAL_ALPHA;
    CODE_VALS[3] = MINAL_BETA + MINAL_ALPHA - MINAL_Q;
}
...
{
    target[0] = (target[0] % KEM_Q);
    target[1] = (target[1] % KEM_Q);
    target[0] = target[0] >= 0 ? target[0] : target[0] + KEM_Q;
    target[1] = target[1] >= 0 ? target[1] : target[1] + KEM_Q;
    // Now: 0 <= target[i] < q
