...
{
	unsigned int i;
	uint8_t buf[2 * KEM_SYMBYTES + 1];
	const uint8_t *publicseed = buf;
	uint8_t *noiseseed = buf + KEM_SYMBYTES;
	...
	int buf1len = KEM_ETA * KEM_N / 4;

	noiseseed[KEM_SYMBYTES] = 0;

	for (i = 0; i < KEM_L; i++)
		{
			pseudoXOF (buf1len * 8, noiseseed, (KEM_SYMBYTES + 1) * 8, buf1);
			noiseseed[KEM_SYMBYTES] += 1;
						...
						const uint8_t coins[KEM_SYMBYTES])
{
	...
	// Convert the message from bytes -> polynomial space
	poly_frommsg (&k, m);

	//  Sample the random matrix from the seed
	gen_at (at, seed);
	...
	// The coins from which we will derive further seed_r for s', e', and e''
	// generation
	for (i = 0; i < KEM_SYMBYTES; i++)
		coins1[i] = coins[i];
	...
	// c_m <- INTT(hat(c)_m) + e'' + encode(m)
	poly_add (&v, &v, &epp);
	poly_add (&v, &v, &k);

	// Compress the polynomial and convert to bytes
						...
						const uint8_t sk[KEM_INDCPA_SECRETKEYBYTES])
{
	...
	// Unpack the ciphertext (decode bytes to polynomial, then decompress)
	// From this we get u', v' representing the coefficents and constant term.
	unpack_ciphertext (&b, &v, c);

	// Unpack the secret key (convert from bytes to polynomial)
	unpack_sk (&skpv, sk);

	// Convert to NTT
	polyvec_ntt (&b);

	// Multiply the secret key with the NTT ciphertext
	polyvec_basemul_acc_montgomery (&mp, &skpv, &b);

	// Convert out of NTT
	poly_invntt_tomont (&mp);

	// Recover the original message by subtracting the error constant terms.
	poly_sub (&mp, &v, &mp);

	// Convert polynomial back to message bytes
	poly_tomsg (m, &mp);
}
