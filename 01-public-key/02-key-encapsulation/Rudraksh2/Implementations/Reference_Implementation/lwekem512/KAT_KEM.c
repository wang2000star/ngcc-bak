...
static void fprintlen (FILE *file_output, char *identifier,
											 unsigned long long len);
static void fprintstr (FILE *file_output, char *identifier, unsigned char *msg,
											 unsigned long long len);
	...
	for (int i = 0; i < 10; i++)
		{
			...
			if (memcmp (ss, ss1, ss_len_bytes))
				{
					fprintf (stderr,
									 "ERROR: decapsulation shared secret key != encapsulation "
									 "shared secret key at %s, line %d. \n",
									 __FILE__, __LINE__);
...
static int
validate_algorithm_instance_name (const char *algorithm)
{
	int rt = 0;
	char c = '\0';
...
static void
fprintlen (FILE *file_output, char *identifier, unsigned long long len)
{
...
static void
fprintstr (FILE *file_output, char *identifier, unsigned char *msg,
					 unsigned long long len)
{
