			...
			for (j = 0; j < 8; j++)
				{
					...
					// Scaling chosen with `find_factor.py` script
					d0 *= KEM_Q_DENOM;
					// Down-shift back to get the proper result
					d0 >>= 26;
					// Take only the bottom LOG2T bits
...
poly_frombytes (poly *r, const uint8_t a[KEM_POLYBYTES])
{
	...
	for (i = 0; i < KEM_N / 8; i++)
		{
			r->coeffs[(8 * i) + 0] = (a[(13 * i) + 0] & (0xff))
															 | (((uint16_t)a[(13 * i) + 1] & 0x1f) << 8);
			r->coeffs[(8 * i) + 1] = (a[(13 * i) + 1] >> 5 & (0x07))
															 | (((uint16_t)a[(13 * i) + 2] & 0xff) << 3)
															 | (((uint16_t)a[(13 * i) + 3] & 0x03) << 11);
			r->coeffs[(8 * i) + 2] = (a[(13 * i) + 3] >> 2 & (0x3f))
															 | (((uint16_t)a[(13 * i) + 4] & 0x7f) << 6);
			r->coeffs[(8 * i) + 3] = (a[(13 * i) + 4] >> 7 & (0x01))
															 | (((uint16_t)a[(13 * i) + 5] & 0xff) << 1)
															 | (((uint16_t)a[(13 * i) + 6] & 0x0f) << 9);
			r->coeffs[(8 * i) + 4] = (a[(13 * i) + 6] >> 4 & (0x0f))
															 | (((uint16_t)a[(13 * i) + 7] & 0xff) << 4)
															 | (((uint16_t)a[(13 * i) + 8] & 0x01) << 12);
			r->coeffs[(8 * i) + 5] = (a[(13 * i) + 8] >> 1 & (0x7f))
															 | (((uint16_t)a[(13 * i) + 9] & 0x3f) << 7);
			r->coeffs[(8 * i) + 6] = (a[(13 * i) + 9] >> 6 & (0x03))
															 | (((uint16_t)a[(13 * i) + 10] & 0xff) << 2)
															 | (((uint16_t)a[(13 * i) + 11] & 0x07) << 10);
			r->coeffs[(8 * i) + 7] = (a[(13 * i) + 11] >> 3 & (0x1f))
															 | (((uint16_t)a[(13 * i) + 12] & 0xff) << 5);
		}
}
...
poly_basemul_montgomery (poly *r, const poly *a, const poly *b)
{
	...
	for (j = 0; j < KEM_N; j++)
		{
			// 4613 = 2^{2*18} % q
			t = montgomery_reduce (MONT_2 * (uint32_t)b->coeffs[j]);
 ...
 * Name:        poly_freeze
 *
 * Description: Apply freeze to each member of a polynomial to reduce to
 * 				0..KEM_Q.
 *
 * Arguments:   - poly *r: pointer to input/output polynomial
 **************************************************/
