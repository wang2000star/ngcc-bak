						 ...
						 unsigned int buflen)
{
	...
	while (ctr < len && pos + 13 <= buflen)
		{
			val[0]
					= ((buf[pos + 0] & (0xff)) | ((buf[pos + 1] & 0x1f) << 8)) & 0x1fff;
			val[1] = ((buf[pos + 1] >> 5 & (0x07)) | ((buf[pos + 2] & 0xff) << 3)
								| ((buf[pos + 3] & 0x03) << 11))
							 & 0x1fff;
			val[2] = ((buf[pos + 3] >> 2 & (0x3f)) | ((buf[pos + 4] & 0x7f) << 6))
							 & 0x1fff;
			val[3] = ((buf[pos + 4] >> 7 & (0x01)) | ((buf[pos + 5] & 0xff) << 1)
								| ((buf[pos + 6] & 0x0f) << 9))
							 & 0x1fff;
			val[4] = ((buf[pos + 6] >> 4 & (0x0f)) | ((buf[pos + 7] & 0xff) << 4)
								| ((buf[pos + 8] & 0x01) << 12))
							 & 0x1fff;
			...
			val[6] = ((buf[pos + 9] >> 6 & (0x03)) | ((buf[pos + 10] & 0xff) << 2)
								| ((buf[pos + 11] & 0x07) << 10))
							 & 0x1fff;
			val[7] = ((buf[pos + 11] >> 3 & (0x1f)) | ((buf[pos + 12] & 0xff) << 5))
							 & 0x1fff;
			pos += 13;
