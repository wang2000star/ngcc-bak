				 ...
				 unsigned long long *ss_len_bytes)
{
	...
	/* Multitarget countermeasure for coins + contributory KEM */
	memcpy (buf + KEM_SYMBYTES, sk + KEM_SECRETKEYBYTES - 2 * KEM_SYMBYTES,
					KEM_SYMBYTES);

	hash_g (kr, buf, 2 * KEM_SYMBYTES);

	/* coins are in kr+KEM_SYMBYTES */
	indcpa_enc (cmp, buf, pk, kr + KEM_SYMBYTES);

	fail = verify (ct, cmp, KEM_CIPHERTEXTBYTES);

	memcpy (ct1, ct, KEM_CIPHERTEXTBYTES);
	memcpy (ct1 + KEM_CIPHERTEXTBYTES, sk + KEM_SECRETKEYBYTES - KEM_SYMBYTES,
					KEM_SYMBYTES);
	hash_h (kr + KEM_SYMBYTES, ct1, (KEM_CIPHERTEXTBYTES + KEM_SYMBYTES));
	cmov (kr, kr + KEM_SYMBYTES, KEM_SYMBYTES, fail);
	memcpy (ss, kr, KEM_SYMBYTES);
	...
	// cmov(kr, sk+KEM_SECRETKEYBYTES-KEM_SYMBYTES, KEM_SYMBYTES, fail);
