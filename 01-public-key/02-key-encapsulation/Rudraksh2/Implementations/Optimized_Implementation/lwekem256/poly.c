...
// #if (KEM_INDCPA_MSGBYTES != KEM_N / 4)
// #error "KEM_INDCPA_MSGBYTES must be equal to KEM_N/8 bytes!"
// #endif
...
// 				{
// 					mask = ((msg[i] >> (2 * j)) & 0x03);
// 					r->coeffs[(4 * i) + j] = ((uint32_t)(mask * KEM_Q) + 2) >> 2;
// 				}
// 		}
// }
...
// 	minal_b2_code_init (&minal, KEM_Q, 0, 2); // beta needs to change
...
// 					minal_b2_code_encode (codeword, msg_bits, &minal);
...
// 			for (j = 0; j < 4; j++)
// 				{
// 					// t  = (((freeze(a->coeffs[(4*i)+j]) << 2) + KEM_Q/2)/KEM_Q) &
// 					// 0x03;
// 					t = a->coeffs[(4 * i) + j];
// 					t <<= 2;
// 					// t += 3840; //KEM_Q/2;
// 					// t *= 139792; //(2^30/KEM_Q)+1;
// 					t += KEM_Q_2;		// KEM_Q/2;
// 					t *= KEM_Q_230; //(2^30/KEM_Q)+1;
// 					t >>= 30;
// 					t &= 0x3;
// 					msg[i] |= t << (2 * j);
...
// 					msg[i] |= (minal_b2_code_decode (codeword, &minal) << j);
