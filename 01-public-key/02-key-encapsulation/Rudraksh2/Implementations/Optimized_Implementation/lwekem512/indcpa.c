						 ...
						 unsigned int buflen)
{
  ...
  while(ctr < len && pos + 13 <= buflen) {
    val[0]= (( buf[ pos + 0 ] & (0xff)) | ((buf[pos + 1] & 0x1f)<<8)) & 0x1fff;
    val[1]= (( buf[ pos + 1 ]>>5 & (0x07)) | ((buf[pos + 2] & 0xff)<<3) | ((buf[pos + 3] & 0x03)<<11)) & 0x1fff;
    val[2]= (( buf[ pos + 3 ]>>2 & (0x3f)) | ((buf[pos + 4] & 0x7f)<<6)) & 0x1fff;
    val[3]= (( buf[ pos + 4 ]>>7 & (0x01)) | ((buf[pos + 5] & 0xff)<<1) | ((buf[pos + 6] & 0x0f)<<9)) & 0x1fff;
    val[4]= (( buf[ pos + 6 ]>>4 & (0x0f)) | ((buf[pos + 7] & 0xff)<<4) | ((buf[pos + 8] & 0x01)<<12)) & 0x1fff;
    val[5]= (( buf[ pos + 8]>>1 & (0x7f)) | ((buf[pos + 9] & 0x3f)<<7)) & 0x1fff;
    val[6]= (( buf[ pos + 9]>>6 & (0x03)) | ((buf[pos + 10] & 0xff)<<2) | ((buf[pos + 11] & 0x07)<<10)) & 0x1fff;
    val[7]= (( buf[ pos + 11]>>3 & (0x1f)) | ((buf[pos + 12] & 0xff)<<5)) & 0x1fff;
    pos += 13;
						...
						const uint8_t coins[KEM_SYMBYTES])
{
	...
	// polyvec b1;
	// poly v1;
	// unpack_ciphertext (&b1, &v1, c);
	// printf ("Ciphertext polynomial b:\n");
	// for (i = 0; i < KEM_L; i++)
	// {
	// 	for (int j = 0; j < KEM_N; j++)
	// 	{
	// 		printf ("b.vec[%d].coeffs[%d] = %d, b1.vec[%d].coeffs[%d] = %d\n", i, j, b.vec[i].coeffs[j], i, j, b1.vec[i].coeffs[j]);
	// 	}
	// }
	// printf ("\n");
	// printf ("Ciphertext polynomial v:\n");
	// for (i = 0; i < KEM_N; i++)
	// {
	// 	printf ("v.coeffs[%d] = %d, v1.coeffs[%d] = %d, v.coeffs[%d]-v.coeffs[%d] = %d\n", i, v.coeffs[i], i, v1.coeffs[i], i, i, v.coeffs[i] - v1.coeffs[i]);
	// }
	// printf ("\n");
}
