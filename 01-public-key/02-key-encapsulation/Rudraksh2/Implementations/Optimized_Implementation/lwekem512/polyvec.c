...
polyvec_compress (uint8_t r[KEM_POLYVECCOMPRESSEDBYTES], const polyvec *a)
{
	...
	for (i = 0; i < KEM_L; i++)
	{
		...
		for (j = 0; j < KEM_N;) {
			...
			r[1] = (uint8_t) (((a->vec[i].coeffs[j] >> 8) & 0x1f) | (a->vec[i].coeffs[j+1] << 5));
			r[2] = (uint8_t) (a->vec[i].coeffs[j+1] >> 3);
			r[3] = (uint8_t) (((a->vec[i].coeffs[j+1] >> 11) & 0x03) | (a->vec[i].coeffs[j+2] << 2));
			r[4] = (uint8_t) (((a->vec[i].coeffs[j+2] >> 6) & 0x7f) | (a->vec[i].coeffs[j+3] << 7));
			r[5] = (uint8_t) (a->vec[i].coeffs[j+3] >> 1);
			r[6] = (uint8_t) (((a->vec[i].coeffs[j+3] >> 9) & 0x0f) | (a->vec[i].coeffs[j+4] << 4));
			r[7] = (uint8_t) (a->vec[i].coeffs[j+4] >> 4);
			r[8] = (uint8_t) (((a->vec[i].coeffs[j+4] >> 12) & 0x01) | (a->vec[i].coeffs[j+5] << 1));
			r[9] = (uint8_t) (((a->vec[i].coeffs[j+5] >> 7) & 0x3f) | (a->vec[i].coeffs[j+6] << 6));
			r[10] = (uint8_t) (a->vec[i].coeffs[j+6] >> 2);
			r[11] = (uint8_t) (((a->vec[i].coeffs[j+6] >> 10) & 0x07) | (a->vec[i].coeffs[j+7] << 3));
...
polyvec_decompress (polyvec *r, const uint8_t a[KEM_POLYVECCOMPRESSEDBYTES])
{
	...
	for (i = 0; i < KEM_L; i++)
	{
		...
		for (j = 0; j < (KEM_N / 8); j++)
		{
			...
			t[1]= ( a[1]>>5 & (0x07)) | (((uint16_t)a[2] & 0xff)<<3) | (((uint16_t)a[3] & 0x03)<<11);
			t[2]= ( a[3]>>2 & (0x3f)) | (((uint16_t)a[4] & 0x7f)<<6);
			t[3]= ( a[4]>>7 & (0x01)) | (((uint16_t)a[5] & 0xff)<<1) | (((uint16_t)a[6] & 0x0f)<<9);
			t[4]= ( a[6]>>4 & (0x0f)) | (((uint16_t)a[7] & 0xff)<<4) | (((uint16_t)a[8] & 0x01)<<12);
			t[5]= ( a[8]>>1 & (0x7f)) | (((uint16_t)a[9] & 0x3f)<<7);
			t[6]= ( a[9]>>6 & (0x03)) | (((uint16_t)a[10] & 0xff)<<2) | (((uint16_t)a[11] & 0x07)<<10);
...
#else
		for (j = 0; j < KEM_N;) {
			r->vec[i].coeffs[j]= ( a[0] & (0xff)) | (((uint16_t)a[1] & 0x1f)<<8);
			r->vec[i].coeffs[j+1]= ( a[1]>>5 & (0x07)) | (((uint16_t)a[2] & 0xff)<<3) | (((uint16_t)a[3] & 0x03)<<11);
			r->vec[i].coeffs[j+2]= ( a[3]>>2 & (0x3f)) | (((uint16_t)a[4] & 0x7f)<<6);
			r->vec[i].coeffs[j+3]= ( a[4]>>7 & (0x01)) | (((uint16_t)a[5] & 0xff)<<1) | (((uint16_t)a[6] & 0x0f)<<9);
			r->vec[i].coeffs[j+4]= ( a[6]>>4 & (0x0f)) | (((uint16_t)a[7] & 0xff)<<4) | (((uint16_t)a[8] & 0x01)<<12);
			r->vec[i].coeffs[j+5]= ( a[8]>>1 & (0x7f)) | (((uint16_t)a[9] & 0x3f)<<7);
			r->vec[i].coeffs[j+6]= ( a[9]>>6 & (0x03)) | (((uint16_t)a[10] & 0xff)<<2) | (((uint16_t)a[11] & 0x07)<<10);
			r->vec[i].coeffs[j+7]= ( a[11]>>3 & (0x1f)) | (((uint16_t)a[12] & 0xff)<<5);
			j += 8;
			a += 13;
#endif
		}
...
{
	basemul_mont_avx(r, &a->vec[0], &b->vec[0], qdata);
	for (int i = 1; i < KEM_L; i++)
	{
		poly temp;
		basemul_mont_avx(&temp, &a->vec[i], &b->vec[i], qdata);
		poly_add(r, r, &temp);
	}
	return;
	// int i, j;
