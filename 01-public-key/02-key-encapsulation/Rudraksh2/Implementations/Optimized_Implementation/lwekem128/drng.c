...
static void
u32_to_u8_big_endian (unsigned int u32, unsigned char u8[4])
{
...
static void
inc_Big_Number (unsigned char *BN, unsigned long long BN_len_bytes)
{
...
static void
plus_Big_Number (unsigned char *BN1, const unsigned char *BN2,
								 ...
								 unsigned long long BN_len_bytes)
{
	...
	for (; BN_len_bytes != 0; BN_len_bytes--)
		{
			sum = BN1[BN_len_bytes - 1] + BN2[BN_len_bytes - 1]
						+ BN3[BN_len_bytes - 1] + BN4[BN_len_bytes - 1] + carry;
			carry = sum / (0xFFU + 1);
...
static int
SM3_df (unsigned char *input_string, unsigned long long input_string_len_bytes)
{
	...
	for (unsigned long long i = 0; i < len; i++)
		{
			data_to_SM3_len_bytes = sizeof (counter)
															+ sizeof (number_of_bits_to_return_big_endian)
															+ input_string_len_bytes;
			...
			memcpy (data_to_SM3 + sizeof (counter),
							number_of_bits_to_return_big_endian,
							sizeof (number_of_bits_to_return_big_endian));
			memcpy (data_to_SM3 + sizeof (counter)
									+ sizeof (number_of_bits_to_return_big_endian),
							input_string, input_string_len_bytes);
...
static int
SM3_DRNG_Instantiate (DRNG_ctx *drng, const unsigned char *nonce,
											unsigned long long nonce_len_bytes)
{
...
static int
SM3_DRNG_Generate (DRNG_ctx *drng, unsigned long long requested_no_of_bits,
									 unsigned char *return_bits)
{
	...
	if (requested_no_of_Byte >= 1)
		{
			return_bits[requested_no_of_Byte - 1] &= HIGH_N_BIT_MASK (
					8 - (8 * requested_no_of_Byte - requested_no_of_bits));
		}
	memset (H, 0, sizeof (H));
	padded_V[0] = 0x03;
	memcpy (padded_V + 1, drng->V, sizeof (drng->V));
...
int
init_random_number (DRNG_ctx *drng, const unsigned char *seed,
...
int
get_random_number (DRNG_ctx *drng, unsigned char *random_number,
