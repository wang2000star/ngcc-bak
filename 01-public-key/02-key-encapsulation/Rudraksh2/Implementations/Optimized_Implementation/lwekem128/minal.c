...
static int16_t CODE_VALS[4]
		= { 0, MINAL_BETA, MINAL_ALPHA, MINAL_BETA + MINAL_ALPHA - MINAL_Q };
...
uint32_t
get_distance_sqr_to_codeword (uint16_t idx,
															uint32_t distsqr_matrix[][N_CODE_VALS])
{
...
internal_minal_code_decode (int16_t target[])
{
	...
	uint32_t min_dist_codeword
			= get_distance_sqr_to_codeword (0, distsqr_matrix);
	for (int i = 1; i < (1 << 2); i++)
		{
			min_dist_codeword = secure_min (
					get_distance_sqr_to_codeword (i, distsqr_matrix), min_dist_codeword);
		}
...
minal_b2_code_decode (int16_t target[])
{
	...
	int16_t internal_target[2]
			= { target[0] - B2_MINAL_HALF_Q, target[1] - B2_MINAL_HALF_Q };
	int16_t internal_decode = internal_minal_code_decode (internal_target);
	uint8_t internal_decode_bits[2]
			= { (internal_decode >> 1) & 1, internal_decode & 1 };
	int16_t internal_cw[2] = { 0, 0 };

	internal_cw[0] = internal_decode_bits[0] * MINAL_ALPHA
									 + internal_decode_bits[1] * MINAL_BETA;
	internal_cw[1] = internal_decode_bits[1] * MINAL_ALPHA
									 + internal_decode_bits[0] * MINAL_BETA;
