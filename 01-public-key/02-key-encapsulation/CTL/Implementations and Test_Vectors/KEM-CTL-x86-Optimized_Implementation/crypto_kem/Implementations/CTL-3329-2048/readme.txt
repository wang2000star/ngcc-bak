CTL-3329-2048 KEM 优化实现
================================================================================

概述
--------------------------------------------------------------------------------
CTL-3329-2048 是基于格密码学的密钥封装机制（KEM）算法优化实现，遵循商用密码标准研究院的编程接口规范。本实现采用 AVX2 指令集优化，显著提升了性能�?
目录结构
--------------------------------------------------------------------------------
CTL-3329-2048/
  +-- api.c              # 核心API实现
  +-- api_3329_2048.c    # CTL-3329-2048特有的API实现
  +-- auxfunc.c/.h       # 辅助函数
...
2. 密钥生成
   unsigned char pk[3009];   // 公钥缓冲�?   unsigned char sk[15617];  // 私钥缓冲�?   unsigned long long pk_len, sk_len;

   int ret = kem_keygen(pk, &pk_len, sk, &sk_len);
   // ret == 0 表示成功

3. 密钥封装（Encapsulation�?   unsigned char ct[2353];   // 密文缓冲�?   unsigned char ss[48];     // 共享密钥缓冲�?   unsigned long long ct_len, ss_len;

   int ret = kem_enc(pk, pk_len, ss, &ss_len, ct, &ct_len);
   // ret == 0 表示成功

4. 密钥解封装（Decapsulation�?   unsigned char ss2[48];   // 解密后的共享密钥
...
KAT 测试向量生成:
  ...
  生成的测试向量将输出�?output/KAT_KEM_CTL-3329-2048.txt 文件�?
性能基准测试:
  # 运行性能基准测试
  ./benchmark.exe
