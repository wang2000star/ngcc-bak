...
static unsigned
bitrev_unsigned(unsigned x, unsigned logn)
{
	unsigned r, u;
...
static fnr
fnr_from_double(double x)
{
	int64_t z;

	z = (int64_t)llround(x * 4294967296.0);
	return fnr_of_scaled32((uint64_t)z);
}

static void
fnr_gm_get(fnr *re, fnr *im, unsigned logn, size_t k)
{
	if (logn <= 10) {
		*re = fnr_gm_tab[(k << 1) + 0];
		*im = fnr_gm_tab[(k << 1) + 1];
...

	{
		...
		rk = bitrev_unsigned((unsigned)k, logn);
		angle = pi * (double)rk / (double)((size_t)1 << logn);
		*re = fnr_from_double(cos(angle));
		*im = fnr_from_double(sin(angle));
	}
}
...
ctl_FFT(fnr *f, unsigned logn)
{
	...
	for (u = 1, m = 2; u < logn; u ++, m <<= 1) {
		...
		for (i1 = 0, j1 = 0; i1 < hm; i1 ++, j1 += t) {
			...
			j2 = j1 + ht;
			fnr_gm_get(&s_re, &s_im, logn, m + i1);
			for (j = j1; j < j2; j ++) {
...
ctl_iFFT(fnr *f, unsigned logn)
{
	...
	for (u = logn; u > 1; u --) {
		...
		for (i1 = 0, j1 = 0; j1 < hn; i1 ++, j1 += dt) {
			...
			j2 = j1 + t;
			fnr_gm_get(&s_re, &s_im, logn, hm + i1);
			s_im = fnr_neg(s_im);
			for (j = j1; j < j2; j ++) {
				fnr x_re, x_im, y_re, y_im;
...
/* see inner.h */
void
ctl_poly_add(fnr *restrict a, const fnr *restrict b, unsigned logn)
{
	size_t u, n;
...
/* see inner.h */
void
ctl_poly_sub(fnr *restrict a, const fnr *restrict b, unsigned logn)
{
	size_t u, n;
...
/* see inner.h */
void
ctl_poly_neg(fnr *a, unsigned logn)
{
	size_t u, n;
...
/* see inner.h */
void
ctl_poly_mulconst(fnr *a, fnr c, unsigned logn)
{
	size_t u, n;
...
/* see inner.h */
void
ctl_poly_mul_fft(fnr *restrict a, const fnr *restrict b, unsigned logn)
{
	size_t u, hn;
...
/* see inner.h */
void
ctl_poly_adj_fft(fnr *a, unsigned logn)
{
	size_t u, n;
...
/* see inner.h */
void
ctl_poly_div_2e(fnr *a, unsigned e, unsigned logn)
{
	size_t u, n;
...
/* see inner.h */
void
ctl_poly_mul_autoadj_fft(fnr *restrict a, const fnr *restrict b, unsigned logn)
{
	size_t u, hn;
...
/* see inner.h */
void
ctl_poly_div_autoadj_fft(fnr *restrict a, const fnr *restrict b, unsigned logn)
{
	size_t u, hn;
...
/* see inner.h */
void
ctl_poly_invnorm_fft(fnr *restrict d,
