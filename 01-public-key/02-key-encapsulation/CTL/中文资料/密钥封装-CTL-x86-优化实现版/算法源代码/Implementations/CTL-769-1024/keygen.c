...
static int
solve_NTRU_i16_3329_Ffirst(unsigned logn,
	int16_t *F, int16_t *G,
	const int8_t *f, const int8_t *g, uint32_t *tmp)
{
	size_t n;
	...
	if (!solve_NTRU_core(logn, g, f, 3329, tmp)) {
		return 0;
	}
	...
	if (!poly_big_to_small_i16(F, tmp + n, lim, logn)) {
		poly_big_i16_overflow_stats(tmp + n, lim, logn,
			&kg_last_smallf_max_abs, &kg_last_smallf_over_count,
			&kg_last_smallf_first_index, &kg_last_smallf_first_value);
		kg_last_smallf_d7_tail_exit = kg_d7_tail_exit;
	...
	if (!ctl_rebuild_G_3329_i16(G, f, g, F, logn, tmp)) {
		kg_diag_stats.solve_fg_fail_poly_small_g ++;
		return 0;
	}
	...
	const void *seed, size_t seed_len, uint32_t *tmp)
{
	...
	case 257:
		...
		break;
	case 769:
		if (!ctl_make_public_769(h, f, g, logn, tmp)) {
			return 0;
		}
		break;
	case 3329:
		if (logn != 11 || !ctl_make_public_3329(h, f, g, logn, tmp)) {
			return 0;
		}
		break;
	}
	...
	uint32_t q, unsigned logn, uint32_t *tmp)
{
	...
	case 769:
		...
		return ctl_rebuild_G_769(G, f, g, F, logn, tmp);
	case 3329:
		(void)G;
		(void)f;
		(void)g;
...
/* see inner.h */
int
ctl_keygen_rebuild_G_3329_i16(int16_t *G,
	...
	unsigned logn, uint32_t *tmp)
{
	...
	return ctl_rebuild_G_3329_i16(G, f, g, F, logn, tmp);
}
