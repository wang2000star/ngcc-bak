...
#define CTL_keygen   Zn(CTL_keygen)
int CTL_keygen(unsigned logn,
	int8_t *f, int8_t *g, int8_t *F, int8_t *G, int32_t *w,
	ntrugen_rng rng, void *rng_context,
	void *tmp, size_t tmp_len);

#define CTL_recover_G   Zn(CTL_recover_G)
int CTL_recover_G(unsigned logn,
	int8_t *G,
 ...
 *    CTL-257-512      512         sqrt(sqrt(2)*257/1024)
 *    CTL-769-1024    1024         sqrt(sqrt(5*log(5)/24)*769/1024)
 *    Hawk-256         256         1.1
 *    Hawk-512         512         1.5
 *    Hawk-1024       1024         2.0
 *
 * All algorithms target f*G - g*F = q for some integer q; Falcon uses
 * q = 12289; CTL uses q = 257 or 769 (depending on the degree n);
 * Hawk uses q = 1.
 *
 * Table format:
 * -------------
...
/* CTL, q = 257, n = 512 -> kmax = 2 */
#define gauss_CTL_257_512   Zn(gauss_CTL_257_512)
extern const uint16_t gauss_CTL_257_512[];

/* CTL, q = 769, n = 1024 -> kmax = 3 */
#define gauss_CTL_769_1024   Zn(gauss_CTL_769_1024)
extern const uint16_t gauss_CTL_769_1024[];
