...
#ifdef VIPER_ARITH_AVX
static void shake128x4_or_scalar(unsigned char *out[4], unsigned long long outlen, const unsigned char in[4][VIPER_GENPUBLIC_INPUT_BYTES], unsigned count) {
  if (count == 4u) {
    shake128x4(out[0], out[1], out[2], out[3], outlen, in[0], in[1], in[2], in[3], VIPER_GENPUBLIC_INPUT_BYTES);
    return;
  }
  for (unsigned i = 0; i < count; i++) shake128(out[i], outlen, in[i], VIPER_GENPUBLIC_INPUT_BYTES);
}
#endif

void viper_gen_public_shake(unsigned char *buf, const unsigned char rho[32]) {
#ifdef VIPER_ARITH_AVX
  unsigned char in[4][VIPER_GENPUBLIC_INPUT_BYTES];
  unsigned char *out[4];
  unsigned lane = 0;
  for (size_t i = 0; i < VIPER_K; i++) {
    for (size_t j = 0; j < VIPER_K; j++) {
      genpublic_input(in[lane], rho, VIPER_DOMAIN_A, i, j);
      out[lane] = buf + genpublic_a_offset(i, j);
      lane++;
      if (lane == 4u) {
        shake128x4_or_scalar(out, VIPER_PUBLIC_A_POLYBYTES, in, lane);
        lane = 0;
      }
    }
  }
  if (lane) shake128x4_or_scalar(out, VIPER_PUBLIC_A_POLYBYTES, in, lane);

  lane = 0;
  for (size_t i = 0; i < VIPER_K; i++) {
    genpublic_input(in[lane], rho, VIPER_DOMAIN_DPK, i, 0);
    out[lane] = buf + genpublic_dpk_offset(i);
    lane++;
    if (lane == 4u) {
      shake128x4_or_scalar(out, VIPER_PUBLIC_DPK_POLYBYTES, in, lane);
      lane = 0;
    }
  }
  if (lane) shake128x4_or_scalar(out, VIPER_PUBLIC_DPK_POLYBYTES, in, lane);
#else
