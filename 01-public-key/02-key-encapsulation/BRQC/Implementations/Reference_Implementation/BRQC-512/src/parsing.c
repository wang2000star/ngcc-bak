/**
 * \file parsing.c
 * \brief Key and ciphertext serialization/deserialization implementation for the BRQC scheme
 *
 * Data formats:
...
/**
 * \fn void brqc_secret_key_to_string(uint8_t* sk, const uint8_t* seed, const rbc_vec sigma, const uint8_t* pk)
 ...
 */
void brqc_secret_key_to_string(uint8_t* sk, const uint8_t* seed, const rbc_vec sigma, const uint8_t* pk) {
  memcpy(sk, seed, SEEDEXPANDER_SEED_BYTES);
  rbc_vec_to_string(sk + SEEDEXPANDER_SEED_BYTES, sigma, BRQC_PARAM_K);
  memcpy(sk + SEEDEXPANDER_SEED_BYTES + BRQC_VEC_K_BYTES, pk, BRQC_PUBLIC_KEY_BYTES);
}


/**
 * \fn void brqc_secret_key_from_string(rbc_qre x, rbc_qre y, rbc_vec sigma, uint8_t* pk, const uint8_t* sk)
 ...
 */
void brqc_secret_key_from_string(rbc_qre x, rbc_qre y, rbc_vec sigma, uint8_t* pk, const uint8_t* sk) {
...
/**
 * \fn void brqc_public_key_to_string(uint8_t* pk, const rbc_qre s, const uint8_t* seed)
 ...
 */
void brqc_public_key_to_string(uint8_t* pk, const rbc_qre s, const uint8_t* seed) {
  rbc_qre_to_string(pk, s);
  memcpy(pk + BRQC_VEC_N_BYTES, seed, SEEDEXPANDER_SEED_BYTES);
}


/**
 * \fn void brqc_public_key_from_string(rbc_qre g, rbc_qre h, rbc_qre s, const uint8_t* pk)
 ...
 *
 * \param[out] g Gabidulin code generator g in R_{q^m} (full rank)
 * \param[out] h Ideal code multiplier h in R_{q^m}
 * \param[out] s Syndrome s in R_{q^m}
 * \param[in]  pk Public key byte string
 */
void brqc_public_key_from_string(rbc_qre g, rbc_qre h, rbc_qre s, const uint8_t* pk) {
  ...
  memcpy(pk_seed, pk + BRQC_VEC_N_BYTES, SEEDEXPANDER_SEED_BYTES);
...
/**
 * \fn void brqc_kem_ciphertext_to_string(uint8_t* ct, const rbc_qre u, const rbc_qre v, const uint8_t* salt)
 ...
 */
void brqc_kem_ciphertext_to_string(uint8_t* ct, const rbc_qre u, const rbc_qre v, const uint8_t* salt) {
  rbc_qre_to_string(ct, u);
  rbc_qre_to_string(ct + BRQC_VEC_N_BYTES, v);
  memcpy(ct + 2 * BRQC_VEC_N_BYTES, salt, BRQC_SALT_BYTES);
}


/**
 * \fn void brqc_kem_ciphertext_from_string(rbc_qre u, rbc_qre v, uint8_t* salt, const uint8_t* ct)
 ...
 */
void brqc_kem_ciphertext_from_string(rbc_qre u, rbc_qre v, uint8_t* salt, const uint8_t* ct) {
  rbc_qre_from_string(u, ct);
  rbc_qre_from_string(v, ct + BRQC_VEC_N_BYTES);
  memcpy(salt, ct + 2 * BRQC_VEC_N_BYTES, BRQC_SALT_BYTES);
}
