...
void rbc_vec_set_random(random_source* ctx, rbc_vec v, uint32_t size) {
  uint32_t bytes = (RBC_127_FIELD_M % 8 == 0) ? RBC_127_FIELD_M / 8 : RBC_127_FIELD_M / 8 + 1;
  uint8_t random[size * bytes];
  uint8_t mask = (1 << RBC_127_FIELD_M % 8) - 1;
...
void rbc_vec_set_random_from_xof(rbc_vec o,
  ...
  uint8_t random[size * bytes];
  uint8_t mask = (1 << RBC_127_FIELD_M % 8) - 1;
  if ((RBC_127_FIELD_M % 8) == 0) { mask = 0xff; }
...
void rbc_vec_set_random_full_rank(random_source* ctx, rbc_vec o, uint32_t size) {
  int32_t rank_max = RBC_127_FIELD_M < size ? RBC_127_FIELD_M : size;
  int32_t rank = -1;
...
uint32_t rbc_vec_gauss(rbc_vec v, uint32_t size, uint8_t reduced_flag, rbc_vec *other_matrices, uint32_t nMatrices) {
  ...
  for(uint32_t p = 0 ; p < size ; p++) {
    ...
    for(uint32_t i=p+1 ; i<size ; i++) {
      for(uint32_t j=0 ; j<RBC_127_ELT_DATA_SIZE ; j++) {
        acc[j] |= v[i][j];
      }
    }
...
void rbc_vec_to_string(uint8_t* str, const rbc_vec v, uint32_t size) {
  ...
  uint32_t str_size = ((size * RBC_127_FIELD_M) % 8 == 0) ? (size * RBC_127_FIELD_M) / 8 : (size * RBC_127_FIELD_M) / 8 + 1;
  ...
  for(size_t i = 0 ; i < size ; i++) {
    for(size_t j = 1 ; j <= bytes2 ; j++) {
      uint8_t bit = rbc_elt_get_coefficient(v[i], RBC_127_FIELD_M - j);
      *(str + index) |= (bit << k % 8);
      k++;
      if(k % 8 == 0) index++;
    }
...
void rbc_vec_from_string(rbc_vec v, uint32_t size, const uint8_t* str) {
  ...
  for(size_t i = 0 ; i < size ; i++) {
    for(size_t j = 1 ; j <= bytes2 ; j++) {
      ...
      rbc_elt_set_coefficient_vartime(v[i], RBC_127_FIELD_M - j, bit);
      k++;
      if(k % 8 == 0) index++;
    }
  }
...
void rbc_vec_from_bytes(rbc_vec o, uint32_t size, uint8_t *random) {
  ...
  for(size_t i = 0 ; i < size ; ++i) {
    random[(i + 1) * RBC_127_ELT_UINT8_SIZE - 1] &= mask;
    memcpy(o[i], &random[i * RBC_127_ELT_UINT8_SIZE], RBC_127_ELT_UINT8_SIZE);
  }
}
...
void rbc_vec_to_bytes( uint8_t *o, const rbc_vec v, uint32_t size) {
  for(size_t i = 0 ; i < size ; ++i) {
    memcpy(&o[i * RBC_127_ELT_UINT8_SIZE], v[i], RBC_127_ELT_UINT8_SIZE);
  }
}
