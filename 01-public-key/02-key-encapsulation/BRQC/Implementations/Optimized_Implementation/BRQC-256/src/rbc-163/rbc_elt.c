...
void rbc_elt_set(rbc_elt o, const rbc_elt e) {
  for(uint8_t i = 0 ; i < RBC_163_ELT_SIZE - 1 ; i++) {
    o[i] = e[i];
  }
  o[RBC_163_ELT_SIZE - 1] = 0;
}
...
void rbc_elt_set_mask2(rbc_elt o1, rbc_elt o2, const rbc_elt e, uint32_t mask) {
  ...
  o1[RBC_163_ELT_SIZE - 1] = 0;
  o2[RBC_163_ELT_SIZE - 1] = 0;
}
...
void rbc_elt_add(rbc_elt o, const rbc_elt e1, const rbc_elt e2) {
  ...
  for(uint8_t i = 0 ; i < RBC_163_ELT_SIZE ; i = i + 2) {
...
void rbc_elt_ur_sqr(rbc_elt_ur o, const rbc_elt e) {
  ...
  e_l = _mm_and_si128(e_0, RBC_163_ELT_SQR_MASK_128);
  e_h = _mm_and_si128(_mm_srli_epi64(e_0, 4), RBC_163_ELT_SQR_MASK_128);

  e_l = _mm_shuffle_epi8(RBC_163_ELT_SQR_LOOKUP_TABLE_128, e_l);
  e_h = _mm_shuffle_epi8(RBC_163_ELT_SQR_LOOKUP_TABLE_128, e_h);
