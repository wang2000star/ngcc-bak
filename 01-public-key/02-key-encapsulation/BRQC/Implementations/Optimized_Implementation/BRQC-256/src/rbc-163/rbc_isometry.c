...
void rbc_isometry_init(rbc_isometry* o, uint32_t n) {
  rbc_mat_fq_init(&(o->P), RBC_163_FIELD_M, RBC_163_FIELD_M);
...
void rbc_isometry_set_zero(rbc_isometry *o, uint32_t n) {
  ...
  rbc_mat_fq_set_zero(o->P, RBC_163_FIELD_M, RBC_163_FIELD_M);
...
void rbc_isometry_set(rbc_isometry *o, const rbc_mat_fq P, const rbc_mat_fq Q, uint32_t n) {
  if (n != o->n) exit(EXIT_FAILURE);
  rbc_mat_fq_set(o->P, P, RBC_163_FIELD_M, RBC_163_FIELD_M);
...
void rbc_isometry_set_random(random_source* ctx, rbc_isometry *o, uint32_t n) {
  if (n != o->n) exit(EXIT_FAILURE);
  rbc_perm perm;
  rbc_perm_init(&perm, RBC_163_FIELD_M);
  rbc_mat_fq_gen(ctx, perm, o->P, RBC_163_FIELD_M);
  rbc_perm_clear(perm);
...
void rbc_isometry_set_random_from_xof(rbc_isometry *o, uint32_t n, void (*xof)(uint8_t *, size_t, const uint8_t *, size_t), const uint8_t *xof_input, uint32_t xof_size) {
  ...
  rbc_perm perm;
  rbc_perm_init(&perm, RBC_163_FIELD_M);
  rbc_mat_fq_gen_from_xof(perm, o->P, RBC_163_FIELD_M, xof, xof_input, xof_size);
  rbc_perm_clear(perm);
  rbc_perm_init(&perm, n);
...
void rbc_isometry_compose(rbc_isometry *o, const rbc_isometry *iso1, const rbc_isometry *iso2) {
  ...
  uint32_t n = iso1->n;
  rbc_mat_fq t1, t2;
  rbc_mat_fq_init(&t1, RBC_163_FIELD_M, RBC_163_FIELD_M);
  rbc_mat_fq_init(&t2, RBC_163_FIELD_M, RBC_163_FIELD_M);
  rbc_mat_fq_set(t1, iso1->P, RBC_163_FIELD_M, RBC_163_FIELD_M);
  rbc_mat_fq_set(t2, iso2->P, RBC_163_FIELD_M, RBC_163_FIELD_M);
  rbc_mat_fq_mul(o->P, t2, t1, RBC_163_FIELD_M, RBC_163_FIELD_M, RBC_163_FIELD_M);
  rbc_mat_fq_clear(t1);
  rbc_mat_fq_clear(t2);
