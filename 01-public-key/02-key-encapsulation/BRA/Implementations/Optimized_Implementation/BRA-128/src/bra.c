/**
 * \file bra.c
 * \brief Implementation of the BRA.PKE public key encryption scheme
 *
 * The BRA.PKE scheme is constructed from the Ideal Blockwise Rank Decoding
 * (IBRD) problem. Main components:
 *   - Augmented Gabidulin code C_{gab}(g, k): an [n, k] Augmented Gabidulin code determined by generator g
...
/**
 * \fn void bra_pke_keygen(uint8_t* pk, uint8_t* sk)
...
void bra_pke_keygen(uint8_t* pk, uint8_t* sk) {
  ...
  rbc_vec sigma_tmp;
  rbc_vec_init(&sigma_tmp, BRA_PARAM_K);
  bra_public_key_to_string(pk, s, pk_seed);
  bra_secret_key_to_string(sk, sk_seed, sigma_tmp, pk);
  ...
  printf("\n\nsupport_x: "); rbc_vspace_print(support_x, BRA_PARAM_W_x);
  printf("\n\nsupport_y: "); rbc_vspace_print(support_y, BRA_PARAM_W_y);
  printf("\n\nx: "); rbc_qre_print(x);
  printf("\n\ny: "); rbc_qre_print(y);
  printf("\n\ng: "); rbc_qre_print(g);
  printf("\n\nh: "); rbc_qre_print(h);
...
/**
 * \fn void bra_pke_encrypt(rbc_qre u, rbc_qre v, const rbc_vec m, uint8_t* theta, const uint8_t* pk)
 ...
 *      Encode uses Augmented Gabidulin code C_{gab}(g, k)
 *
...
void bra_pke_encrypt(rbc_qre u, rbc_qre v, const rbc_vec m, uint8_t* theta, const uint8_t* pk) {
  ...
  rbc_vspace_init(&support_r1, BRA_PARAM_W_r1);
  rbc_vspace_init(&support_r2, BRA_PARAM_W_r2);
  rbc_vspace_init(&support_e, BRA_PARAM_W_e);
  ...
  bra_public_key_from_string(g, h, s, pk);
  ...
  rbc_vspace_init(&support_re, BRA_PARAM_W_r1 + BRA_PARAM_W_r2 + BRA_PARAM_W_e);
  rbc_vspace_set_random_full_rank(&seedexpander, support_re, BRA_PARAM_W_r1 + BRA_PARAM_W_r2 + BRA_PARAM_W_e);

  for(size_t i = 0; i < BRA_PARAM_W_r1; i++) {
    rbc_elt_set(support_r1[i], support_re[i]);
  }
  for(size_t i = 0; i < BRA_PARAM_W_r2; i++) {
    rbc_elt_set(support_r2[i], support_re[BRA_PARAM_W_r1 + i]);
  }
  for(size_t i = 0; i < BRA_PARAM_W_e; i++) {
    rbc_elt_set(support_e[i], support_re[BRA_PARAM_W_r1 + BRA_PARAM_W_r2 + i]);
  }

  rbc_qre_set_random_from_support(&seedexpander, r1, support_r1, BRA_PARAM_W_r1, 1);
  rbc_qre_set_random_from_support(&seedexpander, r2, support_r2, BRA_PARAM_W_r2, 1);
  rbc_qre_set_random_from_support(&seedexpander, e, support_e, BRA_PARAM_W_e, 1);
   ...
   * First encode plaintext using Augmented Gabidulin code: v = m*G */
  rbc_augmented_gabidulin_init(&code, g->v, BRA_PARAM_K, BRA_PARAM_N);
  rbc_augmented_gabidulin_encode(v->v, code, m);
  ...
  printf("\n\nh: "); rbc_qre_print(h);
  printf("\n\ns: "); rbc_qre_print(s);
  printf("\n\nsupport_r1: "); rbc_vspace_print(support_r1, BRA_PARAM_W_r1);
  printf("\n\nsupport_r2: "); rbc_vspace_print(support_r2, BRA_PARAM_W_r2);
  printf("\n\nsupport_e: "); rbc_vspace_print(support_e, BRA_PARAM_W_e);
  printf("\n\nr1: "); rbc_qre_print(r1);
  printf("\n\nr2: "); rbc_qre_print(r2);
  printf("\n\ne: "); rbc_qre_print(e);
  printf("\n\nu: "); rbc_qre_print(u);
...
/**
 * \fn void bra_pke_decrypt(rbc_vec m, const rbc_qre u, const rbc_qre v, const uint8_t* sk)
 ...
 *             = m*G + x*r2 + e - r1*y
 *      Since wt_R(x*r2 + e - r1*y) is small, the Augmented Gabidulin code can correct the error
 *   2. m = Decode(v - u*y), recover plaintext using Augmented Gabidulin decoding
 *
...
void bra_pke_decrypt(rbc_vec m, const rbc_qre u, const rbc_qre v, const uint8_t* sk) {
  ...
  rbc_vec sigma_tmp;
  rbc_vec_init(&sigma_tmp, BRA_PARAM_K);
  bra_secret_key_from_string(x, y, sigma_tmp, pk, sk);
  rbc_vec_clear(sigma_tmp);
  ...
  /* Augmented Gabidulin decoding to recover plaintext m */
  rbc_augmented_gabidulin_init(&code, g->v, BRA_PARAM_K, BRA_PARAM_N);
  rbc_augmented_gabidulin_decode(m, code, tmp->v);
