...
uint64_t RBC_83_SQR_LOOKUP_TABLE[256];
...
void rbc_field_init(void) {
  ...
  if(rbc_init_field == 0) {
    memset(RBC_83_SQR_LOOKUP_TABLE, 0, 8 * 256);
    for(size_t i = 0 ; i < 256 ; ++i) {
      for(size_t j = 0 ; j < 8 ; ++j) {
        mask = 1 << j;
        bit = (mask & i) >> j;
        RBC_83_SQR_LOOKUP_TABLE[i] ^= bit << (2 * j);
      }
    }
...
void rbc_elt_set_zero(rbc_elt o) {
  for(uint8_t i = 0 ; i < RBC_83_ELT_SIZE ; i++) {
    o[i] = 0;
  }
}
...
void rbc_elt_set_one(rbc_elt o) {
  o[0] = 1;
  for(uint8_t i = 1 ; i < RBC_83_ELT_SIZE ; i++) {
    o[i] = 0;
  }
}
...
void rbc_elt_set_random(random_source* ctx, rbc_elt o) {
  ...
  uint8_t random[RBC_83_ELT_UINT8_SIZE];
  random_source_get_bytes(ctx, random, RBC_83_ELT_UINT8_SIZE);
  random[RBC_83_ELT_UINT8_SIZE - 1] &= RBC_83_ELT_MASK;

  memcpy((uint64_t*) o, random, RBC_83_ELT_UINT8_SIZE);
}
...
uint8_t rbc_elt_is_zero(const rbc_elt e) {
  int8_t result = 1;
  for(int i = RBC_83_ELT_DATA_SIZE - 1 ; i >= 0 ; i--) {
    result &= (e[i] == 0);
  }
...
uint8_t rbc_elt_is_equal_to(const rbc_elt e1, const rbc_elt e2) {
  int8_t result = 1;
  for(uint8_t i = 0 ; i < RBC_83_ELT_DATA_SIZE ; i++) {
    result &= (e1[i] == e2[i]);
  }
 return result;
}
...
uint8_t rbc_elt_is_greater_than(const rbc_elt e1, const rbc_elt e2) {
  ...
  for(int8_t i = RBC_83_ELT_DATA_SIZE - 1 ; i >= 0 ; i--) {
    tmp = (e1[i] > e2[i]);
...
void rbc_elt_inv(rbc_elt o, const rbc_elt e) {
  ...
  rbc_elt_set(tmp, e);
  for(int i = 0 ; i < (RBC_83_FIELD_M - 2) ; i++) {
    rbc_elt_sqr(o, tmp);
    rbc_elt_mul(tmp, o, e);
  }
  rbc_elt_sqr(o, tmp);
...
void rbc_elt_nth_root(rbc_elt o, const rbc_elt e, uint32_t n) {
  uint32_t exp = n * (RBC_83_FIELD_M - 1) % RBC_83_FIELD_M;
...
void rbc_elt_ur_set(rbc_elt_ur o, const rbc_elt_ur e) {
  for(uint8_t i = 0 ; i < RBC_83_ELT_UR_SIZE - 1 ; i++) {
    o[i] = e[i];
  }
  o[RBC_83_ELT_SIZE - 1] = 0;
}
...
void rbc_elt_ur_set_zero(rbc_elt_ur o) {
  for(uint8_t i = 0 ; i < RBC_83_ELT_UR_SIZE ; i++) {
    o[i] = 0;
  }
}
...
void rbc_elt_ur_sqr(rbc_elt_ur o, const rbc_elt e) {
  ...
  e_l = _mm_and_si128(tmp1, RBC_83_ELT_SQR_MASK_128);
  e_h = _mm_and_si128(_mm_srli_epi64(tmp1, 4), RBC_83_ELT_SQR_MASK_128);

  e_l = _mm_shuffle_epi8(RBC_83_ELT_SQR_LOOKUP_TABLE_128, e_l);
  e_h = _mm_shuffle_epi8(RBC_83_ELT_SQR_LOOKUP_TABLE_128, e_h);
...
void rbc_elt_to_string(uint8_t* str, const rbc_elt e) {
  uint32_t bytes1 = RBC_83_FIELD_M / 8;
  uint32_t bytes2 = RBC_83_FIELD_M % 8;

  memset(str, 0, RBC_83_ELT_UINT8_SIZE);
  memcpy(str, e, bytes1);

  uint8_t k = 0;
  for(size_t j = 1 ; j <= bytes2 ; j++) {
    uint8_t bit = rbc_elt_get_coefficient(e, RBC_83_FIELD_M - j);
    *(str + bytes1) |= (bit << k % 8);
    k++;
    if(k % 8 == 0) bytes1++;
  }
...
void rbc_elt_from_string(rbc_elt e, const uint8_t* str) {
  ...
  for(size_t j = 1 ; j <= bytes2 ; j++) {
    ...
    rbc_elt_set_coefficient_vartime(e, RBC_83_FIELD_M - j, bit);
    k++;
    if(k % 8 == 0) bytes1++;
  }
}
