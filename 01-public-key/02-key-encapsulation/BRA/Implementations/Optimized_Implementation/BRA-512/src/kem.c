/**
 * \file kem.c
 * \brief Implementation of the BRA.KEM key encapsulation mechanism
 *
 * Based on the salted Fujisaki-Okamoto (FO) transform, converting BRA.PKE
...
/**
 * \fn int bra_keygen(uint8_t* pk, uint8_t* sk)
 * \brief BRA.KEM key generation (KEM.KGen)
 *
 ...
 * \return 0 on success
 */
int bra_keygen(uint8_t *pk, uint8_t *sk) {
  ...
  rbc_vec sigma;
  rbc_field_init();
  rbc_qre_init_modulus(BRA_PARAM_N);
  rbc_vec_init(&sigma, BRA_PARAM_K);

  /* Step 1: Call PKE.KGen to generate (pk', sk')
   * Note: bra_pke_keygen internally calls bra_secret_key_to_string with a
  ...
  rbc_vec_set_random(&prng, sigma, BRA_PARAM_K);
  ...
  bra_secret_key_to_string(sk, sk_seed, sigma, pk);
...
/**
 * \fn int bra_encaps(uint8_t* ct, uint8_t* ss, const uint8_t* pk)
 * \brief BRA.KEM encapsulation (KEM.Encaps)
 *
 ...
 * \return 0 on success
 */
int bra_encaps(uint8_t *ct, uint8_t *ss, const uint8_t *pk) {
  ...
  uint8_t G_in[BRA_PUBLIC_KEY_BYTES + BRA_VEC_K_BYTES + BRA_SALT_BYTES] = {0};
  uint8_t *m_str = &G_in[BRA_PUBLIC_KEY_BYTES];
  uint8_t *salt = &G_in[BRA_PUBLIC_KEY_BYTES + BRA_VEC_K_BYTES];

  /* H hash input buffer: H_in = pk || m || u || v || salt */
  uint8_t H_in[BRA_PUBLIC_KEY_BYTES + BRA_VEC_K_BYTES + 2 * BRA_VEC_N_BYTES + BRA_SALT_BYTES] = {0};
  ...
  rbc_vec_set_random(&prng, m, BRA_PARAM_K);

  /* Step 1: Randomly sample salt in {0,1}^512 */
  get_random_number(&drng_algorithm, salt, BRA_SALT_BYTES * 8);

  /* Step 2: Compute theta = G(pk, m, salt) */
  memcpy(G_in, pk, BRA_PUBLIC_KEY_BYTES);
  rbc_vec_to_string(m_str, m, BRA_PARAM_K);
  pseudohash(512, G_in, (BRA_PUBLIC_KEY_BYTES + BRA_VEC_K_BYTES + BRA_SALT_BYTES) * 8, theta);

  /* Step 3: ct' = PKE.Enc(pk, m; theta), encrypt to obtain (u, v) */
  bra_pke_encrypt(u, v, m, theta, pk);

  /* Step 4: Serialize ciphertext ct = u || v || salt */
  bra_kem_ciphertext_to_string(ct, u, v, salt);

  /* Step 5: Compute shared secret K = H(pk, m, ct, salt)
   *   H_in = pk || m_str || u_bytes || v_bytes || salt */
  memcpy(H_in, pk, BRA_PUBLIC_KEY_BYTES);
  memcpy(H_in + BRA_PUBLIC_KEY_BYTES, m_str, BRA_VEC_K_BYTES);
  rbc_qre_to_string(H_in + BRA_PUBLIC_KEY_BYTES + BRA_VEC_K_BYTES, u);
  rbc_qre_to_string(H_in + BRA_PUBLIC_KEY_BYTES + BRA_VEC_K_BYTES + BRA_VEC_N_BYTES, v);
  memcpy(H_in + BRA_PUBLIC_KEY_BYTES + BRA_VEC_K_BYTES + 2 * BRA_VEC_N_BYTES, salt, BRA_SALT_BYTES);
  pseudohash(512, H_in, (BRA_PUBLIC_KEY_BYTES + BRA_VEC_K_BYTES + 2 * BRA_VEC_N_BYTES + BRA_SALT_BYTES) * 8, ss);

#ifdef VERBOSE
  printf("\n\nm: "); rbc_vec_print(m, BRA_PARAM_K);
  printf("\n\nsalt: "); for(int i = 0 ; i < BRA_SALT_BYTES ; ++i) printf("%02x", salt[i]);
  printf("\n\ntheta: "); for(int i = 0 ; i < SHA512_BYTES ; ++i) printf("%02x", theta[i]);
  printf("\n\nciphertext: "); for(int i = 0 ; i < BRA_CIPHERTEXT_BYTES ; ++i) printf("%02x", ct[i]);
  printf("\n\nsecret 1: "); for(int i = 0 ; i < BRA_SHARED_SECRET_BYTES ; ++i) printf("%02x", ss[i]);
#endif
...
/**
 * \fn int bra_decaps(uint8_t* ss, const uint8_t* ct, const uint8_t* sk)
 * \brief BRA.KEM decapsulation (KEM.Decaps)
 *
 ...
 * \return 0 on success
 */
int bra_decaps(uint8_t *ss, const uint8_t *ct, const uint8_t *sk) {

#ifdef VERBOSE
  printf("\n\n\n\n### DECAPS ###");
#endif

  uint8_t pk[BRA_PUBLIC_KEY_BYTES] = {0};
  uint8_t theta[SHA512_BYTES] = {0};
  ...
  bra_kem_ciphertext_from_string(u, v, salt, ct);
  ...
  bra_pke_encrypt(u2, v2, m, theta, pk);
  ...
  {
    uint8_t u_bytes[BRA_VEC_N_BYTES];
    uint8_t v_bytes[BRA_VEC_N_BYTES];
    uint8_t u2_bytes[BRA_VEC_N_BYTES];
    uint8_t v2_bytes[BRA_VEC_N_BYTES];
    uint8_t sigma_bytes[BRA_VEC_K_BYTES];
    rbc_qre_to_string(u_bytes, u);
    rbc_qre_to_string(v_bytes, v);
    rbc_qre_to_string(u2_bytes, u2);
    rbc_qre_to_string(v2_bytes, v2);
    rbc_vec_to_string(sigma_bytes, sigma, BRA_PARAM_K);

    /* OR-accumulate every byte difference. Seed 0x0100 guarantees that
     * (r - 1) >> 8 is 0 only when every XOR contribution was zero. */
    uint16_t r = 0x0100;
    for(size_t i = 0; i < BRA_VEC_N_BYTES; i++) r |= (uint16_t)(u_bytes[i] ^ u2_bytes[i]);
    for(size_t i = 0; i < BRA_VEC_N_BYTES; i++) r |= (uint16_t)(v_bytes[i] ^ v2_bytes[i]);
    ...
    for(size_t i = 0; i < BRA_VEC_K_BYTES; i++) {
...
#ifdef VERBOSE
  printf("\n\npk: "); for(int i = 0 ; i < BRA_PUBLIC_KEY_BYTES ; ++i) printf("%02x", pk[i]);
  printf("\n\nsk: "); for(int i = 0 ; i < BRA_SECRET_KEY_BYTES ; ++i) printf("%02x", sk[i]);
  printf("\n\nciphertext: "); for(int i = 0 ; i < BRA_CIPHERTEXT_BYTES ; ++i) printf("%02x", ct[i]);
  printf("\n\nm: "); rbc_vec_print(m, BRA_PARAM_K);
  printf("\n\ntheta: "); for(int i = 0 ; i < SHA512_BYTES ; ++i) printf("%02x", theta[i]);
