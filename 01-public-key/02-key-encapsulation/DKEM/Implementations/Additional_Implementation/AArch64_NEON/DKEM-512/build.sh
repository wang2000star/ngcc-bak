#!/usr/bin/env bash
# DKEM-512 Additional (arm64, ARMv8.4-A native). Build on an arm64 host:
#   bash build.sh                       (native gcc on AArch64 Linux / Apple Silicon clang)
#   CC=aarch64-linux-gnu-gcc bash build.sh   (cross; run kat_DKEM-512 under qemu-aarch64)
# All hashing/XOF uses the official auxfunc; -fcommon lets the template's drng_algorithm coexist.
set -e
CC=${CC:-gcc}
CF="-std=c99 -O3 -fcommon -march=armv8.2-a -DDKM_AARCH64 -DDKE_USE_AARCH64 -DDKE_USE_AARCH64_NATIVE -DDKE_MODE=512 -DDKE_HASH=0 -DDKE_RANDOM=0 -I. -Wpedantic -Wall -Wextra -flto -fomit-frame-pointer"
...
$CC -O3 -flto -fomit-frame-pointer build/*.o -o kat_DKEM-512 -lm $LRT
./kat_DKEM-512
