...
#elif defined(DKE_USE_CORTEX_M4) || defined(DKE_USE_CORTEX_M4_BARRETT)
    /* M4: use UMULL to avoid uint64_t software multiply (>> 32 = just take hi word) */
    uint16_t t[DKE_N];
    for (i = 0; i < DKE_K; i++) {
        for (j = 0; j < DKE_N; j++) {
            uint32_t x = v->vec[i].coeffs[j];
            x += ((int16_t)x >> 15) & DKE_Q;
            uint32_t val = (x << 10) + 1665;
            uint32_t lo, hi;
            __asm__ ("umull %0, %1, %2, %3" : "=r"(lo), "=r"(hi) : "r"(val), "r"((uint32_t)1290167));
            t[j] = hi & 0x3ff;  /* >> 32 = hi word */
        }
        for (j = 0; j < DKE_N / 4; j++) {
...
#elif defined(DKE_USE_CORTEX_M4) || defined(DKE_USE_CORTEX_M4_BARRETT)
    /* M4 fast path: avoid uint64_t multiply (M4 has no 64-bit MUL).
     * Use two 32-bit multiplies to compute (val * MUL) >> 31:
     *   hi = (int32_t)((int64_t)val * MUL >> 32)  ← compiler emits SMULL/UMULL
     *   result = (2 * hi + 1) >> 1  (approximate rounding)
     * On GCC -O2 this compiles to UMULL+shifts, ~4 insn vs ~8 for uint64. */
    uint16_t t[DKE_N];
    for (i = 0; i < DKE_K; i++) {
        for (j = 0; j < DKE_N; j++) {
            uint32_t x = v->vec[i].coeffs[j];
            x += ((int16_t)x >> 15) & DKE_Q;
            uint32_t val = (x << 11) + HALF;
            /* (val * MUL) >> 31: split into hi32 = (val*MUL)>>32, then <<1|carry */
            uint32_t lo, hi;
            __asm__ ("umull %0, %1, %2, %3" : "=r"(lo), "=r"(hi) : "r"(val), "r"(MUL));
            t[j] = ((hi << 1) | (lo >> 31)) & 0x7ff;
        }
        for (j = 0; j < DKE_N / 8; j++) {
            uint16_t *tp = &t[8*j];
