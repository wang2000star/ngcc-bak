#!/usr/bin/env bash
# DKEM-128 on ARM Cortex-M4 (STM32, Plantard arithmetic + matacc). Self-contained.
...
DEFS="-DDKE_MODE=128 -DDKE_HASH=0 -DDKE_RANDOM=0 -DDKE_USE_CORTEX_M4_PLANTARD -DDKE_USE_MATACC"
...
$GCC $ARCH $DEFS $INC $CORE qemu/qemu_dke.c qemu/syscalls.c $ASM \
   ...
   -Wl,--start-group -lgcc -lnosys -lc -Wl,--end-group -o kat_DKEM-128.elf
echo "Built kat_DKEM-128.elf — running round-trip self-test on QEMU:"
$QEMU -M netduinoplus2 -nographic -semihosting-config enable=on,target=native -kernel kat_DKEM-128.elf
