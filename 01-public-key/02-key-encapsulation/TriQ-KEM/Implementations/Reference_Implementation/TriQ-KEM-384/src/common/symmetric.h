...
#define TRIQ_XOF_CACHE_SIZE 1024
typedef struct {
    uint8_t input[TRIQ_XOF_INPUT_MAX];
    uint32_t input_len;
    uint32_t offset;          /* consumed bytes in the generated stream */
    uint8_t cache[TRIQ_XOF_CACHE_SIZE];
    uint32_t cached;          /* how many bytes are currently in cache (0 = no cache) */
} triq_xof_ctx;
