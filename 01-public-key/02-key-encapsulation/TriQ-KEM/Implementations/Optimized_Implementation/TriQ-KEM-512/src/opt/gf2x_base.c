...
#define T_5W_256      ((T_TM3R_3W_256 + 2) / 5)
#define T_5W          (T_5W_256 << 8)
#define T2_5W_256     (2 * T_5W_256)
#define t5            (5 * T_5W / WORD)

__m256i a1_times_a2[tTM3R / 2];
__m256i o256[VEC_N_ARRAY_SIZE_VEC];
uint64_t *tmp_reduce = (uint64_t *)o256;
uint64_t bloc64[PARAM_OMEGA_MAX];  // Allocation with the biggest possible weight
uint64_t bit64[PARAM_OMEGA_MAX];   // Allocation with the biggest possible weight
 ...
 */
/**
 * @brief Base AVX2 Karatsuba helper for chunks up to 16 vectors.
 */
...
static inline void karat_mul_rec_25(__m256i *C, const __m256i *A, const __m256i *B) {
    const size_t m = 12;
    const size_t n0 = 12;
    const size_t n1 = 13;
    __m256i z0[2 * 25], z2[2 * 25], zmid[2 * 25], ta[13], tb[13];
    const __m256i zero = _mm256_setzero_si256();

    karat_mult_small(z0, A, B, 12);
    karat_mult_small(z2, A + m, B + m, 13);

    for (size_t i = 0; i < n1; i++) {
        __m256i alo = (i < n0 ? A[i] : zero);
        __m256i blo = (i < n0 ? B[i] : zero);
        ta[i] = alo ^ A[m + i];
        tb[i] = blo ^ B[m + i];
    }

    karat_mult_small(zmid, ta, tb, 13);

    memset(C, 0, 2 * 25 * sizeof(__m256i));
    ...
    for (size_t i = 0; i < 2 * n1; i++) {
        __m256i z0i = (i < 2 * n0 ? z0[i] : zero);
        __m256i z2i = (i < 2 * n1 ? z2[i] : zero);
        C[m + i] ^= zmid[i] ^ z0i ^ z2i;
    }
}
static inline void karat_mul_rec_26(__m256i *C, const __m256i *A, const __m256i *B) {
    const size_t m = 13;
    const size_t n0 = 13;
    const size_t n1 = 13;
    __m256i z0[2 * 26], z2[2 * 26], zmid[2 * 26], ta[13], tb[13];
    const __m256i zero = _mm256_setzero_si256();

    karat_mult_small(z0, A, B, 13);
    karat_mult_small(z2, A + m, B + m, 13);
    ...
    memset(C, 0, 2 * 26 * sizeof(__m256i));
    for (size_t i = 0; i < 2 * n0; i++) {
        C[i] ^= z0[i];
    }
    for (size_t i = 0; i < 2 * n1; i++) {
...
static inline void karat_mul_rec_51(__m256i *C, const __m256i *A, const __m256i *B) {
    const size_t m = 25;
    const size_t n0 = 25;
    const size_t n1 = 26;
    __m256i z0[2 * 51], z2[2 * 51], zmid[2 * 51], ta[26], tb[26];
    const __m256i zero = _mm256_setzero_si256();

    karat_mul_rec_25(z0, A, B);
    karat_mul_rec_26(z2, A + m, B + m);
    ...
    karat_mul_rec_26(zmid, ta, tb);

    memset(C, 0, 2 * 51 * sizeof(__m256i));
    for (size_t i = 0; i < 2 * n0; i++) {
        C[i] ^= z0[i];
    }
    for (size_t i = 0; i < 2 * n1; i++) {
...
static inline void karat_mult_5(__m256i *Out, const __m256i *A, const __m256i *B) {
    ...
    __m256i ro256[t5 >> 1];
    static __m256i mul_tmp[16 * T_5W_256];
    ...
    karat_mul_vec(D0, a0, b0, T_5W_256, mul_tmp);
    karat_mul_vec(D1, a1, b1, T_5W_256, mul_tmp);
    karat_mul_vec(D2, a2, b2, T_5W_256, mul_tmp);
    karat_mul_vec(D3, a3, b3, T_5W_256, mul_tmp);
    karat_mul_vec(D4, a4, b4, T_5W_256, mul_tmp);

    karat_mul_vec(D01, aa01, bb01, T_5W_256, mul_tmp);
    karat_mul_vec(D02, aa02, bb02, T_5W_256, mul_tmp);
    karat_mul_vec(D03, aa03, bb03, T_5W_256, mul_tmp);
    karat_mul_vec(D04, aa04, bb04, T_5W_256, mul_tmp);

    karat_mul_vec(D12, aa12, bb12, T_5W_256, mul_tmp);
    karat_mul_vec(D13, aa13, bb13, T_5W_256, mul_tmp);
    karat_mul_vec(D14, aa14, bb14, T_5W_256, mul_tmp);

    karat_mul_vec(D23, aa23, bb23, T_5W_256, mul_tmp);
    karat_mul_vec(D24, aa24, bb24, T_5W_256, mul_tmp);

    karat_mul_vec(D34, aa34, bb34, T_5W_256, mul_tmp);
...
// clear all
    ...
    memset_s(a1_times_a2, 0, sizeof(a1_times_a2));
#else
    memset(a1_times_a2, 0, sizeof(a1_times_a2));
#endif
}
