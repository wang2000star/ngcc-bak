 ...
 */
 ...
 /**
  * @brief Number of 16-bit words required to hold a syndrome of length 2路PARAM_DELTA bits
  *
  * For the 256-bit implementation, each word is 16 bits, so we round
  * up the total bit-length (2路PARAM_DELTA) to the next multiple of 16 bits.
  */
 ...
 /**
  * @brief Precomputed 256-bit vectors of field elements 伪岬⑹?for GF(2^8) operations (first half).
  *
  * Each entry packs four 64-bit words into a __m256i, corresponding to the
  * powers of the primitive element 伪 raised to the exponents i路j.
  *
  * The table length (45) matches the number of coefficients needed in the
  * 伪岬⑹?power table for the chosen code parameters.
  */
 static const TriQ_ALIGN32 __m256i alpha_ij256_1[41] = {
 ...
 /**
  * @brief Precomputed 256-bit vectors of field elements 伪岬⑹?for GF(2^8) operations (second half).
  *
  * Same format as alpha_ij256_1, providing the remaining set of powers required
  * for optimized Reed鈥揝olomon arithmetic on 256-bit words with AVX2.
  */
 static const TriQ_ALIGN32 __m256i alpha_ij256_2[41] = {
  ...
  **/
 static const TriQ_ALIGN32 __m256i param256[2] = {{0x00e300e5007d00d9, 0x009d00ec002f0036, 0x00e3004a008000e6, 0x00ec006a00ab00c2},
                                      {0x00330003003900b2, 0x00d1004000d000a5, 0x0000000100f100cc, 0x0000000000000000}};
  ...
  * -# Compute the 2路PARAM_DELTA syndromes.
  * -# Compute the error-locator polynomial 蟽(x).
  * -# Use an additive FFT to find the roots of 蟽(x) (the error locations) and take their inverses.
