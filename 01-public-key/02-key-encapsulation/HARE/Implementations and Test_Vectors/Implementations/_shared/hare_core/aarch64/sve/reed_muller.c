/**
 * @file reed_muller.c
 * @brief AArch64/SVE-safe Reed-Muller path for HARE ARM/SVE targets
 *
 * The ARM/SVE target keeps the reference RM peak/second-peak
 * semantics exactly.  It is compiled from the ARM/SVE core kind so that the
 * ARM implementation has an explicit RM path, while avoiding an unvalidated
 * SVE rewrite of the sensitive peak extraction logic.
 */
