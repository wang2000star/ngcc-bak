...
/*
 * Reed-Solomon implementation for the HARE concatenated code.
 *
 * Construction follows the HARE specification:
 *   - GF(2^8) with primitive polynomial 0x11D (provided by gf.c/gf.h)
 *   - generator polynomial g(x)=prod_{i=1}^{n-k}(x + alpha^i)
 *   - syndromes S_i = r(alpha^i), i=1..n-k
 *   - erasure-aware decoding via T(x)=S(x)gamma(x) mod x^{n-k}
 *
 * Side-channel hardening boundary:
 *   This decoder is written as a best-effort hardened C99 reference decoder.
 *   This decoder avoids Gaussian-elimination-style pivot searches,
 *   secret-dependent loop bounds, and early exits in the RS layer.  It uses
 *   fixed public loop bounds and masked selection for erasure processing,
 *   Berlekamp-Massey update, root scan, and value correction.  It does not
 *   claim a formal constant-time proof for the whole KEM: the field arithmetic
 *   uses table/constants from gf.h and the compiler may still transform C code
 *   in target-dependent ways.
 */
...
void compute_generator_poly(uint16_t *poly) {
    ...
    memset(poly, 0, (size_t)PARAM_G * sizeof(uint16_t));
    poly[0] = 1;

    for (uint16_t i = 1; i <= PARAM_RS_PARITY; ++i) {
        uint16_t next[PARAM_G];
        const uint16_t root = gf_pow_alpha((int)i);

        memset(next, 0, sizeof(next));

        /* g(x) = product_{i=1}^{PARAM_RS_PARITY} (x + alpha^i). */
        next[0] = gf_mul(poly[0], root);
        for (uint16_t j = 1; j <= degree; ++j) {
            next[j] = (uint16_t)(poly[j - 1u] ^ gf_mul(poly[j], root));
        }
        next[degree + 1u] = poly[degree];

        memcpy(poly, next, (size_t)(degree + 2u) * sizeof(uint16_t));
        degree++;
    }
}
