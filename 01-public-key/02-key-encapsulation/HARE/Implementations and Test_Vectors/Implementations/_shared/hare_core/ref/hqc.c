/**
 * @file hqc.c
 * @brief High-level HARE-PKE implementation following the HQC framework
 */
...
/**
 * @brief Generates a key pair for the HARE public-key encryption (PKE) scheme.
 *
 * This function creates a public encryption key (`ek_pke`) and a private decryption key (`dk_pke`)
 * for use in the HARE PKE scheme. The key generation process is seeded with the given `seed` input.
 *
...
/**
 * @brief Encrypts a message using the HARE public-key encryption (PKE) scheme.
 *
 * This function performs encryption in the HARE PKE scheme. It uses the given encryption key (`ek_pke`)
...
void hqc_pke_encrypt(ciphertext_pke_t *c_pke, const uint8_t *ek_pke, const uint8_t *m, const uint8_t *theta) {
    ...
    vect_truncate(tmp);
    vect_add(v, v, tmp, VEC_N1N2_SIZE_64);

    // Compress v (first PARAM_L1 bits) via active compression backend
    ciphertext_compress(c_pke->m_tilde, c_pke->v2, v);
    ...
    printf("\n\nc_pke->m_tilde: ");
    vect_print(c_pke->m_tilde, VEC_NM_SIZE_BYTES);
    printf("\n\nc_pke->v2: ");
    vect_print(c_pke->v2, VEC_NV2_SIZE_BYTES);
#endif
    ...
    memset_zero(v, sizeof(v));
}

/**
 * @brief Decrypts a ciphertext using the HARE public-key encryption (PKE) scheme.
 *
 * This function performs decryption in the HARE PKE scheme. It uses the given decryption key (`dk_pke`)
...
uint8_t hqc_pke_decrypt(uint8_t *m, const uint8_t *dk_pke, const ciphertext_pke_t *c_pke) {
    ...
    // Decompress v from active compression backend
    ciphertext_decompress(v, c_pke->m_tilde, c_pke->v2);
    ...
    vect_add(tmp2, v, tmp1, VEC_N1N2_SIZE_64);
