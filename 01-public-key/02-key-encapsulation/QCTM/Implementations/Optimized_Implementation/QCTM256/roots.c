...
static double roots_now_ms(void)
{
	return qctm_now_ms();
}
...
static int roots_profile_enabled = 0;
static double roots_profile_level_ms = 0.0;
static double roots_profile_rem_ms = 0.0;
static double roots_profile_ee_ms = 0.0;
static int roots_profile_aux_calls = 0;
static int roots_profile_level_calls = 0;
static int roots_profile_rem_calls = 0;
static int roots_profile_monic_rem_calls = 0;

static void roots_poly_make_monic(poly_t p)
{
	int i;
	gf_t inv_lc;
	gf_t value;

	if (p == NULL || poly_deg(p) < 0 || gf_is_unit(poly_tete(p))) {
		return;
	}
	gf_inv(inv_lc, poly_tete(p));
	for (i = 0; i <= poly_deg(p); i++) {
		gf_mul_fast(value, inv_lc, poly_coeff(p, i));
		poly_set_coeff(p, i, value);
	}
}
...
void roots_precomp_init_level(int e) {
  if (level_flag[e] == 0) {
		...
		gf_t a, b;
		double t0 = 0.0;
		if (roots_profile_enabled) {
			t0 = roots_now_ms();
			roots_profile_level_calls++;
		}
		if (tr[e] == NULL) {
			tr[e] = poly_alloc(t - 1);
			aux1[e] = poly_alloc(t);
    ...
    poly_calcule_deg(tr[e]);
		level_flag[e] = 1;
		if (roots_profile_enabled) {
			roots_profile_level_ms += roots_now_ms() - t0;
		}
  }
}
...
int roots_berl_aux(poly_t sigma, int d, int e, gfelt_t * res) {
  ...
  if (e >= gf_extd()) {
	...
	if (poly_deg(aux3[e]) >= poly_deg(sigma)) {
		t0 = roots_profile_enabled ? roots_now_ms() : 0.0;
		if (roots_profile_enabled) {
			roots_profile_rem_calls++;
			if (gf_is_unit(poly_tete(sigma))) {
				roots_profile_monic_rem_calls++;
			}
		}
		poly_rem(aux3[e], sigma);
		if (roots_profile_enabled) {
			roots_profile_rem_ms += roots_now_ms() - t0;
		}
	}
	if (roots_profile_enabled) {
		roots_profile_aux_calls++;
		t0 = roots_now_ms();
	}
	i = poly_ee_aux(aux1[e], aux2[e], sigma, aux3[e], 0);
	if (roots_profile_enabled) {
		roots_profile_ee_ms += roots_now_ms() - t0;
	}
	if (i & 1) {
		gcd1 = aux1[e];
		gcd2 = aux3[e];
	...
	poly_adjust_deg(gcd1);
	poly_adjust_deg(gcd2);
	roots_poly_make_monic(gcd1);
	roots_poly_make_monic(gcd2);
...
int roots_berl(poly_t sigma, gfelt_t * res) {
	int i, d, t;
	int profile_roots = 0;
	static int profile_roots_cached = -1;
	double t0 = 0.0;
	double t_after_init = 0.0;
	double t_after_sq = 0.0;
	double t_after_trace = 0.0;
	double t_after_aux = 0.0;

	if (profile_roots_cached < 0) {
		profile_roots_cached = getenv("QCTM_PROFILE_ROOTS") != NULL;
	}
	profile_roots = profile_roots_cached;
	roots_profile_enabled = profile_roots;
	roots_profile_level_ms = 0.0;
	roots_profile_rem_ms = 0.0;
	roots_profile_ee_ms = 0.0;
	roots_profile_aux_calls = 0;
	roots_profile_level_calls = 0;
	roots_profile_rem_calls = 0;
	roots_profile_monic_rem_calls = 0;
	if (profile_roots) {
		t0 = roots_now_ms();
	}
	t = poly_calcule_deg(sigma);
	roots_poly_make_monic(sigma);
	roots_init();
	roots_precomp_init(t);
	if (profile_roots) {
		t_after_init = roots_now_ms();
	}

	poly_sqmod_init(sigma, sq_aux);
	if (profile_roots) {
		t_after_sq = roots_now_ms();
	}
	...
	if (profile_roots) {
		t_after_aux = roots_now_ms();
		fprintf(stderr,
		        "qctm_roots_berl_profile_ms init=%.3f sq=%.3f trace=%.3f aux=%.3f total=%.3f roots=%d degree=%d aux_calls=%d level_calls=%d rem_calls=%d monic_rem=%d level=%.3f rem=%.3f ee=%.3f\n",
		        t_after_init - t0,
		        t_after_sq - t_after_init,
		        t_after_trace - t_after_sq,
		        t_after_aux - t_after_trace,
		        t_after_aux - t0,
		        d, t, roots_profile_aux_calls, roots_profile_level_calls,
		        roots_profile_rem_calls, roots_profile_monic_rem_calls,
		        roots_profile_level_ms, roots_profile_rem_ms,
		        roots_profile_ee_ms);
	}
	roots_profile_enabled = 0;
	return d;
}
