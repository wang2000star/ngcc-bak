...
File Description: Declares NTT arithmetic routines and constants for the optimized POLARLAC-256 instance.
*/
...
/**
 * @brief Inverse mapping table used in small-block determinant inversion.
 * The original reference declared this as qinv[RL_KEM_Q]; here we let the
 * compiler infer the array length from the initializer to avoid a
 * potential mismatch between the macro `RL_KEM_Q` and the literal initializer
 * length introduced during formatting. The effective length should
 * remain the same as in the original reference data.
 */
