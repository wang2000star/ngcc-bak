...
/// @brief Fused pointwise multiplication and modular accumulation in NTT domain.
/// @details Computes acc <- acc + a * b mod RL_KEM_Q.
///          The implementation is backed by the q=257,N=256 AVX2 base-mul kernel.
/// @param[in,out] acc Accumulator polynomial in NTT domain.
/// @param[in] a First NTT-domain multiplicand.
/// @param[in] b Second NTT-domain multiplicand.
void mq_poly_pointwise_accumulate(int16_t *acc, const int16_t *a, const int16_t *b);

/// @brief Fused pointwise multiplication and accumulation for already 64-byte aligned buffers.
/// @details Computes acc <- acc + a * b mod RL_KEM_Q. All pointers must be
///          64-byte aligned because the backend uses aligned AVX2 loads.
void mq_poly_pointwise_accumulate_aligned(int16_t *acc, const int16_t *a, const int16_t *b);

/// @brief Fused sum of two pointwise products for already 64-byte aligned buffers.
/// @details Computes out <- a0 * b0 + a1 * b1 mod RL_KEM_Q.
