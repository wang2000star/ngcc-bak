...
File Description: Declares polynomial compression, encoding, and arithmetic helpers for the optimized POLARLAC-Light instance.
*/
...
/// @brief Compute dst = a + b mod q for a full length-N polynomial.
void poly_add_reduce_q_avx2(int16_t *dst, const int16_t *a, const int16_t *b);

/// @brief Compute dst = dst + src mod q for a full length-N polynomial.
...
/// @brief Normalize a full length-N polynomial to the canonical q interval.
void poly_normalize_q_avx2(int16_t *dst);

/// @brief Compute dst[i] = dst[i] + RATIO * msg_bits[i] mod q for c2/message positions.
...
/// @brief Losslessly compress a canonical polynomial using fixed zero-selector encoding.
/// @details Output length is `PK_POLY_BYTES`: `RL_KEM_N` low bytes followed by
///          `PK_ZERO_SELECTOR_BYTES` selector bytes. Returns 0 on success and
///          -1 when the selector capacity is exceeded.
int poly_compress(uint8_t *code_c, const int16_t *c);

/// @brief Inverse of `poly_compress`.
