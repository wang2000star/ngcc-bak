...
File Description: Implements the MLWE-style POLARLAC public-key encryption layer for the reference POLARLAC-512-Star instance.
*/
...
int PKE_KeyGen(uint8_t *pk, uint8_t *sk, const uint8_t *seed)
{
    ...
    for (int i = 0; i < RL_KEM_K; i++) {
        ...
        for (int j = 0; j < RL_KEM_K; j++) {
            poly_accumulate_ntt_product(&b_ntt.vec[i], &a_ntt.row[i].vec[j], &s_ntt.vec[j]); //[0,Q)
        }
...
void PKE_Encrypt(uint8_t *c, const uint8_t *pk, const uint8_t *m, const uint8_t *seed)
{
    ...
    poly_zero(&v);
    for (int i = 0; i < RL_KEM_K; i++) {
        poly_accumulate_ntt_product(&v, &b_ntt.vec[i], &r_ntt.vec[i]);  // [0,Q)
    }
...
void PKE_Decrypt(uint8_t *m, const uint8_t *c, const uint8_t *sk)
{
    ...
    poly_zero(&su);
    for (int i = 0; i < RL_KEM_K; i++) {
        poly_accumulate_ntt_product(&su, &u.vec[i], &s_ntt.vec[i]);
    }
