...
File Description: Cycle-count benchmark units for the reference POLARLAC-512-Star implementation.
*/
    ...
    const polarlac_polyvec *u, const polarlac_polyvec *s_ntt)
{
    ...
    for (int k = 0; k < RL_KEM_K; k++) {
#if RL_KEM_Q == 769
        mlwe_accumulate_ntt_product(out, u->vec[k].coeffs, s_ntt->vec[k].coeffs);
#else
...
static void benchmark_pke_encrypt_units(void)
{
    ...
    for (int i = 0; i < RL_KEM_K; i++) {
        for (int j = 0; j < RL_KEM_N; j++) {
            u.vec[i].coeffs[j] = rl_kem_mod_q((int32_t)u.vec[i].coeffs[j] + e1_ntt.vec[i].coeffs[j]);
        }
    }
...
static void benchmark_pke_decrypt_units(void)
{
    ...
    for (int i = 0; i < RL_KEM_Lv; i++) {
        int16_t su_i = rl_kem_mod_q(su[i]);
