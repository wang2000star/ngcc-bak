...
File Description: Implements unit tests for the reference POLARLAC-512-Star implementation.
*/
...
static int test_poly_compress_crt_roundtrip(void)
{
    ...
    if (poly_compress(code, src) != 0) {
        fprintf(stderr, "poly_compress CRT unexpectedly rejected roundtrip input\n");
        return 0;
    }
    ...
    for (int i = 0; i < RL_KEM_N; i++) {
        if (dec[i] != src[i]) {
            fprintf(stderr, "poly_compress CRT mismatch at %d: got %d want %d\n",
                i, dec[i], src[i]);
            return 0;
        }
    }
...
static int test_polyvec_compress_crt_roundtrip(void)
{
    ...
    if (polyvec_compress(code, &src) != 0) {
        fprintf(stderr, "polyvec_compress CRT unexpectedly rejected roundtrip input\n");
        return 0;
    }
    ...
    for (int k = 0; k < RL_KEM_K; k++) {
        for (int i = 0; i < RL_KEM_N; i++) {
            if (dec.vec[k].coeffs[i] != src.vec[k].coeffs[i]) {
                fprintf(stderr, "polyvec_compress CRT mismatch at vec %d coeff %d: got %d want %d\n",
...
static int test_conj_ntt_bound_check(void)
{
    ...
    if (score < 0) {
        fprintf(stderr, "conjugate NTT score overflowed: %d\n", score);
        return 0;
    }

    if (con_poly_within_bound(poly, score) != 1) {
        fprintf(stderr, "conjugate NTT bound rejected exact score %d\n", score);
        return 0;
    }

    if (score > 0 && con_poly_within_bound(poly, score - 1) != 0) {
        fprintf(stderr, "conjugate NTT bound accepted score-1 threshold %d\n", score - 1);
        return 0;
    }
...
static int test_polar_encode_decode_ideal(void)
{
    ...
    uint8_t code_m[RL_KEM_N];
    uint8_t dec[PKE_MESSAGE_BYTES];
    int16_t hatm[RL_KEM_N];
    unsigned int expected_n = 0;

    while ((1u << expected_n) < RL_KEM_Lv) {
        expected_n++;
    }

    if (polar.N != RL_KEM_Lv || polar.n != expected_n ||
            polar.K != (unsigned int)(PKE_MESSAGE_BYTES * 8) ||
            polar.ecc_bytes != CODE_LEN) {
        fprintf(stderr,
            "polar parameter mismatch: N=%u/%u n=%u/%u K=%u/%u ecc_bytes=%u/%u\n",
            polar.N, (unsigned int)RL_KEM_N,
            polar.n, expected_n,
            polar.K, (unsigned int)(PKE_MESSAGE_BYTES * 8),
...
int main(void)
{
    ...
    report_test("poly_compress CRT roundtrip", test_poly_compress_crt_roundtrip());
    report_test("polyvec_compress CRT roundtrip", test_polyvec_compress_crt_roundtrip());
