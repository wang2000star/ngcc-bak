...
File Description: Implements NTT arithmetic routines for the reference POLARLAC-512 instance.
*/
...
static const int16_t con_ntt_f_769[128] = {
...
static const int16_t con_ntt_fn_769[128] = {
...
void con_poly_mul_ntt(int16_t *r, const int16_t *a, const int16_t *b)
{
    for (int i = 0; i < RL_KEM_N / 16; i++) {
        con_basemul_769(r + 16 * i, a + 16 * i, b + 16 * i, con_ntt_f_769[64 + i]);
...
void con_poly_adjoint_ntt(int16_t *r, const int16_t *a)
{
    for (int i = 0; i < RL_KEM_N / 16; i++) {
        con_base_adjoint_769(r + RL_KEM_N - 16 * (i + 1) + 8,
            a + 16 * i, con_ntt_f_769[64 + i]);
        con_base_adjoint_769(r + RL_KEM_N - 16 * (i + 1),
            a + 16 * i + 8, (int16_t)-con_ntt_f_769[64 + i]);
    }
}
