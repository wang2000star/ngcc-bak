...
File Description: Implements unit tests for the reference POLARLAC-256 implementation.
*/
...
static int test_uniform_sampler_range(void)
{
    ...
    for (int i = 0; i < RL_KEM_K; i++) {
        for (int j = 0; j < RL_KEM_K; j++) {
            for (int k = 0; k < RL_KEM_N; k++) {
                int16_t coeff = poly.row[i].vec[j].coeffs[k];
                if (coeff < 0 || coeff >= RL_KEM_Q) {
