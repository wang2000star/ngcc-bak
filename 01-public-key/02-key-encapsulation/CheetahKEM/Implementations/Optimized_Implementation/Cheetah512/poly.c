...
void encode_8bit_lsb(const int16_t *data, size_t datalen, uint8_t *out) {
    for (size_t i = 0; i < datalen; i += 8) {
        // 提取低8位并写入（展开8个元素）
        out[i + 0] = (uint8_t)(data[i + 0] & 0xFF);
        out[i + 1] = (uint8_t)(data[i + 1] & 0xFF);
        out[i + 2] = (uint8_t)(data[i + 2] & 0xFF);
        out[i + 3] = (uint8_t)(data[i + 3] & 0xFF);
        out[i + 4] = (uint8_t)(data[i + 4] & 0xFF);
        out[i + 5] = (uint8_t)(data[i + 5] & 0xFF);
        out[i + 6] = (uint8_t)(data[i + 6] & 0xFF);
        out[i + 7] = (uint8_t)(data[i + 7] & 0xFF);
    }
}

void decode_8bit_lsb(const uint8_t *in, size_t outlen, int16_t *out) {
    for (size_t i = 0; i < outlen; i += 8) {
        out[i + 0] = (int16_t)in[i + 0];
        out[i + 1] = (int16_t)in[i + 1];
        out[i + 2] = (int16_t)in[i + 2];
        out[i + 3] = (int16_t)in[i + 3];
        out[i + 4] = (int16_t)in[i + 4];
        out[i + 5] = (int16_t)in[i + 5];
        out[i + 6] = (int16_t)in[i + 6];
        out[i + 7] = (int16_t)in[i + 7];
    }
}
