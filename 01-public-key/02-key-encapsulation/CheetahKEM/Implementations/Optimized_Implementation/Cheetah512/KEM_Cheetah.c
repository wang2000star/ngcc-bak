...
static void indcpa_encrypt(const uint8_t m[MSG_BYTES],
    ...
    encode_8bit_lsb(v, SHARED_KEY_BYTES*8, c+(CHEETAH_K*N)*DU/8);
}
                    ...
                    uint8_t m[MSG_BYTES]) 
{
    ...
    int16_t s[CHEETAH_K*N];
   // decode_vector(sk, s, CHEETAH_K*N, QBITS);
    decode_13bit_lsb(sk, CHEETAH_K*N, s);

    int16_t u[CHEETAH_K*N], v[CHEETAH_K*N];
    //decode_vector(c, u, CHEETAH_K*N, DU);
    decode_11bit_lsb(c, CHEETAH_K*N, u);
    //decode_vector(c+(CHEETAH_K*N)*DU/8, v, SHARED_KEY_BYTES*8, DV);
    decode_8bit_lsb(c+(CHEETAH_K*N)*DU/8, SHARED_KEY_BYTES*8, v);
