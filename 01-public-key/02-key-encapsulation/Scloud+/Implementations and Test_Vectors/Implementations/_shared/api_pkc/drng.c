...
static int SM3_DRNG_Instantiate(DRNG_ctx *drng, const unsigned char *nonce, unsigned long long nonce_len_bytes)
{
    ...
    if (nonce_len_bytes != 0)
    {
        memcpy(seed_material, nonce, nonce_len_bytes);
    }
    rc = SM3_df(seed_material, nonce_len_bytes);
    if (DRNG_SUCCESS != rc)
    {
        free(seed_material);
        return rc;
    ...
    rc = SM3_df(padded_V, sizeof(padded_V));
    if (DRNG_SUCCESS != rc)
    {
        free(seed_material);
        return rc;
...
{
    if (NULL == drng || (NULL == seed && seed_len_bytes != 0))
    {
        return -1;
    }
...
{
    if (NULL == drng || (NULL == random_number && random_number_len_bits != 0))
    {
        return -1;
    }
    if (0 == random_number_len_bits)
    {
        return DRNG_SUCCESS;
    }
