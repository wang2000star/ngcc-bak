...
#ifndef SCLOUDPLUS_SM3_BACKEND_H
#define SCLOUDPLUS_SM3_BACKEND_H
...
extern "C"
{
    ...
    /// @brief Pseudo-XOF output starting from a caller-selected 32-bit counter
    /// @param[in] output_len_bits Total bits of output
    /// @param[in] msg Base address of input message byte array
    /// @param[in] msg_len_bits Total bits of input message
    /// @param[in] start_ct First SM3 KDF counter value to use
    /// @param[out] output Base address of output byte array
    /// @return 0 for success, others for error
    int pseudoXOF_from_counter(unsigned long long output_len_bits, const unsigned char *msg, unsigned long long msg_len_bits, unsigned int start_ct, unsigned char *output);
