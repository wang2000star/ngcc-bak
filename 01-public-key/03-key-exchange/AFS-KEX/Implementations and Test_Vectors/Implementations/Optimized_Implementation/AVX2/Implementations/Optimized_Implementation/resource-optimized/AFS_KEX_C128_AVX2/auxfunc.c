...
{
	unsigned char cascade_msg_ct[msg_len_bytes + 4];

	memcpy(cascade_msg_ct, msg, msg_len_bytes);
	cascade_msg_ct[msg_len_bytes + 0] = (unsigned char)(counter >> 24);
	cascade_msg_ct[msg_len_bytes + 1] = (unsigned char)(counter >> 16);
	cascade_msg_ct[msg_len_bytes + 2] = (unsigned char)(counter >> 8);
	cascade_msg_ct[msg_len_bytes + 3] = (unsigned char)counter;
	sm3_bit(cascade_msg_ct, ((unsigned long long)msg_len_bytes + 4ULL) * 8ULL, output);
...
int iccs_pseudoxof_bytes_scalar(unsigned char *output, size_t output_len_bytes, const unsigned char *msg, size_t msg_len_bytes)
{
	...
	while (off < output_len_bytes)
	{
		size_t take = output_len_bytes - off;
		if (take > sizeof(block))
		{
			take = sizeof(block);
		}
		iccs_sm3_counter_block(block, msg, msg_len_bytes, ct++);
		memcpy(output + off, block, take);
		off += take;
	}
