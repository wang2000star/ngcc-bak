...
void gen_matrix(polyvec *a, const uint8_t seed[KYBER_SYMBYTES], int transposed)
{
  ...
  kyber_iccs_xof_squeezeblocks4(outs, GEN_MATRIX_NBLOCKS, seed, xs, ys);
  ...
  for(idx = 0; idx < KYBER_K * KYBER_K; idx++) {
    ...
    ctr = rej_uniform(a[i].vec[j].coeffs, KYBER_N, bufs[idx], buflen);
    while(ctr < KYBER_N) {
...
#if KYBER_K == 2
  poly_getnoise_eta1_4x(&skpv.vec[0], &skpv.vec[1], &e.vec[0], &e.vec[1], noiseseed, nonce);
  nonce += 4;
#else
  for(i=0;i<KYBER_K;i++)
...
#if KYBER_K == 2
  poly_getnoise_eta1_4x(&sp.vec[0], &sp.vec[1], &ep.vec[0], &ep.vec[1], coins, nonce);
  nonce += 4;
#else
  for(i=0;i<KYBER_K;i++)
