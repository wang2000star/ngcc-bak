...
/* The KEX path draws ALL its randomness from drng_algorithm (seeded with a fixed
 * seed below), which is the sole source of determinism for these vectors. The
 * imported KEM core still references randombytes() from its non-derand
 * keypair/enc (never called on the KEX path), so we provide a deterministic surf
 * implementation purely to satisfy that link symbol (this elf links without the
 * HAL randombytes). */
...
static void printbytes(const unsigned char *x, unsigned long long xlen)
{
  ...
  for (i = 0; i < xlen; i++) {
    ...
    off += 2;
    if (off >= 2 * 64) { outs[off] = 0; hal_send_str(outs); off = 0; }
  }
  if (off) { outs[off] = 0; hal_send_str(outs); }
}
...
int main(void)
{
  ...
  for (i = 0; i < NTESTS; i++) {
    ...
    for (j = 0; (unsigned long long)j < ssa_len; j++) {
      if (ssa[j] != ssb[j]) { hal_send_str("ERROR"); hal_send_str("#"); return -1; }
    }
  }
