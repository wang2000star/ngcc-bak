#!/usr/bin/env bash
# DKEX-128 Additional (arm64 DKE core; ML-DSA reference). Build on an arm64 host (or cross + qemu).
set -e
CC=${CC:-gcc}
CF="-O3 -fcommon -march=armv8.2-a -DDKM_AARCH64 -DDKE_USE_AARCH64 -DDKE_USE_AARCH64_NATIVE -DADKEX_MODE=128 -DDKE_MODE=128 -DDKE_HASH=0 -DDKE_RANDOM=0 -DADKEX_SIG_BACKEND_MLDSA -DADKEX_SIG_MLDSA_LEVEL=2 -DDILITHIUM_MODE=2 -std=c99 -Wpedantic -Wall -Wextra -flto -fomit-frame-pointer"
...
$CC -O3 -flto -fomit-frame-pointer obj/*.o -o kat_DKEX-128 -lm $LRT
./kat_DKEX-128
