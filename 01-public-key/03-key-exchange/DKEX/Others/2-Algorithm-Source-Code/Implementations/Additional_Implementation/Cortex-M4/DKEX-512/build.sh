#!/usr/bin/env bash
# DKEX-512 on ARM Cortex-M4. DKE ephemeral KEM = Plantard + matacc (M4 asm); ML-DSA
...
DEFS="-DADKEX_MODE=512 -DDKE_MODE=512 -DDKE_HASH=0 -DDKE_RANDOM=0 -DDKE_USE_CORTEX_M4_PLANTARD -DDKE_USE_MATACC -DADKEX_SIG_BACKEND_MLDSA -DADKEX_SIG_MLDSA_LEVEL=5 -DDILITHIUM_MODE=5"
...
for s in cortex-m4/sm3_compress_asm_m4.S qemu/startup_mps2.S cortex-m4/plantard/plantard512_fastntt.S cortex-m4/plantard/plantard512_fastinvntt.S cortex-m4/plantard/plantard512_fastbasemul.S cortex-m4/plantard/plantard512_fastaddsub.S cortex-m4/plantard/plantard512_reduce_asm.S cortex-m4/plantard/plantard512_poly_asm.S cortex-m4/plantard/plantard512_matacc_asm.S; do $GCC $ARCH $DEFS -I. -Icortex-m4/plantard -c "$s" -o obj/a$i.o; i=$((i+1)); done
...
$GCC $ARCH obj/*.o -T qemu/mps2.ld -Wl,--gc-sections -Wl,--start-group -lgcc -lnosys -lc -Wl,--end-group -o kat_DKEX-512.elf
echo "Built kat_DKEX-512.elf — running DKEX 3-pass handshake self-test on QEMU:"
$QEMU -M mps2-an386 -nographic -semihosting-config enable=on,target=native -kernel kat_DKEX-512.elf
