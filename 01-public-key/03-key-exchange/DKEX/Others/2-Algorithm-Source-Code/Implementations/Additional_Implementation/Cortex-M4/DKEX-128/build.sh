#!/usr/bin/env bash
# DKEX-128 on ARM Cortex-M4. DKE ephemeral KEM = Plantard + matacc (M4 asm); ML-DSA
...
DEFS="-DADKEX_MODE=128 -DDKE_MODE=128 -DDKE_HASH=0 -DDKE_RANDOM=0 -DDKE_USE_CORTEX_M4_PLANTARD -DDKE_USE_MATACC -DADKEX_SIG_BACKEND_MLDSA -DADKEX_SIG_MLDSA_LEVEL=2 -DDILITHIUM_MODE=2"
...
for s in cortex-m4/sm3_compress_asm_m4.S qemu/startup_qemu.S cortex-m4/plantard/plantard_fastntt.S cortex-m4/plantard/plantard_fastinvntt.S cortex-m4/plantard/plantard_fastbasemul.S cortex-m4/plantard/plantard_fastaddsub.S cortex-m4/plantard/plantard_reduce_asm.S cortex-m4/plantard/plantard_poly_asm.S cortex-m4/plantard/plantard_matacc_asm.S; do $GCC $ARCH $DEFS -I. -Icortex-m4/plantard -c "$s" -o obj/a$i.o; i=$((i+1)); done
...
$GCC $ARCH obj/*.o -T qemu/netduino.ld -Wl,--gc-sections -Wl,--start-group -lgcc -lnosys -lc -Wl,--end-group -o kat_DKEX-128.elf
echo "Built kat_DKEX-128.elf — running DKEX 3-pass handshake self-test on QEMU:"
$QEMU -M netduinoplus2 -nographic -semihosting-config enable=on,target=native -kernel kat_DKEX-128.elf
