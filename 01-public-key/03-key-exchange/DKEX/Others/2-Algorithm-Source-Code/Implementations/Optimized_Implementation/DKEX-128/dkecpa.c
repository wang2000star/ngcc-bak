...
#if defined(DKE_USE_CORTEX_M4_PLANTARD) && DKE_N == 512 && !defined(DKE_NO_MATACC512)
/* Forward declaration: defined in dke_m4_plantard.c (DKE_N==512 section) */
extern void DKE_polymatmul_matacc512(poly res[DKE_K], const polyvec *b,
...
#if !(defined(DKE_USE_CORTEX_M4_PLANTARD) && DKE_N == 512 && !defined(DKE_NO_MATACC512))
    polyvec mat[DKE_K];   /* 16 KB for DKE-512; omitted when matacc is used */
#endif
    polyvec pA, eA, sA;
...
#if defined(DKE_USE_CORTEX_M4_PLANTARD) && DKE_N == 512 && !defined(DKE_NO_MATACC512)
    /* matacc512: fused A-generation + basemul, no 16 KB mat[] on stack */
    (void)0; /* mat[] not declared; matacc runs after noise sampling below */
#else
    gen_a(mat, seed);
...
#endif
#endif /* DKE_USE_CORTEX_M4_PLANTARD && DKE_N==512 */
#if 1
#if defined(DKE_USE_OPT_C) && (DKE_HASH == 2)
...
#if defined(DKE_USE_CORTEX_M4_PLANTARD) && DKE_N == 512 && !defined(DKE_NO_MATACC512)
    /* matacc512: generate A on-the-fly and compute pA = A * sA (no 16 KB mat) */
    DKE_polymatmul_matacc512(pA.vec, &sA, seed, 0);
    for (i = 0; i < DKE_K; i++) DKE_poly_tomont(&pA.vec[i]);
#else
    for (i = 0; i < DKE_K; i++) {
...
#if !(defined(DKE_USE_CORTEX_M4_PLANTARD) && DKE_N == 512 && !defined(DKE_NO_MATACC512))
    polyvec matt[DKE_K];   /* 16 KB for DKE-512; omitted when matacc is used */
#endif
    polyvec pA, pB, sB, eB;
    poly kB, e;
    unsigned int i, nonce = 0;
...
#if defined(DKE_USE_CORTEX_M4_PLANTARD) && DKE_N == 512 && !defined(DKE_NO_MATACC512)
    /* matacc512: A^T generation deferred (fused with basemul below);
     * noise sampling must still happen here, using scalar SM3 path. */
    for (i = 0; i < DKE_K; i++) DKE_getsecretB(sB.vec + i, coins, nonce++);
    for (i = 0; i < DKE_K; i++) DKE_geterrorB(eB.vec + i, coins, nonce++);
#elif defined(DKE_USE_AVX2) && (DKE_HASH == 2) && DKE_K == 2
...
#if defined(DKE_USE_CORTEX_M4_PLANTARD) && DKE_N == 512 && !defined(DKE_NO_MATACC512)
    /* matacc512: generate A^T on-the-fly and compute pB = A^T * sB */
    DKE_polymatmul_matacc512(pB.vec, &sB, seed, 1);
#else
    for (i = 0; i < DKE_K; i++)
...
#if !(defined(DKE_USE_AVX2) && (DKE_HASH == 2) && DKE_K == 2)
