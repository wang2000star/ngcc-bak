#!/usr/bin/env bash
# DKEX-512 Reference (portable scalar C). drng/auxfunc are official API_PKC files.
#  and dilithium/ have colliding header basenames -> separate -I groups.
set -e
CC=${CC:-gcc}
CF="-std=c99 -Wpedantic -Wall -Wextra -O2 -fcommon -DDKE_FORCE_SCALAR -DADKEX_MODE=512 -DDKE_MODE=512 -DDKE_HASH=0 -DDKE_RANDOM=0 -DADKEX_SIG_BACKEND_MLDSA -DADKEX_SIG_MLDSA_LEVEL=5 -DDILITHIUM_MODE=5"
