# DKEX（PKCKEX-359777）自评估材料包 — 密钥交换（KEX，3 趟 + 签名）

依据《新一代商用密码算法 x86 架构 / ARM 架构实现自评估指引》（商用密码标准研究院，2026 年 6 月）整理。
本包为 DKEX 算法的**独立自评估材料**，与 §3.1(1) 电子提交包（will-submit）**平行**提交，不并入电子包。
...
# DKEX (PKCKEX-359777) Self-Assessment Materials — Key Exchange (KEX, 3-pass + signature)
...
This package is the **standalone self-assessment material** for DKEX, submitted **in parallel** with the §3.1(1) electronic submission package (`will-submit`); it is not merged into the electronic package.
