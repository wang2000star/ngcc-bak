#!/usr/bin/env bash
# DKEX-512 Optimized (DKE core AVX2; ML-DSA NIST reference). DKE hashing via auxfunc (sm3x8 removed).
#  and dilithium/ have colliding header basenames -> separate -I groups.
set -e
CC=${CC:-gcc}
CF="-O3 -mavx2 -fcommon -DDKM_LINUX -DDKE_AVX2_NTT256_ASM -DDKE_NTT512_PACKED -DADKEX_MODE=512 -DDKE_MODE=512 -DDKE_HASH=0 -DDKE_RANDOM=0 -DADKEX_SIG_BACKEND_MLDSA -DADKEX_SIG_MLDSA_LEVEL=5 -DDILITHIUM_MODE=5 -std=c99 -Wpedantic -Wall -Wextra -flto -fomit-frame-pointer -march=x86-64 -mtune=native"
...
$CC -O3 -flto -fomit-frame-pointer obj/*.o -o kat_DKEX-512 -lm
./kat_DKEX-512
echo "KAT -> output/KAT_KEX_DKEX-512.txt"
