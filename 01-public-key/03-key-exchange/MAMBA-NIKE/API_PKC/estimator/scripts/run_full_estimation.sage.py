...
# This file was *autogenerated* from the file scripts/run_full_estimation.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_65 = Integer(65); _sage_const_2048 = Integer(2048); _sage_const_0p292 = RealNumber('0.292'); _sage_const_125 = Integer(125); _sage_const_17 = Integer(17); _sage_const_18 = Integer(18)#!/usr/bin/env sage
# ============================================================
...
sys.path.insert(_sage_const_0 , ESTIMATOR_DIR)
...
for name, target, n, q, eta_s, eta_r, d_pk, d_u in PROFILES:
    ...
    matzov_c = primal
    matzov_q = _sage_const_0p292  * beta_m if beta_m else None
...
for r in results:
    name, target, n, q, d_pk, d_u, eta_s, eta_r,         pr, du, dh, bk, mc, mq, beta, best_algo, best_bits, mgc, mgq = r
...
for d in detail_logs:
    ...
    if mgc_v is not None:
        icon_c = "✅" if mgc_v >= _sage_const_0  else "⚠️"
        W(f"- **安全裕度 (MATZOV C)**: {mgc_v:+.1f} bit {icon_c}")
    if mgq_v is not None:
        icon_q = "✅" if mgq_v >= _sage_const_0  else "⚠️"
...
min_mgc = min((r[_sage_const_17 ] for r in results if r[_sage_const_17 ] is not None), default=None)
min_mgq = min((r[_sage_const_18 ] for r in results if r[_sage_const_18 ] is not None), default=None)
...
W("|------|---------------|---------------|")
for r in results:
    mgc = r[_sage_const_17 ]
    mgq = r[_sage_const_18 ]
    c_ok = "✅" if mgc is not None and mgc >= _sage_const_0  else ("⚠️" if mgc is not None else "--")
    q_ok = "✅" if mgq is not None and mgq >= _sage_const_0  else ("⚠️" if mgq is not None else "--")
    W(f"| {r[_sage_const_0 ]} | {c_ok} | {q_ok} |")
W()
