...
static void shake_squeeze(unsigned char *output, unsigned int outputByteLen,
                          uint64_t *s, unsigned int rate)
{
  ...
  {
    keccak_squeezeblocks(output, 1, s, rate);
    output += rate;
    outputByteLen -= rate;
  }
  ...
  if(outputByteLen > 0)
  {
    keccak_squeezeblocks(block, 1, s, rate);
...
void shake128(unsigned char *output, unsigned int outputByteLen, const unsigned char *input, unsigned int inputByteLen)
{
  ...
  shake_squeeze(output, outputByteLen, s, SHAKE128_RATE);
}
...
{
  uint64_t s[25];
  shake256_absorb(s, input, inputByteLen);
  shake_squeeze(output, outputByteLen, s, SHAKE256_RATE);
}
