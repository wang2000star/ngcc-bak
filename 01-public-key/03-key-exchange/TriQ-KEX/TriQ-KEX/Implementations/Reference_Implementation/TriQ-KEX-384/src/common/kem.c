 ...
 * @warning This function calls ::prng_get_bytes() to sample internal randomness. If the PRNG has not been
 *          properly seeded beforehand, the generated keys will be insecure/predictable.
 * @note The API_PKC KEX wrapper seeds the internal PRNG through KEX_AlgorithmInstance.c.
 *       Production integrations must seed the PRNG before calling this function.
...
int crypto_kem_keypair(uint8_t *ek_kem, uint8_t *dk_kem) {
    ...
    // Compute TriQ-PKE keypair
    triq_pke_keygen(ek_pke, dk_pke, seed_pke);

    // Compute TriQ-KEM keypair
...
int crypto_kem_enc(uint8_t *c_kem, uint8_t *K, const uint8_t *ek_kem) {
    ...
    triq_pke_encrypt(&c_kem_t.c_pke, ek_kem, (uint64_t *)m, theta);

    triq_c_kem_to_string(c_kem, &c_kem_t);
...
int crypto_kem_dec(uint8_t *K_prime, const uint8_t *c_kem, const uint8_t *dk_kem) {
    ...
    triq_c_kem_from_string(&c_kem_t.c_pke, c_kem_t.salt, c_kem);

    // Compute message m_prime
    result = triq_pke_decrypt((uint64_t *)m_prime, dk_pke, &c_kem_t.c_pke);
    ...
    triq_pke_encrypt(&c_kem_prime_t.c_pke, ek_pke, (uint64_t *)m_prime, theta_prime);
