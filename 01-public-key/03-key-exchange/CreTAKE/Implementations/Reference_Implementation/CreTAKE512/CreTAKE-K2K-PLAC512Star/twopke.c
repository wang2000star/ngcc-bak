...
{ 
  ...
  if (m == NULL || c1 == NULL || c2 == NULL || sk1 == NULL || sk2 == NULL) {
    ...
    polyvec_decompress(&ubar, c1);
    polyvec_decompress(&uprime, c2);
    deserialize_polyvec_ntt(&s_ntt1, sk1);
    deserialize_polyvec_ntt(&s_ntt2, sk2);

    poly_zero(&subar);
    for (int i = 0; i < RL_KEM_K; i++) {
        poly_accumulate_ntt_product(&subar, &ubar.vec[i], &s_ntt1.vec[i]);
    }
    poly_intt_to_coeff(&subar);

    poly_zero(&suprime);
    for (int i = 0; i < RL_KEM_K; i++) {
        poly_accumulate_ntt_product(&suprime, &uprime.vec[i], &s_ntt2.vec[i]);
    }
