...
#define H3_BITS        8
#define H3_DIV_M       171U      /* ceil(2^9 / 3) */
...
static void pack_polyveck_h_base3(unsigned char *out, const polyveck *h,
    ...
    memset(ov_buf, 0, BIT_POLYVECK_H_OVERFLOW_BYTES);   /* zero padding for verify */
    /* overflow: byte 0 = count, then 10-bit entries packed from bit 0 of byte 1 */
    uint64_t ov_acc = 0;
    int ov_bits = 0, ov_wi = 1;   /* reserve byte 0 for count */
    ...
    /* flush remaining overflow bits, then write count at byte 0 */
    while (ov_bits > 0) {
        ov_buf[ov_wi++] = (uint8_t)ov_acc;
        ov_acc >>= 8;
        ov_bits -= 8;
...
static int unpack_polyveck_h_base3(polyveck *h, const unsigned char *in,
    ...
    /* unpack base3 first (writes ±1 for all positions including overflows) */
    for (int i = 0; i < BIT_K; i++) {
        for (int g = 0; g < BIT_POLY_H_GROUPS; g++) {
            if (bits < H3_BITS) {
            ...
            for (int k = 0; k < BIT_H_PACK_GROUP && idx_base + k < BIT_N; k++) {
                h3_divmod(&q, &r, b);
                h->vec[i].coeffs[idx_base + k] = (int16_t)(r - 1);
                b = q;
            }
        }
    }

    /* apply overflow entries on top (overwrites base3 ±1 with ±2) */
    {
        uint64_t ov_acc = 0;
        ...
        /* reject if unused bits / trailing bytes are non‑zero */
