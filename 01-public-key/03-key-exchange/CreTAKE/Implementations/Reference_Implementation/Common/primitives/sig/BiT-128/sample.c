...
{
    unsigned int i;
    uint8_t __attribute__((aligned(32))) buf[BYTELEN];
...
{
    uint8_t __attribute__((aligned(32))) buf[BYTELEN_11];
    bit_xof256_nonce(buf, sizeof(buf), seed, BIT_SEEDBYTES, *nonce);
    *nonce = (uint16_t)(*nonce + 1U);
...
{
    unsigned int i;
    uint8_t __attribute__((aligned(32))) buf[BYTELEN_3BIT];
    bit_xof256_nonce(buf, sizeof(buf), seed, BIT_SEEDBYTES, *nonce);
    *nonce = (uint16_t)(*nonce + 1U);
...
#define BYTELEN_4BIT (BIT_N)           /* 4-bit → 2 coeffs/byte, 256 coeffs = 128 lo + 128 hi = 256 */

void poly_sample_triangular_4bit(int16_t *out, const uint8_t *seed, uint16_t *nonce)
{
    unsigned int i;
    uint8_t __attribute__((aligned(32))) buf[BYTELEN_4BIT];
    bit_xof256_nonce(buf, sizeof(buf), seed, BIT_SEEDBYTES, *nonce);
    *nonce = (uint16_t)(*nonce + 1U);
    ...
    for(i = 0; i < BIT_N / 4; i++)
    {
        ...
        const uint8_t *hi = buf + BYTELEN_4BIT / 2 + 2 * i;

        uint16_t lo0 = (lo[0] >> 0) & 0x0F;
        uint16_t lo1 = (lo[0] >> 4) & 0x0F;
        uint16_t lo2 = (lo[1] >> 0) & 0x0F;
        uint16_t lo3 = (lo[1] >> 4) & 0x0F;

        uint16_t hi0 = (hi[0] >> 0) & 0x0F;
        uint16_t hi1 = (hi[0] >> 4) & 0x0F;
        uint16_t hi2 = (hi[1] >> 0) & 0x0F;
        uint16_t hi3 = (hi[1] >> 4) & 0x0F;
...
int check_reject_highbits_w1_sparse(const poly *w1, const poly *w0, const sparse_challenge *c) {
    return poly_check_reject_highbits_w1_sparse(w1, w0, c);
}
...
int check_reject_hint_range(const polyveck *h) {
    ...
    for (int i = 0; i < BIT_K; i++)
        for (int j = 0; j < BIT_N; j++)
            ov += (uint32_t)(ct_abs(h->vec[i].coeffs[j]) >> 1);
    reject_flag |= (uint32_t)(BIT_H_OVERFLOW_MAX - ov) >> 31;
#endif
    return (int)reject_flag;
}
