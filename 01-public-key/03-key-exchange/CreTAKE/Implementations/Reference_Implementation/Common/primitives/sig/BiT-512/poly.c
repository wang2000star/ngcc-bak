...
uint32_t reduce_barrett(int64_t a) {
    ...
    a -= t * BIT_Q;
    /* The Barrett estimate t may be off by one for inputs below ~-5q, leaving
     * the residual slightly negative; fold it back into [0, q) with a leading
     * conditional add before the canonical centering step. */
    a += (a >> 63) & BIT_Q;
    a -= BIT_Q;
    a += (a >> 63) & BIT_Q;
    return (uint32_t)a;
...
/* HighBits(w, γ₂) := round(reduce_to_unsigned(w) / γ₂),  result in [0, ALPHA_HINT). */
...
/*
 * Constant-time challenge multiply.
 * The challenge is sampled in {0,1} (τ ones); poly_cneg then applies the
 * bimodal sign (−1)^b to ALL coefficients uniformly, so every sparse entry
 * shares the same sign s = (−1)^b.  We accumulate the *unsigned* negacyclic
 * shift-sum  acc = Σ_t x^{pos[t]}·a  (the wrap negation is from x^N=−1,
 * independent of s) and apply the single sign s branch-free.  This removes
 * the per-position `if (sign>0)` branch that leaked the secret bit b through
 * control flow; the numeric result is unchanged, so KAT is unchanged.
 */
