...
int kex_generate_pass1_msg_a(unsigned char *ska,unsigned long long ska_len_bytes,unsigned char *pkb,unsigned long long pkb_len_bytes,unsigned char *sta,unsigned long long *sta_len_bytes,unsigned char *m1,unsigned long long *m1_len_bytes){
	...
	unsigned char buf[SEED_BYTES + SKI_LEN], k_j[KEM_K_LEN];
	...
	ALIGN32 unsigned char apkb[PAD32(PKR_LEN)] = {0};
	ALIGN32 unsigned char actj[PAD32(C_R_LEN)] = {0};
	...
	memcpy(apkb, pkb, PKR_LEN);
	ret = kem_enc(apkb, PKR_LEN, k_j, &ss_l, actj, &ct_l);
	if (ret != 0) {
        return -3;
    }
	memcpy(ct_j, actj, C_R_LEN);
...
int kex_generate_pass2_msg_b(unsigned char *skb,unsigned long long skb_len_bytes,unsigned char *pka,unsigned long long pka_len_bytes,unsigned char *m1,unsigned long long m1_len_bytes,unsigned char *stb,unsigned long long *stb_len_bytes,unsigned char *m2,unsigned long long *m2_len_bytes){
	...
	unsigned char k_j[KEM_K_LEN];
	unsigned char *ct_i = m2;
	unsigned char pkb[PKR_LEN];
	unsigned char k_i[KEM_K_LEN];

	ALIGN32 unsigned char apka[PAD32(PKI_LEN)] = {0};
	ALIGN32 unsigned char atpk[PAD32(TPK_LEN)] = {0};
	ALIGN32 unsigned char act_i[PAD32(C_I_LEN)] = {0};
	ALIGN32 unsigned char askb[PAD32(SKR_LEN)] = {0};
	ALIGN32 unsigned char act_j[PAD32(C_R_LEN)] = {0};
	(void)m1_len_bytes;

	memcpy(pkb, skb + TSK_LEN, PKR_LEN);
	memcpy(atpk, m1, TPK_LEN);
	memcpy(act_j, m1 + TPK_LEN, C_R_LEN);

	memcpy(apka, pka, PKI_LEN);
	twokem_enc(apka, pka_len_bytes, atpk, TPK_LEN, k_i, &l_ss, act_i, &l_ct);
	*m2_len_bytes = l_ct;
	memcpy(ct_i, act_i, C_I_LEN);

	memcpy(askb, skb, SKR_LEN);
	kem_dec(askb, skb_len_bytes, act_j, C_R_LEN, k_j, &l_ss);

	derive_session_key(stb, atpk, pka, pkb, ct_i, act_j, k_i, KEM_K_LEN, k_j, KEM_K_LEN);
	*stb_len_bytes = K_LEN;
	return 1;
}
