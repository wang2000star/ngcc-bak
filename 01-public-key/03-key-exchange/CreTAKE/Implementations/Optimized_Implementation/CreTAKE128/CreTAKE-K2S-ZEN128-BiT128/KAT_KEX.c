...
static void *aligned_calloc_32(size_t n, size_t size)
{
    ...
    if (bytes == 0) {
        padded_bytes = ALIGNMENT_32;
    } else {
        if (bytes > SIZE_MAX - (ALIGNMENT_32 - 1u)) {
            return NULL;
        }

        padded_bytes =
            (bytes + ALIGNMENT_32 - 1u) &
            ~(size_t)(ALIGNMENT_32 - 1u);
    }

    if (posix_memalign(&ptr, ALIGNMENT_32, padded_bytes) != 0) {
        return NULL;
    }

    memset(ptr, 0, padded_bytes);
    return ptr;
}
...
int main()
{
	...
	FILE *file_output;
	unsigned char *seed, *pka, *ska, *pkb, *skb, *sta, *stb, *ssa, *ssb, *m1, *m2, *ma, *mb;
	unsigned long long pass, pka_len_bytes, ska_len_bytes, pkb_len_bytes,
		...
		m1_len_bytes, m2_len_bytes, ma_len_bytes, mb_len_bytes, total_len_bytes;
	int rtn;
