...
void sha3_256_concat2(uint8_t h[32],
                      ...
                      const uint8_t *in1, size_t in1len)
{
  ...
  keccak_init(s);
  pos = keccak_absorb(s, 0, SHA3_256_RATE, in0, in0len);
  pos = keccak_absorb(s, pos, SHA3_256_RATE, in1, in1len);
  keccak_finalize(s, pos, SHA3_256_RATE, 0x06);
  KeccakF1600_StatePermute(s);
  for(i=0;i<4;i++)
    store64(h+8*i,s[i]);
}
