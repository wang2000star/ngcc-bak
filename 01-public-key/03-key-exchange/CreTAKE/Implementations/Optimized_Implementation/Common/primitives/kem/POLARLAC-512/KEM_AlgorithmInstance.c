...
File Description: Implements the API_PKC KEM wrapper for the optimized POLARLAC-512Light instance.
*/

//  FO transform (BIT_USE_SHAKE selects SHAKE or SM3)：

// G = bit_xof(m || pk) -> ss || seed
// RejectKey = bit_xof(z || ct), where z is stored in sk
...
{
    if (bit_xof_concat2_bytes(ss, KEM_SS_BYTES,
            reject_seed, KEM_REJECT_SEED_BYTES, ct, PKE_CIPHERTEXT_BYTES) != 0) {
        return -13;
    }
    ...
    unsigned char *sk, unsigned long long *sk_len_bytes)
{
    ...
    *sk_len_bytes = kem_get_sk_len_bytes();
    ...
    if (kem_get_random_number(&drng_algorithm, seed_kg, KEM_SEED_LEN_BYTES * 8ULL) != 0) {
        return -2;
    }
    ...
    if (kem_get_random_number(&drng_algorithm,
    ...
    unsigned char *ct, unsigned long long *ct_len_bytes)
{
    ...
    *ct_len_bytes = kem_get_ct_len_bytes();
    ...
    if (kem_get_random_number(&drng_algorithm, m, PKE_MESSAGE_BYTES * 8ULL) != 0) {
        return -3;
    }
