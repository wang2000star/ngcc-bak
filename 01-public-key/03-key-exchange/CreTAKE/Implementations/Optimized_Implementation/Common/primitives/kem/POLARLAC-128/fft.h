...
File Description: Declares FFT-based spectral helper routines and constants for the optimized POLARLAC-128 instance.
*/
 ...
 *
 * Notes:
 * - This header provides a fixed-size 16-bit complex type and a precomputed
 *   integer twiddle table for the fixed-point FFT rejection path.
 * - POLARLAC-128 uses an integer-only AVX2 implementation: no float/double,
 *   no runtime sin/cos and no FFTW are used in the rejection sampler.
...
/**
 * @brief Integer AVX2-backed spectral rejection check.
 * @details For POLARLAC-128 this uses folded half-packing, iterative fixed
 *          stages and a fused final-stage bound check. It is exposed for
 *          benchmarking.
 */
