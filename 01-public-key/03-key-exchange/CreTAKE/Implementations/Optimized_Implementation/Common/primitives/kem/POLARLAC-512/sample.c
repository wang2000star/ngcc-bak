...
static inline void sample_ternary_9_128_avx2(int16_t *a, const uint8_t *buf)
{
    uint8_t kbuf[16] POLARLAC_SAMPLE_ALIGN32;
    const __m256i one = _mm256_set1_epi16(1);
    const __m256i nine = _mm256_set1_epi16(9);
    const __m256i high = _mm256_set1_epi16(118);
    unsigned int in = 0;
    ...
    for (unsigned int j = 0; j < RL_KEM_N; j += 16) {
        ...
        __m256i mneg = _mm256_and_si256(_mm256_cmpgt_epi16(nine, k), one);  /* k < 9 */
        __m256i mpos = _mm256_and_si256(_mm256_cmpgt_epi16(k, high), one);  /* k > 118 */
...
// Ternary sampler: {-1:9/128; 0:110/128; 1:9/128}.
static void poly_generate_tenary_avx(int16_t *a, const uint8_t *seed, uint8_t nonce)
{
    ...
    sample_ternary_9_128_avx2(a, buf);
