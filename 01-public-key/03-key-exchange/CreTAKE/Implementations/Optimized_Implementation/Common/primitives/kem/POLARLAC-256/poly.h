...
File Description: Declares polynomial compression, encoding, and arithmetic helpers for the optimized POLARLAC-256 instance.
*/
...
#define POLARLAC_ALIGN64 __attribute__((aligned(64)))
#else
#define POLARLAC_ALIGN64
#endif
...
typedef struct POLARLAC_ALIGN64 {
...
/// @brief Truncate canonical coefficients to their low byte representation.
/// @param[out] dst Base address of the output byte array of length `C1_LEN_BYTES`.
/// @param[in] src Base address of the input coefficient array of length `RL_KEM_N`.
void byte_compress(uint8_t *dst, const int16_t *src);

/// @brief Decompress a byte string produced by `byte_compress`.
/// @param[out] dst Base address of the recovered coefficient array of length `RL_KEM_N`.
/// @param[in] src Base address of the byte array of length `C1_LEN_BYTES`.
void byte_decompress(int16_t *dst, const uint8_t *src);

/// @brief Truncate a length-K polynomial vector to low bytes.
/// @param[out] dst Output byte array of length `C1_LEN_BYTES`.
/// @param[in] src Input polynomial vector.
void polyvec_byte_compress(uint8_t *dst, const polarlac_polyvec *src);

/// @brief Decompress a byte string produced by `polyvec_byte_compress`.
/// @param[out] dst Recovered polynomial vector.
/// @param[in] src Input byte array of length `C1_LEN_BYTES`.
void polyvec_byte_decompress(polarlac_polyvec *dst, const uint8_t *src);


/// @brief Losslessly encode a canonical polynomial using fixed zero-selector layout.
/// @param[out] code_c Output byte array of length `PK_POLY_BYTES`.
/// @param[in] c Canonical input coefficients in `{0, ..., 256}` of length `RL_KEM_N`.
/// @return 0 on success, -1 if the fixed selector capacity is exceeded.
int poly_compress(uint8_t *code_c, int16_t *c);

/// @brief Decode a canonical polynomial from the fixed zero-selector layout.
/// @param[out] c Recovered coefficients in `{0, ..., 256}` of length `RL_KEM_N`.
/// @param[in] code_c Input byte array of length `PK_POLY_BYTES`.
void poly_decompress(int16_t *c, const uint8_t *code_c);

/// @brief Losslessly encode a length-K polynomial vector as K independently compressed polynomials.
/// @param[out] code_c Output byte array of length `PK_LEN_BYTES`.
/// @param[in] c Canonical input polynomial vector.
/// @return 0 on success, -1 if the fixed selector capacity is exceeded.
...
/// @param[out] c Recovered polynomial vector.
/// @param[in] code_c Input byte array of length `PK_LEN_BYTES`.
