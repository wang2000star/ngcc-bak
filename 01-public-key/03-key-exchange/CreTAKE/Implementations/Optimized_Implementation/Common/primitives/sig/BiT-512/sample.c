...
/* 14-bit triangular: buf = [lo_half(2048B) ‖ hi_half(2048B)].
   4 bytes from lo, 4 from hi → 2 signed int32_t coefficients. */
static void poly_unpack_triangular_14bit(int32_t *RESTRICT out, const uint8_t *buf)
{
    for (unsigned int i = 0; i < BIT_N / 2; i++) {
...
{
    uint8_t ALIGNED_32 buf0[Y_BUF_SZ], buf1[Y_BUF_SZ];
    uint8_t ALIGNED_32 buf2[Y_BUF_SZ], buf3[Y_BUF_SZ];
...
void polyvecy_sample_y2_avx(polyvecy *y, const uint8_t *seed_y, uint16_t *nonce)
{
    for (int i = 0; i < BIT_K; i++)
        poly_sample_triangular_15bit(y->vec[4 + i].coeffs, seed_y, nonce);
}
...
void poly_sample_triangular_15bit(int32_t *out, const uint8_t *seed, uint16_t *nonce)
{
    ...
    *nonce = (uint16_t)(*nonce + 1U);
    for (int i = 0; i < BIT_N / 2; i++) {
        ...
        out[2*i+0] = (int32_t)(lo0 & 0x7FFFU) - (int32_t)(hi0 & 0x7FFFU);
        out[2*i+1] = (int32_t)(lo1 & 0x7FFFU) - (int32_t)(hi1 & 0x7FFFU);
    }
}
