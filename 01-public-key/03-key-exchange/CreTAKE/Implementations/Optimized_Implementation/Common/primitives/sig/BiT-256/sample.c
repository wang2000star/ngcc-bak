...
/* 12-bit triangular: buf = [lo_half(768B) ‖ hi_half(768B)].
   ...
   SAMPLING_*_PAD) so the over-read stays in-bounds; surplus bits are masked off. */
static const int8_t unpack12_shuf[32] __attribute__((aligned(32))) = {
    0,1,2,3, 1,2,3,4, 3,4,5,6, 4,5,6,7,
    6,7,8,9, 7,8,9,10, 9,10,11,12, 10,11,12,13
};
static void poly_unpack_triangular_12bit(int32_t *RESTRICT out, const uint8_t *buf)
{
...
int check_reject_highbits_w1_sparse(const poly *w1, const poly *w0,
...
/* 13-bit triangular (y2): the scalar layout's per-coeff uint32 windows sit at
   ...
   to the scalar.  16-byte loads over-read up to 3 bytes; callers pad the buffer. */
static const int8_t unpack13_shuf[32] __attribute__((aligned(32))) = {
    0,1,2,3, 2,3,4,5, 4,5,6,7, 5,6,7,8,
    7,8,9,10, 8,9,10,11, 9,10,11,12, 11,12,13,14
};
static void poly_unpack_triangular_13bit(int32_t *out, const uint8_t *buf)
{
