...
int kex_generate_pass1_msg_a(unsigned char *ska,unsigned long long ska_len_bytes,unsigned char *pkb,unsigned long long pkb_len_bytes,unsigned char *sta,unsigned long long *sta_len_bytes,unsigned char *m1,unsigned long long *m1_len_bytes){
	(void)ska;(void)ska_len_bytes;(void)pkb_len_bytes;(void)sta_len_bytes;
	ALIGN32 unsigned char seed_kg[SEED_BYTES];
	ALIGN32 unsigned char *tpk = m1;
	ALIGN32 unsigned char *ct_j = m1 + TPK_LEN;
	ALIGN32 unsigned char buf[SEED_BYTES + SKI_LEN], k_j[KEM_K_LEN];
	unsigned long long tpk_l=0, tsk_l=0, ss_l=0, ct_l=0;

	ALIGN32 unsigned char atpk[PAD32(TPK_LEN)] = {0};
	ALIGN32 unsigned char atsk[PAD32(TSK_LEN)] = {0};
	ALIGN32 unsigned char act_j[PAD32(C_R_LEN)] = {0};
	...
	ret = twokem_keygen2_withseed(seed_kg, SEED_BYTES, atpk, &tpk_l, atsk, &tsk_l);
    if (ret != 0) {
        return -2;
    }
	memcpy(tpk, atpk, tpk_l);

	ret = kem_enc(pkb, PKR_LEN, k_j, &ss_l, act_j, &ct_l);
	if (ret != 0) {
        return -3;
    }
	memcpy(ct_j, act_j, C_R_LEN);
...
int kex_generate_pass2_msg_b(unsigned char *skb,unsigned long long skb_len_bytes,unsigned char *pka,unsigned long long pka_len_bytes,unsigned char *m1,unsigned long long m1_len_bytes,unsigned char *stb,unsigned long long *stb_len_bytes,unsigned char *m2,unsigned long long *m2_len_bytes){
	...
	ALIGN32 unsigned char ct_j[C_R_LEN], k_j[KEM_K_LEN];
	ALIGN32 unsigned char *ct_i = m2;
	ALIGN32 unsigned char *pkb = (unsigned char *)calloc(PKR_LEN, sizeof(unsigned char));
	ALIGN32 unsigned char *tpk = (unsigned char *)calloc(TPK_LEN, sizeof(unsigned char));
	ALIGN32 unsigned char k_i[KEM_K_LEN];
	(void)m1_len_bytes;
...
int kex_derive_ss_a(unsigned char *ska,unsigned long long ska_len_bytes,unsigned char *pkb,unsigned long long pkb_len_bytes,unsigned char *mb,unsigned long long mb_len_bytes,unsigned char *sta,unsigned long long sta_len_bytes,unsigned char *ssa,unsigned long long *ssa_len_bytes){
	...
	ALIGN32 unsigned char k_i[KEM_K_LEN];
	ALIGN32 const unsigned char *buf = sta;
	ALIGN32 const unsigned char *k_j = sta + SEED_BYTES;
	ALIGN32 const unsigned char *ct_j = sta + SEED_BYTES + KEM_K_LEN;
	ALIGN32 unsigned char pkI[PKI_LEN];
	ALIGN32 unsigned char buf2[SEED_BYTES + SKI_LEN], tpk[TPK_LEN], tsk[TSK_LEN], seed_kg[SEED_BYTES];
	...
	if (twokem_dec(ska, ska_len_bytes, tsk, tsk_l, ct_i, C_I_LEN, k_i, &k_i_l) != 0) {
        return -2;
    }
