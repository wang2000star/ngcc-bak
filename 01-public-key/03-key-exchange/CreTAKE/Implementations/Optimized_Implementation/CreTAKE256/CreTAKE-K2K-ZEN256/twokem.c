    ...
    unsigned char *ct, unsigned long long *ct_len_bytes)
{
    ...
    ALIGN32 unsigned char m[MSG_LEN_BYTES];
    ALIGN32 unsigned char seed_enc[SEED_BYTES];
    ALIGN32 unsigned char *c1 = ct;
    ALIGN32 unsigned char *c2 = ct + ZEN_CIPHERTEXT_LEN_BYTES;
    unsigned long long c1_len_bytes=0, ss1_len_bytes=0;
    ALIGN32 unsigned char ss1[KEM_K_LEN];
    ALIGN32 unsigned char buf[SEED_BYTES], buf2[MSG_LEN_BYTES + SEED_BYTES];
    ALIGN32 unsigned char buf3[PKI_LEN + KEM_K_LEN + MSG_LEN_BYTES];
	...
	unsigned char *ss, unsigned long long *ss_len_bytes)
{
    ...
    ALIGN32 unsigned char *c1 = ct;
    ALIGN32 unsigned char *c2 = ct + ZEN_CIPHERTEXT_LEN_BYTES;
    ALIGN32 unsigned long long ss1_len_bytes=0;
    ALIGN32 unsigned char m[MSG_LEN_BYTES];
    ALIGN32 const unsigned char *pk1 = sk1 + TSK_LEN;
    ALIGN32 unsigned char buf3[PKI_LEN + KEM_K_LEN + MSG_LEN_BYTES];
