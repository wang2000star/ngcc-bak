...
/* AVX2 fork: only poly_sqnorm is vectorized (the squared-norm gate that
 ...
 * mirror of the scalar centering + square + int64 accumulate, so the avx2
 * KAT stays byte-exact to ref. */
...
int64_t poly_sqnorm(const poly *a)
{
    ...
    /* 8 coeffs / iteration: reduce32 -> center mod q (simd_bmodpm_q) ->
     * square -> accumulate in int64.  The centered |r| <= (q-1)/2 < 2^15,
     * so r*r < 2^30; the two int64x4 lane accumulators (even/odd 32-bit
     * lanes) cannot overflow over N <= 1024 terms (N*(q/2)^2 << 2^63).
     * _mm256_mul_epi32 squares the even 32-bit lanes into int64; the odd
     * lanes are shifted down 32 and squared the same way. */
    __m256i acc_e = _mm256_setzero_si256();
    __m256i acc_o = _mm256_setzero_si256();
    unsigned k;
    for (k = 0; k < N; k += 8) {
        __m256i c = _mm256_loadu_si256((const __m256i *)&a->coeffs[k]);
        __m256i r = simd_bmodpm_q(c); /* centered (-q/2, q/2]          */
        __m256i pe = _mm256_mul_epi32(r, r); /* even lanes r^2 -> int64 */
        __m256i ro = _mm256_srli_epi64(r, 32); /* odd lanes -> even slot */
        __m256i po = _mm256_mul_epi32(ro, ro); /* odd lanes r^2 -> int64 */
        acc_e = _mm256_add_epi64(acc_e, pe);
        acc_o = _mm256_add_epi64(acc_o, po);
    }
    return simd_hsum_epi64(_mm256_add_epi64(acc_e, acc_o));
#else
    int64_t acc = 0;
    unsigned i;
    for (i = 0; i < N; ++i) {
