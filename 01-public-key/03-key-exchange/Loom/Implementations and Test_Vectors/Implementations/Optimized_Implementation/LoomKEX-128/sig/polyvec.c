...
/* At most XOF_STREAMS/XOF_LANES_AVX2 N-way passes cover the 16 logical
 * streams (2 for 8-way SM3, 4 for 4-way SHAKE).  XOF_STREAMS is a multiple
 * of XOF_LANES_AVX2 for both MODEs (16 % 8 == 0, 16 % 4 == 0), so the
 * passes tile the streams exactly with no partial trailing pass; a caller
 * that needs only nfill streams drives ceil(nfill / lanes) of them. */
_Static_assert(
 ...
 * all 16 streams in one pass this is always a single pass; the win is in
 * the SHAKE path, where a caller needing <= lanes streams skips the
 * trailing passes entirely.
 *
                            ...
                            int use_xof128, unsigned nfill)
{
    const unsigned passes =
        (nfill + (unsigned)XOF_LANES_AVX2 - 1) / (unsigned)XOF_LANES_AVX2;
    unsigned pass, k;
    for (pass = 0; pass < passes; pass++) {
        const uint8_t *seedp[XOF_LANES_AVX2];
        uint8_t *outp[XOF_LANES_AVX2];
        xof_ctx_avx2 ctx;
        for (k = 0; k < (unsigned)XOF_LANES_AVX2; k++) {
            unsigned s = pass * (unsigned)XOF_LANES_AVX2 + k;
            seedp[k] = nonces[s];
            outp[k] = dst[s];
        }
        if (use_xof128) {
            xof128_avx2_init(&ctx, seedp, nonce_len);
            xof128_avx2_squeeze(&ctx, outp, block_len);
        } else {
            xof256_avx2_init(&ctx, seedp, nonce_len);
            xof256_avx2_squeeze(&ctx, outp, block_len);
        }
    }
}
              ...
              const uint8_t seedA[SEEDBYTES])
{
     ...
     * (UNIFORM_DRAW bytes) of all XOF_STREAMS lanes N-at-a-time (8-way SM3
     * / 4-way SHAKE), byte-exact to 16 sequential scalar us_fill() slices
     * (lane-equivalence).  ExpandA is the headline lever: this replaces 16
     * sequential xof128 squeezes -- the dominant cost of Verify (~83%) and
     * a big chunk of Sign -- with XOF_STREAMS/XOF_LANES_AVX2 batched
     * squeezes, now right-sized so the wasted tail is never squeezed. Then
     * the (already vectorized) per-lane uniform_reject_chunk runs on each
     * pre-filled buffer, drawing any further slice on the scalar path.
     * UNIFORM_DRAW is a whole multiple of the per-MODE squeeze
     * granularity, so the N-way squeeze is rate-aligned and byte-exact. */
    {
 ...
 * lane buffer, appending accepted coeffs to dst[*cnt..want).  PURE buffer
 * operation -- the caller MUST have ensured >= NOISE_MINIBATCH_RAND_BYTES
 * are resident at gs->buf+gs->pos.  The WHOLE NOISE_MINIBATCH_RAND_BYTES
 * is consumed up front (cursor advances independent of the *cnt==want
 * early break).  This is the EXACT scalar body shared by the scalar (A)
 * and N-way round-robin (B) ExpandS paths so they cannot drift. */
                                 ...
                                 const uint8_t *signs)
{
     ...
     * in SIMD groups and BREAK the moment `count` accepts have landed,
     * eliding the approx_exp / finalize for the unused tail of the
     * GAUSS_BATCH window.  Both paths leave any candidate past `count`
     * DISCARDED (the dst write was already `*coefcnt < count`-gated), and
     * the byte cursor always advances the WHOLE MINIBATCH_RAND_BYTES, so
     * the per-lane byte schedule is byte-identical to processing all 32.
     * The break is on the PUBLIC accept-count (same data-dependence as the
     * mini-batch count). */
    ...
    !defined(GAUSS_FINALIZE_SCALAR)
         ...
         * expect to need (remaining accepts rounded up to the 4-candidate
        ...
        int win = (int)((remain + 3u) & ~3u); /* round up to SIMD group */
        int base = 0;
        if (win > GAUSS_BATCH)
            win = GAUSS_BATCH;
