...
int main()
{
	...
	FILE *file_output;
	unsigned char *seed, *pka, *ska, *pkb, *skb, *sta, *stb, *ssa, *ssb, *m1, *m2, *m3, *m4, *ma, *mb;
	unsigned long long pass, pka_len_bytes, ska_len_bytes, pkb_len_bytes,
		...
		m1_len_bytes, m2_len_bytes, m3_len_bytes, m4_len_bytes, ma_len_bytes, mb_len_bytes, total_len_bytes;
	int rtn;
	...
	m1 = (unsigned char *)calloc(CRYPTO_LOOM_MSG1BYTES, sizeof(unsigned char));
	m2 = (unsigned char *)calloc(CRYPTO_LOOM_MSG2BYTES, sizeof(unsigned char));
	m3 = (unsigned char *)calloc(CRYPTO_LOOM_MSG3BYTES, sizeof(unsigned char));
	m4 = (unsigned char *)calloc(CRYPTO_LOOM_MSG4BYTES, sizeof(unsigned char));
	ma = NULL;
	mb = NULL;
	...
	for (int i = 0; i < 10; i++)
	{
		...
		for (;;)
		{
			...
			/***   generate the fourth-pass message sent by the responder  ***/
			...
			if (rtn < KAT_KEX_SUCCESS)
			{
				fprintf(stderr, "ERROR: kex_generate_pass4_msg_b returned %d at %s, line %d. \n", rtn, __FILE__, __LINE__);
				return KAT_KEX_CRYPTO_FAILURE;
			}
			// print responder state information
			fprintlen(file_output, "Pass4_Stb_Len = ", stb_len_bytes);
			fprintstr(file_output, "Pass4_Stb = ", stb, stb_len_bytes);
			// print message sent by responder
			fprintlen(file_output, "M4_Len = ", m4_len_bytes);
			fprintstr(file_output, "M4 = ", m4, m4_len_bytes);
			mb = m4;
			mb_len_bytes = m4_len_bytes;
			if (rtn == 1)
			{
				if (pass != 4)
				{
		...
		fprintf(file_output, "\n");
		loom_secure_zero(sta, (size_t)sta_len_bytes);
		loom_secure_zero(stb, (size_t)stb_len_bytes);
	}
	printf("[Debug**] all loop SUCCESS!\n");
	free(seed);
	free(m1);
	free(m2);
	free(m3);
