# ADKEX（PKCKEX-142803）自评估材料包 — 密钥交换（KEX，2 趟）

依据《新一代商用密码算法 x86 架构 / ARM 架构实现自评估指引》（商用密码标准研究院，2026 年 6 月）整理。
本包为 ADKEX 算法的**独立自评估材料**，与 §3.1(1) 电子提交包（will-submit）**平行**提交，不并入电子包。
...
# ADKEX (PKCKEX-142803) Self-Assessment Materials — Key Exchange (KEX, 2-pass)
...
This package is the **standalone self-assessment material** for ADKEX, submitted **in parallel** with the §3.1(1) electronic submission package (`will-submit`); it is not merged into the electronic package.
