#!/usr/bin/env bash
# ADKEX-128 on ARM Cortex-M4. DKE core = Plantard arithmetic + matacc; ADKEX = 2-pass
...
DEFS="-DADKEX_MODE=128 -DDKE_MODE=128 -DDKE_HASH=0 -DDKE_RANDOM=0 -DDKE_USE_CORTEX_M4_PLANTARD -DDKE_USE_MATACC"
...
$GCC $ARCH $DEFS $INC $CORE qemu/qemu_kex.c qemu/syscalls.c $ASM    -T qemu/netduino.ld -Wl,--gc-sections    -Wl,--start-group -lgcc -lnosys -lc -Wl,--end-group -o kat_ADKEX-128.elf
echo "Built kat_ADKEX-128.elf — running ADKEX 2-pass handshake self-test on QEMU:"
$QEMU -M netduinoplus2 -nographic -semihosting-config enable=on,target=native -kernel kat_ADKEX-128.elf
