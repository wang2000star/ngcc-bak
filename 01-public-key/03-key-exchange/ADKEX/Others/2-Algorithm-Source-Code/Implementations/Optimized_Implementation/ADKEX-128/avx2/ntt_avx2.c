...
void DKE_ntt(int16_t r[DKE_N]) {
    ...
    /* Vendored ML-KEM AVX2 forward NTT: leaves coeffs in ntt(packed) order, UNREDUCED
     * (like ML-KEM's own poly_ntt). The single reduce comes from the DKE_poly_reduce
    ...
    /* Packed-domain (bitreversed) ML-KEM-style forward NTT for n=512, q=7681.
...
#if DKE_N == 256
    {
        ...
        /* ML-KEM-style NTT, outputs packed format */
...
#endif
#if defined(DKE_NTT512_PACKED) && DKE_N == 512
    /* Packed-domain ML-KEM-style inverse NTT for n=512, q=7681. Input in the
...
#if DKE_N == 256 && defined(_MSC_VER)
    {
        /* ML-KEM-style fused INVNTT: operates directly on packed NTT format,
