...
// SM3 mode: 4x/8x parallel noise sampling via sm3x8 --------------------------

#if defined(DKE_USE_AVX2) && DKE_HASH == 0
#include "avx2/sm3x8_avx2.h"
#elif defined(DKE_USE_AARCH64) && DKE_HASH == 0
#include "aarch64/sm3x4_neon.h"
#endif

#if (defined(DKE_USE_AVX2) || defined(DKE_USE_AARCH64)) && DKE_HASH == 0

/*
 * SM3 noise XOF for one poly: coins = SM3(seed||nonce||1) || SM3(seed||nonce||2) || ...
 * Each SM3 call produces 32 bytes. Need DKE_CBD_BYTES total.
 * num_sm3_blocks = ceil(DKE_CBD_BYTES / 32).
 *
 * DKE-128/256 (N=256, ETA=2): CBD_BYTES=128, blocks=4
 * DKE-256     (N=256, ETA=3): CBD_BYTES=192, blocks=6
 * DKE-512     (N=512, ETA=3): CBD_BYTES=384, blocks=12
 */
#define DKE_NOISE_SM3_BLOCKS ((DKE_CBD_BYTES + 31) / 32)

/*
 * 4x parallel noise sampling using sm3x8.
 * 4 polys × multiple SM3 blocks each.
 * Strategy: pack 8 SM3 calls per sm3x8 invocation.
 *   - If blocks_per_poly is even: 4 polys × 2 counters = 8 lanes per call
 *   - If blocks_per_poly is odd: handle last block with sm3x4
 */
void DKE_getnoise_sm3x8_4x(poly *r0, poly *r1, poly *r2, poly *r3,
                             const uint8_t seed[DKE_SEEDBYTES],
                             uint8_t nonce0, uint8_t nonce1,
                             uint8_t nonce2, uint8_t nonce3) {
    uint8_t coins[4][DKE_CBD_BYTES + 32]; /* +32 for alignment slack */
    uint8_t nonces[4];
    unsigned int ct, k;

    nonces[0] = nonce0; nonces[1] = nonce1;
    nonces[2] = nonce2; nonces[3] = nonce3;

#if DKE_SEEDBYTES == 64
    /* Midstate path: seed is exactly one SM3 block. Compute the midstate
     * once, then hash only the 5-byte suffix nonce||counter per output
     * block — 1 compression instead of 2. Output is identical to hashing
     * seed||nonce||counter in full. */
    {
        sm3_midstate mid;
        uint8_t suf8[8][5];
        sm3_compute_midstate(&mid, seed);
        ...
        for (ct = 0; ct + 2 <= DKE_NOISE_SM3_BLOCKS; ct += 2) {
            unsigned int ctA = ct + 1;
            unsigned int ctB = ct + 2;
            for (k = 0; k < 4; k++) {
                suf8[k][1] = (uint8_t)(ctA >> 24);
                suf8[k][2] = (uint8_t)((ctA >> 16) & 0xff);
                suf8[k][3] = (uint8_t)((ctA >> 8) & 0xff);
                suf8[k][4] = (uint8_t)(ctA & 0xff);
                suf8[k+4][1] = (uint8_t)(ctB >> 24);
                suf8[k+4][2] = (uint8_t)((ctB >> 16) & 0xff);
                suf8[k+4][3] = (uint8_t)((ctB >> 8) & 0xff);
                suf8[k+4][4] = (uint8_t)(ctB & 0xff);
            }
            sm3x8_hash_suffix(
                coins[0] + ct * 32,       coins[1] + ct * 32,
                coins[2] + ct * 32,       coins[3] + ct * 32,
                coins[0] + (ct+1) * 32,   coins[1] + (ct+1) * 32,
                coins[2] + (ct+1) * 32,   coins[3] + (ct+1) * 32,
                &mid,
                suf8[0], suf8[1], suf8[2], suf8[3],
                suf8[4], suf8[5], suf8[6], suf8[7],
                5);
        }

        if (ct < DKE_NOISE_SM3_BLOCKS) {
            unsigned int counter = ct + 1;
            for (k = 0; k < 4; k++) {
                suf8[k][1] = (uint8_t)(counter >> 24);
                suf8[k][2] = (uint8_t)((counter >> 16) & 0xff);
                suf8[k][3] = (uint8_t)((counter >> 8) & 0xff);
                suf8[k][4] = (uint8_t)(counter & 0xff);
            }
            sm3x4_hash_suffix(
                coins[0] + ct * 32, coins[1] + ct * 32,
                coins[2] + ct * 32, coins[3] + ct * 32,
        ...
        centered_binomial(r0, coins[0]);
        centered_binomial(r1, coins[1]);
        centered_binomial(r2, coins[2]);
        centered_binomial(r3, coins[3]);
        return;
    }
#else
    {
    uint8_t msg8[8][DKE_SEEDBYTES + 1 + 4];
    const size_t msg_len = DKE_SEEDBYTES + 1;

    /* Pre-fill fixed part: seed || nonce for each of 4 polys, duplicated for 2 counters */
    for (k = 0; k < 4; k++) {
        memcpy(msg8[k], seed, DKE_SEEDBYTES);
        msg8[k][DKE_SEEDBYTES] = nonces[k];
        memcpy(msg8[k + 4], seed, DKE_SEEDBYTES);
        msg8[k + 4][DKE_SEEDBYTES] = nonces[k];
    }

    /* Process 2 counters × 4 polys = 8 SM3 hashes per sm3x8 call */
    for (ct = 0; ct + 2 <= DKE_NOISE_SM3_BLOCKS; ct += 2) {
        unsigned int ctA = ct + 1;
        unsigned int ctB = ct + 2;
        for (k = 0; k < 4; k++) {
            msg8[k][msg_len + 0] = (uint8_t)(ctA >> 24);
            msg8[k][msg_len + 1] = (uint8_t)((ctA >> 16) & 0xff);
            msg8[k][msg_len + 2] = (uint8_t)((ctA >> 8) & 0xff);
            msg8[k][msg_len + 3] = (uint8_t)(ctA & 0xff);
            msg8[k+4][msg_len + 0] = (uint8_t)(ctB >> 24);
            msg8[k+4][msg_len + 1] = (uint8_t)((ctB >> 16) & 0xff);
            msg8[k+4][msg_len + 2] = (uint8_t)((ctB >> 8) & 0xff);
            msg8[k+4][msg_len + 3] = (uint8_t)(ctB & 0xff);
        }
        sm3x8_hash_equal(
            coins[0] + ct * 32,       coins[1] + ct * 32,
            coins[2] + ct * 32,       coins[3] + ct * 32,
            coins[0] + (ct+1) * 32,   coins[1] + (ct+1) * 32,
            coins[2] + (ct+1) * 32,   coins[3] + (ct+1) * 32,
            msg8[0], msg8[1], msg8[2], msg8[3],
            msg8[4], msg8[5], msg8[6], msg8[7],
            msg_len + 4);
    }

    /* Handle odd remaining block with sm3x4 */
    if (ct < DKE_NOISE_SM3_BLOCKS) {
        unsigned int counter = ct + 1;
        uint8_t msg4[4][DKE_SEEDBYTES + 1 + 4];
        for (k = 0; k < 4; k++) {
            memcpy(msg4[k], seed, DKE_SEEDBYTES);
            msg4[k][DKE_SEEDBYTES] = nonces[k];
            msg4[k][msg_len + 0] = (uint8_t)(counter >> 24);
            msg4[k][msg_len + 1] = (uint8_t)((counter >> 16) & 0xff);
            msg4[k][msg_len + 2] = (uint8_t)((counter >> 8) & 0xff);
            msg4[k][msg_len + 3] = (uint8_t)(counter & 0xff);
        }
        sm3x4_hash_equal(
            coins[0] + ct * 32, coins[1] + ct * 32,
            coins[2] + ct * 32, coins[3] + ct * 32,
            msg4[0], msg4[1], msg4[2], msg4[3],
            msg_len + 4);
    }
...
#endif /* DKE_SEEDBYTES == 64 */
}

/*
 * 8x parallel noise sampling using sm3x8.
 * 8 polys × multiple SM3 blocks each.
 * Strategy: for each counter value, process 8 polys in one sm3x8 call.
 */
void DKE_getnoise_sm3x8_8x(poly *r0, poly *r1, poly *r2, poly *r3,
    ...
    uint8_t coins[8][DKE_CBD_BYTES + 32];
    uint8_t nonces[8];
    unsigned int ct, k;

    nonces[0] = nonce0; nonces[1] = nonce1;
    nonces[2] = nonce2; nonces[3] = nonce3;
    nonces[4] = nonce4; nonces[5] = nonce5;
    nonces[6] = nonce6; nonces[7] = nonce7;

#if DKE_SEEDBYTES == 64
    /* Midstate path: seed is exactly one SM3 block — hash only the 5-byte
     * nonce||counter suffix per output block (see DKE_getnoise_sm3x8_4x). */
    {
        sm3_midstate mid;
        uint8_t suf8[8][5];
        sm3_compute_midstate(&mid, seed);
        ...
        for (ct = 0; ct < DKE_NOISE_SM3_BLOCKS; ct++) {
            unsigned int counter = ct + 1;
            for (k = 0; k < 8; k++) {
                suf8[k][1] = (uint8_t)(counter >> 24);
                suf8[k][2] = (uint8_t)((counter >> 16) & 0xff);
            ...
            sm3x8_hash_suffix(
                coins[0] + ct * 32, coins[1] + ct * 32,
                coins[2] + ct * 32, coins[3] + ct * 32,
                coins[4] + ct * 32, coins[5] + ct * 32,
                coins[6] + ct * 32, coins[7] + ct * 32,
                &mid,
                suf8[0], suf8[1], suf8[2], suf8[3],
                suf8[4], suf8[5], suf8[6], suf8[7],
                5);
    ...
    /* Pre-fill fixed part: seed || nonce for each of 8 polys */
    for (k = 0; k < 8; k++) {
        memcpy(msg8[k], seed, DKE_SEEDBYTES);
        msg8[k][DKE_SEEDBYTES] = nonces[k];
    }

    /* For each counter, process all 8 polys in one sm3x8 call */
    for (ct = 0; ct < DKE_NOISE_SM3_BLOCKS; ct++) {
        unsigned int counter = ct + 1;
        for (k = 0; k < 8; k++) {
            msg8[k][msg_len + 0] = (uint8_t)(counter >> 24);
            msg8[k][msg_len + 1] = (uint8_t)((counter >> 16) & 0xff);
            msg8[k][msg_len + 2] = (uint8_t)((counter >> 8) & 0xff);
            msg8[k][msg_len + 3] = (uint8_t)(counter & 0xff);
        }
        sm3x8_hash_equal(
            coins[0] + ct * 32, coins[1] + ct * 32,
            coins[2] + ct * 32, coins[3] + ct * 32,
    ...
    centered_binomial(r2, coins[2]);
    centered_binomial(r3, coins[3]);
    centered_binomial(r4, coins[4]);
    centered_binomial(r5, coins[5]);
    centered_binomial(r6, coins[6]);
    centered_binomial(r7, coins[7]);
}

#undef DKE_NOISE_SM3_BLOCKS
#endif /* (DKE_USE_AVX2 || DKE_USE_AARCH64) && DKE_HASH == 0 */
...
/*
 * Incremental SHAKE128 XOF state for gen_matrix (HASH=1 or HASH=2).
...
/* ── Exported XOF squeeze for the Plantard matacc ASM ──────────────────
 * plantard_matacc_asm.S / matacc.i refill their sample buffer by calling
 *   dke1_xof_squeezeblocks(out, nblocks=1, state)   // 256-ring (DKE-128/256)
 *   dke3_xof_squeezeblocks(out, nblocks=1, state)   // 512-ring (DKE-512)
 * which is exactly the (out, nblocks, state) ABI of the static
 * dke_xof_squeezeblocks above. Export a thin wrapper so the matacc objects
 * resolve. state is opaque to the ASM (passed straight through). */
/* Provide this for ANY Cortex-M4 backend, not just Plantard: the combined
 * dke_asm.o now contains all 3 backends' asm (incl. the Plantard matacc that
 * references dkeN_xof_squeezeblocks), so the symbol must resolve regardless of
 * which backend the C side selected. */
#if defined(DKE_USE_CORTEX_M4_PLANTARD) || defined(DKE_USE_CORTEX_M4_BARRETT) || defined(DKE_USE_CORTEX_M4)
#if DKE_MODE == 512
void dke3_xof_squeezeblocks(uint8_t *out, size_t nblocks, dke_xof_state *state)
#else
void dke1_xof_squeezeblocks(uint8_t *out, size_t nblocks, dke_xof_state *state)
#endif
{
    dke_xof_squeezeblocks(out, nblocks, state);
}
#endif /* any Cortex-M4 backend */

/* ── Fused matrix-generate + multiply-accumulate (matacc) ─────────────────────
 * Opt-in (DKE_USE_MATACC), Plantard-only, 256-ring (DKE-128/256). Ports the
 * reference dkem-sjg1006 matacc.c driver: on-the-fly generates a row of A (or
 * A^T) from `seed` via the SM3 XOF and multiply-accumulates it with `b`, caching
 * b*zeta into b_prime to skip re-twiddling on later rows. The matacc asm refills
 * its 168-byte block by calling dke1_xof_squeezeblocks (defined above); `state`
 * is opaque to the asm. KAT correctness MUST be checked on QEMU/hardware. */
#if defined(DKE_USE_MATACC) && defined(DKE_USE_CORTEX_M4_PLANTARD) && (DKE_MODE != 512)
extern const int32_t zetas[];

/* matacc asm (plantard_matacc_asm.S). FP regs s16-s21,s26-s29 clobbered. */
extern void matacc_asm_cache_16_32(int32_t *r_tmp, const int16_t *b, int16_t c[4], unsigned char *buf, const int32_t zetas[64], dke_xof_state *state, int16_t *aprimeptr);
extern void matacc_asm_cache_32_32(int32_t *r_tmp, const int16_t *b, int16_t c[4], unsigned char *buf, const int32_t zetas[64], dke_xof_state *state, int16_t *aprimeptr);
extern void matacc_asm_cache_32_16(int16_t *r, const int16_t *b, int16_t c[4], unsigned char *buf, const int32_t zetas[64], dke_xof_state *state, int16_t *aprimeptr, const int32_t *r_tmp);
extern void matacc_asm_opt_16_32(int32_t *r_tmp, const int16_t *b, int16_t c[4], unsigned char *buf, dke_xof_state *state, const int16_t *aprimeptr);
extern void matacc_asm_opt_32_32(int32_t *r_tmp, const int16_t *b, int16_t c[4], unsigned char *buf, dke_xof_state *state, const int16_t *aprimeptr);
extern void matacc_asm_opt_32_16(int16_t *r, const int16_t *b, int16_t c[4], unsigned char *buf, dke_xof_state *state, const int16_t *aprimeptr, const int32_t *r_tmp);

#define MATACC_CLOBBER() __asm__ volatile("" : : : "s16","s17","s18","s19","s20","s21","s26","s27","s28","s29")

static inline void DKE_xof_absorb_ij(dke_xof_state *st, const uint8_t *seed,
                                     unsigned i, unsigned j, int transposed) {
    if (transposed) dke_xof_absorb(st, seed, (uint8_t)i, (uint8_t)j);
    else            dke_xof_absorb(st, seed, (uint8_t)j, (uint8_t)i);
}

void DKE_matacc_cache32(poly *r, const polyvec *b, polyvec *b_prime,
                        unsigned char i, const unsigned char *seed, int transposed) {
    unsigned char buf[DKE_XOF_BLOCKBYTES + 2];
    dke_xof_state state;
    int16_t c[4];
    int32_t r_tmp[DKE_N];
    int j = 0;

    DKE_xof_absorb_ij(&state, seed, i, j, transposed);
...
#endif /* DKE_USE_MATACC && Plantard && MODE!=512 */
...
#if (defined(DKE_USE_AVX2) || defined(DKE_USE_AARCH64)) && DKE_HASH == 0
/* SM3 parallel matrix generation using sm3x8/sm3x4.
 * Generates 4 matrix elements simultaneously by running 4 SM3 XOF streams
 * in parallel. Uses sm3x8_hash_equal to compute 2 counters × 4 streams = 8
 * SM3 hashes per call (AVX2) or 2×4 (NEON).
 *
 * DKE-512 optimization: SEEDBYTES=64 = exactly 1 SM3 block. Precompute
 * midstate once, then use sm3x8_hash_suffix for the 6-byte suffix
 * (x||y||counter), saving ~50% of SM3 compression calls. */
#if defined(DKE_USE_AVX2)
#include "avx2/sm3x8_avx2.h"
#elif defined(DKE_USE_AARCH64)
#include "aarch64/sm3x4_neon.h"
...
#if DKE_SEEDBYTES == 64
    /* Precompute midstate: SM3(seed[0:64]) intermediate state.
     * All matrix elements share the same seed, so this is done once. */
    sm3_midstate mid;
    sm3_compute_midstate(&mid, seed);
#endif

    /* Process 4 matrix elements at a time */
    while (done + 4 <= total) {
        unsigned int idx_i[4], idx_j[4];
        uint8_t bufs[4][DKE_GEN_MATRIX_NBLOCKS * DKE_XOF_BLOCKBYTES + 32];
        unsigned int ctr[4];
        unsigned int k;
        ...
        for (k = 0; k < 4; k++) {
            ...
            idx_j[k] = flat % DKE_K;
            if (transposed) {
                xy[k][0] = (uint8_t)idx_i[k];
                xy[k][1] = (uint8_t)idx_j[k];
            } else {
                xy[k][0] = (uint8_t)idx_j[k];
                xy[k][1] = (uint8_t)idx_i[k];
            }
        }

        {
            unsigned int needed_bytes = DKE_GEN_MATRIX_NBLOCKS * DKE_XOF_BLOCKBYTES;
            unsigned int num_sm3_blocks = (needed_bytes + 31) / 32;
            unsigned int ct;

#if DKE_SEEDBYTES == 64
            /* Midstate path: suffix = x(1B) || y(1B) || counter(4B) = 6 bytes.
             * 6 < 56, so padding always fits in one block → 1 compress per sm3x8 call. */
            uint8_t suf8[8][6]; /* 4 streams × 2 counters */

            /* Pre-fill x,y for each lane */
            for (k = 0; k < 4; k++) {
                suf8[k][0] = xy[k][0];
                suf8[k][1] = xy[k][1];
                suf8[k + 4][0] = xy[k][0];
            ...
            for (ct = 0; ct + 2 <= num_sm3_blocks; ct += 2) {
                unsigned int ctA = ct + 1;
                unsigned int ctB = ct + 2;
                for (k = 0; k < 4; k++) {
                    suf8[k][2] = (uint8_t)(ctA >> 24);
                    suf8[k][3] = (uint8_t)((ctA >> 16) & 0xff);
                    suf8[k][4] = (uint8_t)((ctA >> 8) & 0xff);
                    suf8[k][5] = (uint8_t)(ctA & 0xff);
                    suf8[k+4][2] = (uint8_t)(ctB >> 24);
                    suf8[k+4][3] = (uint8_t)((ctB >> 16) & 0xff);
                    suf8[k+4][4] = (uint8_t)((ctB >> 8) & 0xff);
                    suf8[k+4][5] = (uint8_t)(ctB & 0xff);
                }
                sm3x8_hash_suffix(
                    bufs[0] + ct * 32,       bufs[1] + ct * 32,
                    bufs[2] + ct * 32,       bufs[3] + ct * 32,
                    bufs[0] + (ct+1) * 32,   bufs[1] + (ct+1) * 32,
                    bufs[2] + (ct+1) * 32,   bufs[3] + (ct+1) * 32,
                    &mid,
                    suf8[0], suf8[1], suf8[2], suf8[3],
                    suf8[4], suf8[5], suf8[6], suf8[7],
                    6);
            ...
            if (ct < num_sm3_blocks) {
                unsigned int counter = ct + 1;
                uint8_t suf4[4][6];
                for (k = 0; k < 4; k++) {
                    suf4[k][0] = xy[k][0];
                    suf4[k][1] = xy[k][1];
                    suf4[k][2] = (uint8_t)(counter >> 24);
                    suf4[k][3] = (uint8_t)((counter >> 16) & 0xff);
                    suf4[k][4] = (uint8_t)((counter >> 8) & 0xff);
                    suf4[k][5] = (uint8_t)(counter & 0xff);
                }
                sm3x4_hash_suffix(
                    bufs[0] + ct * 32, bufs[1] + ct * 32,
                    bufs[2] + ct * 32, bufs[3] + ct * 32,
                    &mid,
                    suf4[0], suf4[1], suf4[2], suf4[3],
                    6);
            }
#else
            /* Non-64B seed: use full sm3x8_hash_equal (no midstate optimization) */
            uint8_t extseed[4][DKE_SEEDBYTES + 2];
            uint8_t msg8[8][DKE_SEEDBYTES + 2 + 4];

            for (k = 0; k < 4; k++) {
                memcpy(extseed[k], seed, DKE_SEEDBYTES);
                extseed[k][DKE_SEEDBYTES + 0] = xy[k][0];
                extseed[k][DKE_SEEDBYTES + 1] = xy[k][1];
            }

            for (k = 0; k < 4; k++) {
                memcpy(msg8[k],     extseed[k], DKE_SEEDBYTES + 2);
                memcpy(msg8[k + 4], extseed[k], DKE_SEEDBYTES + 2);
            }
            ...
            for (ct = 0; ct + 2 <= num_sm3_blocks; ct += 2) {
                ...
                unsigned int ctB = ct + 2;
                for (k = 0; k < 4; k++) {
                    msg8[k][DKE_SEEDBYTES + 2 + 0] = (uint8_t)(ctA >> 24);
                    msg8[k][DKE_SEEDBYTES + 2 + 1] = (uint8_t)((ctA >> 16) & 0xff);
                    msg8[k][DKE_SEEDBYTES + 2 + 2] = (uint8_t)((ctA >> 8) & 0xff);
                    msg8[k][DKE_SEEDBYTES + 2 + 3] = (uint8_t)(ctA & 0xff);
                    msg8[k+4][DKE_SEEDBYTES + 2 + 0] = (uint8_t)(ctB >> 24);
                    msg8[k+4][DKE_SEEDBYTES + 2 + 1] = (uint8_t)((ctB >> 16) & 0xff);
                    msg8[k+4][DKE_SEEDBYTES + 2 + 2] = (uint8_t)((ctB >> 8) & 0xff);
                    msg8[k+4][DKE_SEEDBYTES + 2 + 3] = (uint8_t)(ctB & 0xff);
                }
                sm3x8_hash_equal(
                    bufs[0] + ct * 32,       bufs[1] + ct * 32,
                    bufs[2] + ct * 32,       bufs[3] + ct * 32,
            ...
            if (ct < num_sm3_blocks) {
                unsigned int counter = ct + 1;
                uint8_t msg4[4][DKE_SEEDBYTES + 2 + 4];
                for (k = 0; k < 4; k++) {
                    memcpy(msg4[k], extseed[k], DKE_SEEDBYTES + 2);
                    msg4[k][DKE_SEEDBYTES + 2 + 0] = (uint8_t)(counter >> 24);
                    msg4[k][DKE_SEEDBYTES + 2 + 1] = (uint8_t)((counter >> 16) & 0xff);
                    msg4[k][DKE_SEEDBYTES + 2 + 2] = (uint8_t)((counter >> 8) & 0xff);
                    msg4[k][DKE_SEEDBYTES + 2 + 3] = (uint8_t)(counter & 0xff);
                }
                sm3x4_hash_equal(
                    bufs[0] + ct * 32, bufs[1] + ct * 32,
                    bufs[2] + ct * 32, bufs[3] + ct * 32,
        ...
        /* Rejection sampling on each of the 4 streams */
        {
            unsigned int buflen = DKE_GEN_MATRIX_NBLOCKS * DKE_XOF_BLOCKBYTES;
            for (k = 0; k < 4; k++) {
                ctr[k] = rej_uniform(res[idx_i[k]].vec[idx_j[k]].coeffs, DKE_N, bufs[k], buflen);
            }

            {
                /* SIMD pass fix: the previous 4x "extra" refill produced a byte
                 * stream that did not align with the reference incremental XOF at
                 * the initial-buffer boundary (only exercised by high-rejection
                 * seeds, e.g. DKE-256). For any stream that did not fill from the
                 * initial 4x buffer, redo it with the reference scalar XOF so the
                 * generated matrix matches the reference byte-for-byte. Streams
                 * that already filled used the same first NBLOCKS*BLOCKBYTES bytes
                 * as the reference, so they are left untouched. */
                for (k = 0; k < 4; k++) {
                    if (ctr[k] < DKE_N) {
                        dke_xof_state st;
                        uint8_t rbuf[DKE_GEN_MATRIX_NBLOCKS * DKE_XOF_BLOCKBYTES];
                        unsigned int rctr, rbuflen;
                        if (transposed)
                            dke_xof_absorb(&st, seed, (uint8_t)idx_i[k], (uint8_t)idx_j[k]);
                        else
                            dke_xof_absorb(&st, seed, (uint8_t)idx_j[k], (uint8_t)idx_i[k]);
                        dke_xof_squeezeblocks(rbuf, DKE_GEN_MATRIX_NBLOCKS, &st);
                        rbuflen = DKE_GEN_MATRIX_NBLOCKS * DKE_XOF_BLOCKBYTES;
                        rctr = rej_uniform(res[idx_i[k]].vec[idx_j[k]].coeffs, DKE_N, rbuf, rbuflen);
                        while (rctr < DKE_N) {
                            dke_xof_squeezeblocks(rbuf, 1, &st);
                            rbuflen = DKE_XOF_BLOCKBYTES;
                            rctr += rej_uniform(res[idx_i[k]].vec[idx_j[k]].coeffs + rctr,
                                                DKE_N - rctr, rbuf, rbuflen);
                        }
                        dke_xof_release(&st);
                    }
                }
...
#endif
            for (k = 0; k < 4; k++)
                _nttunpack(res[idx_i[k]].vec[idx_j[k]].coeffs);
#undef _nttunpack
        }
#endif
        done += 4;
    ...
    /* Handle remaining elements (< 4) with scalar */
    for (; done < total; done++) {
        unsigned int ctr2, buflen2;
        unsigned int ii = done / DKE_K;
        unsigned int jj = done % DKE_K;
        dke_xof_state state;
        uint8_t buf2[DKE_GEN_MATRIX_NBLOCKS * DKE_XOF_BLOCKBYTES];

        if (transposed)
            dke_xof_absorb(&state, seed, (uint8_t)ii, (uint8_t)jj);
        else
            dke_xof_absorb(&state, seed, (uint8_t)jj, (uint8_t)ii);

        dke_xof_squeezeblocks(buf2, DKE_GEN_MATRIX_NBLOCKS, &state);
        buflen2 = DKE_GEN_MATRIX_NBLOCKS * DKE_XOF_BLOCKBYTES;
        ctr2 = rej_uniform(res[ii].vec[jj].coeffs, DKE_N, buf2, buflen2);

        while (ctr2 < DKE_N) {
            dke_xof_squeezeblocks(buf2, 1, &state);
            buflen2 = DKE_XOF_BLOCKBYTES;
            ctr2 += rej_uniform(res[ii].vec[jj].coeffs + ctr2,
                                DKE_N - ctr2, buf2, buflen2);
        }
        dke_xof_release(&state);
...
#else /* Pure scalar SM3 gen_matrix */
...
#endif /* (DKE_USE_AVX2 || DKE_USE_AARCH64) && DKE_HASH == 0 / Pure scalar */
