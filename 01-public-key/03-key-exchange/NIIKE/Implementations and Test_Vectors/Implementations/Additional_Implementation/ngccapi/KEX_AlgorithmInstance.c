...
{
	make_SecretKey((void*)ska, &drng_algorithm);
	orient_cycle_t pubkey;
	niike_PublicKey(&pubkey, ska);
	encode_orient_cycle(pka, &pubkey);
	return 0;
}
...
{
	make_SecretKey((void*)skb, &drng_algorithm);
	orient_cycle_t pubkey;
	niike_PublicKey(&pubkey, skb);
	encode_orient_cycle(pkb, &pubkey);
	return 0;
}
	...
	unsigned char *ssa, unsigned long long *ssa_len_bytes)
{
	fp2_t sharedkey;
	orient_cycle_t pubkey;
	decode_orient_cycle(&pubkey, pkb);
	niike_SecretAgreement(&sharedkey, &pubkey, (void*)ska);
	fp2_encode(ssa, &sharedkey);
	return 0;
}
	...
	unsigned char *ssb, unsigned long long *ssb_len_bytes)
{
	fp2_t sharedkey;
	orient_cycle_t pubkey;
	decode_orient_cycle(&pubkey, pka);
	niike_SecretAgreement(&sharedkey, &pubkey, (void*)skb);
	fp2_encode(ssb, &sharedkey);
	return 0;
}