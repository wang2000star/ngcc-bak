...
void v8poly_mul(v8poly h, const v8poly f, const int lenf, const v8poly g, const int leng)
{
	...
	if(leng == 1)
	{
	        v8fp2_t fc[lenf], gc; // copy of f and g
		for(i = 0; i < lenf; i++)
			v8fp2_copy(&fc[i], &f[i]);

		v8fp2_copy(&gc, &g[0]);
		for (i = 0; i < lenh; i++)
			v8fp2_mul(&h[i], &fc[i], &gc);
	...
	{
	        v8fp2_t t0, t1, t2;
		v8fp2_add(&t1, &f[0], &f[1]);
		v8fp2_add(&t2, &g[0], &g[1]);
		v8fp2_mul(&t0, &f[0], &g[0]);
		v8fp2_mul(&h[2], &f[1], &g[1]);
		v8fp2_mul(&t1, &t1, &t2);
		v8fp2_sub(&t1, &t1, &t0);
		v8fp2_sub(&h[1], &t1, &h[2]);
		v8fp2_copy(&h[0], &t0);
		return;
	}
	...
	if (lenf == 3)
	{
		...
		if (leng == 2)
		{
		  v8fp2_t t0, t1, t2, t3;
			v8fp2_mul(&t0, &f[0], &g[0]);
			v8fp2_mul(&t2, &f[1], &g[1]);
			v8fp2_add(&t3, &f[0], &f[1]);
			v8fp2_add(&t1, &g[0], &g[1]);
			v8fp2_mul(&t1, &t1, &t3);
			v8fp2_sub(&t1, &t1, &t2);
			v8fp2_sub(&t1, &t1, &t0);
			v8fp2_mul(&t3, &f[2], &g[1]);
			v8fp2_mul(&h[2], &f[2], &g[0]);
			v8fp2_add(&h[2], &h[2], &t2);
		        v8fp2_copy(&h[0], &t0);
			v8fp2_copy(&h[1], &t1);
			v8fp2_copy(&h[3], &t3);
			return;
		...
		if (leng == 3)
		{
		        v8fp2_t fg_high[3], t0, t1, t2;

			v8poly_mul(fg_high, &(f[1]), 2, &(g[1]), 2);

			v8fp2_add(&t0, &f[0], &f[1]);
			v8fp2_add(&t1, &g[0], &g[1]);
			v8fp2_mul(&t1, &t0, &t1);
			v8fp2_add(&t0, &f[0], &f[2]);
			v8fp2_add(&t2, &g[0], &g[2]);
			v8fp2_mul(&t2, &t0, &t2);
			
			v8fp2_mul(&h[0], &f[0], &g[0]);

			v8fp2_sub(&t1, &t1, &h[0]);
			v8fp2_sub(&h[1], &t1, &fg_high[0]);
			v8fp2_sub(&t2, &t2, &h[0]);
			v8fp2_sub(&t2, &t2, &fg_high[2]);
			v8fp2_add(&h[2], &t2, &fg_high[0]);
			v8fp2_copy(&h[3], &fg_high[1]);
			v8fp2_copy(&h[4], &fg_high[2]);
	...
	if (leng <= nf)
	{
		v8fp2_t fg_low[nf + leng -1], fg_high[mf + leng -1];
    
		v8poly_mul(fg_low, f, nf, g, leng);
		v8poly_mul(fg_high, &(f[nf]), mf, g, leng);
		for(i = 0; i < nf; i++)
			v8fp2_copy(&h[i], &fg_low[i]);
    
		for(i = nf; i < nf + leng - 1; i++)
			v8fp2_add(&h[i], &fg_high[i-nf], &fg_low[i]);

		for(i = nf + leng - 1; i < lenh; i++)
			v8fp2_copy(&h[i], &fg_high[i - nf]);
	...
	v8fp2_t f_mid[mf], g_mid[mf], fg_low[2*nf-1], fg_mid[2*mf-1], fg_high[mf+mg-1];
  
	for(i = 0; i < nf; i++)
		v8fp2_add(&f_mid[i], &f[i], &f[i+nf]);

	if(lenf & 1)
		v8fp2_copy(&f_mid[nf], &f[lenf-1]);
  
	i = 0;
	while(i < nf && i < mg)
	{
		v8fp2_add(&g_mid[i], &g[i], &g[nf+i]);
		i++;
	}
	while(i < nf)
	{
		v8fp2_copy(&g_mid[i], &g[i]);
		i++;
	}
	
	v8poly_mul(fg_low, f, nf, g, nf);
	v8poly_mul(fg_high, &(f[nf]), mf, &(g[nf]), mg);
	
	if((lenf & 1) && (mg == mf))
	{
		v8fp2_copy(&g_mid[nf], &g[leng - 1]);
		v8poly_mul(fg_mid, f_mid, mf, g_mid, mf);
	}
	else
	{
	        v8poly_mul(fg_mid, f_mid, mf, g_mid, nf);
	}


	for(i = 0; i < mf + mg - 1; i++)
		v8fp2_sub(&fg_mid[i], &fg_mid[i], &fg_high[i]);

	for(i = 0; i < 2*nf-1; i++)
		v8fp2_sub(&fg_mid[i], &fg_mid[i], &fg_low[i]);

	for(i = 0; i < nf; i++)
		v8fp2_copy(&h[i], &fg_low[i]);

	for(i = nf; i < 2*nf-1; i++)
		v8fp2_add(&h[i], &fg_low[i], &fg_mid[i-nf]);

	v8fp2_copy(&h[2*nf-1], &fg_mid[nf-1]);
  
	for(i = 2*nf; i < 2*nf+mf-1; i++)
		v8fp2_add(&h[i], &fg_mid[i-nf], &fg_high[i-2*nf]);

	for(i = 2*nf+mf-1; i < lenh; i++)
		v8fp2_copy(&h[i], &fg_high[i-2*nf]);
...
void v8poly_mul_low(v8poly h, const int n, const v8poly f, const int lenf, const v8poly g, const int leng)
{
	...
	if(leng > lenf)
	{
		v8poly_mul_low(h, n, g, leng, f, lenf);
		return;
	}
	...
	if(leng == 1)
	{
		v8fp2_t fc[n], gc; // Copies for f,g
		int i;
		for(i = 0; i < n; i++)
			v8fp2_copy(&fc[i], &f[i]);
	...
	if(n == 2)
	{
		...
		v8fp2_mul(&fg[1], &f[1], &g[0]);
		v8fp2_mul(&fg[0], &f[0], &g[1]);
		v8fp2_add(&fg[1], &fg[1], &fg[0]);
		v8fp2_mul(&h[0], &f[0], &g[0]);
		v8fp2_copy(&h[1], &fg[1]);
		return;
	}
	...
	if(n == 3)
	{
		...
		if(leng == 2)
		{
		  v8fp2_t t0, t1, t2;
			...
			v8fp2_add(&t1, &g[0], &g[1]);
			v8fp2_mul(&t1, &t0, &t1);
			v8fp2_mul(&t2, &f[0], &g[0]);
			v8fp2_sub(&t1, &t1, &t2);
			v8fp2_mul(&t0, &f[1], &g[1]);
			v8fp2_sub(&t1, &t1, &t0);
			v8fp2_mul(&h[2], &f[2], &g[0]);
			v8fp2_add(&h[2], &t0, &h[2]);
			v8fp2_copy(&h[1], &t1);
			v8fp2_copy(&h[0], &t2);
			return;
		}
		...
		if(leng == 3)
		{
		        v8fp2_t t0, t1, t2, t3, t4;
			v8fp2_mul(&t0, &f[0], &g[0]);
			v8fp2_mul(&t1, &f[1], &g[1]);
			v8fp2_add(&t2, &f[0], &f[1]);
			v8fp2_add(&t3, &g[0], &g[1]);
			v8fp2_mul(&t2, &t2, &t3);
			v8fp2_sub(&t2, &t2, &t0);
			v8fp2_sub(&t2, &t2, &t1);

			v8fp2_add(&t3, &f[0], &f[2]);
			v8fp2_add(&t4, &g[0], &g[2]);
			v8fp2_mul(&t3, &t3, &t4);
			v8fp2_mul(&t4, &f[2], &g[2]);
			v8fp2_sub(&t3, &t3, &t4);
			v8fp2_sub(&t3, &t3, &t0);
			v8fp2_add(&h[2], &t3, &t1);
	...
	if((n==4) && (leng==4))
	{
		...
		for(i = 0; i < nf; i++)
		{
			for(j = 0; j < n-2*i-1; j++)
			{
				v8fp2_add(&t0, &f[i], &f[i+j+1]);
				v8fp2_add(&t1, &g[i], &g[i+j+1]);
				v8fp2_mul(&c[k], &t0, &t1);
				v8fp2_add(&t0, &c[i], &c[i+j+1]);
				v8fp2_sub(&c[k], &c[k], &t0);
				k++;
			}
		}

		v8fp2_copy(&c[n-1], &c[0]);
		for(i = 1; i < nf; i++)
		{
			for(j = 1; j < n-2*i; j++)
				v8fp2_add(&c[n+2*i-1+j], &c[n+2*i-1+j], &c[(1+i)*n-i*i-1+j]);
		}

		for(i = 1; i < nc; i++)
			v8fp2_add(&c[n+2*i-1], &c[n+2*i-1], &c[i]);

		for(i = n-1; i < 2*n-1; i++)
			v8fp2_copy(&h[i-n+1], &c[i]);
	...
	v8fp2_t f0[lenf0], f1[lenf1];
	for(i = 0; i < lenf1; i++)
	{
		v8fp2_copy(&f0[i], &f[2*i]);
		v8fp2_copy(&f1[i], &f[2*i+1]);
	}

	if(lenf0 > lenf1)
		v8fp2_copy(&f0[lenf0-1], &f[lenf - 1]);

	//Split g
	const int leng1 = leng >> 1;
	const int leng0 = leng - leng1;
	v8fp2_t g0[leng0], g1[leng1];
	for(i = 0; i < leng1; i++)
	{
		v8fp2_copy(&g0[i], &g[2*i]);
		v8fp2_copy(&g1[i], &g[2*i+1]);
	}
	
	if(leng0 > leng1)
		v8fp2_copy(&g0[leng0-1], &g[leng - 1]);
  
	//Compute f01 = f0 + f1
	v8fp2_t f01[lenf0];
	for(i = 0; i < lenf1; i++)
		v8fp2_add(&f01[i], &f0[i], &f1[i]);

	if(lenf0 > lenf1)
		v8fp2_copy(&f01[lenf0-1], &f0[lenf0-1]);

	//Compute g01 = g0 + g1
	v8fp2_t g01[leng0];
	for(i = 0; i < leng1; i++)
		v8fp2_add(&g01[i], &g0[i], &g1[i]);

	if(leng0 > leng1)
		v8fp2_copy(&g01[leng0-1], &g0[leng0-1]);
	...
	v8fp2_t fg_0[n0];
	v8poly_mul_low(fg_0, n0, f0, lenf0, g0, leng0);
	...
	v8fp2_t fg_1[n1];
	v8poly_mul_low(fg_1, n1, f1, lenf1, g1, leng1);
	...
	v8fp2_t fg_01[n01];
	v8poly_mul_low(fg_01, n01, f01, lenf0, g01, leng0);
	...
	while(i < n1)
	{
	        v8fp2_sub(&fg_01[i], &fg_01[i], &fg_0[i]);
		v8fp2_sub(&h[2*i+1], &fg_01[i], &fg_1[i]);
		i++;
	}
	while(i < n01)
	{
	        v8fp2_sub(&h[2*i+1], &fg_01[i], &fg_0[i]);
		i++;
	}

	//Combine the result for fg_1 and fg_0
	v8fp2_copy(&h[0], &fg_0[0]);
	for(i = 1; i < n1; i++)
	        v8fp2_add(&h[2*i], &fg_0[i], &fg_1[i-1]);
	if(2*n1 < n)
	        v8fp2_add(&h[2*n1], &fg_0[n1], &fg_1[n1-1]);
...
void v8quasi_poly_mul_middle(v8poly h, const v8poly g, const int leng, const v8poly f, const int lenf)
{
	...
	v8fp2_t F[lenF], G[leng1];
 
	v8fp2_t A[leng1];
	for(i = 0; i < lenF; i++)
		v8fp2_add(&F[i], &f[i], &f[i+leng1]);

	v8quasi_poly_mul_middle(A, &(g[leng0]), leng1, F, lenF);

	v8fp2_t B[leng1];
	if(leng & 1)
	{
		v8fp2_copy(&G[0], &g[leng0]);
		for(i = 0; i < leng0; i++)
			v8fp2_sub(&G[i+1], &g[leng1+i], &g[i]);

	}
	else
	{
		for(i = 0; i < leng0; i++)
			v8fp2_sub(&G[i], &g[leng1+i], &g[i]);

	}
	
	v8quasi_poly_mul_middle(B, G, leng1, &(f[leng1]), lenF);

	v8fp2_t C[leng0];
	for(i = 0; i < 2*leng0-1; i++)
		v8fp2_add(&F[i], &f[i+leng1], &f[i+2*leng1]);

	v8quasi_poly_mul_middle(C, g, leng0, F, 2*leng0-1);

	for(i = 0; i < leng1; i++)
		v8fp2_sub(&h[i], &A[i], &B[i]);

	for(i = 0; i < leng0; i++)
		v8fp2_add(&h[i+leng1], &C[i], &B[i]);
...
void v8poly_mul_middle(v8poly h, const v8poly g, const int leng, const v8poly f, const int lenf)
{
	...
	if ( (leng == lenf >> 1) && !(lenf & 1) )
	{
		...
		v8fp2_t f1[lenf]; 
		for(i = 0; i < lenf - 1; i++)
			v8fp2_copy(&f1[i], &f[i+1]);

		v8fp2_set(&f1[lenf-1], 0);
		v8quasi_poly_mul_middle(h, g, leng, f1, lenf);
		return;
	}
	...
	if( (leng == (lenf>>1)+1) && !(lenf & 1) )
	{
		...
		v8fp2_set(&f1[0], 0);
		for(i = 0; i < lenf; i++)
			v8fp2_copy(&f1[i+1], &f[i]);

		v8quasi_poly_mul_middle(h, g, leng, f1, lenf+1);
		return;
	}
	...
	if( (leng == (lenf>>1)+1) && (lenf & 1) )
	{
		v8quasi_poly_mul_middle(h, g, leng, f, lenf);
		return;
	}
	...
	v8fp2_t F0[lenF0], G[leng];
	for(i = 0; i < lenF0; i++)
		v8fp2_copy(&F0[i], &f[lenF0-1-i]);

	for( i = 0; i < leng; i++)
		v8fp2_copy(&G[i], &g[leng-1-i]);

	v8fp2_t fg_low[leng-1], fg_low_reverse[leng-1];
	v8poly_mul_low(fg_low, leng-1, F0, lenF0, G, leng);
	for(i = 0; i < leng-1; i++)
		v8fp2_copy(&fg_low_reverse[i], &fg_low[leng-2-i]);

	v8fp2_t fg_high[leng];
	v8poly_mul_low(fg_high, leng, &(f[lenF0]), leng, g, leng);

	for(i = 0; i < leng-1; i++)
		v8fp2_add(&h[i], &fg_low_reverse[i], &fg_high[i]);

	v8fp2_copy(&h[leng-1], &fg_high[leng-1]);
	return;
}
...
void v8poly_mul_selfreciprocal(v8poly h, const v8poly g, const int leng, const v8poly f, const int lenf)
{
	...
	if(lenf==leng)
	{
		...
		{
			v8fp2_t t0, t1;
			v8fp2_add(&t0, &g[0], &g[1]);
			v8fp2_add(&t1, &f[0], &f[1]);
			v8fp2_mul(&t1, &t0, &t1);
			v8fp2_mul(&t0, &g[0], &f[0]);
			v8fp2_mul(&h[2], &g[1], &f[1]);
			v8fp2_add(&h[2], &h[2], &t0);
			v8fp2_copy(&h[0], &t0);
			v8fp2_sub(&h[1], &t1, &h[2]);
			v8fp2_add(&h[2], &h[2], &h[0]);
			v8fp2_copy(&h[3], &h[1]);
			v8fp2_copy(&h[4], &h[0]);
			return;
		}
		...
		if(lenf == 4)
		{
			...
			v8fp2_mul(&t1, &t0, &t1);
			v8fp2_mul(&t0, &g[0], &f[0]);
			v8fp2_mul(&h[3], &g[1], &f[1]);
			v8fp2_copy(&h[2], &t1);
			v8fp2_copy(&h[0], &t0);
			v8fp2_sub(&h[2], &h[2], &h[0]);
			v8fp2_sub(&h[1], &h[2], &h[3]);
			v8fp2_add(&h[3], &h[3], &h[0]);
			v8fp2_add(&h[3], &h[3], &h[3]);
			v8fp2_copy(&h[4], &h[2]);
			v8fp2_copy(&h[5], &h[1]);
			v8fp2_copy(&h[6], &h[0]);
			return;
		...
		{
			v8fp2_t t0, t1, t2, t3, t4;
			v8fp2_sub(&t1, &g[1], &g[0]);
			v8fp2_sub(&t0, &f[0], &f[1]);
			v8fp2_mul(&t1, &t1, &t0);
			v8fp2_sub(&t2, &g[2], &g[0]);
			v8fp2_sub(&t0, &f[0], &f[2]);
			v8fp2_mul(&t2, &t0, &t2);
			v8fp2_sub(&t3, &g[2], &g[1]);
			v8fp2_sub(&t0, &f[1], &f[2]);
			v8fp2_mul(&t3, &t3, &t0);
			v8fp2_mul(&t0, &g[1], &f[1]);
			v8fp2_mul(&t4, &g[2], &f[2]);
			v8fp2_mul(&h[0], &g[0], &f[0]);
			v8fp2_copy(&h[1], &t1);
			v8fp2_copy(&h[2], &t2);
			v8fp2_copy(&h[3], &t3);
			v8fp2_add(&h[5], &t0, &h[0]);
			v8fp2_add(&h[1], &h[1], &h[5]);
			v8fp2_add(&h[3], &h[3], &h[1]);
			v8fp2_add(&h[4], &h[5], &t4);
			v8fp2_add(&h[4], &h[4], &h[5]);
			v8fp2_add(&h[2], &h[2], &h[5]);
			v8fp2_add(&h[2], &h[2], &t4);
			v8fp2_add(&h[3], &h[3], &t0);
			v8fp2_add(&h[3], &h[3], &t4);
			v8fp2_copy(&h[5], &h[3]);
			v8fp2_copy(&h[6], &h[2]);
			v8fp2_copy(&h[7], &h[1]);
			v8fp2_copy(&h[8], &h[0]);
		...
		if(lenf & 1)
		{
			...
			v8fp2_t g0[len0], f0[len0], g1[len1], f1[len1];
			for(i = 0; i < len1; i++)
			{
				v8fp2_copy(&g0[i], &g[2*i]);
				v8fp2_copy(&f0[i], &f[2*i]);
			...
			v8fp2_copy(&g0[len1], &g[2*len1]);
			v8fp2_copy(&f0[len1], &f[2*len1]);

			v8fp2_t h0[2*len0-1];
			v8poly_mul_selfreciprocal(h0, g0, len0, f0, len0);

			v8fp2_t h1[2*len1-1];
			v8poly_mul_selfreciprocal(h1, g1, len1, f1, len1);

			v8fp2_t h01[2*len0-1];
			for(i = 0; i < len1; i++)
			{
				v8fp2_add(&g0[i], &g0[i], &g1[i]);
				v8fp2_add(&f0[i], &f0[i], &f1[i]);
				v8fp2_add(&g0[i+1], &g0[i+1], &g1[i]);
				v8fp2_add(&f0[i+1], &f0[i+1], &f1[i]);
			}
			v8poly_mul_selfreciprocal(h01, g0, len0, f0, len0);
       
			// Mixing results for the odd degree part
			for(i = 0; i < 2*len0-1; i++)
				v8fp2_sub(&h01[i], &h01[i], &h0[i]);

			for(i = 0; i < 2*len1-1; i++)
				v8fp2_sub(&h01[i], &h01[i], &h1[i]);

			for(i = 0; i < 2*len1-1; i++)
			{
				v8fp2_sub(&h01[i+1], &h01[i+1], &h1[i]);
				v8fp2_sub(&h01[i+1], &h01[i+1], &h1[i]);
			}
			for(i = 0; i < 2*len1-1; i++)
				v8fp2_sub(&h01[i+2], &h01[i+2], &h1[i]);

			v8fp2_copy(&h[1], &h01[0]);
			for(i = 1; i < 2*len0-2; i++)
				v8fp2_sub(&h[2*i+1], &h01[i], &h[2*i-1]);

			// Mixing results for the even degree parts
			v8fp2_copy(&h[0], &h0[0]);
			for(i = 1; i < 2*len1; i++)
				v8fp2_add(&h[2*i], &h0[i], &h1[i-1]);

			v8fp2_copy(&h[4*len1], &h0[2*len1]);
		  	return;
		}
		else
		{
			...
			v8poly_mul(h, g, half, f, half);
			v8poly_mul(h1, g, half, &(f[half]), half);

			v8fp2_set(&h[leng-1], 0);
			for(i = 0; i < half; i++)
			{
				v8fp2_add(&h[half+i], &h[half+i], &h1[i]);
				v8fp2_add(&h[half+i], &h[half+i], &h1[leng-2-i]);
			}

			for(i = 0; i < leng-1; i++)
				v8fp2_copy(&h[2*leng-2-i], &h[i]);
	...
	else
	{
		...
		v8poly_mul_low(h, m, g, leng, f, lenf);
		for(i = m; i < lenf+leng-1; i++)
			v8fp2_copy(&h[i], &h[lenf+leng-2-i]);
...
void v8product_tree(v8poly H[], int DEG[], const int root, const v8poly F[], const int LENF, const int n)
{
	...
	if(n == 1)
	{
		H[root] = aligned_alloc(64, sizeof(v8fp2_t)*LENF);
		for(i = 0; i < LENF; i++)
			v8fp2_copy(&H[root][i], &F[0][i]);
	...
	v8product_tree(H, DEG, left, F, LENF, n0);
	v8product_tree(H, DEG, right, &(F[n0]), LENF, n1);
	DEG[root] = DEG[left] + DEG[right];
	H[root] = aligned_alloc(64, sizeof(v8fp2_t)*(DEG[left]+DEG[right]+1));
	v8poly_mul(H[root], H[left], DEG[left]+1, H[right], DEG[right]+1);
	return;
}
...
void v8product_tree_LENFeq2(v8poly H[], int DEG[], const int root, const v8fp2_t F[][2], const int n)
{
	...
	if(n == 1)
	{
		H[root] = aligned_alloc(64, sizeof(v8fp2_t)*2);
		for(i = 0; i < 2; i++)
			v8fp2_copy(&H[root][i], &F[0][i]);
	...
	const int left = 2*root+1;
	const int right = left + 1;
	v8product_tree_LENFeq2(H, DEG, left, F, n0);
	v8product_tree_LENFeq2(H, DEG, right, &(F[n0]), n1);
	DEG[root] = DEG[left] + DEG[right];
	H[root] = aligned_alloc(64, sizeof(v8fp2_t)*(DEG[left]+DEG[right]+1));
	v8poly_mul(H[root], H[left], DEG[left]+1, H[right], DEG[right]+1);
	return;
...
void v8product_tree_LENFeq3(v8poly H[], int DEG[], const int root, const v8fp2_t F[][3], const int n)
{
	...
	if(n == 1)
	{
		H[root] = aligned_alloc(64, sizeof(v8fp2_t)*3);
		for(i = 0; i < 3; i++)
			v8fp2_copy(&H[root][i], &F[0][i]);
	...
	const int left = 2*root+1;
	const int right = left + 1;
	v8product_tree_LENFeq3(H, DEG, left, F, n0);
	v8product_tree_LENFeq3(H, DEG, right, &(F[n0]), n1);
	DEG[root] = DEG[left] + DEG[right];
	H[root] = aligned_alloc(64, sizeof(v8fp2_t)*(DEG[left]+DEG[right]+1));
	v8poly_mul(H[root], H[left], DEG[left]+1, H[right], DEG[right]+1);
	return;
...
void v8product_tree_selfreciprocal(v8poly H[], int DEG[], const int root, const v8poly F[], const int LENF, const int n)
{
	...
	const int left = 2*root+1;
	const int right = left + 1;
	v8product_tree_selfreciprocal(H, DEG, left, F, LENF, n0);
	v8product_tree_selfreciprocal(H, DEG, right, &(F[n0]), LENF, n1);
	DEG[root] = DEG[left] + DEG[right];
	H[root] = aligned_alloc(64, sizeof(v8fp2_t)*(DEG[left]+DEG[right]+1));
	v8poly_mul_selfreciprocal(H[root], H[left], DEG[left]+1, H[right], DEG[right]+1);
	return;
}
...
void v8product_tree_selfreciprocal_LENFeq3(v8poly H[], int DEG[], const int root, const v8fp2_t F[][3], const int n)
{
	...
	const int left = 2*root+1;
	const int right = left + 1;
	v8product_tree_selfreciprocal_LENFeq3(H, DEG, left, F, n0);
	v8product_tree_selfreciprocal_LENFeq3(H, DEG, right, &(F[n0]), n1);
	DEG[root] = DEG[left] + DEG[right];
	H[root] = aligned_alloc(64, sizeof(v8fp2_t)*(DEG[left]+DEG[right]+1));
	v8poly_mul_selfreciprocal(H[root], H[left], DEG[left]+1, H[right], DEG[right]+1);
	return;
...
void v8clear_tree(v8poly H[], const int root, const int n)
{
	...
	v8clear_tree(H, 2*root+1, n-(n>>1));
	v8clear_tree(H, 2*root+2, n>>1);
	free(H[root]);
	return;
}
