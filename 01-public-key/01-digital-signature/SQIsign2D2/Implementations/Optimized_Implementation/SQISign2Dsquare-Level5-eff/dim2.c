                                      ...
                                      const ibz_t *norm_bound)
{
    ...
    if (ibz_cmp(&norm, norm_bound) <= 0) {
        ibz_mul(&norm, &norm, &QUATALG_PINFTY.p);
        assert(ibz_cmp(&norm, n_mu) < 0);
        ibz_sub(&norm, n_mu, &norm);
        ibz_div(&norm, &temp, &norm, n);
        assert(ibz_cmp(&temp, &ibz_const_zero) == 0);
        ibz_div(&norm, &temp, &norm, n);
        assert(ibz_cmp(&temp, &ibz_const_zero) == 0);
        ok = condition(res, &sum, &norm, n);
    }
    ibz_finalize(&norm);
                                         ...
                                         const int max_tries)
{
    ...
    // calculate the norm of target_minus_closest
    ...
    // This represents the quadratic form in vector coordinates over lat_basis.
    // is the norm defined by norm_q
    quat_dim2_lattice_get_qf_on_lattice(&qf_a, &qf_b, &qf_c, lat_basis, norm_q);
    // discriminant computation disc = 4ac-b^2
    ...
    if (ok) {
        ...
        // Iterate y from -bound_y to bound_y.
        while ((!found) && (!stop) && (ibz_cmp(&y, &bound_y) < 0) && (tries < max_tries)) {
            ...
            // Iterate over x; check whether this range is appropriate.
            while ((!found) && (!stop) && (ibz_cmp(&x, &bound_x) < 0) && (tries < max_tries)) {
                ...
                found = quat_dim2_lattice_bound_and_condition(res,
                                                              ...
                                                              norm_q,
                                                              norm_bound);
                // Stop when both x and y are zero.
                                      ...
                                      unsigned int max_tries)
{
    ...
    found = quat_dim2_lattice_qf_enumerate_short_vec(
        res, condition, n_mu, n, &target_minus_closest, &reduced, &norm_q, &norm_bound, max_tries);
