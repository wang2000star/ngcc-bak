...
void
xeval_3(ec_point_t* R, ec_point_t* const Q, const int lenQ, const ec_kps3_t* kps)
{
    fp2_t t0, t1, t2;
    for (int i = 0; i < lenQ; i++) {
        fp2_add(&t0, &Q[i].x, &Q[i].z);
        fp2_sub(&t1, &Q[i].x, &Q[i].z);
        fp2_mul(&t0, &kps->K.x, &t0);
        fp2_mul(&t1, &kps->K.z, &t1);
        fp2_add(&t2, &t0, &t1);
        fp2_sub(&t0, &t1, &t0);
        fp2_sqr(&t2, &t2);
        fp2_sqr(&t0, &t0);
        fp2_mul(&R[i].x, &Q[i].x, &t2);
        fp2_mul(&R[i].z, &Q[i].z, &t0);
    }
}
