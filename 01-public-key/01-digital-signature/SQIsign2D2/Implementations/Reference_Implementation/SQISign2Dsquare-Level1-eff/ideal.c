            ...
            for (b = -int_norm + abs(a); b <= int_norm - abs(a); b++) {
                for (c = -int_norm + abs(a) + abs(b); c <= int_norm - abs(a) - abs(b); c++) {
                    ...
                    if (ibz_is_one(&gcd)) {
						...
						if (found) {
							ibz_mod(&(gen->coord[0]), &(gen->coord[0]), &lideal->norm);
							ibz_mod(&(gen->coord[1]), &(gen->coord[1]), &lideal->norm);
							ibz_mod(&(gen->coord[2]), &(gen->coord[2]), &lideal->norm);
							ibz_mod(&(gen->coord[3]), &(gen->coord[3]), &lideal->norm);
							goto fin;
						}
                    }
                }
