...
ibz_4x4_right_ker_mod_power_of_2(ibz_vec_4_t *ker, const ibz_mat_4x4_t *mat, unsigned short exp)
{
    ...
    ibz_t *full_ker_rows[4];
    ibz_t *howell_rows[4];
    const ibz_t *mat_rows[4];
    const ibz_t *full_ker_const_rows[4];
    for (int i = 0; i < 4; i++) {
        full_ker_rows[i] = full_ker[i];
        howell_rows[i] = howell[i];
        mat_rows[i] = (*mat)[i];
        full_ker_const_rows[i] = full_ker[i];
    }

    ibz_mat_right_ker_mod(4, 4, full_ker_rows, mat_rows, &pow2);
    int zeros = ibz_mat_howell(4, 4, howell_rows, NULL, full_ker_const_rows, &pow2);
