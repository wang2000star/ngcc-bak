...
void
compute_p_quarter(ibz_t* p_quarter, const ibz_t* p)
{
    ibz_t temp;
    ibz_init(&temp);

    // Use ibz_sqrt to compute sqrt(p) first
    mpz_sqrt(temp, *p);

    // Then compute sqrt of sqrt(p) to get fourth root
    mpz_sqrt(*p_quarter, temp);
...
ibz_rand_interval(ibz_t *rand, const ibz_t *a, const ibz_t *b)
{
    ...
    mp_limb_t *r = (mp_limb_t *)malloc(sizeof(mp_limb_t)*len_limbs);

    do {
		randret = get_random_number(&drng_algorithm, (unsigned char *)r, 8*len_bytes);
        if (randret != 0) {
            ret = 0;
            goto err;
        }
