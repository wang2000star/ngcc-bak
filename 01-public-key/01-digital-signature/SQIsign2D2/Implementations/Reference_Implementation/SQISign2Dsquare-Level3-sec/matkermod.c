 ...
 */
static void
swap_col(int rows, int cols, ibz_t **mat, int j, int k)
{
    ibz_t tmp;
    ibz_init(&tmp);
 ...
 */
static int
is_col_zero(int rows, int cols, ibz_t **mat, int j)
{
    int is_zero = 1;
    for (int i = 0; i < rows; i++)
                      ...
                      ibz_t *mod)
{
    ...
    const ibz_t *trans_const[rows];
    for (int i = 0; i < rows; i++) {
        trans_const[i] = trans[i];
    }

    ibz_mat_mulmod(rows, rows + 1, rows + 1, res, (const ibz_t **)test, trans_const, mod);
                      ...
                      ibz_t *mod)
{
    ...
    ibz_t tmp;
    ibz_mat_2x2_t U;
    ibz_t **howell, **trans, **preker, **near_ker;
