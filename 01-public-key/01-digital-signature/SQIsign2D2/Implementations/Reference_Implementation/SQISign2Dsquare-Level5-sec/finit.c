...
void
ibz_mat_init(int rows, int cols, ibz_t ***mat)
{
    *mat = (ibz_t **)malloc(rows * sizeof(ibz_t *));
    for (int i = 0; i < rows; i++)
    {
        (*mat)[i] = (ibz_t *)malloc(cols * sizeof(ibz_t));
...
void
ibz_mat_finalize(int rows, int cols, ibz_t **mat)
{
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++)
            ibz_finalize(&mat[i][j]);
