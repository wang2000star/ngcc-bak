...
quat_lattice_random_elem(quat_alg_elem_t *elem, const quat_lattice_t *lattice, unsigned char n)
{
    ...
    int64_t rand[4];
	int randret = get_random_number(&drng_algorithm, (unsigned char *)rand, 8 * 4 * sizeof(uint64_t));
    if (randret != 0)
        return 0;
                               ...
                               const quat_alg_t *alg)
{
    ...
    (void)ok;
        ...
        ibz_t *trans_basis_rows[4];
        const ibz_t *work_rows[16];
        for (int i = 0; i < 4; i++)
            trans_basis_rows[i] = trans->basis[i];
        for (int i = 0; i < 16; i++)
            work_rows[i] = work[i];
        ibz_mat_right_ker_mod(16, 4, trans_basis_rows, work_rows, &det2);
