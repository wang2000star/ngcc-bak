    ...
    unsigned char *sk, unsigned long long *sk_len_bytes)
{
  ...
  owf_256(sk_key, sk, pk + input_size);
    ...
    unsigned char *sn, unsigned long long *sn_len_bytes)
{
  ...
  unsigned char owf_output[32];
  owf_256(owf_key, owf_input, owf_output);

  unsigned char witness[768 / 8];
  const sig_paramset_t* params = sig_get_paramset(LYNXER_256F);
    ...
    unsigned char *m, unsigned long long m_len_bytes)
{
  ...
  if (sn_len_bytes != 15097) {
    return -1;
  }

  const sig_paramset_t* params = sig_get_paramset(LYNXER_256F);
  return voleith_verify(m, m_len_bytes, sn, pk, pk + 32, params);
}
