...
static_assert(40 == 20 + 20, "invalid public key size");
static_assert(40 == 20 + 160 / 8, "invalid secret key size");
    ...
    unsigned char *sk, unsigned long long *sk_len_bytes)
{
  ...
  owf_160(sk_key, sk, pk + input_size);
    ...
    unsigned char *sn, unsigned long long *sn_len_bytes)
{
  ...
  unsigned char owf_output[20];
  owf_160(owf_key, owf_input, owf_output);

  unsigned char witness[480 / 8];
  const sig_paramset_t* params = sig_get_paramset(LYNXER_160F);
