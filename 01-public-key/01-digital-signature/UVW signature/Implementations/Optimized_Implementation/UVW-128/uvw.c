...
void dec_xy(const mf3_e *h_xy, const vf3_e *s1, const vf3_e *e2, vf3_e **e1, const size_t w) {
    ...
    while (true) {
        counter++;
        if (counter % 100 == 0) fprintf(stderr, "DBG_DEC_XY: counter=%zu\n", counter);
        ...
        *e1 = vf3_alloc(h_xy->n_col);
        ...
        if (counter % 1000 == 0) fprintf(stderr, "DBG_DEC_XY: counter=%zu ew=%zu w=%zu e1e1e2_size=%zu\n", counter, ew, w, e1e1e2->size);
        vf3_free(e1e1e2);

        if (counter % 1000 == 0) fprintf(stderr, "DBG_DEC_XY: counter=%zu ew=%zu w=%zu\n", counter, ew, w);
