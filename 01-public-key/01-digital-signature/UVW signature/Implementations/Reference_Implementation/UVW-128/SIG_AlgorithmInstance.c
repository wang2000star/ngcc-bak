...
/**
 * @brief 生成UVW签名方案的密钥对（公钥+私钥）
 *
 * 密钥生成过程：
 * 1. 使用DRNG生成随机种子(seed)
 * 2. 基于seed和随机nonce生成4个矩阵：
 *    - H_x: (n/2-k1) × (n/2) F₃矩阵
 *    - H_y: (n/2-k1) × (n/2) F₃矩阵
 *    - H_z: (n/2-k2) × (n/2) F₃矩阵
 *    - D: n×n 置换对角矩阵
 ...
 * 4. 序列化输出公钥和私钥
 *
 * @param pk [out] 公钥缓冲区，调用者需分配足够空间（使用sig_get_pk_len_bytes获取大小）
 * @param pk_len_bytes [out] 输出：实际公钥长度（字节）
 * @param sk [out] 私钥缓冲区，调用者需分配足够空间（使用sig_get_sk_len_bytes获取大小）
 * @param sk_len_bytes [out] 输出：实际私钥长度（字节）
 * @return 成功返回0，失败返回-1（内存不足或矩阵不满秩）
 *
 * @note 私钥结构：[seed(32/64B)][nonce_x(2B)][nonce_y(2B)][nonce_z(2B)][nonce_d(2B)]
 * @note 此函数会重试最多10000次以找到满秩矩阵
 * @warning 需要预先调用init_random_number()初始化uvw_drng全局变量
 */
int sig_keygen(
...
/**
 * @brief 使用私钥对消息进行UVW签名
 *
 * 签名过程：
 * 1. 从私钥中提取seed和4个nonce
 * 2. 使用seed+nonce重建完整私钥（H_x, H_y, H_z, D矩阵）
 * 3. 对消息m计算SHAKE256哈希值
 * 4. 基于哈希值生成稀疏签名向量(e1, e2)
 * 5. 生成随机数r并序列化输出签名
 *
 * @param sk [in] 私钥缓冲区（由sig_keygen生成）
 * @param sk_len_bytes [in] 私钥长度（应等于SEED_LEN_BYTES + 8）
 * @param m [in] 待签名的消息
 * @param m_len_bytes [in] 消息长度（字节）
 * @param sn [out] 签名缓冲区，调用者需分配足够空间（使用sig_get_sn_len_bytes获取大小）
 * @param sn_len_bytes [out] 输出：实际签名长度（字节）
 * @return 成功返回0，失败返回-1（参数错误或DRNG未初始化）
 *
 * @note 签名包含：[参数(12B)][随机数r(λ/8 B)][稀疏向量e(k维)]
 * @note 稀疏向量的汉明重量约为w（安全参数相关）
 */
int sig_sign(
    unsigned char *sk, unsigned long long sk_len_bytes,
 ...
 * 1. 反序列化公钥(pk)和签名(sn)
 * 2. 对消息m计算SHAKE256哈希值
 * 3. 调用uvw_verify()验证签名的数学正确性
 *    - 检查 H1^T × e1 + R^T × e2 = r (mod 3)
 *    - 检查 e 的汉明重量是否为 w
 *
 * @param pk [in] 公钥缓冲区（由sig_keygen生成）
 * @param pk_len_bytes [in] 公钥长度（应等于sig_get_pk_len_bytes返回值）
 * @param sn [in] 签名缓冲区（由sig_sign生成）
 * @param sn_len_bytes [in] 签名长度（应等于sig_get_sn_len_bytes返回值）
 * @param m [in] 原始消息
 * @param m_len_bytes [in] 消息长度（字节）
 * @return 验证成功返回0，验证失败返回-1
 *
 * @note 此函数不依赖DRNG或私钥信息
 * @note 验证失败可能原因：签名被篡改、公钥不匹配、消息不一致
 */
int sig_verify(
    unsigned char *pk, unsigned long long pk_len_bytes,
    unsigned char *sn, unsigned long long sn_len_bytes,
