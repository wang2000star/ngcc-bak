...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk)
{
  ...
  poly_uniform_eta_4x(&s1.vec[0], &s1.vec[1], &s1.vec[2], &s2.vec[0],
                      rhoprime, 0, 1, 2, 3);
  poly_uniform_eta_4x(&s2.vec[1], &s2.vec[2], &dummy0, &dummy1,
                      rhoprime, 4, 5, 6, 7);
...
rej:
  poly_uniform_gamma1_4x(&z.vec[0], &z.vec[1], &z.vec[2], &tmp2,
                         rhoprime,
                         (uint16_t)(L*nonce),
                         (uint16_t)(L*nonce + 1),
                         (uint16_t)(L*nonce + 2),
  ...
  sm3hash((unsigned long long)CTILDEBYTES * 8ULL,
          ctmp,
