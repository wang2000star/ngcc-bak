...
static void prepare_xof_input_4x(unsigned char *dst0,
                                 unsigned char *dst1,
                                 unsigned char *dst2,
                                 unsigned char *dst3,
                                 ...
                                 unsigned int ct)
{
	const unsigned long long msg_len_bytes = (msg_len_bits + 7) / 8;
	const unsigned long long remaining_bits = msg_len_bits % 8;

	copy_bytes_4x_avx2(dst0, dst1, dst2, dst3,
	                   msg0, msg1, msg2, msg3, msg_len_bytes);

	if(remaining_bits == 0)
	{
		unsigned char b0 = (unsigned char)(ct >> 24);
		unsigned char b1 = (unsigned char)((ct & 0xFFFFFFU) >> 16);
		unsigned char b2 = (unsigned char)((ct & 0xFFFFU) >> 8);
		unsigned char b3 = (unsigned char)ct;

		dst0[msg_len_bytes + 0] = b0;
		dst1[msg_len_bytes + 0] = b0;
		dst2[msg_len_bytes + 0] = b0;
		dst3[msg_len_bytes + 0] = b0;
		dst0[msg_len_bytes + 1] = b1;
		dst1[msg_len_bytes + 1] = b1;
		dst2[msg_len_bytes + 1] = b1;
		dst3[msg_len_bytes + 1] = b1;
		dst0[msg_len_bytes + 2] = b2;
		dst1[msg_len_bytes + 2] = b2;
		dst2[msg_len_bytes + 2] = b2;
		dst3[msg_len_bytes + 2] = b2;
		dst0[msg_len_bytes + 3] = b3;
		dst1[msg_len_bytes + 3] = b3;
		dst2[msg_len_bytes + 3] = b3;
		dst3[msg_len_bytes + 3] = b3;
	}
	else
	{
		const unsigned char keep_mask = (unsigned char)~((1U << (8U - remaining_bits)) - 1U);
		const unsigned char xor_byte = (unsigned char)(ct >> (32U - (8U - remaining_bits)));
		const unsigned char s0 = (unsigned char)((((uint32_t)ct << (8U  - remaining_bits)) & 0xFFFFFFFFU) >> 24);
		const unsigned char s1 = (unsigned char)((((uint32_t)ct << (16U - remaining_bits)) & 0xFFFFFFFFU) >> 24);
		const unsigned char s2 = (unsigned char)((((uint32_t)ct << (24U - remaining_bits)) & 0xFFFFFFFFU) >> 24);
		const unsigned char s3 = (unsigned char)((((uint32_t)ct << (32U - remaining_bits)) & 0xFFFFFFFFU) >> 24);

		dst0[msg_len_bytes - 1] = (unsigned char)((dst0[msg_len_bytes - 1] & keep_mask) ^ xor_byte);
		dst1[msg_len_bytes - 1] = (unsigned char)((dst1[msg_len_bytes - 1] & keep_mask) ^ xor_byte);
		dst2[msg_len_bytes - 1] = (unsigned char)((dst2[msg_len_bytes - 1] & keep_mask) ^ xor_byte);
		dst3[msg_len_bytes - 1] = (unsigned char)((dst3[msg_len_bytes - 1] & keep_mask) ^ xor_byte);

		dst0[msg_len_bytes + 0] = s0;
		dst1[msg_len_bytes + 0] = s0;
		dst2[msg_len_bytes + 0] = s0;
		dst3[msg_len_bytes + 0] = s0;
		dst0[msg_len_bytes + 1] = s1;
		dst1[msg_len_bytes + 1] = s1;
		dst2[msg_len_bytes + 1] = s1;
		dst3[msg_len_bytes + 1] = s1;
		dst0[msg_len_bytes + 2] = s2;
		dst1[msg_len_bytes + 2] = s2;
		dst2[msg_len_bytes + 2] = s2;
		dst3[msg_len_bytes + 2] = s2;
		dst0[msg_len_bytes + 3] = s3;
		dst1[msg_len_bytes + 3] = s3;
		dst2[msg_len_bytes + 3] = s3;
		dst3[msg_len_bytes + 3] = s3;
	}
}

static void copy_output_4x(unsigned char *output0,
...
{
	copy_bytes_4x_avx2(output0, output1, output2, output3,
	                   src0, src1, src2, src3, out_len_bytes);

	if(output_len_bits % 256 != 0)
	{
		normalize(output0, output_len_bits);
		normalize(output1, output_len_bits);
		normalize(output2, output_len_bits);
		normalize(output3, output_len_bits);
	}
}
...
int pseudoXOF_4x(unsigned long long output_len_bits,
                 const unsigned char *msg0,
                 const unsigned char *msg1,
                 const unsigned char *msg2,
                 const unsigned char *msg3,
                 ...
                 unsigned char *output3)
{
	unsigned int ct = 1;
	const unsigned long long msg_len_bytes = (msg_len_bits + 7) / 8;
	const unsigned long long out_len_bytes = (output_len_bits + 7) / 8;
	const unsigned long long blocks = (output_len_bits + 255) / 256;
	const unsigned long long alloc_len = (output_len_bits + 255) / 8;
	unsigned char *cascade_msg_ct = NULL;
	unsigned char *K = NULL;
	unsigned char *cascade0, *cascade1, *cascade2, *cascade3;
	unsigned char *K0, *K1, *K2, *K3;
	cascade_msg_ct = (unsigned char *)malloc(4 * (msg_len_bytes + 4));
	if(cascade_msg_ct == NULL)
	{
		fprintf(stderr, "ERROR: Memory allocation failed at %s, line %d. \n", __FILE__, __LINE__);
		return MEMORY_ALLOCATION_FAILED;
	...
	K = (unsigned char *)malloc(4 * alloc_len);
	if(K == NULL)
	{
		fprintf(stderr, "ERROR: Memory allocation failed at %s, line %d. \n", __FILE__, __LINE__);
		free(cascade_msg_ct);
	...
	cascade0 = cascade_msg_ct;
	cascade1 = cascade0 + (msg_len_bytes + 4);
	cascade2 = cascade1 + (msg_len_bytes + 4);
	cascade3 = cascade2 + (msg_len_bytes + 4);
	...
	{
		prepare_xof_input_4x(cascade0, cascade1, cascade2, cascade3,
		                     msg0, msg1, msg2, msg3,
		                     msg_len_bits, ct);

		sm3_bit_4x(cascade0, cascade1, cascade2, cascade3,
		           msg_len_bits + 32,
		           K0 + i * 32, K1 + i * 32,
		           K2 + i * 32, K3 + i * 32);
	...
	copy_output_4x(output0, output1, output2, output3,
	               K0, K1, K2, K3, out_len_bytes, output_len_bits);
