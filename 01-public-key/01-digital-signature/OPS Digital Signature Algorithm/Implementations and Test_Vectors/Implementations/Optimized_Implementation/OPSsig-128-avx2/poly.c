...
#elif GAMMA2 == (Q-1)/48
  for(unsigned i = 0; i < N/8; ++i) {
    r[5*i+0] = a->coeffs[4*i+0] | (a->coeffs[4*i+1] << 5);
    r[5*i+1] = (a->coeffs[4*i+1] >> 3) | (a->coeffs[4*i+2] << 2) | (a->coeffs[4*i+3] << 7);
    r[5*i+2] = (a->coeffs[4*i+3] >> 1) | (a->coeffs[4*i+4] << 4);
    r[5*i+3] = (a->coeffs[4*i+4] >> 4) | (a->coeffs[4*i+5] << 1) | (a->coeffs[4*i+6] << 6);
    r[5*i+4] = (a->coeffs[4*i+6] >> 2) | (a->coeffs[4*i+7] << 3);
  }
#elif GAMMA2 == (Q-1)/96
  for(unsigned i = 0; i < N/4; ++i) {
