...
static int unpack_h(polyveck *h, const uint8_t *in) {
  ...
  const uint16_t posmask = (uint16_t)((1u << POLYVECH_POSBITS) - 1u);
  const unsigned used_bits = OMEGA * POLYVECH_POSBITS;
  const unsigned rem = used_bits & 7u;

  if(rem != 0) {
    unsigned bytepos = used_bits >> 3;
    if(in[bytepos] & (uint8_t)~((1u << rem) - 1u))
      return 1;
    for(bytepos += 1; bytepos < POLYVECH_POSBYTES; ++bytepos)
      if(in[bytepos] != 0)
        return 1;
  }
