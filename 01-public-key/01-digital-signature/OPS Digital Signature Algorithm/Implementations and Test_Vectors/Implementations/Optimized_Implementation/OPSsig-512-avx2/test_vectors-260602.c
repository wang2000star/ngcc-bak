...
static void make_sample_hint(polyveck *h) {
  ...
  for(unsigned i = 0; i < K; ++i) {
    unsigned base = 7u + 31u * i;
    h->vec[i].coeffs[(base + 0u) % N] = 1;
    h->vec[i].coeffs[(base + 9u) % N] = 1;
    h->vec[i].coeffs[(base + N/2 + 13u) % N] = 1;
  }
}
...
static int test_module_vectors(void) {
  ...
  printf("OK: module vector checks passed\n");
  return 0;
}

static int test_x4_helpers(void) {
  uint8_t msg[4][43];
  uint8_t xof_ref[4][67];
  uint8_t xof_x4[4][67];
  uint8_t seed[4][DRNG_SEED_BYTES];
  uint8_t drng_ref[4][65];
  uint8_t drng_x4[4][65];
  DRNG_ctx scalar[4];
  DRNG_x4_ctx vector;

  for(unsigned int lane = 0; lane < 4; ++lane) {
    fill_bytes(msg[lane], sizeof(msg[lane]), (uint8_t)(17 + 23 * lane));
    fill_bytes(seed[lane], sizeof(seed[lane]), (uint8_t)(91 + 13 * lane));
  }

  if(pseudoXOF_4x(531, msg[0], msg[1], msg[2], msg[3], 317,
                  xof_x4[0], xof_x4[1], xof_x4[2], xof_x4[3]) != 0) {
    fprintf(stderr, "pseudoXOF_4x failed\n");
    return 1;
  }

  for(unsigned int lane = 0; lane < 4; ++lane) {
    if(pseudoXOF(531, msg[lane], 317, xof_ref[lane]) != 0) {
      fprintf(stderr, "pseudoXOF failed\n");
      return 1;
    }
    if(memcmp(xof_ref[lane], xof_x4[lane], sizeof(xof_ref[lane])) != 0) {
      fprintf(stderr, "pseudoXOF_4x mismatch on lane %u\n", lane);
      return 1;
    }
  }

  if(init_random_number_4x(&vector, seed[0], seed[1], seed[2], seed[3],
                           DRNG_SEED_BYTES) != 0) {
    fprintf(stderr, "init_random_number_4x failed\n");
    return 1;
  }

  for(unsigned int lane = 0; lane < 4; ++lane) {
    if(init_random_number(&scalar[lane], seed[lane], DRNG_SEED_BYTES) != 0) {
      fprintf(stderr, "init_random_number failed in lane %u\n", lane);
      return 1;
    }
  }

  if(get_random_number_4x(&vector, drng_x4[0], drng_x4[1], drng_x4[2], drng_x4[3], 517) != 0) {
    fprintf(stderr, "get_random_number_4x failed\n");
    return 1;
  }

  for(unsigned int lane = 0; lane < 4; ++lane) {
    if(get_random_number(&scalar[lane], drng_ref[lane], 517) != 0) {
      fprintf(stderr, "get_random_number failed in lane %u\n", lane);
      return 1;
    }
    if(memcmp(drng_ref[lane], drng_x4[lane], sizeof(drng_ref[lane])) != 0) {
      fprintf(stderr, "get_random_number_4x mismatch on lane %u\n", lane);
      return 1;
    }
  }

  printf("OK: x4 helper checks passed\n");
  return 0;
}
...
static int test_signing(void) {
  ...
  printf("OK: sign/verify checks passed (%d tests)\n", SIGN_TESTS);
  return 0;
}
...
int main(void) {
  ...
  printf("OK: test_vectors-260602 passed\n");
  return 0;
}
