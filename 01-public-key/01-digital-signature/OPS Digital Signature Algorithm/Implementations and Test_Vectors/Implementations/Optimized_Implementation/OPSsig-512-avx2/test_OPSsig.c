...
int main(void) {
  ...
  for(i = 0; i < NTESTS; i++) {
    ...
    if(ret != 0) {
      fprintf(stderr, "ERROR: verification failed at i=%llu (ret=%d)\n",
              (unsigned long long)i, ret);
      fprintf(stderr, "INFO: seed=");
    ...
    ret = crypto_sign_verify(sig, siglen, m, MLEN, ctxp, ctxlen, pk);
    if(ret == 0) {
      fprintf(stderr, "ERROR: trivial forgeries possible\n");
      return 1;
    }
  }

  printf("INFO: CRYPTO_PUBLICKEYBYTES = %d\n", CRYPTO_PUBLICKEYBYTES);
  printf("INFO: CRYPTO_SECRETKEYBYTES = %d\n", CRYPTO_SECRETKEYBYTES);
  printf("INFO: CRYPTO_BYTES = %d\n", CRYPTO_BYTES);
  printf("OK: sign/verify test passed (%d iterations)\n", NTESTS);
  return 0;
}
