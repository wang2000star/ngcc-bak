...
void decompose_avx(__m256i *a1, __m256i *a0, const __m256i *a) {
  ...
  for(i = 0; i < N/8; ++i) {
    ...
    {
      const __m256i mul = _mm256_set1_epi64x(393241);
    ...
    f1 = _mm256_srli_epi32(f1, 8);
    {
      const __m256i mul = _mm256_set1_epi64x(786481);
      __m256i even = _mm256_mul_epu32(f1, mul);
      __m256i odd = _mm256_mul_epu32(_mm256_srli_epi64(f1, 32), mul);
      uint64_t even64[4];
      uint64_t odd64[4];
...
void use_hint_avx(__m256i *b, const __m256i *a, const __m256i *hint) {
  ...
  for(i = 0; i < N/8; ++i) {
    ...
    t = _mm256_cmpgt_epi32(f, zero);
    h = _mm256_blendv_epi32(_mm256_sub_epi32(zero, h), h, t);
    g = _mm256_add_epi32(g, h);
#if GAMMA2 == (Q-1)/32
    g = _mm256_and_si256(g, mask);
#elif GAMMA2 == (Q-1)/48
    {
      const __m256i max = _mm256_set1_epi32(23);
      __m256i neg = _mm256_cmpgt_epi32(zero, g);
      __m256i over = _mm256_cmpgt_epi32(g, max);
      g = _mm256_blendv_epi32(g, max, neg);
      g = _mm256_blendv_epi32(g, zero, over);
    }
#elif GAMMA2 == (Q-1)/96
    {
      const __m256i max = _mm256_set1_epi32(47);
      __m256i neg = _mm256_cmpgt_epi32(zero, g);
      __m256i over = _mm256_cmpgt_epi32(g, max);
      g = _mm256_blendv_epi32(g, max, neg);
      g = _mm256_blendv_epi32(g, zero, over);
