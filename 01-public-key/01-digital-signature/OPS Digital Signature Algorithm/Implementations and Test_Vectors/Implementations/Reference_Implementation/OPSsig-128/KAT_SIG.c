...
int main()
{
	...
	// Write the test vector into an implementation-isolated directory.
	const char *dir_name = "../../../Test_Vectors";
	char file_path[256] = "";
	snprintf(algoname_output, sizeof(algoname_output),
	         "KAT_SIG_%s-%s.txt", ALGORITHM_INSTANCE, IMPLEMENTATION_VARIANT);
	snprintf(dir_path, sizeof(dir_path), "%s/%s/%s",
	         dir_name, IMPLEMENTATION_VARIANT, ALGORITHM_INSTANCE);
	snprintf(file_path, sizeof(file_path), "%s/%s", dir_path, algoname_output);
	if (0 != create_directories(dir_path))
	{
		fprintf(stderr, "ERROR: Generate folder \"%s\" failed at %s, line %d. \n", dir_path, __FILE__, __LINE__);
