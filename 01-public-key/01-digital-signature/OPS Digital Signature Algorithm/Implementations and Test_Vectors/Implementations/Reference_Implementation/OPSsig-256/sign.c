                                          ...
                                          const uint8_t *sk)
{
  ...
  /* Tables for the smallpoly path — built once before the rej loop */
  ...
  /* Build packing tables once from time-domain s1/s2/t0 */
  prepare_s1_table_ops256(s1_table, &s1);
  prepare_s2_table_ops256(s2_table, &s2);
  prepare_t0_table_ops256(t0_table_lo, t0_table_hi, &t0);
...
rej:
  ...
  /* No poly_ntt(&cp): smallpoly consumes time-domain cp directly */

  /* Fused cs1+cs2: compute z=y+cs1 and w0prime=w0-cs2, early-check both */
  if(evaluate_cs1_cs2_early_check_ops256(&z, &w0prime, &cp,
                                          s1_table, s2_table,
                                          &y, &w0,
                                          GAMMA1 - BETA, GAMMA2 - BETA))
    goto rej;

  /* ct0 smallpoly: compute c*t0 into h, norm-check */
  evaluate_ct0_ops256(&h, &cp, t0_table_lo, t0_table_hi);
