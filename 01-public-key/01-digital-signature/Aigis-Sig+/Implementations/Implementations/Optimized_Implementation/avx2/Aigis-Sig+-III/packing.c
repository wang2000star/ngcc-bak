...
static void eta_2_unpack_avx2(uint8_t table[PARAM_N * 3], const uint8_t *a,
                              int eta) {
	const __m128i gather3 = _mm_setr_epi8(
		0, 1, 2, -1, 3, 4, 5, -1,
		6, 7, 8, -1, 9, 10, 11, -1);
	const __m256i shift = _mm256_setr_epi32(0, 3, 6, 9, 12, 15, 18, 21);
	const __m256i reorder = _mm256_setr_epi32(0, 4, 1, 5, 2, 6, 3, 7);
	const __m256i mask = _mm256_set1_epi32(0x07);
	const __m256i veta = _mm256_set1_epi8(eta);
	__m128i in;
	__m256i packed, secret, neg_secret, f0, f1, f2, f3, t0, t1;
	uint8_t *pos;

	for (int j = 0; j < PARAM_N / 32; ++j) {
		if (j + 1 < PARAM_N / 32) {
			in = _mm_loadu_si128((const __m128i *)(a + 12 * j));
		} else {
			in = _mm_loadl_epi64((const __m128i *)(a + 12 * j));
			in = _mm_or_si128(in, _mm_slli_si128(
				_mm_loadu_si32((const void *)(a + 12 * j + 8)), 8));
		}

		in = _mm_shuffle_epi8(in, gather3);
		packed = _mm256_broadcastsi128_si256(in);

		f0 = _mm256_permutevar8x32_epi32(packed, _mm256_set1_epi32(0));
		f1 = _mm256_permutevar8x32_epi32(packed, _mm256_set1_epi32(1));
		f2 = _mm256_permutevar8x32_epi32(packed, _mm256_set1_epi32(2));
		f3 = _mm256_permutevar8x32_epi32(packed, _mm256_set1_epi32(3));

		f0 = _mm256_and_si256(_mm256_srlv_epi32(f0, shift), mask);
		f1 = _mm256_and_si256(_mm256_srlv_epi32(f1, shift), mask);
		f2 = _mm256_and_si256(_mm256_srlv_epi32(f2, shift), mask);
		f3 = _mm256_and_si256(_mm256_srlv_epi32(f3, shift), mask);

		t0 = _mm256_packus_epi32(f0, f1);
		t1 = _mm256_packus_epi32(f2, f3);
		packed = _mm256_packus_epi16(t0, t1);
		packed = _mm256_permutevar8x32_epi32(packed, reorder);

		neg_secret = _mm256_sub_epi8(packed, veta);
		secret = _mm256_sub_epi8(veta, packed);

		pos = table + 32 * j;
		_mm256_storeu_si256((__m256i *)pos, neg_secret);
		_mm256_storeu_si256((__m256i *)(pos + PARAM_N),
		                    secret);
		_mm256_storeu_si256((__m256i *)(pos + 2 * PARAM_N), neg_secret);
	}
}

void s1_unpack(uint8_t s1_table[PARAM_N * 3], uint8_t *a) {
	const int32_t eta1x2 = 2 * ETA1;
#if ETA1 == 1
	eta_1_unpack_avx2(s1_table, a);
#elif ETA1 == 2
	eta_2_unpack_avx2(s1_table, a, ETA1);
#endif
}


void s2_unpack(s2Word s2_table[PARAM_N * 3], uint8_t *a ) {
	#if ETA2 == 5
...
#elif ETA2 == 2
	eta_2_unpack_avx2(s2_table, a, ETA2);
#endif
}
...
#elif PARAM_D == 14
	const __m256i permu = _mm256_set_epi32(0, 3, 2, 1, 3, 2, 1, 0);
	__m256i mask = _mm256_set1_epi32(0x3FFF);
