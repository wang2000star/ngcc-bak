...
static int SHAKE256_df(unsigned char *input_string, unsigned long long input_string_len_bytes)
{
    ...
    unsigned char *data_to_SHAKE256 = NULL;
    unsigned long long data_to_SHAKE256_len_bytes;
    unsigned char shake256_dgst[OUTLEN];
    ...
    {
        data_to_SHAKE256_len_bytes = sizeof(counter) + sizeof(number_of_bits_to_return_big_endian) + input_string_len_bytes;
        data_to_SHAKE256 = (unsigned char *)malloc(data_to_SHAKE256_len_bytes);
        if (NULL == data_to_SHAKE256)
        {
            fprintf(stderr, "ERROR: Memory allocation failed at %s, line %d. \n", __FILE__, __LINE__);
            return DRNG_MEMORY_ALLOCATION_FAILED;
        }
        data_to_SHAKE256[0] = counter;
        u32_to_u8_big_endian(8 * SEEDLEN, number_of_bits_to_return_big_endian);
        memcpy(data_to_SHAKE256 + sizeof(counter), number_of_bits_to_return_big_endian, sizeof(number_of_bits_to_return_big_endian));
        memcpy(data_to_SHAKE256 + sizeof(counter) + sizeof(number_of_bits_to_return_big_endian), input_string, input_string_len_bytes);
        shake256(shake256_dgst, OUTLEN, data_to_SHAKE256, data_to_SHAKE256_len_bytes);
        memcpy(temp + i * OUTLEN, shake256_dgst, OUTLEN);
        free(data_to_SHAKE256);
        data_to_SHAKE256 = NULL;
        counter++;
    }
