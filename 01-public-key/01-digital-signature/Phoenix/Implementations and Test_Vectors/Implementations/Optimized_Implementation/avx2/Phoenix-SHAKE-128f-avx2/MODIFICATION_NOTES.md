...
```c
#define PH_BYTES (PH_N + COUNTER_SIZE + PH_TFORS_SIG_MAX + PH_D * (PH_GWOTS_BYTES + COUNTER_SIZE) + \
                   PH_FULL_HEIGHT * PH_N)
```
...
```c
#define PH_BYTES (PH_N + COUNTER_SIZE + 2 + PH_TFORS_SIG_MAX + PH_D * (PH_GWOTS_BYTES + COUNTER_SIZE) + \
                   PH_FULL_HEIGHT * PH_N)
```
...
```c
*siglen = PH_N + COUNTER_SIZE + 2 + tfors_siglen + PH_D * (PH_GWOTS_BYTES + PH_TREE_HEIGHT * PH_N + COUNTER_SIZE);
```
...
{
    const size_t gwots_layer_len = PH_GWOTS_BYTES + PH_TREE_HEIGHT * PH_N + COUNTER_SIZE;
    const size_t fixed_siglen = PH_N + COUNTER_SIZE + 2 + (size_t)PH_D * gwots_layer_len;
