#ifndef YUANYANG_2048_SIGN_NTT_H
#define YUANYANG_2048_SIGN_NTT_H
...
typedef struct {
	/* Same polynomial transformed under Q1 and Q2 for CRT reconstruction. */
	uint32_t q1[YUANYANG_D];
...
typedef struct {
	/*
	 * Signing-only expanded key material.
	 *
	 * Plain int64_t arrays are coefficient-domain fixed-point values kept for
	 * documentation/debug use and for products that start from coefficients.
	 * The *_ntt arrays are the corresponding key-dependent transforms reused by
	 * Algorithm 6; unless stated otherwise by the helper name, those NTT arrays
	 * are stored in Montgomery representation.
	 */
	...
	yuanyang_sign_ntt_crt_poly basis_crt_ntt[YUANYANG_MAT2_POLYS];
...
typedef struct {
	/* Offline presampler output p = y + round(A_hat*x), still scaled by L. */
	int32_t p0_scaled[YUANYANG_D];
	int32_t p1_scaled[YUANYANG_D];
} yuanyang_sign_presample;

typedef struct {
	/* Ring buffer owned by the caller; the signing code only moves indices. */
