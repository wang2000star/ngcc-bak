	...
	const uint16_t challenge[YUANYANG_D])
{
	...
	const fpr q_scaled_inv = { 0x11107f7337e40000 };
	...
	prng *rng)
{
	 ...
	 * c1' = hat0 + u_hat*z2, and z2 is kept in NTT form for the later
...
materialize_lattice_point_and_delta2_error(
	int64_t lattice0[YUANYANG_D],
	int64_t lattice1[YUANYANG_D],
	int64_t delta2_err1[YUANYANG_D],
	const yuanyang_sign_ntt_precomp *ntt_precomp,
	...
	const uint16_t challenge[YUANYANG_D])
{
	...
	for (size_t u = 0; u < YUANYANG_D; u++) {
		delta2_err1[u] = lattice1[u] - (int64_t)challenge[u];
	}
}
...
delta2_probability_for_rejection(
	const yuanyang_sign_ntt_precomp *ntt_precomp,
	const int64_t lattice0[YUANYANG_D],
	const int64_t delta2_err1[YUANYANG_D])
{
	fpr_q62 delta2;
...
sample_lattice_point(
	int *accepted,
	yuanyang_sample_reject_reason *reject_reason,
	int64_t v0[YUANYANG_D], int64_t v1[YUANYANG_D],
	const yuanyang_sign_ntt_precomp *ntt_precomp,
	...
	prng *rng)
{
	...
	int64_t lattice0[YUANYANG_D], lattice1[YUANYANG_D];
	int64_t delta2_err1[YUANYANG_D];
	yuanyang_sign_ntt_crt_poly z2_ntt;
	...
	*accepted = 0;
	...
	memcpy(v0, lattice0, sizeof lattice0);
	memcpy(v1, lattice1, sizeof lattice1);
	*accepted = 1;
	return YUANYANG_SUCCESS;
}
...
derive_signature_halves(
	...
	const uint16_t challenge[YUANYANG_D],
	const int64_t v0[YUANYANG_D], const int64_t v1[YUANYANG_D])
{
	/*
	 ...
	 * verification from c, h, and s1.  Reject non-canonical s2 values before
	 * narrowing so the signing-side value matches what verification can
	 * reconstruct modulo q.
	 */
	for (size_t u = 0; u < YUANYANG_D; u++) {
		int64_t x1, x2;

		x1 = -v0[u];
		x2 = (int64_t)challenge[u] - v1[u];
		if (x1 < INT16_MIN || x1 > INT16_MAX
			|| x2 < -(int64_t)(YUANYANG_Q >> 1)
			|| x2 > (int64_t)(YUANYANG_Q >> 1))
		{
			return 0;
		}
	...
	yuanyang_sign_stats *stats)
{
	...
	for (unsigned attempt = 0; attempt < YUANYANG_SIGN_MAX_ATTEMPTS; attempt++) {
		int64_t v0[YUANYANG_D], v1[YUANYANG_D];
		...
		{
			accepted = derive_signature_halves(s1, s2, challenge, v0, v1);
		}
		if (!accepted) {
			if (stats != NULL) {
				stats->s2_range_reject++;
			}
			continue;
		}
