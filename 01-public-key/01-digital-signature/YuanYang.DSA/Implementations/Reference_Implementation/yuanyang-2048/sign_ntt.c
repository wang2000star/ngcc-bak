...
static void
dot_add_i64_i64(yy_u128 *pos, yy_u128 *neg, int64_t a, int64_t b)
{
	/*
...
{
	/*
	 * Inverse Q2 transform.  For 1024 and 2048, the final inverse butterfly is
	 * fused with multiplication by d^{-1}; this mirrors the shared Q1 inverse
	 * structure while keeping Q2 signing-only.
	 */
	...
	const int64_t F[YUANYANG_D], const int64_t G[YUANYANG_D])
{
	...
	crt_precompute_i64(&dst->basis_crt_ntt[YUANYANG_MAT2_00], f);
	crt_precompute_i64(&dst->basis_crt_ntt[YUANYANG_MAT2_01], F);
	crt_precompute_i64(&dst->basis_crt_ntt[YUANYANG_MAT2_10], g);
	crt_precompute_i64(&dst->basis_crt_ntt[YUANYANG_MAT2_11], G);
...
yuanyang_sign_ntt_basis_mul_with_z2_ntt(
	int64_t out0[YUANYANG_D],
	int64_t out1[YUANYANG_D],
	const yuanyang_sign_ntt_precomp *precomp,
	const int32_t z1[YUANYANG_D],
	const yuanyang_sign_ntt_crt_poly *z2_ntt)
{
	/* Compute B*(z1,z2) in the coefficient domain. */
	yuanyang_sign_ntt_crt_poly z1_ntt;

	crt_precompute_i32(&z1_ntt, z1);
	mul_crt_pretransformed_add2(out0,
		&precomp->basis_crt_ntt[YUANYANG_MAT2_00], &z1_ntt,
		&precomp->basis_crt_ntt[YUANYANG_MAT2_01], z2_ntt);
	mul_crt_pretransformed_add2(out1,
		&precomp->basis_crt_ntt[YUANYANG_MAT2_10], &z1_ntt,
		&precomp->basis_crt_ntt[YUANYANG_MAT2_11], z2_ntt);
}
...
{
	yuanyang_sign_ntt_crt_poly z2_ntt;

	crt_precompute_i32(&z2_ntt, z2);
...
yuanyang_sign_ntt_delta2_weight(
	const yuanyang_sign_ntt_precomp *precomp,
	const int64_t err0[YUANYANG_D],
	const int64_t err1[YUANYANG_D])
{
	/*
	...
	crt_precompute_i64(&err0_ntt, err0);
	crt_precompute_i64(&err1_ntt, err1);
	...
	dot_neg.lo = 0;
	for (unsigned i = 0; i < YUANYANG_D; i++) {
		dot_add_i64_i64(&dot_pos, &dot_neg, err0[i], action0[i]);
		dot_add_i64_i64(&dot_pos, &dot_neg, err1[i], action1[i]);
	}
