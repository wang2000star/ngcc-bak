...
void bigint_mul_scalar(bigint *res, const bigint *a, int64_t scalar)
{
    ...
    for (; i < n; i++) {
        ...
        __extension__ unsigned __int128 p = (unsigned __int128)ad[i] * us;
...
void bigint_mul_quadratic(bigint *r, const bigint *a, const bigint *b)
{
    ...
    for (size_t j = 0; j < bn; ++j) {
        ...
        for (; i < an; ++i) {
            ...
            uint64_t mid = (p00 >> 32) + (uint32_t)p01 + (uint32_t)p10;
            uint64_t zlo = (mid << 32) | (uint32_t)p00;
            uint64_t zhi = p11 + (p01 >> 32) + (p10 >> 32) + (mid >> 32);

            lo = zlo & LIMB_MASK;
            hi = (zlo >> BASE) | (zhi << (64 - BASE));
#endif
...
#if (defined(__GNUC__) || defined(__clang__)) && defined(__SIZEOF_INT128__)
            __extension__ unsigned __int128 p = (unsigned __int128)a->d[i] * b->d[j];
            hi = (uint64_t)(p >> BASE);
#else
            uint64_t x = a->d[i];
            uint64_t y = b->d[j];
