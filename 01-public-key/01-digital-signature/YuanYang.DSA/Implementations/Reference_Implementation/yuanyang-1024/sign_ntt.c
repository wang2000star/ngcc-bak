 ...
 *     the specification/report.md.  The full T matrix is not materialized.
 */
...
q2_norm_i64(int64_t x)
{
	...
	/* Q2NTT = 2^31 - 10239; three fixed folds cover int64_t. */
	ux = (uint64_t)x;
	sign = (uint64_t)0 - (uint64_t)(x < 0);
	mag = (ux ^ sign) - sign;
	for (unsigned i = 0; i < 3u; i++) {
		hi = mag >> 31;
		mag = (mag & UINT64_C(0x7FFFFFFF)) + hi * UINT64_C(10239);
	}
	r = (uint32_t)mag;
	d = r - Q2NTT;
