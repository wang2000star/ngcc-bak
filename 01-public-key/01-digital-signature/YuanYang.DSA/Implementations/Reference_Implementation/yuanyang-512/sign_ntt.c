 ...
 *
 * Scaling conventions used by the signing path:
 *   - A_hat is rounded to coefficient-domain integers at 2^22 scale.
 *   - u_hat is rounded at 2^11 scale.
 *   - B_hat^{-1} is stored as an adjugate numerator; the caller applies the
 *     q denominator and any presampler scale when converting back to fpr.
 *   - Delta2 uses G = Sigma_delta/sigma^2 at
 *     YUANYANG_SIGN_DELTA_NTT_SCALE_BITS and solves the required
 *     inverse-covariance action with the fixed Jacobi iteration described in
 *     the specification. The full T matrix is not materialized.
 */

#define YY_SIGN_NTT_Q1        UINT32_C(2147352577)
#define YY_SIGN_NTT_Q1_HALF   UINT32_C(1073676288)

#define YY_SIGN_NTT_Q1_INV_Q2_MONTY UINT32_C(181953727)

static uint32_t
q2_norm_i64(int64_t x)
{
	/*
	 * Reduce a signed coefficient to the residue format consumed by the local
	 * Q2 NTT. Q1 normalization stays in shared ntt.c; Q2 exists only for the
	 * signing CRT products in this file.
	 */
...
{
	/*
	 * Accumulate a signed dot product as separate positive and negative
	 * magnitudes. Delta2 can need more headroom than int64_t before the final
	 * sign is resolved.
	 */
	uint64_t sign;
	uint64_t aa;
...
{
	/*
	 * Q2 uses the same direct negacyclic layout as shared qntt.c. The stage
	 * list is parameter-set specific and deliberately unrolled so the reference
	 * code shows the transform depth.
	 */
#define Q2_FORWARD_STAGE(M) do {                              \
		const unsigned ht = YUANYANG_D / (2u * (M));          \
		for (unsigned i = 0, j1 = 0; i < (M);                 \
			i++, j1 += 2u * ht)                               \
		{                                                     \
...
#define Q2_INVERSE_STAGE(HM) do {                              \
	...
	/*
	 * This 512 path keeps every inverse butterfly explicit, then applies the
	 * Montgomery normalization to all coefficients. The larger parameter-set
	 * folders use an equivalent fused final stage.
	 */
	switch (YUANYANG_LOGD) {
	case 11:
		Q2_INVERSE_STAGE(1u << 10); /* fall through */
	case 10:
		Q2_INVERSE_STAGE(1u << 9);  /* fall through */
	case 9:
		Q2_INVERSE_STAGE(1u << 8);  /* fall through */
	case 8:
		Q2_INVERSE_STAGE(1u << 7);  /* fall through */
	case 7:
		Q2_INVERSE_STAGE(1u << 6);  /* fall through */
	case 6:
		Q2_INVERSE_STAGE(1u << 5);  /* fall through */
	case 5:
		Q2_INVERSE_STAGE(1u << 4);  /* fall through */
	case 4:
		Q2_INVERSE_STAGE(1u << 3);  /* fall through */
	case 3:
		Q2_INVERSE_STAGE(1u << 2);  /* fall through */
	case 2:
		Q2_INVERSE_STAGE(1u << 1);  /* fall through */
	case 1:
		Q2_INVERSE_STAGE(1u << 0);  /* fall through */
	default:
		break;
	}
...
#undef Q2_INVERSE_STAGE
	...
	for (unsigned i = 0; i < YUANYANG_D; i++) {
		a[i] = q2_montymul(a[i], Q2NTT_INV_N_MONTY);
	}
}
...
{
	/* Import normal residues with a*R^2/R = a*R (mod Q2NTT). */
	for (unsigned i = 0; i < YUANYANG_D; i++) {
...
q1_muladd_montgomery_pretransformed_residue(
	...
	const uint32_t b1_ntt[YUANYANG_D])
{
	qntt_pointwise_montgomery_muladd_to(out,
...
q1_muladd_montgomery_pretransformed_centered(
	int64_t out[YUANYANG_D],
	const uint32_t a0_ntt[YUANYANG_D],
	const uint32_t b0_ntt[YUANYANG_D],
	const uint32_t a1_ntt[YUANYANG_D],
	const uint32_t b1_ntt[YUANYANG_D])
{
	uint32_t residue[YUANYANG_D];

	q1_muladd_montgomery_pretransformed_residue(residue, a0_ntt, b0_ntt,
		a1_ntt, b1_ntt);
	qntt_center_i64(out, residue);
}
	...
	const uint32_t b_ntt[YUANYANG_D])
{
	...
	q1_mul_montgomery_pretransformed_residue(residue,
		a_ntt_montgomery, b_ntt);
	qntt_center_i64(out, residue);
}
	...
	const int32_t b[YUANYANG_D])
{
	...
	qntt_forward(b_ntt);
	{
		...
		q1_mul_montgomery_pretransformed_residue(residue, a_ntt, b_ntt);
		qntt_center_i64(out, residue);
	}
}
	...
	const yuanyang_sign_ntt_crt_poly *b_ntt)
{
	...
	q1_mul_montgomery_pretransformed_residue(r1,
		a_ntt->q1, b_ntt->q1);
	q2_mul_montgomery_pretransformed_residue(r2,
		a_ntt->q2, b_ntt->q2);
	crt_combine_residues(out, r1, r2);
}
	...
	const yuanyang_sign_ntt_crt_poly *b1_ntt)
{
	...
	q1_muladd_montgomery_pretransformed_residue(r1,
		a0_ntt->q1, b0_ntt->q1,
		a1_ntt->q1, b1_ntt->q1);
	q2_muladd_montgomery_pretransformed_residue(r2,
		a0_ntt->q2, b0_ntt->q2,
		a1_ntt->q2, b1_ntt->q2);
	crt_combine_residues(out, r1, r2);
}
...
{
	/*
	 * Drop fixed-point fractional bits with round-to-nearest-even, matching
	 * the fpr trace generation used to validate these scaled products.
	 */
	uint64_t neg;
	uint64_t mag;
	uint64_t q;
...
/*
 * Multiply two fixed-point polynomials and round back by `bits` fractional
 * bits. Delta2 uses this after CRT reconstruction so the fixed-point scale is
 * kept stable across the Jacobi iterations.
 */
#if YUANYANG_SIGN_NTT_DEBUG
static void
poly_mul_crt_scaled(
...
{
	/*
	 * Precompute the truncated correction
	 *   g + g^2 + ... + g^k
	 * used as (I - g)^-1 - I in the diagonal blocks of the Delta2 Jacobi
	 * solve. The identity term is applied by
		...
		iter++)
	{
		...
		/* The power after the final series term is never consumed. */
...
{
	/*
	 * Apply (I + corr)*in, where corr is the truncated series above. In the
	 * Delta2 recurrence this approximates the inverse of each diagonal block
	 * without materializing a dense T matrix.
	 */
...
static void
load_scaled_precompute_inputs(
	yuanyang_sign_ntt_precomp *dst,
	const yuanyang_expanded_sk *expanded,
	int64_t f[YUANYANG_D], int64_t g[YUANYANG_D],
	int64_t F[YUANYANG_D], int64_t G[YUANYANG_D])
{
	/*
	 * First convert expanded-key fpr values to the fixed-point integer scales
	 * consumed by the signing NTT products.
	 */
	for (unsigned i = 0; i < YUANYANG_D; i++) {
...
static void
precompute_basis_and_u_hat(
	yuanyang_sign_ntt_precomp *dst,
	int64_t uf_q11[YUANYANG_D], int64_t ug_q11[YUANYANG_D],
	const int64_t f[YUANYANG_D], const int64_t g[YUANYANG_D],
	const int64_t F[YUANYANG_D], const int64_t G[YUANYANG_D])
{
	/*
	 * Algorithm 6 needs B*(z1,z2) and u_hat*z2 during every attempt. Build
	 * the single-prime Q1 NTT tables once, then compute u_hat*f and u_hat*g
	 * before importing the reusable tables into Montgomery form.
	 */
	...
	/*
	 * Subsequent products reuse these key transforms. Import them once into
	 * Montgomery form so each pointwise product needs one reduction, not two.
	 */
...
static void
precompute_a_hat_tables(yuanyang_sign_ntt_precomp *dst)
{
	/* Presampling uses A_hat*x and needs the wider CRT product path. */
...
static void
precompute_b_hat_inverse_tables(
	yuanyang_sign_ntt_precomp *dst,
	const yuanyang_expanded_sk *expanded,
	const int64_t uf_q11[YUANYANG_D],
	const int64_t ug_q11[YUANYANG_D])
{
	 ...
	 * applied when the caller converts the exact numerator back to fpr. The
	 * bottom row is just (-g, f), while the top row uses F_hat and G_hat,
	 * including the u_hat*f and u_hat*g corrections.
	 */
	for (unsigned i = 0; i < YUANYANG_D; i++) {
		int64_t F_hat_q11;
		int64_t G_hat_q11;
...
static void
precompute_delta2_tables(yuanyang_sign_ntt_precomp *dst)
{
	/*
	 * Delta2 uses G = Sigma_delta/sigma^2. Store both the CRT NTT forms used
	 * by the Jacobi products and the truncated diagonal inverse corrections.
	 */
	for (unsigned j = 0; j < YUANYANG_MAT2_POLYS; j++) {
...
{
	/*
	 * Convert the key-dependent signing products to the coefficient-domain
	 * fixed-point integers consumed by the online sampler, then precompute
	 * their Q1 or CRT NTT forms. This keeps the online Algorithm 6 path close
	 * to the spec: presampling uses A_hat, lattice-center recovery uses
	 * B_hat^{-1}, lattice materialization uses B, and Delta2 uses
	 * Sigma_delta/sigma^2.
	 */
	...
	load_scaled_precompute_inputs(dst, expanded, f, g, F, G);
	precompute_basis_and_u_hat(dst, uf_q11, ug_q11, f, g, F, G);
	precompute_a_hat_tables(dst);
	precompute_b_hat_inverse_tables(dst, expanded, uf_q11, ug_q11);
	precompute_delta2_tables(dst);
}
	...
	const int32_t err1[YUANYANG_D])
{
	 ...
	 * product is converted back to the default fpr format. This computes the
	 * action t = (I - G)^-1 G err needed for the exponent err^T t / 2; the
	 * reference intentionally avoids storing the obsolete materialized T matrix.
	 */
	...
	if (dot_cmp <= 0) {
		...
		abs_weight_raw = u128_pow2_scale_rne(dot_mag,
			YUANYANG_FPR_FRAC_BITS + YUANYANG_LOGD - 1u,
			YUANYANG_SIGN_DELTA_NTT_SCALE_BITS);
	...
	weight.v = u128_pow2_scale_rne(dot_mag,
		YUANYANG_FPR_FRAC_BITS + YUANYANG_LOGD - 1u,
		YUANYANG_SIGN_DELTA_NTT_SCALE_BITS);
#if YUANYANG_SIGN_NTT_DEBUG
	printf("ntt_weight: %lu (%lu)\n",
		(unsigned long)weight.v,
		(unsigned long)(weight.v >> YUANYANG_FPR_FRAC_BITS));
#endif
	return weight;
}
