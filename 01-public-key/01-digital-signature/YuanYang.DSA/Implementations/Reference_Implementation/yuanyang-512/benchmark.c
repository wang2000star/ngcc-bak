/*
 * Local signature self-assessment benchmark.
 *
 * This exercises the submission-facing keygen/sign/verify API in the order
 * expected by the performance-guideline workflow.  It is deliberately outside
 * the implementation core: timing, CSV output, and memory accounting stay here
 * so the reference signing code remains focused on the specification.
 */
...
main(int argc, char **argv)
{
	...
	/*
	 * Each run creates one key, signs the fixed benchmark message, then
	 * verifies that signature.  The software DRNG is reseeded for keygen and
	 * signing so the measurement sequence is replayable under KAT-style builds.
	 */
	for (i = 0; i < runs; i++) {
