 ...
 *
 * Parameter-set-specific roots and Montgomery constants are provided by
 * ntttable.h.
 *
 ...
 */
void
qntt_forward_partial(uint32_t a[YUANYANG_D], int logn)
{
    unsigned n;
 ...
 */
void
qntt_inverse_partial(uint32_t a[YUANYANG_D], int modq, int logn)
{
    unsigned n;
...
#pragma omp simd
	for (unsigned i = 0; i < YUANYANG_D; i++) {
		...
		p0 = qntt_montymul(a0_montgomery[i], b0[i]);
		p1 = qntt_montymul(a1_montgomery[i], b1[i]);
		out[i] = qntt_add(p0, p1);
	}
}
