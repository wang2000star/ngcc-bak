/*
 * yuanyang-1024 reference byte encodings.
 *
 *   PK = q || encode(h),
...
#error The embedded yuanyang-1024 rANS model assumes rejection bound 4317.
#endif
 ...
 * quantization and tail-smoothing overhead seen with the yy-512 2^12 model,
...
// 	}
// 	off = 0;
// 	yuanyang_decode_uniform(decoded->h, YUANYANG_D, YUANYANG_Q,
// 		YUANYANG_PK_BLOCK_COEFFS, sk + off);
// 	off += YUANYANG_H_BYTES;
// 	for (size_t i = 0; i < YUANYANG_D; i++) {
// 		if (decoded->h[i] >= YUANYANG_Q) {
// 			return YUANYANG_BADARG;
// 		}
// 	}
// 	for (size_t i = 0; i < YUANYANG_D; i++, off += 1u) {
// 		decoded->f[i] = load_i8(sk + off);
// 	}
// 	return YUANYANG_SUCCESS;
// }
