...
{
#define Q2INTT_STAGE(HM) do {                                            \
        enum { T = YUANYANG_D / (2u * (HM)) };                           \
        enum { DT = 2u * T };                                            \
        for (unsigned i = 0, j1 = 0; i < (HM); i++, j1 += DT) {           \
            uint32_t s = q2ntt_iGMb[(HM) + i];                           \
            unsigned j2 = j1 + T;                                        \
            _Pragma("omp simd")                                          \
            for (unsigned j = j1; j < j2; j++) {                         \
                uint32_t u = a[j];                                       \
                uint32_t v = a[j + T];                                   \
                a[j]     = q2_add(u, v);                                 \
                a[j + T] = q2_montymul(q2_sub(u, v), s);                 \
            }                                                            \
        }                                                                \
    } while (0)

    switch (YUANYANG_LOGD) {
    case 11: Q2INTT_STAGE(1u << 10); /* fall through */
    case 10: Q2INTT_STAGE(1u << 9);  /* fall through */
    case 9:  Q2INTT_STAGE(1u << 8);  /* fall through */
    case 8:  Q2INTT_STAGE(1u << 7);  /* fall through */
    case 7:  Q2INTT_STAGE(1u << 6);  /* fall through */
    case 6:  Q2INTT_STAGE(1u << 5);  /* fall through */
    case 5:  Q2INTT_STAGE(1u << 4);  /* fall through */
    case 4:  Q2INTT_STAGE(1u << 3);  /* fall through */
    case 3:  Q2INTT_STAGE(1u << 2);  /* fall through */
    case 2:  Q2INTT_STAGE(1u << 1);  /* fall through */
    case 1:  Q2INTT_STAGE(1u << 0);  /* fall through */
    default:
        ;
    }
