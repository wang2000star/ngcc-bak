...
/*************************************************
* Name:        shake384_absorb
**************************************************/
void shake384_absorb(keccak_state *state, const uint8_t *in, size_t inlen)
{
  state->pos = keccak_absorb(state->s, state->pos, SHAKE384_RATE, in, inlen);
}

/*************************************************
* Name:        shake384_finalize
**************************************************/
void shake384_finalize(keccak_state *state)
{
  /* 0x1F is SHAKE domain separation */
  keccak_finalize(state->s, state->pos, SHAKE384_RATE, 0x1F);
  state->pos = SHAKE384_RATE;
}

/*************************************************
* Name:        shake384_squeeze
**************************************************/
void shake384_squeeze(uint8_t *out, size_t outlen, keccak_state *state)
{
  state->pos = keccak_squeeze(out, outlen, state->s, state->pos, SHAKE384_RATE);
}

/*************************************************
* Name:        shake384_absorb_once
**************************************************/
void shake384_absorb_once(keccak_state *state, const uint8_t *in, size_t inlen)
{
  keccak_absorb_once(state->s, SHAKE384_RATE, in, inlen, 0x1F);
  state->pos = SHAKE384_RATE;
}

/*************************************************
* Name:        shake384_squeezeblocks
**************************************************/
void shake384_squeezeblocks(uint8_t *out, size_t nblocks, keccak_state *state)
{
  keccak_squeezeblocks(out, nblocks, state->s, SHAKE384_RATE);
}
...
{
  state->pos = keccak_absorb(state->s, state->pos, SHAKE512_RATE, in, inlen);
}

/*************************************************
* Name:        shake512_finalize
**************************************************/
void shake512_finalize(keccak_state *state)
{
  /* 0x1F is SHAKE domain separation */
  keccak_finalize(state->s, state->pos, SHAKE512_RATE, 0x1F);
  state->pos = SHAKE512_RATE;
}

/*************************************************
* Name:        shake512_squeeze
**************************************************/
void shake512_squeeze(uint8_t *out, size_t outlen, keccak_state *state)
{
  state->pos = keccak_squeeze(out, outlen, state->s, state->pos, SHAKE512_RATE);
}

/*************************************************
* Name:        shake512_absorb_once
**************************************************/
void shake512_absorb_once(keccak_state *state, const uint8_t *in, size_t inlen)
{
  keccak_absorb_once(state->s, SHAKE512_RATE, in, inlen, 0x1F);
  state->pos = SHAKE512_RATE;
}
...
{
  keccak_squeezeblocks(out, nblocks, state->s, SHAKE512_RATE);
}
...
void shake384(uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen)
{
  ...
  shake384_absorb_once(&state, in, inlen);
  nblocks = outlen / SHAKE384_RATE;
  shake384_squeezeblocks(out, nblocks, &state);
  outlen -= nblocks * SHAKE384_RATE;
  out += nblocks * SHAKE384_RATE;
  shake384_squeeze(out, outlen, &state);
}
...
void shake512(uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen)
{
  ...
  shake512_absorb_once(&state, in, inlen);
  nblocks = outlen / SHAKE512_RATE;
  shake512_squeezeblocks(out, nblocks, &state);
  outlen -= nblocks * SHAKE512_RATE;
  out += nblocks * SHAKE512_RATE;
  shake512_squeeze(out, outlen, &state);
}
