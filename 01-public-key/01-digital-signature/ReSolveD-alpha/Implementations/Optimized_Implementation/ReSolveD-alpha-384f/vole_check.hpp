...
template <typename P>
static void load_vole_check_hash(const uint8_t* in, block_secpar<P::secpar_v> out[2])
{
    constexpr size_t lambda_bytes = P::secpar_bytes;
    constexpr size_t hash_bytes = CONSTANTS<P>::VOLE_CHECK::HASH_BYTES;

    out[0] = block_secpar<P::secpar_v>::set_zero();
    out[1] = block_secpar<P::secpar_v>::set_zero();
    memcpy(&out[0], in, lambda_bytes);
    memcpy(&out[1], in + lambda_bytes, hash_bytes - lambda_bytes);
}

template <typename P>
static void store_vole_check_hash(const block_secpar<P::secpar_v> in[2], uint8_t* out)
{
    constexpr size_t lambda_bytes = P::secpar_bytes;
    constexpr size_t hash_bytes = CONSTANTS<P>::VOLE_CHECK::HASH_BYTES;

    memcpy(out, &in[0], lambda_bytes);
    memcpy(out + lambda_bytes, &in[1], hash_bytes - lambda_bytes);
}
                ...
                uint8_t* __restrict__ proof, hash_state& hasher)
{
    ...
    if (verifier)
        load_vole_check_hash<P>(proof, u_hash);
    ...
    for (int col = -!verifier; col < static_cast<ssize_t>(P::delta_bits_v); ++col)
    {
        ...
        SIG_ASSERT(i == CP::QUICKSILVER_ROWS - (CP::QUICKSILVER_ROWS % P::secpar_bits));
        ...
        for (i = 0; i + 64 <= CP::QUICKSILVER_ROWS; i += 64)
            state_64.update(&chal.hasher_key_64, poly64::load(to_hash + i / 8));

        if (CP::QUICKSILVER_ROWS % 64)
        {
            uint8_t last_chunk[sizeof(uint64_t)] = {};
            memcpy(last_chunk, to_hash + i / 8, (CP::QUICKSILVER_ROWS - i) / 8);
            state_64.update(&chal.hasher_key_64, poly64::load(last_chunk));
        }
        ...
        load_vole_check_hash<P>(to_hash + CP::QUICKSILVER_ROWS / 8, mask);
        for (size_t j = 0; j < 2; ++j)
        ...
        uint8_t hash_output_bytes[CP::VOLE_CHECK::HASH_BYTES];
        store_vole_check_hash<P>(hash_output, hash_output_bytes);
        hasher.update(hash_output_bytes, sizeof(hash_output_bytes));
    }
    ...
    if (!verifier)
        store_vole_check_hash<P>(u_hash, proof);
}
