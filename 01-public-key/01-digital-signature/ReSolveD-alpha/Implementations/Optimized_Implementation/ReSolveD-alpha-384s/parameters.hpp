...
constexpr unsigned int owf_algo_rsd = 3;
...
enum class owf : unsigned int
{
    ...
    rsd = owf_algo_rsd << owf_algo_shift,
...
constexpr bool is_owf_with_rsd(owf o)
{
    return (static_cast<std::underlying_type_t<owf>>(o) >> owf_algo_shift) == owf_algo_rsd;
}
...
constexpr bool is_owf_with_norm_proof(owf o)
{
    return static_cast<std::underlying_type_t<owf>>(o) & owf_flag_norm_proof;
}
...
constexpr bool is_owf_with_ctr_input(owf o)
{
    return static_cast<std::underlying_type_t<owf>>(o) & owf_flag_ctr_input;
}
...
template <secpar S, uint32_t MAX_TWEAKS> struct leaf_hash_type<S, MAX_TWEAKS, leaf_hash::tccr_prg>
{
    static_assert(S == secpar::s384, "TCCR leaf PRG is defined for CSP 384");
    using type = prg_leaf_hash<ref_tccr_prg<S>>;
};
template <secpar S, uint32_t MAX_TWEAKS>
struct leaf_hash_type<S, MAX_TWEAKS, leaf_hash::shacal2_prg>
{
    static_assert(S == secpar::s512, "SHACAL-2 leaf PRG is defined for CSP 512");
    using type = prg_leaf_hash<ref_shacal2_prg<S>>;
};
template <secpar S, uint32_t MAX_TWEAKS> struct leaf_hash_type<S, MAX_TWEAKS, leaf_hash::shake>
{
...
namespace resolved_alpha
{

using resolved_alpha_160_s = parameter_set<secpar::s160, 14, owf::rsd, prg::aes_ctr, prg::tccr_hash,
                                           leaf_hash::aes_ctr, 6, {bavc::one_tree, 129}>;
using resolved_alpha_160_f = parameter_set<secpar::s160, 21, owf::rsd, prg::aes_ctr, prg::tccr_hash,
                                           leaf_hash::aes_ctr, 8, {bavc::one_tree, 139}>;

using resolved_alpha_256_s = parameter_set<secpar::s256, 22, owf::rsd, prg::aes_ctr, prg::tccr_hash,
                                           leaf_hash::aes_ctr, 12, {bavc::one_tree, 224}>;
using resolved_alpha_256_f = parameter_set<secpar::s256, 35, owf::rsd, prg::aes_ctr, prg::tccr_hash,
                                           leaf_hash::aes_ctr, 8, {bavc::one_tree, 223}>;

using resolved_alpha_384_s =
    parameter_set<secpar::s384, 34, owf::rsd, prg::ref_tccr, prg::tccr_hash, leaf_hash::tccr_prg,
                  10, {bavc::one_tree, 332}>;
using resolved_alpha_384_f =
    parameter_set<secpar::s384, 53, owf::rsd, prg::ref_tccr, prg::tccr_hash, leaf_hash::tccr_prg, 9,
                  {bavc::one_tree, 336}>;

using resolved_alpha_512_s =
    parameter_set<secpar::s512, 46, owf::rsd, prg::ref_shacal2, prg::tccr_hash,
                  leaf_hash::shacal2_prg, 6, {bavc::one_tree, 439}>;
using resolved_alpha_512_f =
    parameter_set<secpar::s512, 72, owf::rsd, prg::ref_shacal2, prg::tccr_hash,
                  leaf_hash::shacal2_prg, 8, {bavc::one_tree, 447}>;

} // namespace resolved_alpha

#define ALL_RESOLVED_ALPHA_INSTANCES                                                               \
    resolved_alpha::resolved_alpha_160_s, resolved_alpha::resolved_alpha_160_f,                    \
        resolved_alpha::resolved_alpha_256_s, resolved_alpha::resolved_alpha_256_f,                \
        resolved_alpha::resolved_alpha_384_s, resolved_alpha::resolved_alpha_384_f,                \
        resolved_alpha::resolved_alpha_512_s, resolved_alpha::resolved_alpha_512_f
