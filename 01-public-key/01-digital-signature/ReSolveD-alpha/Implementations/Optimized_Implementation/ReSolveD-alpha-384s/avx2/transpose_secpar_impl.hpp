                                               ...
                                               unsigned int i_start, unsigned int i, size_t j_chunk)
{
        ...
        for (size_t j_unrotated = 0; j_unrotated < TRANSPOSE_CHUNK_SIZE; ++j_unrotated)
    {
        ...
        else
        {
            ...
            size_t used_bytes = S == secpar::s192 ? sizeof(block192) :
                                S == secpar::s160 ? sizeof(block160) :
                                sizeof(block256);
...
void transpose_secpar(const void* input, void* output, size_t stride, size_t rows)
{
    ...
    // For s160: sizeof(block160)=24 ≠ secpar_bits(160)=20. The AVX2 transpose path
    // (transposeVxN) uses sizeof(block_secpar) for output elements. Compute the
    // correct byte offset per chunk.
    constexpr size_t out_factor = S == secpar::s160
        ? (TRANSPOSE_BITS_COLS<S> / (TRANSPOSE_BITS_ROWS / 8)) * sizeof(block160)
        : secpar_to_bits(S);
    for (size_t i = 0; i < rows / 8; i += TRANSPOSE_BITS_ROWS / 8)
    {
        if constexpr (S == secpar::s384 || S == secpar::s512)
            transposeVxN_scalar<S>(in + i, out + i * out_factor, stride);
        else
            transposeVxN<S>(in + i, out + i * out_factor, stride);
    }
}
