...
constexpr std::size_t FIXED_KEY_PREFERRED_WIDTH_SHIFT = []
{
    ...
    {
        static_assert(dependent_false<S>, "unsupported security parameter for fixed-key AES");
    }
}();
template <secpar S>
...
template <secpar S, owf O>
    requires(is_owf_with_rsd(O))
struct OWF_CONSTANTS<S, O>
{
    constexpr static std::size_t RSD_BLOCK_SIZE = 6;
    constexpr static std::size_t RSD_PACKED_BLOCK_BITS = RSD_BLOCK_SIZE - 1;

    constexpr static std::size_t RSD_CODE_LENGTH = []
    {
        if constexpr (S == secpar::s160)
            ...
            static_assert(dependent_false<S>, "unsupported security parameter for RSD OWF");
    }();

    constexpr static std::size_t RSD_DIMENSION = []
    {
        if constexpr (S == secpar::s160)
            return 1056;
        else if constexpr (S == secpar::s256)
    ...
    constexpr static std::size_t RSD_NOISE_WEIGHT = []
    {
        if constexpr (S == secpar::s160)
            return 310;
        else if constexpr (S == secpar::s256)
    ...
    constexpr static std::size_t RSD_SYNDROME_BITS = RSD_CODE_LENGTH - RSD_DIMENSION;
    static_assert(RSD_SYNDROME_BITS % RSD_BLOCK_SIZE == 0);
    constexpr static std::size_t RSD_LINEAR_CHECK_BLOCKS = RSD_SYNDROME_BITS / RSD_BLOCK_SIZE;
    constexpr static std::size_t RSD_LINEAR_CHECK_CONSTRAINTS =
        (RSD_LINEAR_CHECK_BLOCKS + secpar_to_bits(S) - 1) / secpar_to_bits(S);
    constexpr static std::size_t RSD_WITNESS_BITS =
        (RSD_DIMENSION / RSD_BLOCK_SIZE) * RSD_PACKED_BLOCK_BITS;

    constexpr static std::size_t OWF_ROUNDS = 0;
    constexpr static std::size_t OWF_BLOCK_SIZE = (RSD_SYNDROME_BITS + 7) / 8;
    constexpr static std::size_t OWF_BLOCKS = 1;
    constexpr static std::size_t OWF_OUTPUT_BYTES = OWF_BLOCK_SIZE;
    constexpr static std::size_t OWF_KEY_SCHEDULE_SBOXES = 0;
    constexpr static std::size_t OWF_KEY_SCHEDULE_CONSTRAINTS = 0;
    constexpr static std::size_t OWF_KEY_WITNESS_BITS = 0;
    constexpr static std::size_t OWF_ENC_SBOXES = 0;
    constexpr static std::size_t OWF_ENC_CONSTRAINTS =
        RSD_NOISE_WEIGHT + RSD_LINEAR_CHECK_CONSTRAINTS;
    constexpr static std::size_t OWF_ENC_WITNESS_BITS_PER_BLOCK = ((RSD_WITNESS_BITS + 7) / 8) * 8;
    constexpr static std::size_t OWF_ENC_WITNESS_BITS = OWF_ENC_WITNESS_BITS_PER_BLOCK;
    constexpr static std::size_t OWF_NUM_CONSTRAINTS = OWF_ENC_CONSTRAINTS;
    constexpr static std::size_t WITNESS_BITS = OWF_ENC_WITNESS_BITS;
