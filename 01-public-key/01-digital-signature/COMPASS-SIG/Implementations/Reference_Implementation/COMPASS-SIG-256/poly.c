...
* Name:        polyw1_pack
*
* Description: Bit-pack polynomial w1. Extracts low
*              POLYW1_BITS bits from each coefficient
*              and packs them densely.
*
* Arguments:   - uint8_t *r: output byte array
...
void polyeta_pack(uint8_t *r, const poly *a, int8_t eta) {
  ...
  if (eta == 1) {
    /* ETA=1: 2 bits per coefficient, 4 per byte */
    for(i = 0; i < N/4; ++i) {
    ...
    /* ETA=2: 3 bits per coefficient, 8 per 3 bytes */
    for(i = 0; i < N/8; ++i) {
...
void polyeta_unpack(poly *r, const uint8_t *a, int8_t eta) {
  ...
  if (eta == 1) {
    /* eta=1: unpack: 1 byte -> 4 coefficients */
    for(i = 0; i < N/4; ++i) {
    ...
    /* eta=2: unpack: 3 bytes -> 8 coefficients */
    for(i = 0; i < N/8; ++i) {
...
#elif Z_BITS == 19
  /* 19 bit: 8 coefficients -> 19 bytes */
  for(i = 0; i < N/8; ++i) {
