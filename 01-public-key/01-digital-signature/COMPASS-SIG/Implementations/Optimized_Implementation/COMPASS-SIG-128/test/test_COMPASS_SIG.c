...
/* DRNG context needed by sign.c:crypto_sign_keypair (initialized via randombytes) */
...
int main(void)
{
  ...
  /* Initialize DRNG with random seed for testing */
  uint8_t drng_seed[64];
  randombytes(drng_seed, sizeof(drng_seed));
  init_random_number(&drng_algorithm, drng_seed, sizeof(drng_seed));

  snprintf((char*)ctx,CTXLEN,"test_COMPASS_SIG");
