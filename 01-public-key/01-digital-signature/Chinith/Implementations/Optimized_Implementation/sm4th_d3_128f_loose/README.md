# sm4th-d3-128f-loose - Optimized Implementation

Optimized implementation of **sm4th-d3-128f-loose** with x86-oriented acceleration.

Protocol parameters follow `SM4th-d3-128f-loose`:
`lambda=128`, `lambda_f=128`, `lambda_iv=128`, `ell=1808`, `tau=16`,
`w_grind=8`, `T_open=110`, `B=16`, `d=3`. The loose variant keeps SM4 as
the PRG, uses a 128-bit IV, and its d3 leaf PRG path emits 276 bytes.
...
├── sm4th_d3_128f_loose.c / sm4th_d3_128f_loose.h      # Public API wrapper
├── sm4th_d3_128f_loose_AlgorithmInstance.c / .h # Algorithm instance entry points
├── sm4th_d3_128f_loose_bench.c                  # Benchmark entry
...
```bash
make sm4th_d3_128f_loose_bench
```
...
```bash
make clean
make PRG_ACCE=0 HYGON_SM4_ACCE=1 KAT_SIG sm4th_d3_128f_loose_bench
```
...
```bash
./sm4th_d3_128f_loose_bench
./sm4th_d3_128f_loose_bench 100
./sm4th_d3_128f_loose_bench 100 8
```

`./sm4th_d3_128f_loose_bench` runs 100 fixed-56-byte sign/verify iterations by default.
...
```bash
bash Chinith/check_kat_hash.sh --variant sm4th_d3_128f_loose
```
