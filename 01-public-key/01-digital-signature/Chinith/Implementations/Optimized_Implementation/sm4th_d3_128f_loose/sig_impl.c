...
#if defined(__x86_64__)
  const __m128i mm0 = _mm_set1_epi8((char)m0);
  const __m128i mm1 = _mm_set1_epi8((char)m1);
  const __m128i mm2 = _mm_set1_epi8((char)m2);
  const __m128i mm3 = _mm_set1_epi8((char)m3);
  size_t j = 0;
  for (; j + 16u <= utilde_bytes; j += 16u) {
    const __m128i u = _mm_loadu_si128((const __m128i*)(u_tilde + j));
    __m128i d0 = _mm_loadu_si128((const __m128i*)(q0 + j));
    __m128i d1 = _mm_loadu_si128((const __m128i*)(q1 + j));
    __m128i d2 = _mm_loadu_si128((const __m128i*)(q2 + j));
    __m128i d3 = _mm_loadu_si128((const __m128i*)(q3 + j));
    d0 = _mm_xor_si128(d0, _mm_and_si128(u, mm0));
    d1 = _mm_xor_si128(d1, _mm_and_si128(u, mm1));
    d2 = _mm_xor_si128(d2, _mm_and_si128(u, mm2));
    d3 = _mm_xor_si128(d3, _mm_and_si128(u, mm3));
    _mm_storeu_si128((__m128i*)(q0 + j), d0);
    _mm_storeu_si128((__m128i*)(q1 + j), d1);
    _mm_storeu_si128((__m128i*)(q2 + j), d2);
    _mm_storeu_si128((__m128i*)(q3 + j), d3);
  }
  for (; j < utilde_bytes; ++j) {
...
static bool pseudoXOF_leq256_byte_aligned_local(unsigned long long output_len_bits,
  ...
  uint8_t digest[32];
  const bool ok = sm3hash(256, buf, (unsigned long long)total_len * 8ULL, digest) == 0;
  if (ok) {
...
static bool chall3_batch4_portable_hash(chall3_batch4_portable_ctx_t* batch_ctx, uint32_t ctr_base,
  ...
  for (size_t lane = 0; lane < 4u; ++lane) {
    ...
    } else if (batch_ctx->output_bits > 0 && batch_ctx->output_bits <= 256) {
      rc = pseudoXOF_leq256_byte_aligned_local((unsigned long long)batch_ctx->output_bits, dst,
                                               batch_ctx->lane_msg_len, digest[lane])
               ? 0
               : -1;
    } else {
...
void sm4th_sign(const params_t* params, uint8_t* sig, const uint8_t* msg, size_t msg_len,
  ...
  SIG_TSTART(t_sign_alloc_mu);
  uint8_t* mu = malloc(params->lambda_bytes * 2);
  assert(mu);
  SIG_TEND("sign.misc_alloc_mu", t_sign_alloc_mu);
  ...
  SIG_TSTART(t_sign_alloc_rootkey);
  uint8_t* rootkey = malloc(params->lambda_bytes);
  uint8_t iv[IV_SIZE];
  assert(rootkey);
  SIG_TEND("sign.misc_alloc_rootkey", t_sign_alloc_rootkey);
  ...
  SIG_TEND("sign.misc_alloc_vole", t_sign_alloc_vole);
  ...
  SIG_TSTART(t_sign_alloc_chall1);
  uint8_t* chall_1 = malloc(5u * lambda_f_bytes + 8u);
  assert(chall_1);
  SIG_TEND("sign.misc_alloc_chall1", t_sign_alloc_chall1);
  ...
  SIG_TSTART(t_sign_alloc_chall2);
  uint8_t* chall_2 = malloc(3u * lambda_f_bytes + 8u);
  assert(chall_2);
  SIG_TEND("sign.misc_alloc_chall2", t_sign_alloc_chall2);
  ...
  SIG_TSTART(t_sign_alloc_a0);
  uint8_t* a0_tilde = malloc(lambda_f_bytes);
  assert(a0_tilde);
  SIG_TEND("sign.misc_alloc_a0", t_sign_alloc_a0);
  SIG_TSTART(t_sm4_prove);
  sm4_128_prover(params, a0_tilde, signature_a1_tilde(params, sig),
                 signature_a2_tilde(params, sig), witness, u + ell_bytes, V, owf_input,
                 owf_output, chall_2);
  SIG_TEND("sign.sm4_prove", t_sm4_prove);
  SIG_PRINT("[%s sign] sm4_prove done\n", proto_name);

  SIG_TSTART(t_sign_free_vole);
  free_pointer_array(&V);
  free(u);
  u = NULL;
  SIG_TEND("sign.misc_free_vole", t_sign_free_vole);
  ...
  SIG_TSTART(t_sign_alloc_chall3_decode);
  uint16_t* decoded_chall_3 = malloc(sizeof(uint16_t) * tau);
  assert(decoded_chall_3);
  SIG_TEND("sign.misc_alloc_chall3_decode", t_sign_alloc_chall3_decode);
  ...
  SIG_TEND("sign.misc_cleanup", t_sign_cleanup);
  SIG_TEND("sign.total", t_sign_total);
  sm4th_timing_report("sign");
...
int sm4th_verify(const params_t* params, const uint8_t* msg, size_t msglen,
  ...
  if (!check_challenge_3(dsignature_chall_3(params, sig), lambda_f - params->w_grind,
                         lambda_f)) {
    SIG_TEND("verify.check_challenge_3", t_check_chall3);
    SIG_TEND("verify.total", t_verify_total);
    sm4th_timing_report("verify");
    return -1;
  }
  SIG_TEND("verify.check_challenge_3", t_check_chall3);

  SIG_TSTART(t_verify_alloc_mu);
  uint8_t* mu = malloc(params->lambda_bytes * 2);
  assert(mu);
  SIG_TEND("verify.misc_alloc_mu", t_verify_alloc_mu);
  ...
  SIG_TSTART(t_verify_alloc_vole);
  ...
  assert(hcom);
  SIG_TEND("verify.misc_alloc_vole", t_verify_alloc_vole);
  ...
  SIG_TSTART(t_verify_alloc_chall1);
  uint8_t* chall_1 = malloc(5u * lambda_f_bytes + 8u);
  assert(chall_1);
  SIG_TEND("verify.misc_alloc_chall1", t_verify_alloc_chall1);
  ...
  SIG_TSTART(t_verify_alloc_chall2);
  uint8_t* chall_2 = malloc(3u * lambda_f_bytes + 8u);
  assert(chall_2);
  SIG_TEND("verify.misc_alloc_chall2", t_verify_alloc_chall2);
  hash_challenge_2_finalize(params, chall_2, &chall_2_ctx, dsignature_d(params, sig), lambda_f, ell);
  SIG_TEND("verify.hash_challenge_2.pseudoXOF_3lambda+64", t_chall2_v);
  SIG_PRINT("[%s verify] chall_2 done\n", proto_name);

  const uint8_t* d = dsignature_d(params, sig);
  SIG_TSTART(t_verify_alloc_a0);
  uint8_t* a0_tilde = malloc(lambda_f_bytes);
  assert(a0_tilde);
  SIG_TEND("verify.misc_alloc_a0", t_verify_alloc_a0);
  SIG_TSTART(t_sm4_verify);
  sm4_128_verifier(params, a0_tilde, d, q, owf_input, owf_output, chall_2,
                   dsignature_chall_3(params, sig), dsignature_a1_tilde(params, sig),
                   dsignature_a2_tilde(params, sig));
  SIG_TEND("verify.sm4_verify", t_sm4_verify);
  SIG_PRINT("[%s verify] sm4_verify done\n", proto_name);
  SIG_TSTART(t_verify_free_q);
  free_pointer_array(&q);
  SIG_TEND("verify.misc_free_q", t_verify_free_q);

  SIG_TSTART(t_verify_alloc_chall3);
  uint8_t* chall_3 = malloc(lambda_f_bytes);
  assert(chall_3);
  SIG_TEND("verify.misc_alloc_chall3", t_verify_alloc_chall3);
  ...
  free(hcom);
  free(mu);
  SIG_TEND("verify.misc_cleanup", t_verify_cleanup);
  SIG_TEND("verify.total", t_verify_total);
  sm4th_timing_report("verify");
  return result;
}
