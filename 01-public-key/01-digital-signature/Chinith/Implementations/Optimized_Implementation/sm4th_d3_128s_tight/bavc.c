...
static int h1_finalize_batch4_same_len_portable(unsigned int out_bytes_len,
  ...
  const int output_bits = (int)((size_t)out_bytes_len * 8u);
...
static void em_leaf_commit_columns_from_nodes(const params_t* params, const uint8_t* iv,
                                              ...
                                              uint8_t* com_out, uint8_t* vole_out_288) {
  ...
  assert(ell_hat_bytes == 288u);
  assert(vole_out_288 != NULL);
  ...
  for (unsigned int i = 0; i < tau; ++i) {
    ...
    uint8_t* dst_col = sd_out + dst_idx * seed_bytes;
    for (unsigned int j = 0; j < half_leaf_cnt; ++j) {
      memcpy(dst_col + (size_t)j * seed_bytes,
             leaf_nodes + ((size_t)j * tau + i) * seed_bytes, seed_bytes);
    }
  }
  for (unsigned int i = 0; i < tau1; ++i) {
    ...
    for (unsigned int j = 0; j < half_leaf_cnt; ++j) {
      memcpy(dst_col + (size_t)j * seed_bytes,
             second_leaf_nodes + ((size_t)j * tau1 + i) * seed_bytes, seed_bytes);
    }
  }
  ...
  for (unsigned int i = 0; i < tau; ++i) {
    ...
    const size_t dst_idx = (i < tau1) ? ((size_t)(2u * i) * half_leaf_cnt)
                                      : ((size_t)(i + tau1) * half_leaf_cnt);
    const uint8_t* key_col = sd_out + dst_idx * seed_bytes;
    prg_2_lambda_and_288_fixed_tweak_independent_batch_with_sd(
        key_col, seed_bytes, iv, tweak, vole_tweak, sd_out + dst_idx * seed_bytes,
        seed_bytes, com_out + dst_idx * com_size, com_size,
        vole_out_288 + dst_idx * ell_hat_bytes, ell_hat_bytes, prg_lambda, half_leaf_cnt);
  }
  ...
  SIG_TEND("bavc.commit.leaf_prg_2lambda_288", t_bavc_commit_leaf_prg);
}
...
static void bavc_commit_em(const params_t* params,
  ...
  assert(nodes);
  const bool precompute_sd_prg288 = (params->lambda_prg == 256u) && (params->ell_hat_bytes == 288u);
  assert(precompute_sd_prg288);
  bavc->h   = malloc(h1_bytes);
  bavc->com = malloc(L * com_size);
  bavc->sd  = malloc(L * seed_bytes);
  bavc->sd_prg288 = precompute_sd_prg288 ? malloc((size_t)L * params->ell_hat_bytes) : NULL;
  assert(bavc->h && bavc->com && bavc->sd);
  assert(!precompute_sd_prg288 || bavc->sd_prg288);
  ...
  seed_prg_em_pipeline_commit(params, nodes, iv, bavc->sd, bavc->com, bavc->sd_prg288);
...
static bool bavc_reconstruct_em(const params_t* params, bavc_rec_t* bavc_rec, 
  ...
  assert(params->lambda == 128u);
  assert(params->lambda_prg == 256u);
  assert(ell_hat_bytes == 288u);
  assert(bavc_rec->s_prg288 != NULL);
  ...
  const uint8_t* first_leaf_nodes = keys + (size_t)(L - 1) * seed_bytes;
  const uint8_t* second_leaf_nodes = keys + (size_t)second_layer_base * seed_bytes;
  const size_t first_leaf_stride = (size_t)tau * seed_bytes;
  const size_t second_leaf_stride = (size_t)tau_1 * seed_bytes;
  ...
  if (tau_1 == 0u) {
    for (int ii = 0; ii < (int)tau; ++ii) {
      ...
      const uint8_t* key_base_first = first_leaf_nodes + (size_t)i * seed_bytes;
      uint8_t* out_first = com_batch + com_base_off[i] * com_size;
      uint8_t* sd_i_out = bavc_rec->s + sd_base_off[i] * seed_bytes;
      uint8_t* sd_i_prg288_out = bavc_rec->s_prg288 + sd_base_off[i] * ell_hat_bytes;
      const uint8_t* decom_com_i = decom_i + (size_t)i * com_size;

      memset(sd_i_out, 0, seed_bytes);
      memset(sd_i_prg288_out, 0, ell_hat_bytes);

      prg_2_lambda_and_288_fixed_tweak_independent_batch_with_sd_xor_map(
          key_base_first, first_leaf_stride, iv, tweak, vole_tweak, sd_i_out, seed_bytes,
          0u, opened, out_first, com_size, sd_i_prg288_out, ell_hat_bytes, prg_lambda,
          half_leaf_cnt);
    ...
    for (int ii = 0; ii < (int)tau; ++ii) {
      ...
      if (i < tau_1) {
        const uint8_t* key_base_second = second_leaf_nodes + (size_t)i * seed_bytes;
        ...
        prg_2_lambda_and_288_fixed_tweak_independent_batch_with_sd_xor_map(
            ...
            seed_bytes, half_leaf_cnt, opened, out_second, com_size, sd_i_prg288_out,
