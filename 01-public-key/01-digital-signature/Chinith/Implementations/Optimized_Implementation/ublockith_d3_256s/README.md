...
├── ublockith_d3_256s_bench.c                  # Benchmark entry (ms + Mcyc)
...
- Thus `./ublockith_d3_256s_bench` and `./ublockith_d3_256s_bench 100` run 100 fixed-56-byte sign/verify iterations, while `./ublockith_d3_256s_bench 100 8` forces the increasing-message run.

Baseline check command:

```bash
make NDEBUG=1 PRG_ACCE=1 FIELD_PCLMUL=1 USE_BALLET=1 ublockith_d3_256s_bench && ./ublockith_d3_256s_bench 20
```
