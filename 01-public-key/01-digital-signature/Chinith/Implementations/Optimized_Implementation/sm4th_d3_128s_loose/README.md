# sm4th-d3-128s-loose - Optimized Implementation

Optimized implementation of **sm4th-d3-128s-loose** with x86-oriented acceleration.

Protocol parameters follow `SM4th-d3-128s-loose`:
`lambda=128`, `lambda_f=128`, `lambda_iv=128`, `ell=1808`, `tau=11`,
`w_grind=7`, `T_open=102`, `B=16`, `d=3`. The loose variant keeps SM4 as
...
├── sm4th_d3_128s_loose.c / sm4th_d3_128s_loose.h      # Public API wrapper
...
```bash
make clean
make PRG_ACCE=0 HYGON_SM4_ACCE=1 KAT_SIG sm4th_d3_128s_loose_bench
```
...
`./sm4th_d3_128s_loose_bench` runs 100 fixed-56-byte sign/verify iterations by default.
