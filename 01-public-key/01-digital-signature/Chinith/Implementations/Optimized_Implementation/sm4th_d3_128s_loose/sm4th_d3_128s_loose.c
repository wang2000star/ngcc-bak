...
int sm4th_d3_128s_loose_keygen(uint8_t* pk, uint8_t* sk, const uint8_t* sk_key, const uint8_t* sk_input) {
  ...
  sm4th_d3_128s_loose_owf(SK_KEY(sk), SK_INPUT(sk), PK_OUTPUT(pk));
...
int sm4th_d3_128s_loose_unpack_private_key(sm4th_d3_128s_loose_unpacked_private_key_t* unpacked_sk, const uint8_t* sk) {
  ...
  sm4th_d3_128s_loose_owf(SK_KEY(sk), SK_INPUT(sk), unpacked_sk->owf_output);
...
int sm4th_d3_128s_loose_validate_keypair(const uint8_t* pk, const uint8_t* sk) {
  ...
  uint8_t pk_check[32];
  sm4th_d3_128s_loose_owf(SK_KEY(sk), SK_INPUT(sk), PK_OUTPUT(pk_check));
...
int sm4th_d3_128s_loose_sign_with_randomness(const uint8_t* sk, const uint8_t* message, size_t message_len, const uint8_t* rho, size_t rho_len, uint8_t* signature, size_t* signature_len) {
  ...
  if (!sk || !signature || !signature_len || *signature_len < SM4TH_D3_S_LOOSE_SIGNATURE_SIZE || (!rho && rho_len) || (!message && message_len)) {
    return -1;
  }

  sm4th_d3_128s_loose_unpacked_private_key_t unpacked_sk;
  int ret = sm4th_d3_128s_loose_unpack_private_key(&unpacked_sk, sk);
  // as long as ret!=0, this would enter into the if branch
  // if ret==0, then continue to sign
  if (ret) {
    sm4th_d3_128s_loose_clear_unpacked_private_key(&unpacked_sk);
    return ret;
  }

  ret = sm4th_d3_128s_loose_unpacked_sign_with_randomness(&unpacked_sk, message, message_len, rho, rho_len, signature, signature_len);
  sm4th_d3_128s_loose_clear_unpacked_private_key(&unpacked_sk);
  return ret;
}

int sm4th_d3_128s_loose_unpacked_sign_with_randomness(const sm4th_d3_128s_loose_unpacked_private_key_t* sk, const uint8_t* message, size_t message_len, const uint8_t* rho, size_t rho_len, uint8_t* signature, size_t* signature_len) {
  if (sm4th_opt_require_x86() != 0) {
    return -1;
  }
  if (!sk || !signature || !signature_len || *signature_len < SM4TH_D3_S_LOOSE_SIGNATURE_SIZE || (!message && message_len)) {
    return -1;
  }
  ...
  *signature_len = SM4TH_D3_S_LOOSE_SIGNATURE_SIZE;
...
int sm4th_d3_128s_loose_verify(const uint8_t* pk, const uint8_t* message, size_t message_len, const uint8_t* signature, size_t signature_len) {
  ...
  if (!pk || !signature || signature_len != SM4TH_D3_S_LOOSE_SIGNATURE_SIZE || (!message && message_len)) {
    return -1;
  }
...
void sm4th_d3_128s_loose_clear_private_key(uint8_t* key) {
  explicit_bzero(key, SM4TH_D3_S_LOOSE_PRIVATE_KEY_SIZE);
}
