...
static params_t sm4th_params_em_128f_g = {
    .lambda = 128,
    .lambda_q = 80,
    .lambda_f = 160,
    .lambda_iv = 256,
    .lambda_prg = 256,
    .owf_block_bits = SM4_OWF_BLOCK_BITS,
    .prg_block_bits = PRG_BLOCK_BITS,
    .ell = 1024,
    .ske = 0,
    .senc = 128,
    .r = 32,
    ...
    .lambda_bytes = 128 / 8,
    .lambda_q_bytes = 80 / 8,
    .lambda_f_bytes = 160 / 8,
    .lambda_iv_bytes = 256 / 8,
    .lambda_prg_bytes = 256 / 8,
    .owf_block_bytes = SM4_OWF_BLOCK_BYTES,
    .prg_block_bytes = PRG_BLOCK_BYTES,
    .iv_bytes = IV_SIZE,
    .ell_bytes = 1024 / 8,
    .ell_hat_bits = 1024 + 2 * 160 + UNIVERSAL_HASH_B_BITS,
    .ell_hat_bytes = (1024 + 2 * 160 + UNIVERSAL_HASH_B_BITS) / 8,
    .utilde_bytes = (160 / 8) + UNIVERSAL_HASH_B,
};
...
static void init_params(params_t* params) {
  ...
  assert(params->lambda_prg % 8 == 0);
  assert(params->owf_block_bits % 8 == 0);
  assert(params->prg_block_bits % 8 == 0);
  ...
  assert(params->lambda_prg_bytes == params->lambda_prg / 8);
  assert(params->owf_block_bytes == params->owf_block_bits / 8);
  assert(params->prg_block_bytes == params->prg_block_bits / 8);
  assert(params->owf_block_bits == SM4_OWF_BLOCK_BITS);
  assert(params->prg_block_bits == PRG_BLOCK_BITS);
...
const params_t* sm4th_params_em_128f(void) {
  static int initialized = 0;
  if (!initialized) {
    init_params(&sm4th_params_em_128f_g);
    initialized = 1;
  }
  return &sm4th_params_em_128f_g;
}

unsigned int compute_sig_size(const params_t* params) {
  const unsigned int prg_lambda_bytes = params->lambda_prg_bytes;
  ...
  const unsigned int E = (n_leaf * prg_lambda_bytes) * tau + t_open * prg_lambda_bytes;
