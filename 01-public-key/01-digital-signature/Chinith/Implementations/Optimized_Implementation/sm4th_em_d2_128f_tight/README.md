# sm4th-em-d2-128f-tight - Optimized Implementation

Optimized implementation of **sm4th-em-d2-128f-tight** with x86-oriented
acceleration. This tight variant uses SM4 for the OWF and Ballet-256/256 as the
PRG, with 256-bit IVs and 256-bit PRG seeds.

Protocol parameters follow `SM4th-EM-d2-128f-tight` in `Chinith-260621.pdf`:
`lambda=128`, `lambda_f=160`, `lambda_iv=256`, `lambda_prg=256`, `ell=1024`,
`tau=21`, `w_grind=8`, `T_open=140`, `B=16`, `d=2`, and
`ell_hat=1360` bits (`170` bytes).
...
├── sm4th_em_d2_128f_tight.c / .h          # Public API wrapper
├── sm4th_em_d2_128f_tight_AlgorithmInstance.c / .h # Algorithm instance entry points
├── sm4th_em_d2_128f_tight_bench.c         # Benchmark entry
...
```bash
make sm4th_em_d2_128f_tight_bench
```
...
```bash
make clean
make NDEBUG=1 PRG_ACCE=0 SM4_OWF_ACCE=0 FIELD_PCLMUL=0 KAT_SIG sm4th_em_d2_128f_tight_bench
```
...
```bash
./sm4th_em_d2_128f_tight_bench
./sm4th_em_d2_128f_tight_bench 100
./sm4th_em_d2_128f_tight_bench 100 8
```

`./sm4th_em_d2_128f_tight_bench` runs 100 fixed-56-byte sign/verify iterations
by default. The signed message is the deterministic 56-byte ASCII string
...
- On x86: build/run `KAT_SIG` with default Ballet PRG optimized flags.
- On non-x86: prints one log line and forces the portable Ballet PRG path.
