...
void H1_final_keep(const params_t* params, H1_context_t* ctx, uint8_t* digest) {
  size_t out_bytes_len = (size_t)params->n_leaf * params->prg_block_bytes;
...
void H3_final(const params_t* params, H3_context_t* ctx, uint8_t* digest, uint8_t* iv) {
  const size_t out_bytes_len = params->lambda_prg_bytes + params->lambda_iv_bytes;
  new_hash_final(ctx, domain_sep_H3, out_bytes_len);
  new_hash_squeeze(ctx, digest, params->lambda_prg_bytes);
  new_hash_squeeze(ctx, iv, params->lambda_iv_bytes);
  new_hash_clear(ctx);
}
...
void H4_update(const params_t* params, H4_context_t* ctx, const uint8_t* iv) {
  new_hash_update(ctx, iv, params->lambda_iv_bytes);
}

void H4_final(const params_t* params, H4_context_t* ctx, uint8_t* iv) {
  new_hash_final(ctx, domain_sep_H4, params->iv_bytes);
  new_hash_squeeze(ctx, iv, params->iv_bytes);
  new_hash_clear(ctx);
}
