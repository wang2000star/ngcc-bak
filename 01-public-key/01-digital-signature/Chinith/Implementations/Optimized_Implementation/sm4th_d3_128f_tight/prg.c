...
#endif
#else
#error "sm4th_d3_128f_tight requires USE_BALLET=1 and HAVE_BALLET_CORE=1"
#endif
...
static void prg_2lambda_and_288_batch4_256_avx2(
    const uint8_t* keys, size_t key_stride, const uint8_t* iv, uint32_t tweak_2lambda,
    uint32_t tweak_288, uint8_t* const out_2lambda_ptrs[4], size_t out_2lambda_len,
    uint8_t* const out_288_ptrs[4]) {
  ...
  prg_ballet256_emit_fixed_4keys_avx2(rk_pack, iv, tweak_2lambda, out_2lambda_ptrs,
                                      out_2lambda_len);
  prg_ballet256_emit_fixed_4keys_avx2(rk_pack, iv, tweak_288, out_288_ptrs, PRG_288_BYTES);
}
...
#if PRG_HAS_BALLET_AVX2
  if (PRG_HAS_4KEY_FIXED_TWEAK_AVX2 && seclvl == 256u && sm4th_x86_has_avx2() &&
      out_288_stride >= PRG_288_BYTES && count >= 4u) {
    const size_t n4 = count & ~(size_t)3u;
    for (; i < n4; i += 4u) {
      uint8_t* out2_ptrs[4];
      uint8_t* out288_ptrs[4];
      for (size_t lane = 0; lane < 4u; ++lane) {
        ...
        out288_ptrs[lane] = out_288 + idx * out_288_stride;
      }
      prg_2lambda_and_288_batch4_256_avx2(keys + i * key_stride, key_stride, iv,
                                          tweak_2lambda, tweak_288, out2_ptrs,
                                          out_2lambda_len, out288_ptrs);
    }
  }
#endif
  ...
  if (PRG_HAS_4KEY_FIXED_TWEAK_AVX2 && seclvl == 256u && sm4th_x86_has_avx2() &&
    ...
    for (; i < n4; i += 4u) {
      ...
      for (size_t lane = 0; lane < 4u; ++lane) {
        ...
        out2_ptrs[lane] = out_2lambda + idx * out_2lambda_stride;
        out288_ptrs[lane] = prg_sd_xor_map_slot(out_288, out_288_stride, idx, sd_index_base,
                                                sd_index_xor);
      }
      prg_2lambda_and_288_batch4_256_avx2(keys + i * key_stride, key_stride, iv,
                                          tweak_2lambda, tweak_288, out2_ptrs,
