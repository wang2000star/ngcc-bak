...
static bfSSS_t* column_to_row_major_and_shrink_V_SSS_avx2(uint8_t** v, unsigned int ell) {
  ...
  for (; row + 8u <= rows; row += 8u) {
    ...
    for (unsigned int chunk = 0; chunk < (UBLOCKITH_LAMBDA / 32u); ++chunk) {
      const unsigned int base_col = chunk * 32u;
      uint8_t gathered[32];
      for (unsigned int c = 0; c < 32u; ++c) {
        gathered[c] = v[base_col + c][row_byte];
      }

      const __m256i cols = _mm256_loadu_si256((const __m256i*)gathered);
      for (unsigned int bit = 0; bit < 8u; ++bit) {
        const int mask = _mm256_movemask_epi8(_mm256_slli_epi16(cols, 7 - (int)bit));
        uint8_t* dst = &packed_rows[bit][chunk * 4u];
        dst[0] = (uint8_t)(mask);
        dst[1] = (uint8_t)(mask >> 8);
        dst[2] = (uint8_t)(mask >> 16);
        dst[3] = (uint8_t)(mask >> 24);
      }
    }
...
static bfSSS_t* column_to_row_major_and_shrink_V_SSS(uint8_t** v, unsigned int ell) {
  ...
  if (__builtin_cpu_supports("avx2")) {
    return column_to_row_major_and_shrink_V_SSS_avx2(v, ell);
  }
#endif
