...
#include "ublockith_em_d3_256s.h"
...
int ublockith_em_d3_256s_keygen(uint8_t* pk, uint8_t* sk, const uint8_t* sk_key,
  ...
  ublockith_em_d3_256s_owf(SK_KEY(sk), SK_INPUT(sk), PK_OUTPUT(pk));
...
int ublockith_em_d3_256s_unpack_private_key(ublockith_em_d3_256s_unpacked_private_key_t* unpacked_sk,
  ...
  ublockith_em_d3_256s_owf(SK_KEY(sk), SK_INPUT(sk), unpacked_sk->owf_output);

  const params_t* params = ublockith_params_em_d3_256s();
...
int ublockith_em_d3_256s_validate_keypair(const uint8_t* pk, const uint8_t* sk) {
  ...
  uint8_t pk_check[UBLOCKITH_EM_D3_256S_PUBLIC_KEY_SIZE];
  ublockith_em_d3_256s_owf(SK_KEY(sk), SK_INPUT(sk), PK_OUTPUT(pk_check));
...
int ublockith_em_d3_256s_sign_with_randomness(const uint8_t* sk, const uint8_t* message,
  ...
  if (!sk || !signature || !signature_len || *signature_len < UBLOCKITH_EM_D3_256S_SIGNATURE_SIZE ||
      (!rho && rho_len) || (!message && message_len)) {
    return -1;
  }

  ublockith_em_d3_256s_unpacked_private_key_t unpacked_sk;
  int ret = ublockith_em_d3_256s_unpack_private_key(&unpacked_sk, sk);
  if (ret) {
    ublockith_em_d3_256s_clear_unpacked_private_key(&unpacked_sk);
    return ret;
  }

  ret = ublockith_em_d3_256s_unpacked_sign_with_randomness(&unpacked_sk, message, message_len, rho,
...
int ublockith_em_d3_256s_unpacked_sign_with_randomness(
    const ublockith_em_d3_256s_unpacked_private_key_t* sk, const uint8_t* message,
  ...
  *signature_len = UBLOCKITH_EM_D3_256S_SIGNATURE_SIZE;
  return 0;
}

int ublockith_em_d3_256s_verify(const uint8_t* pk, const uint8_t* message, size_t message_len,
                             const uint8_t* signature, size_t signature_len) {
  if (!pk || !signature || signature_len != UBLOCKITH_EM_D3_256S_SIGNATURE_SIZE ||
      (!message && message_len)) {
    return -1;
  }
...
void ublockith_em_d3_256s_clear_private_key(uint8_t* key) {
  explicit_bzero(key, UBLOCKITH_EM_D3_256S_PRIVATE_KEY_SIZE);
}
