...
#if defined(VISTRUTITH_ENABLE_AESNI) && VISTRUTITH_ENABLE_AESNI && \
    (defined(__x86_64__) || defined(__i386__))
#    include <immintrin.h>
#    define VISTRUTITH_X86_AESNI 1
#else
#    define VISTRUTITH_X86_AESNI 0
#endif

#if VISTRUTITH_X86_AESNI && defined(VISTRUTITH_ASSUME_AESNI) && VISTRUTITH_ASSUME_AESNI
#    define VISTRUTITH_X86_AESNI_ALWAYS 1
#else
#    define VISTRUTITH_X86_AESNI_ALWAYS 0
#endif
...
#if VISTRUTITH_X86_AESNI
#if !VISTRUTITH_X86_AESNI_ALWAYS
static int vistrutah_runtime_aes_checked = 0;
static int vistrutah_runtime_aes_available = 0;

static int
vistrutah_runtime_has_aes(void)
{
    if (!vistrutah_runtime_aes_checked) {
#    if defined(__GNUC__) || defined(__clang__)
        __builtin_cpu_init();
        vistrutah_runtime_aes_available = __builtin_cpu_supports("aes");
#    else
        vistrutah_runtime_aes_available = 1;
#    endif
        vistrutah_runtime_aes_checked = 1;
    }
    return vistrutah_runtime_aes_available;
}
#endif

static inline void
vistrutah_aesenc_block(uint8_t state[16], const uint8_t round_key[16])
{
    __m128i s = _mm_loadu_si128((const __m128i*) state);
    __m128i k = _mm_loadu_si128((const __m128i*) round_key);
    s         = _mm_aesenc_si128(s, k);
    _mm_storeu_si128((__m128i*) state, s);
}

static inline void
vistrutah_aesenclast_block(uint8_t state[16], const uint8_t round_key[16])
{
    __m128i s = _mm_loadu_si128((const __m128i*) state);
    __m128i k = _mm_loadu_si128((const __m128i*) round_key);
...
static inline void
vistrutah_aesenc_zero_block(uint8_t state[16])
{
    __m128i s = _mm_loadu_si128((const __m128i*) state);
    s         = _mm_aesenc_si128(s, _mm_setzero_si128());
    _mm_storeu_si128((__m128i*) state, s);
}

static inline __m128i
vistrutah_rotl16_5_m128(__m128i x)
{
#if defined(__SSSE3__)
    return _mm_alignr_epi8(x, x, 5);
#else
    return _mm_or_si128(_mm_srli_si128(x, 5), _mm_slli_si128(x, 11));
#endif
}

static inline __m128i
vistrutah_rotl16_10_m128(__m128i x)
{
#if defined(__SSSE3__)
    return _mm_alignr_epi8(x, x, 10);
#else
    return _mm_or_si128(_mm_srli_si128(x, 10), _mm_slli_si128(x, 6));
#endif
}

/* Same lane interleave strategy as utils_vistrutah/vistrutah_512_intel.c. */
static inline void
vistrutah_mixing_layer_512_sse(__m128i* s0, __m128i* s1, __m128i* s2, __m128i* s3)
{
...
static inline void
vistrutah_512_encrypt_prepared_aesni_same_plaintext(
    __m128i p0, __m128i p1, __m128i p2, __m128i p3, uint8_t* ciphertext,
    const vistrutah_512_prepared_key_t* prepared, const uint8_t* fixed_key, int steps)
{
    __m128i s0 = p0;
    __m128i s1 = p1;
    __m128i s2 = p2;
    __m128i s3 = p3;
    __m128i rk0 = _mm_loadu_si128((const __m128i*) prepared->round_key);
    __m128i rk1 = _mm_loadu_si128((const __m128i*) (prepared->round_key + 16));
    __m128i rk2 = _mm_loadu_si128((const __m128i*) (prepared->round_key + 32));
    __m128i rk3 = _mm_loadu_si128((const __m128i*) (prepared->round_key + 48));

    const __m128i fk0 = _mm_loadu_si128((const __m128i*) fixed_key);
    const __m128i fk1 = _mm_loadu_si128((const __m128i*) (fixed_key + 16));
    const __m128i fk2 = _mm_loadu_si128((const __m128i*) (fixed_key + 32));
    const __m128i fk3 = _mm_loadu_si128((const __m128i*) (fixed_key + 48));
    ...
    s0 = _mm_aesenc_si128(s0, fk0);
    s1 = _mm_aesenc_si128(s1, fk1);
    s2 = _mm_aesenc_si128(s2, fk2);
    s3 = _mm_aesenc_si128(s3, fk3);

    for (int i = 1; i < steps; i++) {
        s0 = _mm_aesenc_si128(s0, zero);
        s1 = _mm_aesenc_si128(s1, zero);
        s2 = _mm_aesenc_si128(s2, zero);
        s3 = _mm_aesenc_si128(s3, zero);

        vistrutah_mixing_layer_512_sse(&s0, &s1, &s2, &s3);

        rk0 = vistrutah_rotl16_5_m128(rk0);
        rk1 = vistrutah_rotl16_10_m128(rk1);
        rk2 = vistrutah_rotl16_5_m128(rk2);
        rk3 = vistrutah_rotl16_10_m128(rk3);
        ...
        const __m128i rc = _mm_loadu_si128((const __m128i*) &ROUND_CONSTANTS[16 * (i - 1)]);
    ...
    s0 = _mm_aesenclast_si128(s0, rk0);
    s1 = _mm_aesenclast_si128(s1, rk1);
    s2 = _mm_aesenclast_si128(s2, rk2);
    s3 = _mm_aesenclast_si128(s3, rk3);
...
#if VISTRUTITH_X86_AESNI_ALWAYS
    return true;
#elif VISTRUTITH_X86_AESNI
    return vistrutah_runtime_has_aes() != 0;
#else
    return false;
#endif
}
...
{
#if VISTRUTITH_X86_AESNI_ALWAYS
    return "Intel AES-NI S-box path, compile-time dispatch)";
#elif VISTRUTITH_X86_AESNI
    if (vistrutah_runtime_has_aes()) {
        return "Intel AES-NI S-box path, runtime dispatch)";
    }
#endif
#ifdef SBOX_TABLE
    return "Portable (T-table S-box, no hardware acceleration)";
#else
...
{
#if VISTRUTITH_X86_AESNI_ALWAYS
    vistrutah_aesenc_block(state, round_key);
    return;
#elif VISTRUTITH_X86_AESNI
    if (vistrutah_runtime_has_aes()) {
        vistrutah_aesenc_block(state, round_key);
...
{
#if VISTRUTITH_X86_AESNI_ALWAYS
    vistrutah_aesenclast_block(state, round_key);
    return;
#elif VISTRUTITH_X86_AESNI
    if (vistrutah_runtime_has_aes()) {
        vistrutah_aesenclast_block(state, round_key);
                               ...
                               const vistrutah_512_prepared_key_t* prepared)
{
    ...
    if (vistrutah_runtime_has_aes()) {
#endif
        __m128i p0 = _mm_loadu_si128((const __m128i*) plaintext);
        __m128i p1 = _mm_loadu_si128((const __m128i*) (plaintext + 16));
        __m128i p2 = _mm_loadu_si128((const __m128i*) (plaintext + 32));
        __m128i p3 = _mm_loadu_si128((const __m128i*) (plaintext + 48));
        vistrutah_512_encrypt_prepared_aesni_same_plaintext(p0, p1, p2, p3, ciphertext, prepared,
                                                            fixed_key, steps);
        return;
    }
#endif
    ...
    uint8_t* out_base, size_t out_stride)
{
    ...
    if (vistrutah_runtime_has_aes()) {
        ...
        for (size_t lane = 0; lane < lane_count; ++lane) {
            const vistrutah_512_prepared_key_t* prepared_lane = &prepared[lane];
            vistrutah_512_encrypt_prepared_aesni_same_plaintext(
                p0, p1, p2, p3, out_base + lane * out_stride, prepared_lane,
                prepared_lane->fixed_key, steps);
        }
        return;
    }
#endif
