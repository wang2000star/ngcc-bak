...
static void em_leaf_commit_columns_from_nodes(const params_t* params, const uint8_t* iv,
                                              ...
                                              uint8_t* com_out, uint8_t* vole_out_162) {
  ...
  assert(lambda == 128u);
  assert(ell_hat_bytes == 162u);
  assert(vole_out_162 != NULL);
  ...
  for (unsigned int i = 0; i < tau; ++i) {
    ...
    prg_2_lambda_and_162_fixed_tweak_independent_batch_with_sd(
        key_col, tau_stride, iv, tweak, vole_tweak, sd_out + dst_idx * lambda_bytes,
        lambda_bytes, com_out + dst_idx * com_size, com_size,
        vole_out_162 + dst_idx * ell_hat_bytes, ell_hat_bytes, lambda, half_leaf_cnt);
  }
...
static void bavc_commit_em(const params_t* params,
  ...
  assert(nodes);
  const bool precompute_sd_prg162 = (params->lambda == 128u) && (ell_hat_bytes == 162u);
  assert(precompute_sd_prg162);
  bavc->h   = malloc(lambda_bytes * 2);
  bavc->com = malloc(L * com_size);
  bavc->sd  = malloc(L * lambda_bytes);
  bavc->sd_prg162 = precompute_sd_prg162 ? malloc((size_t)L * ell_hat_bytes) : NULL;
  assert(bavc->h && bavc->com && bavc->sd);
  assert(!precompute_sd_prg162 || bavc->sd_prg162);
#if SM4TH_COMPONENT_TIMING && SM4TH_COMPONENT_TIMING_VERBOSE
  ...
  bavc_detail_start = sm4th_timing_now_ms();
#endif
  seed_prg_em_pipeline_commit(params, nodes, iv, bavc->sd, bavc->com, bavc->sd_prg162);
#if SM4TH_COMPONENT_TIMING && SM4TH_COMPONENT_TIMING_VERBOSE
...
static bool bavc_reconstruct_em(const params_t* params, bavc_rec_t* bavc_rec, 
  ...
  if (tau_1 == 0u) {
    for (int ii = 0; ii < (int)tau; ++ii) {
      ...
      uint8_t* sd_i_prg162_out = bavc_rec->s_prg162 + sd_base_off[i] * ell_hat_bytes;
      const uint8_t* decom_com_i = decom_i + (size_t)i * com_size;

      memset(sd_i_out, 0, lambda_bytes);
      memset(sd_i_prg162_out, 0, ell_hat_bytes);

      prg_2_lambda_and_162_fixed_tweak_independent_batch_with_sd_xor_map(
          key_base_first, first_layer_key_stride, iv, tweak, vole_tweak, sd_i_out, lambda_bytes,
          0u, opened, out_first, com_size, sd_i_prg162_out, ell_hat_bytes, params->lambda,
          half_leaf_cnt);
    ...
    for (int ii = 0; ii < (int)tau; ++ii) {
      ...
      if (i < tau_1) {
        ...
        prg_2_lambda_and_162_fixed_tweak_independent_batch_with_sd_xor_map(
            ...
            lambda_bytes, half_leaf_cnt, opened, out_second, com_size, sd_i_prg162_out,
