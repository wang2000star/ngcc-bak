...
{
	return VISTRUTITH_D3_512S_PUBLIC_KEY_SIZE;
}

unsigned long long sig_get_sk_len_bytes(void)
{
	return VISTRUTITH_D3_512S_PRIVATE_KEY_SIZE;
}

unsigned long long sig_get_sn_len_bytes(void)
{
	return VISTRUTITH_D3_512S_SIGNATURE_SIZE;
}
	...
	unsigned char *sk, unsigned long long *sk_len_bytes)
{
	...
	bool done = false;
	uint8_t sk_key[512 / 8];
	uint8_t sk_input[512 / 8];

	while (!done) {
		get_random_number(&drng_algorithm, sk_key, 512);
		done = (ptr_get_bit(sk_key, 0) & ptr_get_bit(sk_key, 1)) == 0;
	}

	get_random_number(&drng_algorithm, sk_input, 512);
	
	return vistrutith_d3_512s_keygen(pk, sk, sk_key, sk_input);
}
	...
	unsigned char *sn, unsigned long long *sn_len_bytes)
{
	...
	return vistrutith_d3_512s_sign_with_randomness(sk, m, (size_t) m_len_bytes, rho, sizeof(rho), sn, (size_t*) sn_len_bytes);
}
	...
	unsigned char *m, unsigned long long m_len_bytes)
{
	...
	return vistrutith_d3_512s_verify(pk, m, (size_t) m_len_bytes, sn, (size_t) sn_len_bytes);
}
