...
#include "utils_vistrutah/vistrutith_vistrutah_512.h"
...
void vistrutith_sign_opt_d3_512s(const params_t* params, uint8_t* sig, const uint8_t* msg,
                                 size_t msg_len, const uint8_t* owf_key,
                                 const uint8_t* owf_input, const uint8_t* owf_output,
                                 const uint8_t* witness, const uint8_t* rho,
                                 size_t rholen) {
  const char* proto_name = "vistrutith";
  ...
  hash_mu(params, mu, owf_input, VISTRUTAH_512_BLOCK_SIZE, owf_output, VISTRUTAH_512_BLOCK_SIZE, msg, msg_len, lambda);
  ...
  {
    uint8_t V_tilde_batch[4u * (BF512_NUM_BYTES + UNIVERSAL_HASH_B)];
  ...
  assert(a0_tilde);
  SIG_TSTART(t_vistrutith_prove);
  vistrutith_512_prover(params, a0_tilde, signature_a1_tilde(params, sig), signature_a2_tilde(params, sig),
               witness, u + ell_bytes, V, owf_input, owf_output, chall_2);
  SIG_TEND("sign.vistrutith_prove", t_vistrutith_prove);
...
int vistrutith_verify_opt_d3_512s(const params_t* params, const uint8_t* msg, size_t msglen,
  ...
  hash_mu(params, mu, owf_input, VISTRUTAH_512_BLOCK_SIZE, owf_output, VISTRUTAH_512_BLOCK_SIZE, msg, msglen, lambda);
...

  {
    ...
    uint8_t Q_tilde_batch[4u * (BF512_NUM_BYTES + UNIVERSAL_HASH_B)];
  ...
  uint8_t* a0_tilde = malloc(params->lambda_bytes);
  assert(a0_tilde);
  SIG_TSTART(t_vistrutith_verify);
  vistrutith_512_verifier(params, a0_tilde, d, q, owf_input, owf_output, chall_2, dsignature_chall_3(params, sig), dsignature_a1_tilde(params, sig), dsignature_a2_tilde(params, sig));
  SIG_TEND("verify.vistrutith_verify", t_vistrutith_verify);
  free_pointer_array(&q);
