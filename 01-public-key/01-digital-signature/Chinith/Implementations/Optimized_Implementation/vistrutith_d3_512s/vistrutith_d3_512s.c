...
int vistrutith_d3_512s_keygen(uint8_t* pk, uint8_t* sk, const uint8_t* sk_key, const uint8_t* sk_input) {
  ...
  vistrutith_d3_512s_owf(SK_KEY(sk), SK_INPUT(sk), PK_OUTPUT(pk));
...
int vistrutith_d3_512s_unpack_private_key(vistrutith_d3_512s_unpacked_private_key_t* unpacked_sk, const uint8_t* sk) {
  ...
  vistrutith_d3_512s_owf(SK_KEY(sk), SK_INPUT(sk), unpacked_sk->owf_output);
  // declassify OWF output

  // generate extended witness using sk: owf_key, owf_input
  const params_t* params = vistrutith_params_d3_512s();
...
int vistrutith_d3_512s_validate_keypair(const uint8_t* pk, const uint8_t* sk) {
  ...
  uint8_t pk_check[VISTRUTITH_D3_512S_PUBLIC_KEY_SIZE];
  vistrutith_d3_512s_owf(SK_KEY(sk), SK_INPUT(sk), PK_OUTPUT(pk_check));
...
int vistrutith_d3_512s_sign_with_randomness(const uint8_t* sk, const uint8_t* message, size_t message_len, const uint8_t* rho, size_t rho_len, uint8_t* signature, size_t* signature_len) {
  if (!sk || !signature || !signature_len || *signature_len < VISTRUTITH_D3_512S_SIGNATURE_SIZE || (!rho && rho_len) || (!message && message_len)) {
    return -1;
  }

  vistrutith_d3_512s_unpacked_private_key_t unpacked_sk;
  int ret = vistrutith_d3_512s_unpack_private_key(&unpacked_sk, sk);
  // as long as ret!=0, this would enter into the if branch
  // if ret==0, then continue to sign
  if (ret) {
    vistrutith_d3_512s_clear_unpacked_private_key(&unpacked_sk);
    return ret;
  }

  ret = vistrutith_d3_512s_unpacked_sign_with_randomness(&unpacked_sk, message, message_len, rho, rho_len, signature, signature_len);
  vistrutith_d3_512s_clear_unpacked_private_key(&unpacked_sk);
  return ret;
}

int vistrutith_d3_512s_unpacked_sign_with_randomness(const vistrutith_d3_512s_unpacked_private_key_t* sk, const uint8_t* message, size_t message_len, const uint8_t* rho, size_t rho_len, uint8_t* signature, size_t* signature_len) {
  if (!sk || !signature || !signature_len || *signature_len < VISTRUTITH_D3_512S_SIGNATURE_SIZE || (!message && message_len)) {
    return -1;
  }

  const params_t* params = vistrutith_params_d3_512s();
  vistrutith_sign_opt_d3_512s(params, signature, message, message_len, sk->owf_key,
                              sk->owf_input, sk->owf_output, sk->witness, rho,
                              rho_len);
  *signature_len = VISTRUTITH_D3_512S_SIGNATURE_SIZE;
...
int vistrutith_d3_512s_verify(const uint8_t* pk, const uint8_t* message, size_t message_len, const uint8_t* signature, size_t signature_len) {
  if (!pk || !signature || signature_len != VISTRUTITH_D3_512S_SIGNATURE_SIZE || (!message && message_len)) {
    return -1;
  }

  const params_t* params = vistrutith_params_d3_512s();
  return vistrutith_verify_opt_d3_512s(params, message, message_len, signature, PK_INPUT(pk),
                                       PK_OUTPUT(pk));
}

void vistrutith_d3_512s_clear_private_key(uint8_t* key) {
  explicit_bzero(key, VISTRUTITH_D3_512S_PRIVATE_KEY_SIZE);
}
