...
void vole_commit(const params_t* params, const uint8_t* rootKey, const uint8_t* iv,
  ...
  const uint8_t* sd_precomp_base = bavc->sd_prg170;
...
bool vole_reconstruct(const params_t* params, uint8_t* com, uint8_t** q, const uint8_t* iv,
  ...
  bavc_rec.s_prg170 = malloc((size_t)L * ellhat_bytes);
  assert(bavc_rec.s);
  assert(bavc_rec.s_prg170);

  if (!bavc_reconstruct(params, &bavc_rec, decom_i, i_delta, iv)) {
    free(bavc_rec.s_prg170);
    free(bavc_rec.s);
    free(i_delta);
    return false;
  }
  ...
  const uint8_t* sd_precomp_base = bavc_rec.s_prg170;
