...
#define V(idx) (v + (idx) * outlen)
  ...
  } else if (lambda == 256u || lambda == 512u) {
    const unsigned int start_idx = sd0_bot ? 1u : 0u;
    if (start_idx < num_instances) {
#ifdef SIG_TIMING
...
void vole_commit(const params_t* params, const uint8_t* rootKey, const uint8_t* iv,
  ...
  uint8_t* sd_prg_i  = bavc->sd_prg;
  for (unsigned int i = 0; i < tau; ++i) {
#ifdef SIG_TIMING
    const uint64_t t_convert = vole_now_ns();
#endif
    // Step 6
    v_idx += convert_to_vole(params, iv, sd_i, sd_prg_i, false, i, ellhat_bytes,
...
bool vole_reconstruct(const params_t* params, uint8_t* com, uint8_t** q, const uint8_t* iv,
  ...
  bavc_rec.s_prg = malloc((size_t)L * ellhat_bytes);
  assert(bavc_rec.s && bavc_rec.s_prg);
  ...
  uint8_t* sd_prg_i  = bavc_rec.s_prg;
  for (unsigned int i = 0; i < tau; i++) {
      // Step: 2
      ...
      const uint64_t t_convert = vole_now_ns();
#endif
      const unsigned int ki = convert_to_vole(params, iv, sd_i, sd_prg_i, true, i, ellhat_bytes, NULL, qtmp,
                          r_workspace
    #ifdef SIG_TIMING
                          , &convert_stats
    #endif
      ...
      sd_prg_i += (size_t)ellhat_bytes * Ni;
  }
