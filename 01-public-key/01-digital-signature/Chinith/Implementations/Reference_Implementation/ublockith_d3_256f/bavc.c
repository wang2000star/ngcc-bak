...
static void em_leaf_commit_columns_from_nodes(const params_t* params, const uint8_t* iv,
                                              ...
                                              uint8_t* com_out, uint8_t* vole_out) {
  ...
  const unsigned int ellhat_bytes = params->ell_hat_bytes;
  ...
  for (unsigned int i = 0; i < tau; ++i) {
    ...
    prg_2_lambda_and_fixed_tweak_independent_batch_with_sd(
        key_col, tau_stride, iv, tweak, vole_tweak, sd_out + dst_idx * lambda_bytes,
        lambda_bytes, com_out + dst_idx * com_size, com_size, vole_out + dst_idx * ellhat_bytes,
        ellhat_bytes, lambda, ellhat_bytes, half_leaf_cnt);
  }
  ...
  for (unsigned int i = 0; i < tau1; ++i) {
    ...
    prg_2_lambda_and_fixed_tweak_independent_batch_with_sd(
        key_col, tau1_stride, iv, tweak, vole_tweak, sd_out + dst_idx * lambda_bytes,
        lambda_bytes, com_out + dst_idx * com_size, com_size, vole_out + dst_idx * ellhat_bytes,
        ellhat_bytes, lambda, ellhat_bytes, half_leaf_cnt);
  }
}
...
static void seed_prg_em_pipeline_commit(const params_t* params, uint8_t* nodes, const uint8_t* iv,
                                        uint8_t* sd_out, uint8_t* com_out, uint8_t* vole_out
#ifdef SIG_TIMING
                                        ,
...
static void bavc_commit_em(const params_t* params,
  ...
  bavc->sd_prg = malloc((size_t)L * params->ell_hat_bytes);
  assert(bavc->h && bavc->com && bavc->sd && bavc->sd_prg);
#ifdef SIG_TIMING
  ...
  seed_prg_em_pipeline_commit(params, nodes, iv, bavc->sd, bavc->com, bavc->sd_prg
#ifdef SIG_TIMING
                              ,
  ...
  if (use_h1_batch4) {
    ...
    if (tau1 == 0u) {
      ...
      for (; i + 4u <= tau; i += 4u) {
        ...
        com_off += 4u * (size_t)N_region;

#ifdef SIG_TIMING
        t_stage = bavc_now_ns();
#endif
        const int h1_rc = h1_finalize_batch4_same_len_dispatch(
            lambda_bytes, msg, msg_len_region, digest, 4u, h1_batch_scratch,
        ...
        t_stage = bavc_now_ns();
#endif
        H1_update(&h1_com_ctx, hi4[0], 4u * 2u * lambda_bytes);
#ifdef SIG_TIMING
      ...
      if (i < tau) {
        const size_t lane_count = (size_t)(tau - i);
        const uint8_t* msg[4] = {NULL, NULL, NULL, NULL};
        uint8_t* digest[4] = {hi4[0], hi4[1], hi4[2], hi4[3]};
        const uint8_t* com_ptr = com_base + com_off * com_size;
        for (size_t lane = 0; lane < lane_count; ++lane) {
          msg[lane] = com_ptr + lane * msg_len_region;
        }
      ...
      for (unsigned int region = 0; region < 2; ++region) {
        ...
        if (i < i_end) {
          const size_t lane_count = (size_t)(i_end - i);
          const uint8_t* msg[4] = {NULL, NULL, NULL, NULL};
          uint8_t* digest[4] = {hi4[0], hi4[1], hi4[2], hi4[3]};
          const uint8_t* com_ptr = com_base + com_off * com_size;
          for (size_t lane = 0; lane < lane_count; ++lane) {
...
static bool bavc_reconstruct_em(const params_t* params, bavc_rec_t* bavc_rec, 
  ...
  if (tau_1 == 0u) {
    for (int ii = 0; ii < (int)tau; ++ii) {
      ...
      uint8_t* sd_i_prg_out = bavc_rec->s_prg + sd_base_off[i] * ellhat_bytes;
      const uint8_t* decom_com_i = decom_i + (size_t)i * com_size;

      memset(sd_i_out, 0, lambda_bytes);
      memset(sd_i_prg_out, 0, ellhat_bytes);
      prg_2_lambda_and_fixed_tweak_independent_batch_with_sd_xor_map(
          key_base_first, first_layer_key_stride, iv, tweak, i ^ VOLE_TWEAK_OFFSET, sd_i_out,
          lambda_bytes, 0u, opened, out_first, com_size, sd_i_prg_out, ellhat_bytes,
          params->lambda, ellhat_bytes, half_leaf_cnt);
    ...
    for (int ii = 0; ii < (int)tau; ++ii) {
      ...
      if (i < tau_1) {
        ...
        prg_2_lambda_and_fixed_tweak_independent_batch_with_sd_xor_map(
            key_base_second, second_layer_key_stride, iv, tweak, i ^ VOLE_TWEAK_OFFSET, sd_i_out,
            lambda_bytes, half_leaf_cnt, opened, out_second, com_size, sd_i_prg_out,
            ellhat_bytes, params->lambda, ellhat_bytes, half_leaf_cnt);
  ...
  for (unsigned int i = 0; i < tau;) {
    ...
    const size_t msg_len_region = msg_len_arr[i];
    uint8_t hi4[4][2 * lambda_bytes];
    uint8_t* digest[4] = {hi4[0], hi4[1], hi4[2], hi4[3]};

    if (use_h1_batch4) {
      for (; i + 4u <= region_end; i += 4u) {
        const uint8_t* msg4[4];
        msg4[0] = com_batch + com_base_off[i + 0u] * com_size;
        msg4[1] = com_batch + com_base_off[i + 1u] * com_size;
        msg4[2] = com_batch + com_base_off[i + 2u] * com_size;
        msg4[3] = com_batch + com_base_off[i + 3u] * com_size;
        const int h1_rc = h1_finalize_batch4_same_len_dispatch(
            lambda_bytes, msg4, msg_len_region, digest, 4u, h1_batch_scratch,
            h1_batch_scratch_stride);
        assert(h1_rc == 0);
        (void)h1_rc;
        H1_update(&h1_com_ctx, hi4[0], 4u * 2u * lambda_bytes);
      ...
      if (i < region_end) {
        const size_t lane_count = (size_t)(region_end - i);
        const uint8_t* msg[4] = {NULL, NULL, NULL, NULL};
        for (size_t lane = 0; lane < lane_count; ++lane) {
          msg[lane] = com_batch + com_base_off[i + (unsigned int)lane] * com_size;
        }
        const int h1_rc = h1_finalize_batch4_same_len_dispatch(
            lambda_bytes, msg, msg_len_region, digest, lane_count, h1_batch_scratch,
            h1_batch_scratch_stride);
...

    {
      ...
      H1_reset(&h1_ctx);
      H1_update(&h1_ctx, msg[0], msg_len[0]);
      H1_final_keep(params, &h1_ctx, hi);
      H1_update(&h1_com_ctx, hi, lambda_bytes * 2);
    }
