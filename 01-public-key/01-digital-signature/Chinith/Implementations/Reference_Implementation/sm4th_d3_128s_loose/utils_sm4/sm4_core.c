/* The SM4 set_key and encryption functions, obtained from openssl
...
/* ================================================================== *
 *  SM4 T-box tables  (SM4_SBOX_T[j] = L(S[j]) in the four rotation  *
 *  positions used by the round function)                             *
 * ================================================================== */
...
{
    uint32_t t = 0;
    t |= ((uint32_t)sm4_compute_sbox((uint8_t)(X >> 24))) << 24;
    t |= ((uint32_t)sm4_compute_sbox((uint8_t)(X >> 16))) << 16;
    t |= ((uint32_t)sm4_compute_sbox((uint8_t)(X >>  8))) <<  8;
    t |= (uint32_t)sm4_compute_sbox((uint8_t) X        );
    return t;
}
...
int SM4_set_key(const uint8_t *key, SM4_KEY *ks)
{
    ...
    for (i = 0; i < SM4_KEY_SCHEDULE; i += 4) {
...
/* SM4_encrypt_store_state: Encrypt while recording intermediate states X[4-35]
 * 
 * This function performs SM4 encryption and stores intermediate state values
 * into the provided states array. This is useful for witness generation and
 * verification in cryptographic proofs.
 * 
 * Parameters:
 *   in: 16-byte plaintext input
 *   out: 16-byte ciphertext output
 *   ks: SM4 key schedule
 *   states: array of 32 uint32_t values to store X[4-35]
 *           (states[0] = X[4], states[1] = X[5], ..., states[31] = X[35])
 */
void SM4_encrypt_store_state(const uint8_t *in, uint8_t *out, const SM4_KEY *ks, uint32_t *states)
{
    ...
    /* Round 0: B0^=F(B1^B2^B3^rk[0]), then store as X[4] (after first round) */
    B0 ^= SM4_T(B1 ^ B2 ^ B3 ^ ks->rk[0]);
    states[0] = B0;  /* X[4] */
    
    B1 ^= SM4_T(B2 ^ B3 ^ B0 ^ ks->rk[1]);
    states[1] = B1;  /* X[5] */
    
    B2 ^= SM4_T(B3 ^ B0 ^ B1 ^ ks->rk[2]);
    states[2] = B2;  /* X[6] */
    
    B3 ^= SM4_T(B0 ^ B1 ^ B2 ^ ks->rk[3]);
    states[3] = B3;  /* X[7] */

    /* Rounds 1-7: Main encryption rounds */
    for (int round = 1; round < 8; round++) {
        int base_idx = round * 4;
        int state_idx = 4 + base_idx - 4;  /* Offset in states array */
        
        B0 ^= SM4_T(B1 ^ B2 ^ B3 ^ ks->rk[base_idx    ]);
        states[state_idx] = B0;
        
        B1 ^= SM4_T(B2 ^ B3 ^ B0 ^ ks->rk[base_idx + 1]);
        states[state_idx + 1] = B1;
        
        B2 ^= SM4_T(B3 ^ B0 ^ B1 ^ ks->rk[base_idx + 2]);
        states[state_idx + 2] = B2;
        
        B3 ^= SM4_T(B0 ^ B1 ^ B2 ^ ks->rk[base_idx + 3]);
        states[state_idx + 3] = B3;
    }

    /* Final rounds */
    B0 ^= SM4_T(B1 ^ B2 ^ B3 ^ ks->rk[28]);
    states[28] = B0;  /* X[32] */
    
    B1 ^= SM4_T(B2 ^ B3 ^ B0 ^ ks->rk[29]);
    states[29] = B1;  /* X[33] */
    
    B2 ^= SM4_T(B3 ^ B0 ^ B1 ^ ks->rk[30]);
    states[30] = B2;  /* X[34] */
    
    B3 ^= SM4_T(B0 ^ B1 ^ B2 ^ ks->rk[31]);
    states[31] = B3;  /* X[35] */
