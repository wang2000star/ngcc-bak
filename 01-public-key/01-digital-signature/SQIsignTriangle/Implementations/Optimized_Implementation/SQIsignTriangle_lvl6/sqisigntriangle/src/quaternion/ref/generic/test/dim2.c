...
quat_test_dim2_ibz_mat_2x2_inv_with_det_as_denom(void)
{
    ...
    if (ibz_mat_2x2_inv_with_det_as_denom(&inv, &m, &a)) {
        ibz_2x2_mul(&prod, &inv, &a);
        res = res || ibz_cmp(&(prod[0][0]), &m);
        res = res || ibz_cmp(&(prod[0][1]), &ibz_const_zero);
        res = res || ibz_cmp(&(prod[1][0]), &ibz_const_zero);
        res = res || ibz_cmp(&(prod[1][1]), &m);
    } else {
        res = 1;
    }
    ibz_set(&m, 12);
