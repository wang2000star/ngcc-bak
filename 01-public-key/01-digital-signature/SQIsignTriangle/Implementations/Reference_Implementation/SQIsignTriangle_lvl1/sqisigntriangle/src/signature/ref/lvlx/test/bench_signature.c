...
bench_sqisign(uint64_t bench)
{
    ...
    t0 = rdtsc();
    for (uint64_t i = 0; i < bench; ++i) {
        new_protocols_sign(&(sigs[i]), &(pks[i]), &(sks[i]), msg, sizeof(msg) / sizeof(*msg));
    }
    t1 = rdtsc();
    ms = (1000. * (float)(clock() - t) / CLOCKS_PER_SEC);
    ...
    t0 = rdtsc();
    for (uint64_t i = 0; i < bench; ++i) {
        int check = new_protocols_verify(&(sigs[i]), &(pks[i]), msg, sizeof(msg) / sizeof(*msg));
        if (!check) {
            printf("verif failed ! \n");
        }
    }
