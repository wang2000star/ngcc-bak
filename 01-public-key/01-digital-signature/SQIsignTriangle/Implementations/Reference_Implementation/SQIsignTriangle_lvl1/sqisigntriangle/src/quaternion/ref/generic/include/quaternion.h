...
/**
 * @brief Computes an equivalent left ideal J from I such that its norm satisfies a congruence.
 *
 * Given a left ideal I and integers d1, d2, this function finds an equivalent
 * left ideal J = I * alpha_conjuate / n(I) such that n(J) = k * d1 + d2. This is used in
 * the SQIsignTriangle protocol to construct specific modular degree challenges.
 *
 * @param[out] J   The output equivalent left ideal. Must be initialized before calling.
 * @param[out] k   The output integer multiplier. Must be initialized before calling.
 * @param[in]  I   The input quaternion left ideal.
 * @param[in]  d1  The integer modulus (challenge d1).
 * @param[in]  d2  The target integer residue (challenge d2).
 * @param[in]  alg The quaternion algebra.
 * @return 1 on success, 0 on failure (e.g., degeneracy condition met).
 */
int quat_equivalent_special_norm_ideal(quat_left_ideal_t *J,
                                       ibz_t *k,
