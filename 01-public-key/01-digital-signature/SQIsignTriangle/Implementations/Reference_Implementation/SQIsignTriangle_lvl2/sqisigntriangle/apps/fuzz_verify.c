...
static int load_signature(signature_t *sig, int iter) {
  char filename[256];
  int filename_len = snprintf(filename, sizeof(filename), "testcases/%s/signature%06d.bin", CRYPTO_ALGNAME, iter);
  if (filename_len < 0 || (size_t)filename_len >= sizeof(filename)) {
    fprintf(stderr, "Testcase filename is too long\n");
    return 1;
  }
...
static void verify_signature(signature_t corpus[], int testcases) {
  ...
  if (res || msglen != sizeof(msg) || memcmp(msg, sm + CRYPTO_BYTES, msglen)) {
