...
{
	// Reject keys longer than the inner-block size to prevent stack overflow
	// on memcpy(K, key, key_len_bits/8) — K is fixed L1/8 = 64 bytes.
	if (key_len_bits > L1)
	{
		fprintf(stderr, "ERROR: HMAC key longer than %d bits (L1) at %s, line %d. \n", L1, __FILE__, __LINE__);
		return HASH_FAILED;
	}
	// key expansion
