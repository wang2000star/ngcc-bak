...
void batch_S_FT_madd_gf16(unsigned char *P,
                                   const unsigned char *S,
                                   const unsigned char *FT,
                                   int m, int m_byte, int nb_monoms)
{
    for (int col = 0; col < m; col++) {
        const unsigned char *s_col = S + col * m_byte;
        for (int k = 0; k < nb_monoms; k++) {
            gf16v_madd(P + k * m_byte, s_col, gf16v_get_ele(FT + k * m_byte, col), m_byte);
        }
    }
}

void batch_S_FT_madd_gf256(unsigned char *P,
                                    const unsigned char *S,
                                    const unsigned char *FT,
                                    int m, int m_byte, int nb_monoms)
{
    for (int col = 0; col < m; col++) {
        const unsigned char *s_col = S + col * m_byte;
        for (int k = 0; k < nb_monoms; k++) {
            gf256v_madd(P + k * m_byte, s_col, gf256v_get_ele(FT + k * m_byte, col), m_byte);
        }
    }
}
