...
static void apply_S_to_FT(unsigned char *P, const unsigned char *FT, const unsigned char *S)
{
    ...
    for (int k = 0; k < nb_monoms; k++)
    {
        for (int row = 0; row < VDOO_M; row++)
        {
            ...
            {
                uint8_t s_elt = gfv_get_ele(S + col * VDOO_M_BYTE, row);
                uint8_t ft_elt = gfv_get_ele(FT + k * VDOO_M_BYTE, col);
                sum ^= gfv_mul(s_elt, ft_elt);
            }
            gfv_set_ele(P + k * VDOO_M_BYTE, row, sum);
        }
    }
}
