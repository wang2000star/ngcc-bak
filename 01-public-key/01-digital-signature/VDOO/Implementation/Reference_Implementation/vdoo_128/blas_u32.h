...
static inline void _gf256v_add_u32(uint8_t *accu_b, const uint8_t *a, unsigned _num_byte) {
    ...
    for (unsigned i = 0; i < n_u32; i++) {
      uint32_t bx;
      uint32_t ax;
      /// charges 4 bytes of accu_b in bx ///
      memcpy(&bx,accu_b+4*i,4);
      /// charges 4 bytes of a in ax ///
      memcpy(&ax,a+4*i,4);
      /// bx XOR ax ///
      bx ^= ax;
    ...
    /// computes the remaining bytes if _numb_bytes % 4 != 0 ///
    unsigned rem = _num_byte & 3;
    if( !rem ) return;
    a += (n_u32 << 2);
    accu_b += (n_u32 << 2);
    /// here thers is not a proble m to access the table ///
...
static inline void _gf16v_madd_u32(uint8_t *accu_c, const uint8_t *a, uint8_t gf16_b, unsigned _num_byte) {
    /// unsigned n = a >> b  <=>  n = a / 2^b,  if a > 0 ///
    unsigned n_u32 = _num_byte >> 2;
    for (unsigned i = 0; i < n_u32; i++) {
      uint32_t ax;
      uint32_t cx;
      /// charges 4 bytes of a ///
      memcpy(&ax,a+4*i,4);
      /// charges 4 bytes of accu_c ///
      memcpy(&cx,accu_c+4*i,4);
      /// add mult(ax, gf16_b)  in cx, in GF(16) ///
