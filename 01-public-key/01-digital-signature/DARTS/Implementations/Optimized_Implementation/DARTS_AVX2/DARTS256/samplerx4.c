...
#define NUM_GAUSSIANS (((N+STREAM256_BLOCKBYTES-1)/STREAM256_BLOCKBYTES)*STREAM256_BLOCKBYTES) 
...
//static void preprocessing_x4(sample_candidates.vec, sqr.vec, rejection.vec, &outbuf.vec[N/4]); // first four vectors are just signs
...
void sample_gauss_N_4x(uint64_t *r0, uint64_t *r1, uint64_t *r2, uint64_t *r3, 
    ...
    shake256x4_squeezeblocks_vec((uint8_t *)&outbuf.vec[0], POLY_HYPERBALL_NBLOCKS_4X, &state);
#else
    ...
    preprocessing_x4(sample_candidates.vec, sqr.vec, rejection.vec, &outbuf.vec[N/64]); // first four vectors are just signs
