...
void polyveck_cneg(polyveck *v, const uint8_t b) {
    unsigned int i, j;
    const int32x4_t v_factor = vdupq_n_s32(1 - 2 * (int32_t)b);

    for (i = 0; i < K; i++) {
        for (j = 0; j < N; j += 4) {
            int32x4_t x = vld1q_s32(&v->vec[i].coeffs[j]);
            vst1q_s32(&v->vec[i].coeffs[j], vmulq_s32(x, v_factor));
        }
    }
}
...
void polyveck_frommont(polyveck *r) {
    ...
    for (i = 0; i < K; ++i) {
        for (j = 0; j < N; j += 4) {
            int32x4_t v = vld1q_s32(&r->vec[i].coeffs[j]);
            vst1q_s32(&r->vec[i].coeffs[j], darts_neon_fqmul(v, v_montsq));
        }
    }
}
