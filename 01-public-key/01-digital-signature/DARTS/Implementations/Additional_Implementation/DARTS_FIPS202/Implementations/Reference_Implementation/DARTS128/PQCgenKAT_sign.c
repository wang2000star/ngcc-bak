...
main()
{
    ...
    do {
        ...
        m1 = (unsigned char *)calloc(mlen + CRYPTO_SIGNATUREBYTES, sizeof(unsigned char));
        sm = (unsigned char *)calloc(mlen + CRYPTO_SIGNATUREBYTES, sizeof(unsigned char));
        ...
        if ( (ret_val = crypto_sign_sign(sm, &smlen, m, mlen, sk)) != 0) {
            printf("crypto_sign_sign returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }
