                 ...
                 commitment coms[TAU][N], hash_t H_com, node seeds[TAU][N])
{
    ...
    for (int level = 0; level < height; level++) {
        // 使用 1LL 防止移位溢出
        int start = (1LL << level) - 1;
        int end = (1LL << (level + 1)) - 2;
        ...
        // 隐式屏障：确保当前层所有子节点生成完毕，再进入下一层
    }

    #pragma omp parallel
    {
        
        unsigned char msg_private[SALT_SIZE + NODE_SIZE + 5];
        msg_private[0] = 3;
        memcpy(msg_private + 1, salt, SALT_SIZE);
        ...
        for (int e = 0; e < TAU; e++)
        {
            for (int i = 0; i < N; i++)
            {
                ...
                memcpy(msg_private + 1 + SALT_SIZE, tree[idx], NODE_SIZE);
                memcpy(msg_private + 1 + SALT_SIZE + NODE_SIZE, &idx, 4);                
                pseudohash(sizeof(hash_t)*8, msg_private, (SALT_SIZE + NODE_SIZE + 5) * 8, coms[e][i]);          
                ...
                ff12b delta[TAU], hash_t h_sh)
{
    ...
    #pragma omp parallel
    {
        ...
        unsigned char nonce_private[SALT_SIZE + NODE_SIZE];
        ...
        for (int e = 0; e < TAU; e++)
        {
            for (int i = 0; i < N; i++)
            {
                memcpy(nonce_private, salt, SALT_SIZE);
                memcpy(nonce_private + SALT_SIZE, seeds[e][i], NODE_SIZE);                                
                init_random_number(&(seed_expander[e][i]), nonce_private, SALT_SIZE + NODE_SIZE);
            }
        }
    }

    // 使用 OpenMP 并行化外层 e 循环
    #pragma omp parallel for schedule(static)
    for (int e = 0; e < TAU; e++)
    {
        // ====== 将所有累加器和临时变量移入循环内部，使其成为线程私有 ======
        ff12b delta_acc = 0;
        ff12b delta_base = 0;
               ...
               unsigned char *v_grinding, int points[TAU], ff12b evals[TAU][2 * N_TUPLE - 3], hash_t h_sh)
{
    ...
    for (int e = 0; e < TAU; e++)
    {
        ...
        ff12b delta_rnd, a, tmp, t;

        memset(alpha_eval, 0, sizeof(alpha_eval));
        memset(beta_eval, 0, sizeof(beta_eval)); // 修正了原代码的 sizeof(alpha_eval) 笔误
