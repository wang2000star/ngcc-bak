                 ...
                 commitment coms[TAU][N], hash_t H_com, node seeds[TAU][N])
{
    ...
    int current_level_size = 1;       // 第一层（根）只有1个节点
    int current_level_start = 0;      // 根节点的索引为0

    while (total_internal > 0) {
        int actual_level_size = (current_level_size < total_internal) ? current_level_size : total_internal;
        
        #pragma omp parallel for
        for (int i = 0; i < actual_level_size; i++) {
            int node_idx = current_level_start + i;
            ChildNodeGen(salt, tree[node_idx], node_idx, tree[2 * node_idx + 1], tree[2 * node_idx + 2]);
        }
        
        total_internal -= actual_level_size;
        current_level_start += actual_level_size;
    ...
    #pragma omp parallel
    {
        ...
        // collapse(2) 将双重循环扁平化，将 49152 次迭代均匀分配给 16 个线程
        ...
        for (int e = 0; e < TAU; e++)
        {
            for (int i = 0; i < N; i++)
            {
                ...
                memcpy(msg + 1 + SALT_SIZE, (tree[idx]), NODE_SIZE);
                memcpy(msg + 1 + SALT_SIZE + NODE_SIZE, &idx, 4);
                pseudoXOF(320, msg, (SALT_SIZE + NODE_SIZE + 5) * 8, coms[e][i]);
            }
        }
    }
    ...
    pseudoXOF(320, msg1, N_LEAVES * COMMIT_SIZE * 8 + 8, H_com);
}
              ...
              unsigned char *salt, hash_t H_com, node seeds[TAU][N])
{
    ...
    for (int level = 0; level <= max_level; level++)
    {
        ...
        if (end > N_LEAVES - 2) end = N_LEAVES - 2;
        if (start > end) break; 
        
        #pragma omp parallel for
        for (int i = start; i <= end; i++)
        {
            // 优化：使用 64 位指针快速检查 16 字节节点是否全 0
            // NODE_SIZE (16字节) 正好是 2 个 uint64_t
            uint64_t *ptr64 = (uint64_t *)tree[i];
            if (ptr64[0] != 0 || ptr64[1] != 0)
            {
                ...
                ff12b delta[TAU], hash_t h_sh)
{
    ...
    for (int e = 0; e < TAU; e++)
    {
        ...
        ff12b delta_acc = 0;
        ff12b delta_base = 0;
        unsigned char alpha_acc[N_TUPLE_SIZE] = {0};
        unsigned char beta_acc[N_TUPLE_SIZE] = {0};
        ff12b alpha_base[N_TUPLE - 2] = {0};
        ff12b beta_base[N_TUPLE - 2] = {0};

        unsigned char alpha_rnd[N_TUPLE_SIZE];
        unsigned char beta_rnd[N_TUPLE_SIZE];
        ff12b delta_rnd, tmp;
        
        // buffer 必须是线程私有的，防止多线程同时写入导致冲突
        unsigned char buffer[(MU + 2 * N_TUPLE - 4 + 7) / 8];
    ...
    pseudoXOF(320, msg, (1 + SALT_SIZE + sizeof(H_com))*8 + (N_TUPLE-2)*2*TAU, h_sh);
               ...
               unsigned char *v_grinding, int points[TAU], ff12b evals[TAU][2 * N_TUPLE - 3], hash_t h_sh)
{
    ...
    // 使用 OpenMP 并行化外层循环 e
    // schedule(static) 保证每个线程分配连续的 e 索引，负载均衡
    #pragma omp parallel for schedule(static)
    for (int e = 0; e < TAU; e++)
    {
    ...
    for (int e = 0; e < TAU; e++)
    {
        // 声明线程私有的累加器和临时变量
        ff12b delta_eval = 0;
        ff12b alpha_eval[N_TUPLE - 2] = {0};
        ff12b beta_eval[N_TUPLE - 2] = {0};

        unsigned char buffer[(MU + 2 * N_TUPLE - 4 + 7) / 8];
        unsigned char alpha_rnd[N_TUPLE_SIZE];
        unsigned char beta_rnd[N_TUPLE_SIZE];
        ff12b delta_rnd, a, tmp;
        
        // 缓存当前线程的挑战点
        int challenge_pt = challenge_points[e];
        ...
        points[e] = challenge_pt;
        ff12b tmp_phi = points[e];

       
        ff12b_cond_add(alpha_eval, aux[e], N_TUPLE - 2, tmp_phi);
        ff12b_cond_add(beta_eval, aux[e] + N_TUPLE_SIZE, N_TUPLE - 2, tmp_phi);
