...
{
    uint8_t *buf = dst;
    uint8_t tmp[NWORDS_FIELD * 8];
    size_t copy_len = FP_ENCODED_BYTES;
    fp_t x;
...
fp_decode_reduce(fp_t *d, const void *src, size_t len)
{
    ...
    if (copy_len > FP_ENCODED_BYTES) {
        copy_len = FP_ENCODED_BYTES;
    }
    if (copy_len > sizeof(tmp)) {
        copy_len = sizeof(tmp);
    }
...
fp_decode(fp_t *d, const void *src)
{
    ...
    memset(tmp, 0, sizeof(tmp));
    memcpy(tmp, buf, FP_ENCODED_BYTES > sizeof(tmp) ? sizeof(tmp) : FP_ENCODED_BYTES);
