...
/**
 * SQIsign detached signature generation.
 *
 * The caller is responsible to allocate sufficient memory to hold sig.
 *
 * @param[out] sig Detached signature
 ...
 * @return int 0 if signature generation succeeded, non-zero otherwise.
 */
int sqisign_signature(unsigned char *sig,
 ...
 * @return int 0 if verification and opening succeeded, non-zero otherwise.
 */
