...
static int hmac(const unsigned char *msg, unsigned long long msg_len_bits, const unsigned char *hmac_key, unsigned long long hmac_key_len_bits, unsigned char *mac)
{
	// key expansion
	unsigned char K[L1 / 8] = {0};
	memcpy(K, hmac_key, hmac_key_len_bits / 8);
