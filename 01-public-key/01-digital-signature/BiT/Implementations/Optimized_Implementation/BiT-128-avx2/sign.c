...
int bit_sig_keygen(unsigned char *pk, unsigned long long *pk_len_bytes,
    ...
    polyvec_sample_S1_avx(&s_0, &e, secret_seed, &nonce);
...
int bit_sig_sign(unsigned char *sk, unsigned long long sk_len_bytes,
    ...
    unsigned char secret_seed[BIT_SEEDBYTES];
    unsigned char tr[BIT_TRBYTES];
    /* Bimodal bit b is drawn from the (NGCC-mandated) DRNG once per signature
       as a 32-byte batch (256 b-bits) instead of one DRNG call per rejection
       iteration — the DRNG cost is per-call (~5100 cy) regardless of length. */
    ...
    bit_xof256_3(temp_seed_y, BIT_SEEDBYTES, secret_seed, BIT_SEEDBYTES, rnd, BIT_SEEDBYTES, message_hash, BIT_MESSAGEBYTES);
    
    uint16_t nonce_y = 0;
    polyvecy_sample_y0y1_avx(&y, temp_seed_y, &nonce_y);
    polyvecy_sample_y2_avx(&y, temp_seed_y, &nonce_y);

    /* Force a batch fill on the first iteration. */
    ...
    while (1) {
        ...
        /* ============================================================
           Phase 1+2: z0+z1 via x4 Keccak pre-fetch + AVX2 batch check.
           nonce+0:z0, nonce+1..3:z1[0..2], fixed +4 advance.
           ============================================================ */
        ...
        /* ============================================================
           Phase 3: z2 via x2 Keccak pre-fetch + AVX2 batch check.
           nonce+0..1:z2[0..1], fixed +2 advance.
           ============================================================ */
        ...
        /* ============================================================
           Phase 4: later checks → full resample on any reject
           ============================================================ */
