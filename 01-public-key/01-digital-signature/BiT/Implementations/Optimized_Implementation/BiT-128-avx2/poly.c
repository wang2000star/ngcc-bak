...
{
    size_t ctr = 0, pos = 0;
    /* fast path: ctr+3 < len guarantees unconditional 4-element store stays in bounds */
    while (ctr + 4 <= len && pos < buflen) {
        uint8_t b = buf[pos++];
        __builtin_memcpy(a + ctr, rej_s1_vals[b], 8);  /* 4 × int16_t */
        ctr += rej_s1_cnt[b];
    }
    ...
    while (ctr < len && pos < buflen) {
        ...
        if (t0 < 3 && ctr < len) a[ctr++] = (int16_t)t0 - 1;
        if (t1 < 3 && ctr < len) a[ctr++] = (int16_t)t1 - 1;
        if (t2 < 3 && ctr < len) a[ctr++] = (int16_t)t2 - 1;
        if (t3 < 3 && ctr < len) a[ctr++] = (int16_t)t3 - 1;
    }
    return ctr;
}
...
{
    uint8_t ALIGNED_32 buf0[S1_BUFLEN], ALIGNED_32 buf1[S1_BUFLEN];
    /* dummy lanes avoid bit_xof256x2's malloc; 0xFFFF/0xFFFE are out-of-range nonces */
    uint8_t ALIGNED_32 dummy2[S1_BUFLEN], ALIGNED_32 dummy3[S1_BUFLEN];
    uint16_t n0 = *nonce, n1 = (uint16_t)(*nonce + 1U);
    *nonce = (uint16_t)(*nonce + 2U);
    bit_xof256x4(buf0, buf1, dummy2, dummy3, S1_BUFLEN, seed, BIT_SEEDBYTES,
                 n0, n1, (uint16_t)0xFFFFU, (uint16_t)0xFFFEU);
    if (rej_S1(p0->coeffs, BIT_N, buf0, S1_BUFLEN) < BIT_N) poly_sample_S1(p0, seed, &n0);
    if (rej_S1(p1->coeffs, BIT_N, buf1, S1_BUFLEN) < BIT_N) poly_sample_S1(p1, seed, &n1);
}
