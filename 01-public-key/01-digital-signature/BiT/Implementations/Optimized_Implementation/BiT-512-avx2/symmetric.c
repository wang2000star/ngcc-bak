                         ...
                         uint16_t nonce0, uint16_t nonce1)
{
#if BIT_USE_SHAKE
    uint8_t extseeds[4][BIT_SEEDBYTES + sizeof(uint16_t)];
    unsigned char dummy2[out_len_bytes], dummy3[out_len_bytes];
    ...
    shake256x4(out0, out1, dummy2, dummy3, out_len_bytes,
               extseeds[0], extseeds[1], extseeds[2], extseeds[3],
               seed_len_bytes + sizeof(uint16_t));
    secure_zero(extseeds, sizeof(extseeds));
    secure_zero(dummy2, out_len_bytes);
    secure_zero(dummy3, out_len_bytes);
#else
    (void)bit_xof256_nonce(out0, out_len_bytes, seed, seed_len_bytes, nonce0);
    (void)bit_xof256_nonce(out1, out_len_bytes, seed, seed_len_bytes, nonce1);
#endif
                         ...
                         uint16_t nonce0, uint16_t nonce1, uint16_t nonce2)
{
#if BIT_USE_SHAKE
    uint8_t extseeds[4][BIT_SEEDBYTES + sizeof(uint16_t)];
    unsigned char dummy3[out_len_bytes];
    ...
    shake256x4(out0, out1, out2, dummy3, out_len_bytes,
               extseeds[0], extseeds[1], extseeds[2], extseeds[3],
               seed_len_bytes + sizeof(uint16_t));
    secure_zero(extseeds, sizeof(extseeds));
    secure_zero(dummy3, out_len_bytes);
...
// Fix: add bit_h512 to avx2 symmetric.c
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
#if BIT_USE_SHAKE
...
{
#if BIT_USE_SHAKE
    sha3_512(out, in, inlen);          /* MUST match ref: SHA3-512, not SHAKE256 */
#else
...
void bit_h512_2(uint8_t out[64],
                ...
                const uint8_t *in1, size_t inlen1)
{
    unsigned char buf[BIT_SEEDBYTES + BIT_TRBYTES];
    size_t total = inlen0 + inlen1;
    if (total <= sizeof(buf)) {
        memcpy(buf, in0, inlen0);
        memcpy(buf + inlen0, in1, inlen1);
        bit_h512(out, buf, total);
    } else {
        unsigned char *msg = malloc(total);
        if (!msg) return;
        memcpy(msg, in0, inlen0);
        memcpy(msg + inlen0, in1, inlen1);
        bit_h512(out, msg, total);
        free(msg);
    }
}
