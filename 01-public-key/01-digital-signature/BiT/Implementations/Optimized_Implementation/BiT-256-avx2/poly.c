...
{
    size_t ctr = 0, pos = 0;
    /* fast path: ctr+4 <= len guarantees unconditional 4-element store stays in bounds */
    while (ctr + 4 <= len && pos < buflen) {
        uint8_t b = buf[pos++];
        __builtin_memcpy(a + ctr, rej_s1_vals[b], 16);  /* 4 × int32_t */
        ctr += rej_s1_cnt[b];
    }
    /* scalar tail for last ≤3 slots */
    while (ctr < len && pos < buflen) {
        uint8_t b = buf[pos++];
        uint8_t t0 = b&3, t1=(b>>2)&3, t2=(b>>4)&3, t3=b>>6;
        if (t0 < 3 && ctr < len) a[ctr++] = (int32_t)t0 - 1;
        if (t1 < 3 && ctr < len) a[ctr++] = (int32_t)t1 - 1;
        if (t2 < 3 && ctr < len) a[ctr++] = (int32_t)t2 - 1;
        if (t3 < 3 && ctr < len) a[ctr++] = (int32_t)t3 - 1;
    }
    return ctr;
}
...
/* x2 batched S1 sampling — uses bit_xof256x4 directly to avoid malloc in bit_xof256x2 */
void poly_sample_S1_x2(poly *p0, poly *p1, const uint8_t *seed, uint16_t *nonce) {
    uint8_t ALIGNED_32 buf0[BIT_SAMPLE_S1_BUFLEN], buf1[BIT_SAMPLE_S1_BUFLEN];
    uint8_t ALIGNED_32 dummy2[BIT_SAMPLE_S1_BUFLEN], dummy3[BIT_SAMPLE_S1_BUFLEN];
    uint16_t n0 = *nonce, n1 = (uint16_t)(*nonce + 1U);
    *nonce = (uint16_t)(*nonce + 2U);
    bit_xof256x4(buf0, buf1, dummy2, dummy3, BIT_SAMPLE_S1_BUFLEN,
                 seed, BIT_SEEDBYTES, n0, n1, (uint16_t)0xFFFFU, (uint16_t)0xFFFEU);
    if (rej_S1(p0->coeffs, BIT_N, buf0, BIT_SAMPLE_S1_BUFLEN) < BIT_N) poly_sample_S1(p0, seed, &n0);
    if (rej_S1(p1->coeffs, BIT_N, buf1, BIT_SAMPLE_S1_BUFLEN) < BIT_N) poly_sample_S1(p1, seed, &n1);
}
...
/* ================================================================
   poly_pack_w1 — AVX2 5-bit packing: 8 int32 coeffs → 5 bytes.
   Proof of equivalence with scalar (v = Σ t_i × 2^(5i)):
     madd([1,32,...]) → [t0|t1*32, t2|t3*32, t4|t5*32, t6|t7*32]
     OR-shift → t0|(t1<<5)|(t2<<10)|(t3<<15)|(t4<<20)|(t5<<25)|(t6<<30)|(t7<<35) = v
   v[7:0]=r[0], v[15:8]=r[1], ..., v[39:32]=r[4]. Bit-identical to scalar.
   ================================================================ */
void poly_pack_w1(unsigned char *r, const poly *a) {
    const __m256i mask5 = _mm256_set1_epi32(0x1F);
    const __m128i mult  = _mm_set1_epi32((int32_t)0x00200001); /* 16-bit: [1,32,1,32,...] */

    for (int i = 0; i < BIT_N / 8; i++) {
        __m256i c = _mm256_load_si256((const __m256i *)&a->coeffs[8*i]);
        c = _mm256_and_si256(c, mask5);

        /* int32 → int16 (safe: values ≤ 20 ≪ 32767) */
        __m128i c16 = _mm_packs_epi32(_mm256_castsi256_si128(c),
                                       _mm256_extracti128_si256(c, 1));
        /* madd: c[2k] + c[2k+1]*32 → 4 × int32, each 10 bits */
        __m128i paired = _mm_madd_epi16(c16, mult);

        /* Combine into 40-bit value: v = p0|(p1<<10)|(p2<<20)|(p3<<30) */
        uint64_t v = (uint64_t)(uint32_t)_mm_cvtsi128_si32(paired)
                   | ((uint64_t)(uint32_t)_mm_extract_epi32(paired, 1) << 10)
                   | ((uint64_t)(uint32_t)_mm_extract_epi32(paired, 2) << 20)
                   | ((uint64_t)(uint32_t)_mm_extract_epi32(paired, 3) << 30);

        r[5*i+0] = (uint8_t)v;
        r[5*i+1] = (uint8_t)(v >>  8);
        r[5*i+2] = (uint8_t)(v >> 16);
        r[5*i+3] = (uint8_t)(v >> 24);
        r[5*i+4] = (uint8_t)(v >> 32);
    }
}
...
/* Constant-time sparse challenge multiply: extended-array AVX2, sign (−1)^b applied branch-free. */
void poly_mul_challenge_no_reduce(poly *result, const poly *a, const sparse_challenge *c) {
    int32_t A_ext[BIT_N * 2];
    ...
    /* rotation pointers — no sign select (sign is uniform, applied at the end) */
    ...
    PRAGMA_UNROLL_2
    for (int pass = 0; pass < BIT_N / 64; pass++) {
