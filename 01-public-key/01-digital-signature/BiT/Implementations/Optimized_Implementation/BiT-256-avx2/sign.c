...
int bit_sig_keygen(unsigned char *pk, unsigned long long *pk_len_bytes,
    ...
    if (get_random_number(&drng_algorithm, seed_A,      BIT_SEEDBYTES*8ULL) != 0) return -1;
    if (get_random_number(&drng_algorithm, secret_seed, BIT_SEEDBYTES*8ULL) != 0) return -1;
    ...
    bit_h512(tr, pk, BIT_PUBLICKEYBYTES);
...
int bit_sig_sign(unsigned char *sk, unsigned long long sk_len_bytes,
    ...
    unsigned char secret_seed[BIT_SEEDBYTES];
    unsigned char tr[BIT_TRBYTES];
    /* Bimodal bit b: drawn once per signature as a 32-byte batch (256 b-bits)
       instead of one DRNG call per rejection iteration. */
    ...
    bit_h512_2(message_hash, tr, BIT_TRBYTES, m, (size_t)mlen);
    ...
    while (1) {
        ...
        bit_h512(challenge, hash_input, sizeof(hash_input));
...
int bit_sig_verify(const unsigned char *sig, unsigned long long siglen,
    ...
    bit_h512(c_prime, hash_input, sizeof(hash_input));
