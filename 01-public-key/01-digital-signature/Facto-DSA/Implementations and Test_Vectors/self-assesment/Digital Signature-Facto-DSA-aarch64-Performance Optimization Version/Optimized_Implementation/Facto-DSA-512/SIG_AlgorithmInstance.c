...
static int precomp_ready = 0;
static uint16_t pre_cidx[FACTO_QUAD_TERMS * FACTO_V];
...
{
    const unsigned char label[] = "FDSA-H-512";
    ...
    for (int i = 0; i < FACTO_V; i++) {
        for (int j = i; j < FACTO_V; j++, q++) {
            for (int k = 0; k < FACTO_V; k++) {
                pre_cidx[q * FACTO_V + k] = (uint16_t)cubic_index(i, j, k);
            }
        }
    }
...
static void add_linlin_to_quad_acc(uint64_t * row,
                                   fe_t coeff,
                                   const fe_t * A,
                                   const fe_t * B)
{
...
static void reduce_quad_rows(const uint64_t * src,
                             fe_t * dst,
                             int rows)
{
    for (int i = 0; i < rows * FACTO_QUAD_TERMS; i++) {
        dst[i] = f_reduce_u64(src[i]);
    }
}

static void add_quad_to_cubic_acc(uint64_t * row,
                                  const fe_t * A,
                                  const fe_t * B)
{
    for (int q = 0; q < FACTO_QUAD_TERMS; q++) {
        fe_t aij = A[q];
        if (!aij) {
            continue;
        }
        for (int k = 0; k < FACTO_V; k++) {
            int idx = pre_cidx[q * FACTO_V + k];
            row[idx] += (uint64_t)aij * B[k];
        }
    }
}

static void reduce_cubic_rows(const uint64_t * src,
                              fe_t * dst,
                              int rows)
{
    for (int i = 0; i < rows * FACTO_CUBIC_TERMS; i++) {
        dst[i] = f_reduce_u64(src[i]);
    }
}


static void add_scaled_crow_to_pacc_scalar(uint64_t *Pacc,
                                           const fe_t *Crow,
                                           fe_t trl,
                                           int len)
{
    for (int q = 0; q < len; q++) {
        Pacc[q] += (uint64_t)trl * Crow[q];
    }
}

#if FACTO_SIMD_AVX2
static void add_scaled_crow_to_pacc_avx2(uint64_t *Pacc,
                                         const fe_t *Crow,
                                         fe_t trl,
                                         int len)
{
    ...
    for (; q + 8 <= len; q += 8) {
        ...
        __m256i p32 = _mm256_mullo_epi32(c32, trl32);
        __m128i p32_lo = _mm256_castsi256_si128(p32);
        __m128i p32_hi = _mm256_extracti128_si256(p32, 1);
        __m256i p64_lo = _mm256_cvtepu32_epi64(p32_lo);
        __m256i p64_hi = _mm256_cvtepu32_epi64(p32_hi);
        __m256i acc_lo = _mm256_loadu_si256((const __m256i *)(const void *)(Pacc + q));
        __m256i acc_hi = _mm256_loadu_si256((const __m256i *)(const void *)(Pacc + q + 4));

        acc_lo = _mm256_add_epi64(acc_lo, p64_lo);
        acc_hi = _mm256_add_epi64(acc_hi, p64_hi);

        _mm256_storeu_si256((__m256i *)(void *)(Pacc + q), acc_lo);
        _mm256_storeu_si256((__m256i *)(void *)(Pacc + q + 4), acc_hi);
    }
...
static void add_scaled_crow_to_pacc_sve(uint64_t *Pacc,
                                        const fe_t *Crow,
                                        fe_t trl,
                                        int len)
{
    uint64_t q = 0;
    const svuint32_t trl32 = svdup_u32((uint32_t)trl);
    const uint64_t step16 = svcnth();
    const uint64_t step32 = svcntw();
    const uint64_t step64 = svcntd();

    while (q < (uint64_t)len) {
        svbool_t pg16 = svwhilelt_b16(q, (uint64_t)len);
        svuint16_t c16 = svld1_u16(pg16, (const uint16_t *)(const void *)(Crow + q));
        svuint32_t c32_lo = svunpklo_u32(c16);
        svuint32_t c32_hi = svunpkhi_u32(c16);
        svuint32_t p32_lo = svmul_u32_x(svwhilelt_b32(q, (uint64_t)len), c32_lo, trl32);
        svuint32_t p32_hi = svmul_u32_x(svwhilelt_b32(q + step32, (uint64_t)len), c32_hi, trl32);
        ...
        svbool_t pg64_0 = svwhilelt_b64(q, (uint64_t)len);
        svbool_t pg64_1 = svwhilelt_b64(q + step64, (uint64_t)len);
        svbool_t pg64_2 = svwhilelt_b64(q + step32, (uint64_t)len);
        svbool_t pg64_3 = svwhilelt_b64(q + step32 + step64, (uint64_t)len);
        svuint64_t a0 = svld1_u64(pg64_0, Pacc + q);
        svuint64_t a1 = svld1_u64(pg64_1, Pacc + q + step64);
        svuint64_t a2 = svld1_u64(pg64_2, Pacc + q + step32);
        svuint64_t a3 = svld1_u64(pg64_3, Pacc + q + step32 + step64);

        svst1_u64(pg64_0, Pacc + q, svadd_u64_x(pg64_0, a0, p64_0));
        svst1_u64(pg64_1, Pacc + q + step64, svadd_u64_x(pg64_1, a1, p64_1));
        svst1_u64(pg64_2, Pacc + q + step32, svadd_u64_x(pg64_2, a2, p64_2));
        svst1_u64(pg64_3, Pacc + q + step32 + step64, svadd_u64_x(pg64_3, a3, p64_3));
...
#if FACTO_SIMD_NEON && !defined(__ARM_FEATURE_SVE)
static void add_scaled_crow_to_pacc_neon(uint64_t *Pacc,
                                         const fe_t *Crow,
                                         fe_t trl,
                                         int len)
{
    ...
    for (; q + 8 <= len; q += 8) {
        ...
        uint32x4_t p32_lo = vmulq_n_u32(c32_lo, (uint32_t)trl);
        uint32x4_t p32_hi = vmulq_n_u32(c32_hi, (uint32_t)trl);
        uint64x2_t p64_0 = vmovl_u32(vget_low_u32(p32_lo));
        uint64x2_t p64_1 = vmovl_u32(vget_high_u32(p32_lo));
        uint64x2_t p64_2 = vmovl_u32(vget_low_u32(p32_hi));
        uint64x2_t p64_3 = vmovl_u32(vget_high_u32(p32_hi));
        uint64x2_t a0 = vld1q_u64(Pacc + q);
        uint64x2_t a1 = vld1q_u64(Pacc + q + 2);
        uint64x2_t a2 = vld1q_u64(Pacc + q + 4);
        uint64x2_t a3 = vld1q_u64(Pacc + q + 6);

        vst1q_u64(Pacc + q, vaddq_u64(a0, p64_0));
        vst1q_u64(Pacc + q + 2, vaddq_u64(a1, p64_1));
        vst1q_u64(Pacc + q + 4, vaddq_u64(a2, p64_2));
        vst1q_u64(Pacc + q + 6, vaddq_u64(a3, p64_3));
    }
...
static void add_scaled_crow_to_pacc(uint64_t *Pacc,
                                    const fe_t *Crow,
                                    fe_t trl,
                                    int len)
{
...
#if FACTO_SIMD_AVX2
    add_scaled_crow_to_pacc_avx2(Pacc, Crow, trl, len);
#elif FACTO_SIMD_NEON && defined(__ARM_FEATURE_SVE)
    add_scaled_crow_to_pacc_sve(Pacc, Crow, trl, len);
#elif FACTO_SIMD_NEON
    add_scaled_crow_to_pacc_neon(Pacc, Crow, trl, len);
#else
    add_scaled_crow_to_pacc_scalar(Pacc, Crow, trl, len);
#endif
}
                            ...
                            unsigned char *pk)
{
    ...
    Aacc = (uint64_t *)calloc((size_t)FACTO_N * FACTO_QUAD_TERMS, sizeof(uint64_t));
    A = (fe_t *)malloc((size_t)FACTO_N * FACTO_QUAD_TERMS * sizeof(fe_t));
    Cacc = (uint64_t *)calloc((size_t)FACTO_D * FACTO_CUBIC_TERMS, sizeof(uint64_t));
    C = (fe_t *)malloc((size_t)FACTO_D * FACTO_CUBIC_TERMS * sizeof(fe_t));
    Pacc = (uint64_t *)calloc(FACTO_CUBIC_TERMS, sizeof(uint64_t));

    if (!Aacc || !A || !Cacc || !C || !Pacc) {
        free(Aacc);
        free(A);
        free(Cacc);
        free(C);
    ...
    for (int a = 0; a < FACTO_N; a++) {
        uint64_t *Arow = Aacc + (size_t)a * FACTO_QUAD_TERMS;
        int qpos = q_offset(a);
        int rpos = 0;
        const fe_t *L1a = S + (size_t)a * FACTO_V;

        add_linlin_to_quad_acc(Arow, Q[qpos++], L1a, L1a);

        for (int u = 0; u < a; u++) {
            const fe_t *Lu = S + (size_t)u * FACTO_V;
            add_linlin_to_quad_acc(Arow, Q[qpos++], Lu, L1a);
        }
        ...
        for (int u = 0; u < a; u++) {
            for (int v = u; v < a; v++) {
                ...
                add_linlin_to_quad_acc(Arow, Q[qpos++], Lu, Lv);
            }
        }
        ...
        for (int u = 0; u < FACTO_N; u++) {
            for (int v = u; v < FACTO_N; v++, rpos++) {
                ...
                add_linlin_to_quad_acc(Arow, R[a * FACTO_R_TERMS + rpos], Lu, Lv);
            }
        }
    }

    reduce_quad_rows(Aacc, A, FACTO_N);

    for (int l = 0; l < FACTO_D; l++) {
        uint64_t *Crow = Cacc + (size_t)l * FACTO_CUBIC_TERMS;
        ...
        for (int a = amin; a <= amax; a++) {
            ...
            add_quad_to_cubic_acc(Crow, Arow, Yb);
        }
    }

    reduce_cubic_rows(Cacc, C, FACTO_D);

    for (int r = 0; r < FACTO_M; r++) {
        memset(Pacc, 0, FACTO_CUBIC_TERMS * sizeof(uint64_t));

        for (int l = 0; l < FACTO_D; l++) {
            fe_t trl = T[r * FACTO_D + l];
            const fe_t *Crow = C + (size_t)l * FACTO_CUBIC_TERMS;

            add_scaled_crow_to_pacc(Pacc, Crow, trl, FACTO_CUBIC_TERMS);
        }

        for (int q = 0; q < FACTO_CUBIC_TERMS; q++) {
            store_fe(pk + ((size_t)r * FACTO_CUBIC_TERMS + q) * FE_BYTES,
                     f_reduce_u64(Pacc[q]));
        }
    }
