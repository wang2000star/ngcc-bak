...
static inline uint64_t cycle_read_raw(void)
{
    ...
    __asm__ __volatile__("rdcycle %0" : "=r" (x) :: "memory");
    return x;
#else
    uint32_t lo;
    uint32_t hi;
    ...
    do {
        __asm__ __volatile__("rdcycleh %0" : "=r" (hi) :: "memory");
        __asm__ __volatile__("rdcycle %0" : "=r" (lo) :: "memory");
        __asm__ __volatile__("rdcycleh %0" : "=r" (hi2) :: "memory");
    } while (hi != hi2);
...
static uint64_t riscv_perf_read(void)
{
    uint64_t x;
    ssize_t n;

    x = 0;
    n = read(riscv_perf_fd, &x, sizeof(x));
...
static int riscv_perf_init(void)
{
    ...
    riscv_perf_fd = (int)syscall(__NR_perf_event_open, &attr, 0, -1, -1, 0);
    if (riscv_perf_fd < 0) {
        return 0;
    }

    if (ioctl(riscv_perf_fd, PERF_EVENT_IOC_RESET, 0) != 0 ||
        ioctl(riscv_perf_fd, PERF_EVENT_IOC_ENABLE, 0) != 0) {
        close(riscv_perf_fd);
        riscv_perf_fd = -1;
        return 0;
    }
    ...
    cycle_ok = 1;
    cycle_32bit = 0;
    cycle_source_name = "linux_perf_cpu_cycles";
    return 1;
}
#endif
...
static int cycle_init(void)
{
    ...
    if (!riscv_rdcycle_available()) {
        return riscv_perf_init();
    }
#endif

    cycle_ok = 1;
    cycle_32bit = 0;
    cycle_source_name = "riscv_cycle";
    return 1;
}
