                ...
                uint8_t* __restrict__ proof, hash_state& hasher)
{
    ...
    for (int col = -!verifier; col < static_cast<ssize_t>(P::delta_bits_v); ++col)
    {
        ...
        auto load_64_in_secpar_chunk = [](const uint8_t* chunk, size_t byte_offset) {
            if constexpr (P::secpar_bits % 64 == 0)
            {
                return poly64::load(chunk + byte_offset);
            }
            else
            {
                uint8_t tmp[8] = {0};
                constexpr size_t secpar_bytes = P::secpar_bytes;
                size_t bytes_left = secpar_bytes - byte_offset;
                size_t bytes_to_copy = std::min(sizeof(tmp), bytes_left);
                memcpy(tmp, chunk + byte_offset, bytes_to_copy);
                return poly64::load(tmp);
            }
        };
        ...
        size_t padded_gf64_coefficients =
            (padded_rows / hasher_chunk_size) * gf64_chunks_per_secpar;

        hasher_gfsecpar_state<S> state_secpar;
        hasher_gf64_state state_64;
        state_secpar.init(padded_rows / hasher_chunk_size);
        state_64.init(padded_gf64_coefficients);
        ...
        if (padded_rows - CP::QUICKSILVER_ROWS >= P::secpar_bits)
        {
            ...
            for (size_t j = 0; j < hasher_chunk_size; j += 64)
                state_64.update(&chal.hasher_key_64,
                                load_64_in_secpar_chunk(first_chunk, j / 8));
        ...
        for (; i + hasher_chunk_size <= CP::QUICKSILVER_ROWS; i += hasher_chunk_size)
        {
            ...
            for (size_t j = 0; j < hasher_chunk_size; j += 64)
                state_64.update(&chal.hasher_key_64,
                                load_64_in_secpar_chunk(to_hash + i / 8, j / 8));
        }

        SYDO_ASSERT(i == CP::QUICKSILVER_ROWS - (CP::QUICKSILVER_ROWS % P::secpar_bits));
        ...
        if (CP::QUICKSILVER_ROWS % P::secpar_bits)
        {
            ...
            for (size_t j = 0; j < hasher_chunk_size; j += 64)
                state_64.update(&chal.hasher_key_64,
                                load_64_in_secpar_chunk(last_chunk, j / 8));
        }
