...
struct ggm_forest_bavc
{
    ...
    using iv_t = typename tree_prg_t::iv_t;
    ...
    static void commit(const block_secpar<S> seed, iv_t iv,
    ...
    static void commit_from_roots(block_secpar<S>* roots, iv_t iv,
    ...
    static bool verify(iv_t iv, const uint8_t* __restrict__ opening,
...
struct one_tree_bavc
{
    ...
    static void commit(const block_secpar<S> seed, iv_t iv, block_secpar<S>* __restrict__ tree,
                       block_secpar<S>* __restrict__ leaves,
                       unsigned char* __restrict__ hashed_leaves);
    ...
    static bool verify(iv_t iv, const unsigned char* __restrict__ opening,
                       const uint8_t* __restrict__ delta, block_secpar<S>* __restrict__ leaves,
                       unsigned char* __restrict__ hashed_leaves);
};
...
using grinding_hash_state = hash_state_x8;
...
bool grind_and_open(const block_secpar<BAVC::secpar_v>* __restrict__ forest,
                    ...
                    const grinding_hash_state* hasher, uint32_t* counter_out);
...
// Collision-resistant leaf hash. First secpar bits become the output key, then the subsequent
// 2 secpar bits become the hash.
