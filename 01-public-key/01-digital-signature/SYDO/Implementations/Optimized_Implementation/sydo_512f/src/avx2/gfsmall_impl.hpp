...
{
    // SYDO_ASSERT that the upper 64 bits are zero.
    SYDO_ASSERT(_mm_extract_epi64(x.data, 1) == 0);
...
{
    // SYDO_ASSERT that the upper 8 bits are zero.
    SYDO_ASSERT((_mm_extract_epi64(x.data, 1) & 0xff00000000000000) == 0);
    // The modulus polynomial.
    const auto q8 = block128::set_low64(0b100011011);
    // X^64 // q8
    const auto m = block128::set_low64(0x11a59bcec98e023);
...
inline __m128i compress_gf16_vector(__m128i x)
{
    ...
    // SYDO_ASSERT that the upper 64 bits are zero.
    SYDO_ASSERT(_mm_extract_epi64(y, 1) == 0);
    return y;
}
