...
template <secpar S, bool verifier, size_t max_deg,
          quicksilver_term TERM = quicksilver_term::leading>
    requires (max_deg >= 1)
...
template <secpar S, size_t max_deg, quicksilver_term TERM, size_t deg> requires (deg <= max_deg)
struct quicksilver_gfsecpar<quicksilver_state<S, false, max_deg, TERM>, deg>
{
    typedef quicksilver_state<S, false, max_deg, TERM> quicksilver;
    ...
    poly_secpar<S>& value()
    {
        if constexpr (TERM == quicksilver_term::constant)
            return mac.coeffs[0];
        else
            return mac.coeffs[deg];
    }
    const poly_secpar<S>& value() const
    {
        if constexpr (TERM == quicksilver_term::constant)
            return mac.coeffs[0];
        else
...
template <secpar S, size_t max_deg, quicksilver_term TERM, size_t deg> requires (deg <= max_deg)
struct quicksilver_gf2<quicksilver_state<S, false, max_deg, TERM>, deg>
{
    typedef quicksilver_state<S, false, max_deg, TERM> quicksilver;
...
template <secpar S, size_t max_deg, quicksilver_term TERM, size_t deg> requires (deg <= max_deg)
struct quicksilver_gfsecpar<quicksilver_state<S, true, max_deg, TERM>, deg>
{
    typedef quicksilver_state<S, true, max_deg, TERM> quicksilver;
    ...
    quicksilver_gfsecpar operator+(const quicksilver_gfsecpar& other) const
    {
        quicksilver_gfsecpar out;
        out.mac = mac + other.mac;
        SYDO_ASSERT(delta_powers == other.delta_powers);
...
template <secpar S, size_t max_deg, quicksilver_term TERM, size_t deg> requires (deg <= max_deg)
struct quicksilver_gf2<quicksilver_state<S, true, max_deg, TERM>, deg> :
    public quicksilver_gfsecpar<quicksilver_state<S, true, max_deg, TERM>, deg>
{
    typedef quicksilver_state<S, true, max_deg, TERM> quicksilver;
...
public:
    ...
    quicksilver_gf2 operator+(const quicksilver_gf2& other) const
    {
        ...
        SYDO_ASSERT(base::delta_powers == other.delta_powers);
...
struct quicksilver_state_base<S, true, max_deg>
{
    ...
    poly_secpar<S> high_coeff_eval_scale;
...
template <secpar S> inline poly_secpar<S> gfsecpar_inverse(poly_secpar<S> x)
{
    poly_secpar<S> y = x;
    for (size_t i = 1; i + 1 < secpar_to_bits(S); ++i)
    {
        if constexpr (S == secpar::s512)
        {
            y = detail::mul_reduce_512(y, y);
            y = detail::mul_reduce_512(y, x);
        }
        else
        {
            y = (y * y).template reduce_to<secpar_to_bits(S)>();
            y = (y * x).template reduce_to<secpar_to_bits(S)>();
        }
    }
    if constexpr (S == secpar::s512)
        return detail::mul_reduce_512(y, y);
    else
        return (y * y).template reduce_to<secpar_to_bits(S)>();
}

template <secpar S>
ALWAYS_INLINE poly_secpar<S> gfsecpar_mul_reduce(poly_secpar<S> x, poly_secpar<S> y)
{
    if constexpr (S == secpar::s512)
...
{
    template <secpar S_, bool verifier, size_t max_deg, quicksilver_term TERM>
        requires (max_deg >= 1)
    friend struct quicksilver_state;
    ...
    static poly_2secpar<S> combine_mac_masks_scaled(const block_secpar<S>* macs,
                                                    size_t mac_masks_offset_in_witness,
                                                    poly_secpar<S> scale)
    {
        poly_2secpar<S> accumulators[8];
        ...
        for (int j = 0; j < 8; ++j)
        {
            auto x = gfsecpar_mul_reduce<S>(
                poly_secpar<S>::load(&macs[offset + secpar_to_bits(S) - 8 + j]), scale);
            accumulators[j] = poly_2secpar<S>::from(x);
        }
        ...
        for (int i = secpar_to_bits(S) - 16; i >= 0; i -= 8)
        {
            ...
            {
                auto x = gfsecpar_mul_reduce<S>(
                    poly_secpar<S>::load(&macs[offset + i + j]), scale);
                accumulators[j] = accumulators[j].shift_left_8() + poly_2secpar<S>::from(x);
            }
        }
...
template <secpar S, bool verifier, size_t max_deg, quicksilver_term TERM>
    requires (max_deg >= 1)
...
public:
    ...
    static constexpr quicksilver_term term_v = TERM;
    ...
    const uint8_t* challenge_bytes = nullptr;
                      ...
                      const uint8_t* challenge) requires (!verifier)
    {
        ...
        this->challenge_bytes = challenge;
                      ...
                      const uint8_t* challenge) requires (verifier)
    {
        ...
        const auto raw_delta = poly_secpar<S>::load_dup(&delta);
        if constexpr (TERM == quicksilver_term::constant)
        {
            delta_powers[0] = gfsecpar_inverse<S>(raw_delta);
            this->high_coeff_eval_scale = raw_delta;
        }
        else
        {
            delta_powers[0] = raw_delta;
            this->high_coeff_eval_scale = poly_secpar<S>::set_low32(1);
        }
        for (size_t i = 1; i < max_degree; ++i)
            delta_powers[i] = gfsecpar_mul_reduce<S>(delta_powers[i - 1], this->delta());
    ...
    inline quicksilver_gf2<quicksilver_state> get_witness_bit(size_t index) const
    {
        ...
        if constexpr (verifier && TERM == quicksilver_term::constant)
            out.mac = gfsecpar_mul_reduce<S>(out.mac, this->delta());
        return out;
    }
    ...
    inline void add_constraint(quicksilver_gfsecpar<quicksilver_state, deg> x)
    {
        ...
        if constexpr (verifier)
        {
            auto mac = x_max.mac;
            if constexpr (TERM == quicksilver_term::constant)
                mac = gfsecpar_mul_reduce<S>(mac, this->high_coeff_eval_scale);
            state_secpar[0].update(&key_secpar, mac);
            state_64[0].update(&key_64, mac);
        }
        else
        {
            SYDO_ASSERT(x_max.value() == poly_secpar<S>::set_zero());
            for (size_t i = 0; i < max_degree; ++i)
            {
                const size_t coeff_idx =
                    (TERM == quicksilver_term::constant) ? (i + 1) : i;
                state_secpar[i].update(&key_secpar, x_max.mac.coeffs[coeff_idx]);
                state_64[i].update(&key_64, x_max.mac.coeffs[coeff_idx]);
            }
        }
    }

    template <size_t deg>
    inline void add_prover_constraint_coeffs(
        const std::array<poly_secpar<S>, deg>& coeffs) requires (!verifier)
    {
        static_assert(deg <= max_degree);
        if constexpr (TERM == quicksilver_term::leading && deg == max_degree)
        {
            for (size_t i = 0; i < max_degree; ++i)
            {
                state_secpar[i].update(&key_secpar, coeffs[i]);
                state_64[i].update(&key_64, coeffs[i]);
            }
            return;
        }

        const auto zero = poly_secpar<S>::set_zero();
        for (size_t i = 0; i < max_degree; ++i)
        {
            poly_secpar<S> coeff = zero;
            if constexpr (TERM == quicksilver_term::constant)
            {
                if (i < deg)
                    coeff = coeffs[deg - 1 - i];
            }
            else
            {
                constexpr size_t shift = max_degree - deg;
                if (i >= shift)
                    coeff = coeffs[i - shift];
            }
            state_secpar[i].update(&key_secpar, coeff);
            state_64[i].update(&key_64, coeff);
        }
    }
        ...
        requires (!verifier)
    {
        ...
        for (size_t i = 0; i < max_degree - 1; ++i, witness_bits += secpar_to_bits(S))
        {
            ...
            if constexpr (TERM == quicksilver_term::constant)
            {
                masks.coeffs[i] += mask_value;
                masks.coeffs[i + 1] += mask_mac;
            }
            else
            {
    ...
    void verify(size_t witness_bits, const uint8_t* __restrict__ proof,
        ...
        requires (verifier)
    {
        const auto load_mask = [this](size_t offset) {
            if constexpr (TERM == quicksilver_term::constant)
                return helpers::combine_mac_masks_scaled(macs, offset, this->delta());
            else
                return helpers::combine_mac_masks(macs, offset);
        };
        auto mask = poly_2secpar<S>::set_zero();
        if (max_degree >= 2)
        {
            mask = load_mask(witness_bits);
            for (size_t i = 1; i < max_degree - 1; ++i, proof += secpar_to_bytes(S))
            {
                ...
                auto coeff = poly_secpar<S>::load_dup(proof)
                    + load_mask(witness_bits).template reduce_to<secpar_to_bits(S)>();
...
template <secpar S, size_t max_deg, quicksilver_term TERM, size_t deg> requires (deg <= max_deg)
template <size_t deg2>
inline quicksilver_gfsecpar<quicksilver_state<S, true, max_deg, TERM>, deg>::quicksilver_gfsecpar(
    poly1 const_value, const quicksilver_gfsecpar<quicksilver, deg2>& state_from) :
    delta_powers(state_from.delta_powers)
{
    if constexpr (quicksilver::term_v == quicksilver_term::constant)
    ...
    delta_powers(state_from.delta_powers)
{
    if constexpr (quicksilver::term_v == quicksilver_term::constant)
        ...
        mac = gfsecpar_mul_reduce<S>(const_value, (*delta_powers)[deg - 1]);
    else
        mac = const_value;
}

template <secpar S, size_t max_deg, quicksilver_term TERM, size_t deg> requires (deg <= max_deg)
inline quicksilver_gfsecpar<quicksilver_state<S, false, max_deg, TERM>, deg>::quicksilver_gfsecpar(
//    const quicksilver_gf2<quicksilver_state<S, false, max_deg, TERM>, deg>& other) : TODO
    ...
    mac(other.mac)
{
    if constexpr (quicksilver::term_v == quicksilver_term::constant)
    {
        ...
        for (size_t i = 0; i < deg; ++i)
            mac.coeffs[i + 1] = other.mac.coeffs[i];
    }
    else
    {
        value() = poly_secpar<S>::from_1(other.value());
...
template <size_t deg2>
inline quicksilver_gfsecpar<quicksilver_state<S, false, max_deg, TERM>, deg>::quicksilver_gfsecpar(
    const quicksilver_gfsecpar<quicksilver_state<S, false, max_deg, TERM>, deg2>& other)
    requires (deg > deg2)
{
    if constexpr (quicksilver::term_v == quicksilver_term::constant)
        mac = other.mac;
...
template <secpar S, size_t max_deg, quicksilver_term TERM, size_t deg> requires (deg <= max_deg)
template <size_t deg2>
inline quicksilver_gf2<quicksilver_state<S, false, max_deg, TERM>, deg>::quicksilver_gf2(
    const quicksilver_gf2<quicksilver_state<S, false, max_deg, TERM>, deg2>& other)
    requires (deg > deg2)
{
    if constexpr (quicksilver::term_v == quicksilver_term::constant)
        mac = other.mac;
...
template <size_t deg2>
inline quicksilver_gfsecpar<quicksilver_state<S, true, max_deg, TERM>, deg>::quicksilver_gfsecpar(
    const quicksilver_gfsecpar<quicksilver_state<S, true, max_deg, TERM>, deg2>& other)
    requires (deg > deg2)
{
    ...
    if constexpr (quicksilver::term_v == quicksilver_term::constant)
        ...
        mac = gfsecpar_mul_reduce<S>(other.mac, (*delta_powers)[deg - deg2 - 1]);
}

template <secpar S, size_t max_deg, quicksilver_term TERM, size_t deg> requires (deg <= max_deg)
template <size_t deg2>
inline quicksilver_gf2<quicksilver_state<S, true, max_deg, TERM>, deg>::quicksilver_gf2(
    const quicksilver_gf2<quicksilver_state<S, true, max_deg, TERM>, deg2>& other)
    requires (deg > deg2) :
    base(other) {}
...
inline quicksilver_gfsecpar<quicksilver_state<S, false, max_deg, TERM>, deg>::quicksilver_gfsecpar(
    ...
    requires (deg > deg2) :
    quicksilver_gfsecpar(
        quicksilver_gfsecpar<quicksilver_state<S, false, max_deg, TERM>, deg2>(other)) {}
...
inline quicksilver_gfsecpar<quicksilver_state<S, true, max_deg, TERM>, deg>::quicksilver_gfsecpar(
    ...
    requires (deg > deg2) :
    quicksilver_gfsecpar(
        quicksilver_gfsecpar<quicksilver_state<S, true, max_deg, TERM>, deg2>(other)) {}
...
ALWAYS_INLINE poly_secpar<QS::secpar_v>
quicksilver_mac_mul_reduce(poly_secpar<QS::secpar_v> x, poly_secpar<QS::secpar_v> y)
{
    return gfsecpar_mul_reduce<QS::secpar_v>(x, y);
}

template <typename QS, size_t size_a_1, size_t size_b_1>
ALWAYS_INLINE poly2d<size_a_1 + size_b_1 - 1, QS::secpar_bits>
quicksilver_mac_mul_reduce(const poly2d<size_a_1, QS::secpar_bits>& a,
                           const poly2d<size_b_1, QS::secpar_bits>& b)
{
    if constexpr (QS::secpar_v == secpar::s512)
        return mul_reduce_to<QS::secpar_bits>(a, b);
    else
        return (a * b).template reduce_to<QS::secpar_bits>();
}
...
operator*(const quicksilver_gfsecpar<QS, deg_a>& a, const quicksilver_gfsecpar<QS, deg_b>& b)
{
    ...
    out.mac = quicksilver_mac_mul_reduce<QS>(a.mac, b.mac);
    if constexpr (QS::is_verifier)
    {
        SYDO_ASSERT(a.delta_powers == b.delta_powers);
...
operator*(const quicksilver_gfsecpar<QS, deg_a>& a, const quicksilver_gf2<QS, deg_b>& b)
{
    if constexpr (!QS::is_verifier)
    {
        ...
        {
            out.mac = quicksilver_mac_mul_reduce<QS>(a.mac, b.mac).template shift_left<1>();
            out.mac += (a.mac * b.value()).template reduce_to<QS::secpar_bits>();
        }
        else
        {
            out.mac = quicksilver_mac_mul_reduce<QS>(a.mac, b.mac);
...
operator*(const quicksilver_gf2<QS, deg_a>& a, const quicksilver_gf2<QS, deg_b>& b)
{
    ...
    if constexpr (!QS::is_verifier)
    {
        if constexpr (QS::term_v == quicksilver_term::constant)
        {
            out.mac = quicksilver_mac_mul_reduce<QS>(a.mac, b.mac).template shift_left<1>();
            out.mac += (a.mac * b.value()).template reduce_to<QS::secpar_bits>() +
                       (b.mac * a.value()).template reduce_to<QS::secpar_bits>();
        }
        else
        {
            out.mac = quicksilver_mac_mul_reduce<QS>(a.mac, b.mac);
...
template <secpar S, size_t max_deg, quicksilver_term TERM, size_t deg> requires (deg <= max_deg)
quicksilver_gfsecpar<quicksilver_state<S, false, max_deg, TERM>, deg>
quicksilver_gfsecpar<quicksilver_state<S, false, max_deg, TERM>, deg>::combine_8_bits(
    const quicksilver_gf2<quicksilver_state<S, false, max_deg, TERM>, deg>* qs_bits)
{
    ...
    for (size_t i = 0; i < deg; ++i)
    {
        ...
        const size_t coeff_idx = (TERM == quicksilver_term::constant) ? (i + 1) : i;
        out.mac.coeffs[coeff_idx] = poly_secpar<S>::from_8_self(macs);
    }
...
template <secpar S, size_t max_deg, quicksilver_term TERM, size_t deg> requires (deg <= max_deg)
quicksilver_gfsecpar<quicksilver_state<S, true, max_deg, TERM>, deg>
quicksilver_gfsecpar<quicksilver_state<S, true, max_deg, TERM>, deg>::combine_8_bits(
    const quicksilver_gf2<quicksilver_state<S, true, max_deg, TERM>, deg>* qs_bits)
{
    quicksilver_gfsecpar<quicksilver, deg> out(0, qs_bits[0]);
