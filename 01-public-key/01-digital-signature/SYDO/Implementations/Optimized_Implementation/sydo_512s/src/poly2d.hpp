...
template <size_t out_size_2, size_t size_a_1, size_t size_a_2, size_t size_b_1, size_t size_b_2>
inline poly2d<size_a_1 + size_b_1 - 1, out_size_2>
mul_reduce_to(const poly2d<size_a_1, size_a_2>& a, const poly2d<size_b_1, size_b_2>& b)
{
    typedef poly2d<size_a_1 + size_b_1 - 1, out_size_2> output_poly;
    output_poly out = output_poly::set_zero();
    PRAGMA_UNROLL(4)
    for (size_t i = 0; i < size_a_1; ++i)
        PRAGMA_UNROLL(4)
        for (size_t j = 0; j < size_b_1; ++j)
        {
            if constexpr (out_size_2 == 512 && size_a_2 == 512 && size_b_2 == 512)
                out.coeffs[i + j] += detail::mul_reduce_512(a.coeffs[i], b.coeffs[j]);
            else
                out.coeffs[i + j] +=
                    (a.coeffs[i] * b.coeffs[j]).template reduce_to<out_size_2>();
        }
    return out;
}
