/*
 * Reference protocol core for SIG128_REF.
 ...
 * verification for the submitted 128-bit parameter set.
 *
...
void sig_build_public_state_direct(const unsigned char H_seed[SEED_BYTES], const unsigned char e_seed[SEED_BYTES], unsigned long e_bits[M], unsigned long y_bits[M_K + 12], RawGF2EX H_hat_raw[ROWS][COLS], RawGF2EX e_hat_prime_raw[N_PRIME_LEN], uint16_t y_hat_prime_raw[ROWS], RawGF2EX ye_raw[ROWS])
{
    unsigned long long bit_count = (unsigned long long)embded_num * (unsigned long long)M;
...
void sig_build_public_state(const unsigned char H_seed[SEED_BYTES], const unsigned char e_seed[SEED_BYTES], unsigned long H_bits[M_K + 12][M], unsigned long e_bits[M], unsigned long y_bits[M_K + 12], RawGF2EX H_hat_raw[ROWS][COLS], RawGF2EX e_hat_prime_raw[N_PRIME_LEN], uint16_t y_hat_prime_raw[ROWS], RawGF2EX ye_raw[ROWS])
{
