...
/*
 * Submission-facing wrapper for the SIG256_REF reference implementation.
 *
 * Responsibilities:
...
static int alloc_verify_buffers(VerifyBuffers *bufs)
{
    ...
    if (bufs->H_bits == NULL || bufs->y_bits == NULL || bufs->fs_commitment_input == NULL ||
        bufs->H_hat_raw == NULL || bufs->y_hat_prime_raw == NULL)
    {
        return -1;
    }
    return 0;
...
static void build_verify_public_state(const unsigned char H_seed[SEED_BYTES], unsigned long y_bits[embded_num], unsigned long H_bits[embded_num][M], RawGF2EX H_hat_raw[ROWS][COLS], uint16_t y_hat_prime_raw[ROWS])
{
...
int sig_sign(unsigned char *sk, unsigned long long sk_len_bytes, unsigned char *m, unsigned long long m_len_bytes, unsigned char *sn, unsigned long long *sn_len_bytes)
{
    ...
    Prover(ws, m, (size_t)m_len_bytes, bufs.fs_commitment_input, bufs.e_bits, bufs.H_hat_raw, bufs.y_hat_prime_raw, bufs.e_hat_prime_raw, bufs.ye_raw, &z_commit, &e_eval_commit, &v_eval_commit, &delta_raw, &outputs);
...
int sig_verify(unsigned char *pk, unsigned long long pk_len_bytes, unsigned char *sn, unsigned long long sn_len_bytes, unsigned char *m, unsigned long long m_len_bytes)
{
    ...
    rc = Verifier(ws, m, (size_t)m_len_bytes, bufs.fs_commitment_input, bufs.H_hat_raw, bufs.y_hat_prime_raw, &e_eval_commit, &v_eval_commit, root_hash, p_poly_interactive, rv_out_interactive, G_coeffs_raw, w_hat_prime_raw, commitment_open_hashes, commitment_open_hash_count, e_hat_encoded_commitment_open_output, &z_commit, &delta_raw, &vc_poly_challenge_raw, &vc_verifier_check_raw);
