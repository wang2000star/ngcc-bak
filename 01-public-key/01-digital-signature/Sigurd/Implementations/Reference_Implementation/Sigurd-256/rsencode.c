/*
 * GF(2^16) Reed-Solomon encoder for SIG256_REF.
 * The retained fixed transform covers the historical 17-to-136 packed shape;
...
enum
{
    ...
    kPrimitive = 0x1002D,
    kGenerator = 2,
    kFixedOriginalCount = 17,
    kFixedRecoveryCount = 136,
    kFixedSymbolCount = 512,
...
static uint16_t g_fixed_dft3_fwd[kFixedRadix3][kFixedRadix3];
static uint16_t g_fixed_dft3_inv[kFixedRadix3][kFixedRadix3];
...
static uint16_t g_fixed_crt_index[kFixedRadix3][kFixedRadix5][kFixedRadix17];
...
static void small_dft_3(const uint16_t input[kFixedRadix3], uint16_t output[kFixedRadix3], const uint16_t matrix[kFixedRadix3][kFixedRadix3])
{
    unsigned row;

    for (row = 0; row < kFixedRadix3; ++row)
    {
        uint16_t acc = 0u;
        unsigned col;

        for (col = 0; col < kFixedRadix3; ++col)
        {
            if (input[col] != 0u)
            {
...
/* Good-Thomas decomposition of the retained length-255 NTT as 3 x 5 x 17 small DFTs. */
static void fixed_ntt_255(
    const uint16_t input[kFixedNTTLen],
    uint16_t output[kFixedNTTLen],
    const uint16_t dft3[kFixedRadix3][kFixedRadix3],
    const uint16_t dft17[kFixedRadix17][kFixedRadix17],
    const uint16_t dft5[kFixedRadix5][kFixedRadix5])
{
    uint16_t stage0[kFixedRadix3][kFixedRadix5][kFixedRadix17];
    uint16_t stage1[kFixedRadix3][kFixedRadix5][kFixedRadix17];
    uint16_t stage2[kFixedRadix3][kFixedRadix5][kFixedRadix17];
    unsigned r3;

    for (r3 = 0; r3 < kFixedRadix3; ++r3)
    {
        unsigned r5;
        for (r5 = 0; r5 < kFixedRadix5; ++r5)
        {
            ...
            {
                stage0[r3][r5][r17] = input[g_fixed_crt_index[r3][r5][r17]];
            }
        }
    }
    ...
    for (r3 = 0; r3 < kFixedRadix3; ++r3)
    {
        ...
        for (r5 = 0; r5 < kFixedRadix5; ++r5)
        {
            small_dft_17(stage0[r3][r5], stage1[r3][r5], dft17);
        }
    }
    ...
    for (r3 = 0; r3 < kFixedRadix3; ++r3)
    {
        ...
        for (k17 = 0; k17 < kFixedRadix17; ++k17)
        {
            ...
            {
                column_in[k5] = stage1[r3][k5][k17];
            }
            small_dft_5(column_in, column_out, dft5);
            for (k5 = 0; k5 < kFixedRadix5; ++k5)
            {
                stage2[r3][k5][k17] = column_out[k5];
            }
        }
    }

    {
        ...
        for (k5 = 0; k5 < kFixedRadix5; ++k5)
        {
            ...
            for (k17 = 0; k17 < kFixedRadix17; ++k17)
            {
                uint16_t column_in[kFixedRadix3];
                uint16_t column_out[kFixedRadix3];
                unsigned k3;

                for (k3 = 0; k3 < kFixedRadix3; ++k3)
                {
                    column_in[k3] = stage2[k3][k5][k17];
                }
                small_dft_3(column_in, column_out, dft3);
                for (k3 = 0; k3 < kFixedRadix3; ++k3)
                {
                    output[g_fixed_crt_index[k3][k5][k17]] = column_out[k3];
                }
            }
        }
    }
...
static RsencodeResult ensure_fixed_tables(void)
{
    ...
    for (row = 0; row < kFixedRadix3; ++row)
    {
        ...
        for (col = 0; col < kFixedRadix3; ++col)
        {
            g_fixed_dft3_fwd[row][col] = field_pow_signed((uint32_t)(kFieldOrder / kFixedRadix3), kFixedScale3, row, col, 0);
            g_fixed_dft3_inv[row][col] = field_pow_signed((uint32_t)(kFieldOrder / kFixedRadix3), kFixedScale3, row, col, 1);
        }
    }
    for (row = 0; row < kFixedRadix5; ++row)
    {
    ...
    {
        unsigned col5;
        for (col5 = 0; col5 < kFixedRadix5; ++col5)
        {
            unsigned col17;
            for (col17 = 0; col17 < kFixedRadix17; ++col17)
            {
                g_fixed_crt_index[row][col5][col17] =
                    (uint16_t)((row * kFixedCRTStride3 + col5 * kFixedCRTStride5 + col17 * kFixedCRTStride17) % kFixedNTTLen);
            }
        }
    }
    ...
    }
    {
        ...
        fixed_ntt_255(kernel_time, g_fixed_kernel_ntt, g_fixed_dft3_fwd, g_fixed_dft17_fwd, g_fixed_dft5_fwd);
    }
    ...
    void **work_data)
{
    ...
    for (symbol = 0; symbol < kFixedSymbolCount; ++symbol)
    {
        ...
        fixed_ntt_255(poly_time, poly_ntt, g_fixed_dft3_fwd, g_fixed_dft17_fwd, g_fixed_dft5_fwd);
        for (idx = 0; idx < kFixedNTTLen; ++idx)
        {
            conv_ntt[idx] = gf_mul(poly_ntt[idx], g_fixed_kernel_ntt[idx]);
        }
        fixed_ntt_255(conv_ntt, conv_time, g_fixed_dft3_inv, g_fixed_dft17_inv, g_fixed_dft5_inv);
