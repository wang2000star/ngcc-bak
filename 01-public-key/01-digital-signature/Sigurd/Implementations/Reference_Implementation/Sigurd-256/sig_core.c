/*
 * Reference protocol core for SIG256_REF.
 ...
 * verification for the submitted 256-bit parameter set.
 *
...
/*
 * SIG256_REF uses the sparse extension modulus
 *   y^16 + y^11 + y + x
 ...
 */
void Prover(ProverWorkspace *ws, const unsigned char *message, size_t message_len, unsigned char fs_commitment_input[FS_COMMITMENT_INPUT_BYTES], unsigned long e_bits[M], RawGF2EX H_hat_raw[ROWS][COLS], uint16_t y_hat_prime_raw[ROWS], RawGF2EX e_hat_prime_raw[N_PRIME_LEN], const RawGF2EX ye_raw[ROWS], RawGF2EX *z_commit, RawGF2EX *e_eval_commit, RawGF2EX *v_eval_commit, RawGF2EX *delta_raw, ProverOutputs *outputs)
{
