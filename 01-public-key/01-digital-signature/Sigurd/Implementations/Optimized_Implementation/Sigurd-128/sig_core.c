...
enum
{
    ...
    GF_FIELD_ORDER = (1u << 16),
    GF_EXP_PERIOD = GF_FIELD_ORDER - 1u
};

static uint16_t gf_log_table[GF_FIELD_ORDER];
static uint16_t gf_exp_table[GF_EXP_PERIOD * 2u];
...
static void Initial_Computation_Verify_Raw(
    RawGF2EX yw_hat[ROWS],
...
typedef struct
{
    int16_t opening_index_group[group_size];
    const RawGF2EX *e_hat_encoded_output;
    const RawGF2EX *w_encoded_output;
    const RawGF2EX *delta_raw;
    const unsigned char (*hash_segment)[HASH_DIGEST_LENGTH];
    size_t hash_segment_count;
...
{
    RawGF2EX *yw_hat;
    const RawNZTerms (*H_hat_terms)[COLS];
    const RawGF2EX *w_hat_prime_raw;
} VerifyProjectedProductsTask;
...
static void *sig_malloc_zero_simd(size_t bytes)
{
    void *ptr = SIMDSafeAllocate(bytes);
    if (ptr == NULL)
    {
        sig_abort("ERROR: SIMD allocation failed");
    }
    return ptr;
}
...
{
    if (sm3hash(256, input, ((unsigned long long)input_len) * 8ULL, out) != 0)
    {
        sig_abort("ERROR: sm3hash failed");
    }
}
...
static void gf_init_tables(void)
{
    ...
    gf_log_table[0] = 0;
    for (i = GF_EXP_PERIOD; i < (GF_EXP_PERIOD * 2u); ++i)
    {
        gf_exp_table[i] = gf_exp_table[i - GF_EXP_PERIOD];
    }
    gf_tables_ready = 1;
}
...
static uint16_t mul_gf2_16(uint16_t a, uint16_t b)
{
    ...
    return gf_exp_table[(uint32_t)gf_log_table[a] + (uint32_t)gf_log_table[b]];
}
...
static void raw_copy_coeffs(uint16_t dest[RAW_POLY_LEN], const uint16_t src[RAW_POLY_LEN])
{
#if defined(__AVX2__)
    __m128i vec_lo = _mm_loadu_si128((const __m128i *)src);
    __m256i vec = _mm256_castsi128_si256(vec_lo);
    _mm_storeu_si128((__m128i *)dest, _mm256_castsi256_si128(vec));
    dest[8] = src[8];
    dest[9] = src[9];
#else
    memcpy(dest, src, sizeof(uint16_t) * RAW_POLY_LEN);
#endif
}

static void raw_xor_coeffs(uint16_t dest[RAW_POLY_LEN], const uint16_t lhs[RAW_POLY_LEN], const uint16_t rhs[RAW_POLY_LEN])
{
#if defined(__AVX2__)
    __m256i vec_lhs = _mm256_castsi128_si256(_mm_loadu_si128((const __m128i *)lhs));
    __m256i vec_rhs = _mm256_castsi128_si256(_mm_loadu_si128((const __m128i *)rhs));
    __m256i vec_dst = _mm256_xor_si256(vec_lhs, vec_rhs);
    _mm_storeu_si128((__m128i *)dest, _mm256_castsi256_si128(vec_dst));
    dest[8] = (uint16_t)(lhs[8] ^ rhs[8]);
    dest[9] = (uint16_t)(lhs[9] ^ rhs[9]);
#else
    int i;
    for (i = 0; i < RAW_POLY_LEN; ++i)
    {
        dest[i] = (uint16_t)(lhs[i] ^ rhs[i]);
    }
#endif
}

static int raw_equal_coeffs(const uint16_t lhs[RAW_POLY_LEN], const uint16_t rhs[RAW_POLY_LEN])
{
#if defined(__AVX2__)
    __m256i vec_lhs = _mm256_castsi128_si256(_mm_loadu_si128((const __m128i *)lhs));
    __m256i vec_rhs = _mm256_castsi128_si256(_mm_loadu_si128((const __m128i *)rhs));
    __m256i cmp = _mm256_cmpeq_epi16(vec_lhs, vec_rhs);

    return (_mm256_movemask_epi8(cmp) & 0xFFFF) == 0xFFFF && lhs[8] == rhs[8] && lhs[9] == rhs[9];
#else
    return memcmp(lhs, rhs, sizeof(uint16_t) * RAW_POLY_LEN) == 0;
#endif
}

static void add_raw(RawGF2EX *r, const RawGF2EX *a, const RawGF2EX *b)
{
    raw_xor_coeffs(r->coeffs, a->coeffs, b->coeffs);
}
...
{
    return gf_exp_table[(uint32_t)log_a + (uint32_t)log_b];
}
...
static uint16_t mul_gf2_16_by_log(uint16_t value, uint16_t log_factor)
{
    ...
    return gf_exp_table[(uint32_t)gf_log_table[value] + (uint32_t)log_factor];
}
...
static void reduced_term_xor(uint16_t out[RAW_POLY_LEN], unsigned degree, uint16_t value)
{
    ...
    {
        reduced_term_xor(out, degree - 6u, value);
        reduced_term_xor(out, degree - 7u, value);
        reduced_term_xor(out, degree - 9u, mul_gf2_16_x(value));
        reduced_term_xor(out, degree - 10u, value);
    }
}

static void mul_reduce_terms_into_coeffs(uint16_t out[RAW_POLY_LEN], const RawNZTerms *a_terms, const RawNZTerms *b_terms)
{
    uint8_t ia;
    ...
    for (ia = 0u; ia < a_terms->count; ++ia)
    {
        ...
        for (ib = 0u; ib < b_terms->count; ++ib)
        {
            reduced_term_xor(
                out,
                base_degree + (unsigned)b_terms->index[ib],
    ...
    for (ih = 0u; ih < h_terms->count; ++ih)
    {
        ...
        for (ip = 0u; ip < poly_terms->count; ++ip)
        {
            ...
            else
            {
                out[degree - 6u] ^= value;
                out[degree - 7u] ^= value;
                out[degree - 9u] ^= mul_gf2_16_x(value);
                out[degree - 10u] ^= value;
            }
        }
    }
}

static void mul_reduce_raw(RawGF2EX *r, const RawGF2EX *a, const RawGF2EX *b)
{
    RawNZTerms a_terms;
    RawNZTerms b_terms;
    ...
    memset(r->coeffs, 0, sizeof(r->coeffs));
    mul_reduce_terms_into_coeffs(r->coeffs, &a_terms, &b_terms);
}
...
static void rem_raw(RawGF2EX *r, const RawGF2EX_Product *c)
{
    ...
    for (i = RAW_PRODUCT_LEN - 1; i >= RAW_POLY_LEN; --i)
    {
        ...
        /* y^10 = y^4 + y^3 + x*y + 1 in characteristic two. */
        tmp[i - 6] ^= lead;
        tmp[i - 7] ^= lead;
        tmp[i - 9] ^= mul_gf2_16_x(lead);
        tmp[i - 10] ^= lead;
        tmp[i] = 0u;
    }
    for (i = 0; i < RAW_POLY_LEN; ++i)
...
{
    raw_copy_coeffs(dest->coeffs, src->coeffs);
}

static int raw_equals(const RawGF2EX *a, const RawGF2EX *b)
{
    return raw_equal_coeffs(a->coeffs, b->coeffs);
}
...
void generate_y(unsigned long y[M_K + 12], unsigned long e[M], unsigned long H[M_K + 12][M])
{
    ...
    {
        const unsigned long *row_ptr = H[i];
        unsigned long parity = 0u;
        int block;

        for (block = 0; block < COLS; ++block)
        {
            int base = block * 6;
            parity ^= (row_ptr[base + 0] & e[base + 0]) ^
                      (row_ptr[base + 1] & e[base + 1]) ^
                      (row_ptr[base + 2] & e[base + 2]) ^
                      (row_ptr[base + 3] & e[base + 3]) ^
                      (row_ptr[base + 4] & e[base + 4]) ^
                      (row_ptr[base + 5] & e[base + 5]);
        }
        y[i] = parity & 1u;
    }
}

#define SIG_XOF_LABEL_H "SIG128:H"
#define SIG_XOF_LABEL_E "SIG128:e"
#define SIG_XOF_LABEL_COMMITMENT "SIG128:commitment"
#define SIG_XOF_LABEL_INTERCHECK "SIG128:intercheck"
#define SIG_XOF_LABEL_POLYRES "SIG128:polyres"
#define SIG_XOF_LABEL_MASK "SIG128:mask"
#define SIG_XOF_LABEL_OPEN "SIG128:open"
    ...
    unsigned work_count)
{
    if (workspace->original_buffer == NULL)
    {
        ...
        workspace->original_buffer = (uint16_t *)sig_malloc_zero_simd((size_t)k * elements_per_block * sizeof(uint16_t));
        workspace->original_ptrs = (const void **)sig_malloc_zero((size_t)k * sizeof(void *));
        workspace->work_storage = (uint16_t *)sig_malloc_zero_simd((size_t)work_count * elements_per_block * sizeof(uint16_t));
...
static void rs_encode_with_workspace(RSWorkspace *workspace, RawGF2EX e_hat_encoded[N], const RawGF2EX e_hat_prime[N_PRIME_LEN])
{
    ...
    const unsigned recovery_block_count = 4u * k;
...
static void rs_evaluate_output_position(RawGF2EX *out, const RawGF2EX e_hat_prime[N_PRIME_LEN], unsigned logical_index)
{
    ...
    const unsigned chunk_count = 32u;
    const unsigned symbols_per_chunk = padded_n_logical / chunk_count;
    const unsigned recovery_block_count = 4u * symbols_per_chunk;
...
/* Keep the historical helper name while routing hashing through SM3. */
...
static void merkle_build_group(MerkleBuildTask *task)
{
    ...
    memcpy(task->hash_nodes, g_zero_merkle_nodes, sizeof(g_zero_merkle_nodes));
    for (j = 0; j < group_size; ++j)
    {
        int node_index = MERKLE_LEAF_OFFSET + j;
        raw_poly_to_bytes(e_bytes, &task->e_hat_group[j]);
        raw_poly_to_bytes(v_bytes, &task->v_group[j]);
    ...
    for (j = 0; j < g_merkle_recompute_count; ++j)
    {
        ...
        memcpy(combined_node, task->hash_nodes[node * 2 + 2], HASH_DIGEST_LENGTH);
        memcpy(combined_node + HASH_DIGEST_LENGTH, task->hash_nodes[node * 2 + 3], HASH_DIGEST_LENGTH);
        compute_sha256_vec(task->hash_nodes[node], combined_node, (unsigned long)sizeof(combined_node));
    }

    memcpy(combined_node, task->hash_nodes[0], HASH_DIGEST_LENGTH);
    memcpy(combined_node + HASH_DIGEST_LENGTH, task->hash_nodes[1], HASH_DIGEST_LENGTH);
...
static void merkle_verify_group(MerkleVerifyTask *task)
{
    uint8_t opened[MERKLE_LEAF_COUNT];
    uint8_t required[MERKLE_NODE_COUNT];
    ...
    for (leaf = 0; leaf < group_size; ++leaf)
    {
        if (task->opening_index_group[leaf] >= 0)
        {
            opened[leaf] = 1;
            has_opening = 1;
        }
    ...
    for (leaf = 0; leaf < group_size; ++leaf)
    {
        int open_idx = task->opening_index_group[leaf];
        if (open_idx >= 0)
        {
            RawGF2EX temp;
            ...
            mul_reduce_raw(&temp, &task->e_hat_encoded_output[open_idx], task->delta_raw);
            add_raw(&recovered_v, &task->w_encoded_output[open_idx], &temp);
            raw_poly_to_bytes(e_bytes, &task->e_hat_encoded_output[open_idx]);
...

    {
        ...
        for (idx = 0; idx < MERKLE_NODE_COUNT; ++idx)
        {
            if (required[idx])
            {
                if (hash_offset >= task->hash_segment_count || node_known[idx])
                {
                    task->success = 0;
                    return;
                }
                memcpy(node_hash[idx], task->hash_segment[hash_offset], HASH_DIGEST_LENGTH);
                node_known[idx] = 1;
                ++hash_offset;
            }
        }
...

    {
        ...
        if (!node_known[0] || !node_known[1] || hash_offset != task->hash_segment_count)
        {
            task->success = 0;
            return;
        }
        ...
        compute_sha256_vec(hash_temp, combined_node, (unsigned long)sizeof(combined_node));
        task->success = memcmp(hash_temp, task->root_hash, HASH_DIGEST_LENGTH) == 0;
    }
}

static void *merkle_verify_group_thread(void *arg)
{
    merkle_verify_group((MerkleVerifyTask *)arg);
    return NULL;
}

static void *initial_computation_verify_thread(void *arg)
{
    VerifyProjectedProductsTask *task = (VerifyProjectedProductsTask *)arg;
#ifdef SIG_PROFILE_STEPS
    uint64_t t0 = sig_profile_now_ns();
#endif

    Initial_Computation_Verify_Raw(task->yw_hat, task->H_hat_terms, task->w_hat_prime_raw);
#ifdef SIG_PROFILE_STEPS
...
int Commitment_Open_Verifier_Raw(RawGF2EX e_hat_encoded_iutput[opennum], const int trees[opennum], const unsigned char hash[MERKLE_MAX_PROOF_HASHES][HASH_DIGEST_LENGTH], size_t hash_count, const int choices[opennum], const unsigned char root_Hash[total_groups][HASH_DIGEST_LENGTH], const RawGF2EX w_hat_prime_raw[N_PRIME_LEN], const RawGF2EX *z, const RawGF2EX *e_eval, const RawGF2EX *v_eval, const RawGF2EX *delta_raw)
{
    ...
    pthread_t threads[total_groups];
    MerkleVerifyTask tasks[total_groups];
    int launched[total_groups];
    size_t hash_offset = 0;
    int i;
    ...
    for (i = 0; i < total_groups; ++i)
    {
        uint8_t opened[MERKLE_LEAF_COUNT];
        uint8_t required[MERKLE_NODE_COUNT];
        size_t group_hash_count;
        int leaf;

        memset(tasks[i].opening_index_group, -1, sizeof(tasks[i].opening_index_group));
        memset(opened, 0, sizeof(opened));
        memset(required, 0, sizeof(required));
        launched[i] = 0;

        for (leaf = 0; leaf < opennum; ++leaf)
        {
            if (trees[leaf] == i)
            {
                int leaf_index = choices[leaf] - MERKLE_LEAF_OFFSET;
                tasks[i].opening_index_group[leaf_index] = leaf;
                opened[leaf_index] = 1;
            }
        }

        group_hash_count = (size_t)collect_required_merkle_nodes(opened, required);
        tasks[i].e_hat_encoded_output = e_hat_encoded_iutput;
        tasks[i].w_encoded_output = w_encoded_open;
        tasks[i].delta_raw = delta_raw;
        tasks[i].hash_segment = hash + hash_offset;
        tasks[i].hash_segment_count = group_hash_count;
        tasks[i].root_hash = root_Hash[i];
        tasks[i].success = 0;

        if (hash_offset + group_hash_count > hash_count)
        {
            int joined;
        ...
        hash_offset += group_hash_count;

        if (pthread_create(&threads[i], NULL, merkle_verify_group_thread, &tasks[i]) == 0)
        {
            launched[i] = 1;
        }
        else
        {
            merkle_verify_group(&tasks[i]);
        }
    }

    mul_reduce_raw(&delta_times_e, delta_raw, e_eval);
    add_raw(&rhs, &delta_times_e, v_eval);
    for (i = N_PRIME_LEN - 1; i >= 0; --i)
    {
        mul_reduce_raw(&temp, &w_eval, z);
...
static void horner_eval_Raw(RawGF2EX *result, const RawGF2EX *x_val, const RawGF2EX effective_coeffs[], int degree)
{
    ...
    *result = effective_coeffs[degree];
    for (k = degree - 1; k >= 0; --k)
    {
        ...
        mul_reduce_raw(result, result, x_val);
...
void Polynomial_Challenge_Raw(RawGF2EX *vc, long n, const RawGF2EX tau[], uint16_t scalar, const RawGF2EX w[], const RawGF2EX wr[], const RawGF2EX f_coeffs[], const RawGF2EX *delta)
{
    ...
    for (i = 2; i <= 6; ++i)
    {
        mul_reduce_raw(&delta_pows[i], &delta_pows[i - 1], delta);
    }
    for (i = 0; i <= 6; ++i)
    {
        mul_reduce_raw(&effective_coeffs[i], &f_coeffs[i], &delta_pows[6 - i]);
    }
    for (i = 0; i < n; ++i)
    {
        ...
        mul_reduce_raw(&term, &tau[i], &f_prime_eval);
    ...
    for (i = 0; i < 5; ++i)
    {
        ...
        mul_reduce_raw(&term, &wr[i], &delta_pows[4 - i]);
...
static void get_gi_coeffs_from_formula_Raw(RawGF2EX gi_coeffs[6], const RawGF2EX *e_i, const RawGF2EX *v_i, const RawGF2EX f_coeffs[7])
{
    ...
    e_pows[1] = *e_i;
    mul_reduce_raw(&e_pows[2], &e_pows[1], e_i);
    mul_reduce_raw(&e_pows[4], &e_pows[2], &e_pows[2]);

    v_pows[1] = *v_i;
    for (j = 2; j <= 6; ++j)
    {
        mul_reduce_raw(&v_pows[j], &v_pows[j - 1], v_i);
    }

    mul_reduce_raw(&term1, &f_coeffs[3], &e_pows[2]);
    mul_reduce_raw(&term2, &f_coeffs[5], &e_pows[4]);
    add_raw(&sum, &f_coeffs[1], &term1);
    add_raw(&sum, &sum, &term2);
    mul_reduce_raw(&gi_coeffs[5], &sum, &v_pows[1]);

    mul_reduce_raw(&term1, &f_coeffs[3], &e_pows[1]);
    mul_reduce_raw(&term2, &f_coeffs[6], &e_pows[4]);
    add_raw(&sum, &f_coeffs[2], &term1);
    add_raw(&sum, &sum, &term2);
    mul_reduce_raw(&gi_coeffs[4], &sum, &v_pows[2]);

    mul_reduce_raw(&gi_coeffs[3], &f_coeffs[3], &v_pows[3]);

    mul_reduce_raw(&term1, &f_coeffs[5], &e_pows[1]);
    mul_reduce_raw(&term2, &f_coeffs[6], &e_pows[2]);
    add_raw(&sum, &f_coeffs[4], &term1);
    add_raw(&sum, &sum, &term2);
    mul_reduce_raw(&gi_coeffs[2], &sum, &v_pows[4]);

    mul_reduce_raw(&gi_coeffs[1], &f_coeffs[5], &v_pows[5]);
    mul_reduce_raw(&gi_coeffs[0], &f_coeffs[6], &v_pows[6]);
}
...
void ProverPolynomialResponse_Final_Raw(RawGF2EX G_coeffs[6], long n, const RawGF2EX e[], const RawGF2EX v[], const RawGF2EX f_coeffs[7], const RawGF2EX tau[], const RawGF2EX r_hat[], const RawGF2EX v_hat[], uint16_t scalar)
{
    ...
    for (i = 0; i < n; ++i)
    {
        ...
        for (j = 0; j < 6; ++j)
        {
            ...
            mul_reduce_raw(&term, &tau[i], &gi_coeffs[j]);
...
static void Masking_Setup_Raw(RawGF2EX *delta, const RawGF2EX e_hat_prime[], const RawGF2EX v_hat_prime[], RawGF2EX w_hat_prime[])
{
    ...
    for (i = 0; i < N_PRIME_LEN; ++i)
    {
        ...
        mul_reduce_raw(&term, &e_hat_prime[i], delta);
...
int Interactive_Check_Raw_verify(const RawGF2EX p_poly[SIGMA_REPETITIONS], const RawGF2EX rv_out[SIGMA_REPETITIONS], const RawGF2EX yw_hat[ROWS], const RawGF2EX w_hat_prime[N_PRIME_LEN], const RawGF2EX *delta, const uint16_t y_hat[ROWS], const uint16_t rand_challenges[SIGMA_REPETITIONS][ROWS], const uint16_t gamma_vec[SIGMA_REPETITIONS])
{
    ...
    for (s = 0; s < SIGMA_REPETITIONS; ++s)
    {
...

        {
            ...
            mul_reduce_raw(&delta_term, &p1_plus_gamma, delta);
...
void sig_build_public_state(const unsigned char H_seed[SEED_BYTES], const unsigned char e_seed[SEED_BYTES], unsigned long H_bits[M_K + 12][M], unsigned long e_bits[M], unsigned long y_bits[M_K + 12], RawGF2EX H_hat_raw[ROWS][COLS], RawNZTerms H_hat_terms[ROWS][COLS], RawGF2EX e_hat_prime_raw[N_PRIME_LEN], uint16_t y_hat_prime_raw[ROWS], RawGF2EX ye_raw[ROWS])
{
...
void sig_build_public_state_direct(const unsigned char H_seed[SEED_BYTES], const unsigned char e_seed[SEED_BYTES], unsigned long e_bits[M], unsigned long y_bits[M_K + 12], RawGF2EX H_hat_raw[ROWS][COLS], RawNZTerms H_hat_terms[ROWS][COLS], RawGF2EX e_hat_prime_raw[N_PRIME_LEN], uint16_t y_hat_prime_raw[ROWS], RawGF2EX ye_raw[ROWS])
{
    unsigned long long bit_count = (unsigned long long)embded_num * (unsigned long long)M;
...
void Prover(ProverWorkspace *ws, const unsigned char *message, size_t message_len, unsigned char fs_commitment_input[FS_COMMITMENT_INPUT_BYTES], unsigned long e_bits[M], const RawNZTerms H_hat_terms[ROWS][COLS], uint16_t y_hat_prime_raw[ROWS], RawGF2EX e_hat_prime_raw[N_PRIME_LEN], const RawGF2EX ye_raw[ROWS], RawGF2EX *z_commit, RawGF2EX *e_eval_commit, RawGF2EX *v_eval_commit, RawGF2EX *delta_raw, ProverOutputs *outputs)
{
    ...
    memcpy(ws->f_coeffs_raw, g_polydef_coeffs, sizeof(g_polydef_coeffs));
...
{
    pthread_t verify_products_thread;
    VerifyProjectedProductsTask verify_products_task;
    int verify_products_launched = 0;
    uint16_t (*rand_challenges)[ROWS];
    uint16_t zeta_raw = 0;
...
#endif
    gf_init_tables();
    verify_products_task.yw_hat = ws->yw_raw;
    verify_products_task.H_hat_terms = H_hat_terms;
    verify_products_task.w_hat_prime_raw = w_hat_prime_raw;
    if (pthread_create(&verify_products_thread, NULL, initial_computation_verify_thread, &verify_products_task) == 0)
    {
        verify_products_launched = 1;
    }
...
#endif
        if (verify_products_launched)
        {
            pthread_join(verify_products_thread, NULL);
        }
        return 0;
    }
    ...
    ensure_polydef_cache();
    memcpy(f_coeffs_raw, g_polydef_coeffs, sizeof(g_polydef_coeffs));
...
int sig_run_self_test(void)
{
    ...
    memset(&z_pow10, 0, sizeof(z_pow10));
    basis.coeffs[1] = 1u;
    z_pow10.coeffs[0] = 1u;
    for (i = 0; i < 10; ++i)
    {
        mul_raw(&prod, &z_pow10, &basis);
        rem_raw(&z_pow10, &prod);
    }
    if (z_pow10.coeffs[0] != 1u ||
        z_pow10.coeffs[1] != mul_gf2_16_x(1u) ||
        z_pow10.coeffs[3] != 1u ||
        z_pow10.coeffs[4] != 1u)
    {
        return -8;
    }
    for (i = 2; i < RAW_POLY_LEN; ++i)
    {
        if (i != 3 && i != 4 && z_pow10.coeffs[i] != 0u)
        {
            return -8;
        }
    }
