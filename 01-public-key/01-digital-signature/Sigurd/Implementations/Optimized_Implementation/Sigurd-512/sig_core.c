...
static uint16_t g_open_selection_pool[total_groups * group_size];
...
static void raw_copy_coeffs(uint16_t dest[RAW_POLY_LEN], const uint16_t src[RAW_POLY_LEN])
{
    ...
    __m256i v1 = _mm256_loadu_si256((const __m256i *)(src + 16));
    _mm256_storeu_si256((__m256i *)dest, v0);
    _mm256_storeu_si256((__m256i *)(dest + 16), v1);
#else
...
{
#if defined(__AVX2__)
    __m256i lhs0 = _mm256_loadu_si256((const __m256i *)lhs);
    __m256i lhs1 = _mm256_loadu_si256((const __m256i *)(lhs + 16));
    __m256i rhs0 = _mm256_loadu_si256((const __m256i *)rhs);
    __m256i rhs1 = _mm256_loadu_si256((const __m256i *)(rhs + 16));
    _mm256_storeu_si256((__m256i *)dest, _mm256_xor_si256(lhs0, rhs0));
    _mm256_storeu_si256((__m256i *)(dest + 16), _mm256_xor_si256(lhs1, rhs1));
#else
    int i;
...
static int raw_equal_coeffs(const uint16_t lhs[RAW_POLY_LEN], const uint16_t rhs[RAW_POLY_LEN])
{
    ...
    __m256i rhs0 = _mm256_loadu_si256((const __m256i *)rhs);
    __m256i rhs1 = _mm256_loadu_si256((const __m256i *)(rhs + 16));
    __m256i cmp0 = _mm256_cmpeq_epi16(lhs0, rhs0);
    __m256i cmp1 = _mm256_cmpeq_epi16(lhs1, rhs1);
    return _mm256_movemask_epi8(cmp0) == -1 && _mm256_movemask_epi8(cmp1) == -1;
#else
...
static void mul_raw_terms(RawGF2EX_Product *r, const RawNZTerms *a_terms, const RawNZTerms *b_terms)
{
    uint8_t ia;
...
static void mul_raw(RawGF2EX_Product *r, const RawGF2EX *a, const RawGF2EX *b)
{
    ...
    mul_raw_terms(r, &a_terms, &b_terms);
}
...
static void reduced_term_xor(uint16_t out[RAW_POLY_LEN], unsigned degree, uint16_t value)
{
    ...
    {
        reduced_term_xor(out, degree - 1u, value);
        reduced_term_xor(out, degree - 27u, value);
        reduced_term_xor(out, degree - 32u, mul_gf2_16_x(value));
    }
}
...
static void mul_reduce_low_terms_into_coeffs(uint16_t out[RAW_POLY_LEN], const RawNZTerms *h_terms, const RawNZTerms *poly_terms)
{
    ...
    for (ih = 0u; ih < h_terms->count; ++ih)
    {
        ...
        for (ip = 0u; ip < poly_terms->count; ++ip)
        {
            ...
            else if (degree <= RAW_POLY_LEN + 4u)
            {
                unsigned extra = degree - RAW_POLY_LEN;
                unsigned t;
                uint16_t x_value = mul_gf2_16_x(value);

                out[RAW_POLY_LEN - 1u] ^= value;
                for (t = 0u; t <= extra; ++t)
                {
                    out[5u + t] ^= value;
                    out[t] ^= x_value;
            ...
            {
                reduced_term_xor(out, degree, value);
            }
        }
    }
}
...
static void rs_evaluate_output_position(RawGF2EX *out, const RawGF2EX e_hat_prime[N_PRIME_LEN], unsigned logical_index)
{
    ...
    const unsigned chunk_count = 32u;
    const unsigned symbols_per_chunk = padded_n_logical / chunk_count;
    const unsigned recovery_block_count = SIG512_RS_RATE_FACTOR * symbols_per_chunk;
    ...
    for (i = 0; i < total_groups; ++i)
    {
        ...
        for (leaf = 0; leaf < opennum; ++leaf)
        {
            if (trees[leaf] == i)
            {
                ...
                tasks[i].opening_index_group[leaf_index] = (int16_t)leaf;
                opened[leaf_index] = 1;
            }
        }
...
static void merkle_verify_group(MerkleVerifyTask *task)
{
    ...
    for (leaf = 0; leaf < group_size; ++leaf)
    {
        ...
        if (open_idx >= 0)
        {
            ...
            raw_poly_pair_to_bytes(combined, &task->e_hat_encoded_output[open_idx], &recovered_v);
...
static void hash_to_rand_Open(int commitment_open_trees[opennum], int commitment_open_choices[opennum], const unsigned char hash_value[HASH_DIGEST_LENGTH])
{
    ...
    const int total_slots = total_groups * group_size;
    uint16_t *pool = g_open_selection_pool;
    int i;
