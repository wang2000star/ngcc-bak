...
static void build_verify_public_state_opt(const unsigned char H_seed[SEED_BYTES], unsigned long y_bits[embded_num], unsigned long H_bits[embded_num][M], RawGF2EX H_hat_raw[ROWS][COLS], RawNZTerms H_hat_terms[ROWS][COLS], uint16_t y_hat_prime_raw[ROWS])
{
