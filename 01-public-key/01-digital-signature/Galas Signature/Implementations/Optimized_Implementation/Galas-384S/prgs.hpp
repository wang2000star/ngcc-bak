...
template <secpar S>
struct shake_prg : public prg_base<shake_prg<S>>
{
    typedef prg_base<shake_prg> base;
    using typename base::key_t;
    using typename base::expanded_key_t;
    using typename base::iv_t;
    using typename base::block_t;
                         ...
                         const tweak_t* tweaks, const count_t* counters, block_t* output)
    {
        for (size_t i = 0; i < num_keys; ++i)
        {
            hash_state h;
            h.init_prg(S);
            h.update(&expanded_keys[i], sizeof(expanded_keys[i]));
            h.update(&iv, sizeof(iv));
            h.update(&tweaks[i], sizeof(tweaks[i]));
            h.update(&counters[i], sizeof(counters[i]));
            h.finalize(&output[i * blocks_per_key], blocks_per_key * sizeof(block_t));
        }
    }
};
