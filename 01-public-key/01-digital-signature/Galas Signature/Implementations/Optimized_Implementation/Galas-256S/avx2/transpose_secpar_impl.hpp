                                               ...
                                               unsigned int i_start, unsigned int i, size_t j_chunk)
{
        ...
        for (size_t j_unrotated = 0; j_unrotated < TRANSPOSE_CHUNK_SIZE; ++j_unrotated)
    {
        ...
        else
        {
            ...
            // Remove padding for non-256-bit secpars.  The 128-bit variant uses
            // 128 columns and stores two 128-bit output rows per 256-bit write.
            size_t used_bytes = S == secpar::s128 ? sizeof(block256) : secpar_to_bytes(S);
...
static void transposeVxN_blocks8x8(const unsigned char* input, block256* output, size_t stride)
{
    ...
    // Padding with 0s handles secpars with fewer columns than the AVX2
    // transpose width, e.g. 160 and 192.
    if constexpr (secpar_to_bits(S) < TRANSPOSE_BITS_COLS<S>)
...
template <secpar S>
static ALWAYS_INLINE void transposeVxN_scalar(const unsigned char* __restrict__ input,
                                              unsigned char* __restrict__ output, size_t stride)
{
    constexpr size_t row_bytes = secpar_to_bytes(S);
    constexpr size_t rows = TRANSPOSE_BITS_ROWS;

    memset(output, 0, rows * row_bytes);
    for (size_t col = 0; col < secpar_to_bits(S); ++col)
    {
        const uint8_t* col_in = input + col * stride;
        for (size_t row = 0; row < rows; ++row)
        {
            const uint8_t bit = (col_in[row / 8] >> (row % 8)) & 1u;
            output[row * row_bytes + col / 8] |= static_cast<uint8_t>(bit << (col % 8));
        }
    }
}

template <secpar S>
static void transposeVxN_tiled256(const unsigned char* __restrict__ input,
                                  unsigned char* __restrict__ output, size_t stride)
{
    static_assert(secpar_to_bits(S) > 256);
    static_assert(secpar_to_bits(S) % 8 == 0);

    constexpr size_t row_bytes = secpar_to_bytes(S);
    constexpr size_t tile_bits = secpar_to_bits(secpar::s256);
    constexpr size_t tile_bytes = secpar_to_bytes(secpar::s256);
    constexpr size_t rows = TRANSPOSE_BITS_ROWS;

    alignas(32) unsigned char tile_output[rows * tile_bytes];
    alignas(32) unsigned char tile_input[tile_bits * sizeof(block256)];

    for (size_t col = 0; col < secpar_to_bits(S); col += tile_bits)
    {
        const size_t remaining_bits = secpar_to_bits(S) - col;
        const size_t current_bits = remaining_bits < tile_bits ? remaining_bits : tile_bits;
        const size_t current_bytes = (current_bits + 7) / 8;

        if (current_bits == tile_bits)
        {
            transposeVxN<secpar::s256>(input + col * stride, tile_output, stride);
        }
        else
        {
            memset(tile_input, 0, sizeof(tile_input));
            for (size_t j = 0; j < current_bits; ++j)
                memcpy(tile_input + j * sizeof(block256), input + (col + j) * stride,
                       sizeof(block256));
            transposeVxN<secpar::s256>(tile_input, tile_output, sizeof(block256));
        }

        for (size_t row = 0; row < rows; ++row)
            memcpy(output + row * row_bytes + col / 8, tile_output + row * tile_bytes,
                   current_bytes);
    }
}
...
void transpose_secpar(const void* input, void* output, size_t stride, size_t rows)
{
    ...
    {
        if constexpr (S == secpar::s384 || S == secpar::s512)
            transposeVxN_tiled256<S>(in + i, out + i * secpar_to_bits(S), stride);
        else
