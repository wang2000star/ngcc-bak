                ...
                uint8_t* __restrict__ proof, hash_state& hasher)
{
    ...
    for (int col = -!verifier; col < static_cast<ssize_t>(P::delta_bits_v); ++col)
    {
        ...
        constexpr size_t hasher_chunk_size = P::secpar_bits;
        constexpr size_t gf64_chunks_per_secpar = (P::secpar_bits + 63) / 64;
        size_t padded_rows = ((CP::QUICKSILVER_ROWS + hasher_chunk_size - 1) / hasher_chunk_size) *
                             hasher_chunk_size;

        auto update_state_64 = [&](hasher_gf64_state& state, const uint8_t* chunk)
        {
            for (size_t j = 0; j < gf64_chunks_per_secpar; ++j)
            {
                constexpr size_t gf64_bytes = sizeof(uint64_t);
                size_t byte_offset = j * gf64_bytes;
                if constexpr (P::secpar_bits % 64 == 0)
                {
                    state.update(&chal.hasher_key_64, poly64::load(chunk + byte_offset));
                }
                else
                {
                    uint8_t limb[gf64_bytes] = {};
                    if (byte_offset < P::secpar_bytes)
                    {
                        size_t bytes = std::min(gf64_bytes, P::secpar_bytes - byte_offset);
                        memcpy(limb, chunk + byte_offset, bytes);
                    }
                    state.update(&chal.hasher_key_64, poly64::load(limb));
                }
            }
        };
        ...
        state_64.init((padded_rows / hasher_chunk_size) * gf64_chunks_per_secpar);
        ...
        if (padded_rows - CP::QUICKSILVER_ROWS >= P::secpar_bits)
        {
            ...
            update_state_64(state_64, first_chunk);
        ...
        for (; i + hasher_chunk_size <= CP::QUICKSILVER_ROWS; i += hasher_chunk_size)
        {
            ...
            update_state_64(state_64, to_hash + i / 8);
        }
        ...
        if (CP::QUICKSILVER_ROWS % P::secpar_bits)
        {
            ...
            update_state_64(state_64, last_chunk);
        }
