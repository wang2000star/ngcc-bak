...
static void* vector_aligned_alloc(size_t alignment, size_t size)
{
#if defined(_WIN32)
...
static void vector_aligned_free(void* ptr)
{
#if defined(_WIN32)
	_aligned_free(ptr);
#else
...
#define EXPAND_VERIFIER_SUBTREES_RECURSION(n, next) \
	...
	{ \
		...
		{ \
			...
			assert(this_chunk_size == level_lim - i); \
	...
	block_secpar* restrict leaves, block_2secpar* restrict hashed_leaves)
{
	...
	block_secpar* verifier_subtrees = vector_aligned_alloc(
	...
	vector_aligned_free(verifier_subtrees);
	return true;
}
	...
	block_2secpar* restrict hashed_leaves)
{
	...
	block_secpar *prg_output = vector_aligned_alloc(alignof(block_secpar), 3*BATCH_VECTOR_COMMIT_LEAVES * sizeof(block_secpar));
	...
	vector_aligned_free(prg_output);
}
	...
	block_secpar* restrict leaves, block_2secpar* restrict hashed_leaves)
{
	...
	init_fixed_keys(&fixed_key_tree, &fixed_key_leaf, fixed_key_iv);
	block_secpar* tree =
		vector_aligned_alloc(alignof(block_secpar), BATCH_VECTOR_COMMIT_NODES * sizeof(block_secpar));
