...
static void* vole_aligned_alloc(size_t alignment, size_t size)
{
#if defined(_WIN32)
...
static void vole_aligned_free(void* ptr)
{
#if defined(_WIN32)
	_aligned_free(ptr);
#else
...
{
	block_secpar* leaves =
		vole_aligned_alloc(alignof(block_secpar), VECTOR_COMMIT_LEAVES * sizeof(block_secpar));
	...
	const uint8_t* restrict commitment, const uint8_t* restrict opening, uint8_t* restrict check)
{
	...
	block_2secpar* hashed_leaves =
		vole_aligned_alloc(alignof(block_2secpar), VECTOR_COMMIT_LEAVES * sizeof(block_2secpar));
	...
	vole_aligned_free(hashed_leaves);
	vole_aligned_free(leaves);
	return success;
}
