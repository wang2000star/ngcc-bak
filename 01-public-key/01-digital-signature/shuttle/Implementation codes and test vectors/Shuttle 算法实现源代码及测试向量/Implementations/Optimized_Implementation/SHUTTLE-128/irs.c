 ...
 *
 * === AVX2/AVX-512 FORK ===
 * This fork vectorizes the four scalar full-poly per-coefficient passes
 * (inner_y_v, poly_shift_negacyclic, the sign-normalize negate, and
 * poly_axpy_flag) over the signed int32 `poly` type, behind
 * SHUTTLE_IRS_SIMD.  Each SIMD pass is a LANE-WISE BIT-EXACT mirror of the
 * scalar arithmetic (signed 32x32->64 cross products with int64
 * accumulate; contiguous copy + two's-complement negate; mullo by the +-1
 * flag), so the fork stays byte-identical to ref -- IRS is rejection-free
 * (exactly tau transitions) and feeds the masking, so any divergence would
 * break the KAT.  The 96-bit CDT compare and the SamplerU CLZ stay
 * scalar/already-SIMD (not the target).  CT discipline preserved:
 * branchless, no idiv/%, no float; j is PUBLIC (a challenge index) so its
 * index arithmetic is fine.
 */
#include "irs.h"
...
/* The four full-poly passes are vectorized in the avx2/avx512 forks (same
 * guard the rest of the SIMD scheme code uses).  The SIMD path is a
 * lane-wise bit-exact mirror of the scalar int32 arithmetic. */
#if (defined(USE_AVX2_NTT) && defined(__AVX2__)) || \
    (defined(USE_AVX512_NTT) && defined(__AVX512F__))
#    define SHUTTLE_IRS_SIMD 1
#    include <immintrin.h>
#endif
...
#if defined(SHUTTLE_IRS_SIMD)
/* ------------------------------------------------------------------- *
 *  SIMD mirrors of the four full-poly int32 passes.                   *
 *  Each is a lane-wise BIT-EXACT mirror of the scalar arithmetic.     *
 *  N is a multiple of 16 (256/512/1024) so the loops are exact with   *
 *  no scalar tail at the per-poly granularity.                        *
 * ------------------------------------------------------------------- */

/* signed 32x32->64 dot product of two int32[N] arrays, int64 accumulate,
 * bit-exact to the scalar  sum a[k]*b[k].  The even/odd 32-bit lanes are
 * multiplied with the signed widening multiply (mul_epi32) into int64 and
 * carried in two int64 lane accumulators (associativity over int64 is
 * exact; |a*b| <= ~2^28 and N <= 1024 so the running sum stays well within
 * int64). */
static int64_t simd_dot_i32(const int32_t *a, const int32_t *b, unsigned n)
{
#    if defined(__AVX512F__)
    ...
    for (k = 0; k < n; k += 16) {
        ...
        /* even 32-bit lanes -> eight signed 64-bit products */
        acc_e = _mm512_add_epi64(acc_e, _mm512_mul_epi32(va, vb));
        /* odd lanes shifted into even slots -> eight more products */
        __m512i ao = _mm512_srli_epi64(va, 32);
        __m512i bo = _mm512_srli_epi64(vb, 32);
        acc_o = _mm512_add_epi64(acc_o, _mm512_mul_epi32(ao, bo));
    }
    return _mm512_reduce_add_epi64(_mm512_add_epi64(acc_e, acc_o));
#    else
    ...
    for (k = 0; k < n; k += 8) {
        ...
        acc_e = _mm256_add_epi64(acc_e, _mm256_mul_epi32(va, vb));
        __m256i ao = _mm256_srli_epi64(va, 32);
        __m256i bo = _mm256_srli_epi64(vb, 32);
        acc_o = _mm256_add_epi64(acc_o, _mm256_mul_epi32(ao, bo));
    }
    __m256i s = _mm256_add_epi64(acc_e, acc_o);
    __m128i lo = _mm256_castsi256_si128(s);
    __m128i hi = _mm256_extracti128_si256(s, 1);
    __m128i t = _mm_add_epi64(lo, hi);
    return (int64_t)_mm_extract_epi64(t, 0) +
           (int64_t)_mm_extract_epi64(t, 1);
#    endif
}

/* in-place conditional negate of int32[n] by a uniform two's-complement
 * mask m (0 or -1): d[k] = (d[k]^m) - m.  Bit-exact to the scalar negate.
 * n is ARBITRARY (poly_shift_negacyclic calls this with the public shift
 * j, not a multiple of the vector width), so the tail is handled with an
 * AVX-512 write-mask / an AVX2 scalar remainder. */
static void simd_cneg_i32(int32_t *d, int32_t m, unsigned n)
{
    unsigned k = 0;
#    if defined(__AVX512F__)
    __m512i vm = _mm512_set1_epi32(m);
    for (; k + 16 <= n; k += 16) {
        __m512i c = _mm512_loadu_si512((const void *)&d[k]);
        c = _mm512_sub_epi32(_mm512_xor_si512(c, vm), vm);
        _mm512_storeu_si512((void *)&d[k], c);
    }
    if (k < n) {
        __mmask16 tail = (__mmask16)((1u << (n - k)) - 1u);
        __m512i c = _mm512_maskz_loadu_epi32(tail, (const void *)&d[k]);
        c = _mm512_sub_epi32(_mm512_xor_si512(c, vm), vm);
        _mm512_mask_storeu_epi32((void *)&d[k], tail, c);
    }
#    else
    __m256i vm = _mm256_set1_epi32(m);
    for (; k + 8 <= n; k += 8) {
        __m256i c = _mm256_loadu_si256((const __m256i *)&d[k]);
        c = _mm256_sub_epi32(_mm256_xor_si256(c, vm), vm);
        _mm256_storeu_si256((__m256i *)&d[k], c);
    }
    for (; k < n; ++k)
        d[k] = (d[k] ^ m) - m;
#    endif
...
/* z[k] += f * v[k] over int32[N], f in {-1,+1} uniform.  Bit-exact to the
 * scalar  z += f*v  (mullo low 32 bits == the two's-complement product).
 */
static void simd_axpy_i32(int32_t *z, const int32_t *v, int32_t f,
                          unsigned n)
{
    unsigned k;
#    if defined(__AVX512F__)
    ...
    for (k = 0; k < n; k += 16) {
        __m512i zc = _mm512_loadu_si512((const void *)&z[k]);
        __m512i vc = _mm512_loadu_si512((const void *)&v[k]);
        zc = _mm512_add_epi32(zc, _mm512_mullo_epi32(vc, vf));
        _mm512_storeu_si512((void *)&z[k], zc);
    }
#    else
    __m256i vf = _mm256_set1_epi32(f);
    for (k = 0; k < n; k += 8) {
        __m256i zc = _mm256_loadu_si256((const __m256i *)&z[k]);
        __m256i vc = _mm256_loadu_si256((const __m256i *)&v[k]);
        zc = _mm256_add_epi32(zc, _mm256_mullo_epi32(vc, vf));
        _mm256_storeu_si256((__m256i *)&z[k], zc);
    }
#    endif
}
#endif /* SHUTTLE_IRS_SIMD */
...
static int64_t inner_y_v(const poly y[KVEC], const poly v[KVEC])
{
    ...
    unsigned i;
    for (i = 0; i < KVEC; ++i)
        acc += simd_dot_i32(y[i].coeffs, v[i].coeffs, N);
    return acc;
#else
    int64_t acc = 0;
    unsigned i, k;
 ...
 * reference, S10.)
 *
 * SIMD form: the rotation is two contiguous blocks per poly -- the high
 * [j,N) coeffs copy src[0..N-j) verbatim, the low [0,j) coeffs copy the
 * wrapped tail src[N-j..N) negated.  We memcpy the high block (no
 * arithmetic) and copy-then-conditional-negate the low block; the negate
 * is the same (x^(-1))-(-1) = -x two's-complement form as the scalar, so
 * the result is bit-identical.  (j is PUBLIC, so the block lengths leak
 * nothing secret.)
 */
                                  ...
                                  unsigned int j)
{
    ...
    unsigned i;
    for (i = 0; i < KVEC; ++i) {
        const int32_t *s = src[i].coeffs;
        int32_t *d = v[i].coeffs;
        /* high block: d[j..N) = s[0..N-j)  (verbatim copy). */
        memcpy(d + j, s, (size_t)(N - j) * sizeof(int32_t));
        /* low block: d[0..j) = -s[N-j..N)  (copy then negate; j is the
         * public shift, handled to arbitrary length by simd_cneg_i32). */
        memcpy(d, s + (N - j), (size_t)j * sizeof(int32_t));
        simd_cneg_i32(d, -1, j);
    }
#else
    unsigned i, k;
...
static void poly_axpy_flag(poly z[KVEC], const poly v[KVEC], int64_t flag)
{
    ...
    unsigned i;
    for (i = 0; i < KVEC; ++i)
        simd_axpy_i32(z[i].coeffs, v[i].coeffs, f, N);
#else
    unsigned i, k;
    for (i = 0; i < KVEC; ++i)
        for (k = 0; k < N; ++k)
                         ...
                         sampler_u_res ell)
{
    ...
    flagmask = (t - 1) >> 63; /* -1 iff t <= 0, else 0 */
    {
        ...
        unsigned ii;
        for (ii = 0; ii < KVEC; ++ii)
            simd_cneg_i32(v[ii].coeffs, m32, N);
#else
        unsigned k, ii;
        for (ii = 0; ii < KVEC; ++ii)
            for (k = 0; k < N; ++k) {
