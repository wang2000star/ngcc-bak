/*
 * ntt_ref.c -- portable SCALAR reference implementation of the complete n=512
 * negacyclic NTT / INTT over Z_q[x]/(x^512+1), q=61441, 16-bit Montgomery.
 *
 * COMPLETE NTT: q-1 = 2^12*3*5, so 2n=1024 | q-1; x^512+1 splits into 512
 * linear factors, hence 9 butterfly levels and POINTWISE (coefficient-wise)
...
static uint16_t ZMONT[NTT_N];    /* omega^{brv9(k)} * R mod q */
...
static int brv9(int x){ int r=0; for(int i=0;i<9;i++) r|=((x>>i)&1)<<(8-i); return r; }
...
void ntt_ref_init(void){
    ...
    uint32_t w = prim_root(2*NTT_N);              /* smallest primitive 2N-th root (=21), w^N=-1 */
    for(int k=0;k<NTT_N;k++) ZMONT[k]=(uint16_t)((uint64_t)powmod(w,brv9(k))*(R%NTT_Q)%NTT_Q);
