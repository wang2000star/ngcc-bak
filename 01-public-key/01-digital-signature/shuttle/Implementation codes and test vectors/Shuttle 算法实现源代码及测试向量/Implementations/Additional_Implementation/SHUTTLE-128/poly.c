...
/* AVX-512 fork: only poly_sqnorm is vectorized (the squared-norm gate that
 * dominates Sign normcheck + Verify z-reconstruction).  Every other helper
 * is identical to ref/poly.c.  The SIMD path is a lane-wise bit-exact
 * mirror of the scalar centering + square + int64 accumulate, so the
 * avx512 KAT stays byte-exact to ref. */
#if defined(USE_AVX2_NTT) || defined(USE_AVX512_NTT)
#    define SHUTTLE_POLY_SIMD 1
#    include "simd_red.h"
#endif
...
{
#ifdef SHUTTLE_POLY_SIMD
    /* 16 coeffs / iteration: reduce32 -> center mod q (simd_bmodpm_q) ->
     * square -> accumulate in int64.  The centered |r| <= (q-1)/2 < 2^15,
     * so r*r < 2^30; the two int64x8 lane accumulators (even/odd 32-bit
     * lanes) cannot overflow over N <= 1024 terms (N*(q/2)^2 << 2^63).
     * _mm512_mul_epi32 squares the even 32-bit lanes into int64; the odd
     * lanes are shifted down 32 and squared the same way. */
    __m512i acc_e = _mm512_setzero_si512();
    __m512i acc_o = _mm512_setzero_si512();
    unsigned k;
    for (k = 0; k < N; k += 16) {
        __m512i c = _mm512_loadu_si512((const void *)&a->coeffs[k]);
        __m512i r = simd_bmodpm_q(c); /* centered (-q/2, q/2]           */
        __m512i pe = _mm512_mul_epi32(r, r); /* even lanes r^2 -> int64  */
        __m512i ro = _mm512_srli_epi64(r, 32); /* odd lanes -> even slot */
        __m512i po = _mm512_mul_epi32(ro, ro); /* odd lanes r^2 -> int64 */
        acc_e = _mm512_add_epi64(acc_e, pe);
        acc_o = _mm512_add_epi64(acc_o, po);
    }
