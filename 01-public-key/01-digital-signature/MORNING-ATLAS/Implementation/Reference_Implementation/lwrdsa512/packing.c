...
*              - const polyveck *r0: pointer to vector t0
**************************************************/
             ...
             const polyveck *t0)
{
  ...
  for(i = 0; i < L; ++i)
    polyeta_pack(sk + i*POLETA_SIZE_PACKED, s1->vec+i);//change done
  ...
  //   for(int j = 0; j < N; ++j)
  //   {
  //   printf("s1->vect[%d].coeffs[%d]=%04x\t", i, j, s1->vec[i].coeffs[j]);
  //   }
  //   printf("\n");
  // }
  ...
  //for(i = 0; i < K; ++i)
    //polyeta_pack(sk + i*POLETA_SIZE_PACKED, s2->vec+i); //change done
  ...
  //   for(int j = 0; j < N; ++j)
  //   {
  //   printf("t0->vect[%d].coeffs[%d]=%04x\t", i, j, t0->vec[i].coeffs[j]);
  //   }
  //   printf("\n");
  // }
              ...
              const poly *c)
{
  ...
  for (i = 0, j = 0; i < OMEGA; i += 8, j += 9) {
    sig[j] = h_coeffs[i] & 0xff;
    sig[j+1] = (h_coeffs[i] >> 8) | ((h_coeffs[i+1] & 0x7f) << 1);
    sig[j+2] = (h_coeffs[i+1] >> 7) | ((h_coeffs[i+2] & 0x3f) << 2);
    sig[j+3] = (h_coeffs[i+2] >> 6) | ((h_coeffs[i+3] & 0x1f) << 3);
    sig[j+4] = (h_coeffs[i+3] >> 5) | ((h_coeffs[i+4] & 0xf) << 4);
    sig[j+5] = (h_coeffs[i+4] >> 4) | ((h_coeffs[i+5] & 0x7) << 5);
    sig[j+6] = (h_coeffs[i+5] >> 3) | ((h_coeffs[i+6] & 0x3) << 6);
    sig[j+7] = (h_coeffs[i+6] >> 2) | ((h_coeffs[i+7] & 0x1) << 7);
    sig[j+8] = h_coeffs[i+7] >> 1;
  }
                ...
                const unsigned char sig[SIG_SIZE_PACKED])
{
  ...
  /* Decode h */
  for (i = 0, j = 0; i < OMEGA; i += 8, j += 9) {
    h_coeffs[i] = sig[j] | ((uint16_t)(sig[j+1] & 0x1) << 8);
    h_coeffs[i+1] = (sig[j+1] >> 1) | ((uint16_t)(sig[j+2] & 0x3) << 7);
    h_coeffs[i+2] = (sig[j+2] >> 2) | ((uint16_t)(sig[j+3] & 0x7) << 6);
    h_coeffs[i+3] = (sig[j+3] >> 3) | ((uint16_t)(sig[j+4] & 0xf) << 5);
    h_coeffs[i+4] = (sig[j+4] >> 4) | ((uint16_t)(sig[j+5] & 0x1f) << 4);
    h_coeffs[i+5] = (sig[j+5] >> 5) | ((uint16_t)(sig[j+6] & 0x3f) << 3);
    h_coeffs[i+6] = (sig[j+6] >> 6) | ((uint16_t)(sig[j+7] & 0x7f) << 2);
    h_coeffs[i+7] = (sig[j+7] >> 7) | ((uint16_t)sig[j+8] << 1);
  }
  k = 0;
  for(i = 0; i < K; ++i) {
    for(j = 0; j < N; ++j)
    ...
    for(j = k; j < sig[(9*(OMEGA>>3)) + i]; ++j)
      h->vec[i].coeffs[h_coeffs[j]] = 1;
