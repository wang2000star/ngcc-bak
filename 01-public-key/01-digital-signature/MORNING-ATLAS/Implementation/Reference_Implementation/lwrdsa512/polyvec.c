  ...
  for (unsigned int i1 = 0; i1 < K; i1++)
  {
    for (unsigned int j1 = 0; j1 < L; j1++)
    {
      ...
      for (unsigned int i2 = 0; i2 < N; i2++)
      {
        ...
        //printf("\n1t::%d",t.vec[i1].coeffs[i2]);
        t->vec[i1].coeffs[i2] = t->vec[i1].coeffs[i2] & (Q-1);
        //printf("\n2t::%d",t.vec[i1].coeffs[i2]);
      }
    }
  }
}
...
void polyveck_power2round(polyveck *v1, polyveck *v0, const polyveck *v) {
  ...
  {
    for(j = 0; j < N; ++j){
      // printf("v->vect[%d].coeffs[%d] =%04x\n", i, j, v->vec[i].coeffs[j]);
      v1->vec[i].coeffs[j] = decompose_keygen(v->vec[i].coeffs[j], &v0->vec[i].coeffs[j]);
      // printf("v1->vect[%d].coeffs[%d] =%04x, v0->vect[%d].coeffs[%d]=%04x\n", i, j, v1->vec[i].coeffs[j], i, j, v0->vec[i].coeffs[j]);
    }
    // printf("\n");
  }
  // printf("\n\n");
...
void polyveck_decompose(polyveck *v1, polyveck *v0, const polyveck *v) {
  ...
  for(i = 0; i < K; ++i)
    for(j = 0; j < N; ++j)
      v1->vec[i].coeffs[j] = decompose_sign(v->vec[i].coeffs[j],
                                       &v0->vec[i].coeffs[j]);
}
