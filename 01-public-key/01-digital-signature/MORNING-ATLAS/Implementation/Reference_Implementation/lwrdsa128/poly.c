...
{
  uint32_t aw1[SB], aw2[SB], aw3[SB], aw4[SB], aw5[SB], aw6[SB], aw7[SB];
  uint32_t bw1[SB], bw2[SB], bw3[SB], bw4[SB], bw5[SB], bw6[SB], bw7[SB];

  uint32_t w1[SB_RES]={0}, w2[SB_RES]={0}, w3[SB_RES]={0},
           w4[SB_RES]={0}, w5[SB_RES]={0}, w6[SB_RES]={0}, w7[SB_RES]={0};

  memset(res, 0, (2*N32-1)*sizeof(uint32_t));

  const uint32_t *A0 = a;
  const uint32_t *A1 = a + SB;
  const uint32_t *A2 = a + 2*SB;
  const uint32_t *A3 = a + 3*SB;

  const uint32_t *B0 = b;
  const uint32_t *B1 = b + SB;
  const uint32_t *B2 = b + 2*SB;
  const uint32_t *B3 = b + 3*SB;
