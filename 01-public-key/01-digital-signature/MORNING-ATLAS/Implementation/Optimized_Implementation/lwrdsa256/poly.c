...
void schoolbook_16_wrap(uint32_t *a, uint32_t *b, uint32_t *res)
{
    // --- Only for testing multiplication between two 16 degree polynomials modulo x^16+1 ---
    memset(res,0,N16*sizeof(int32_t));
    uint32_t prod;
    ...
    for(int i=0;i<N16;i++){
        ...
        for(int j=0;j<N16;j++){
            ...
            prod = OVERFLOWING_MUL(a[i], b[j]);//(int32_t)a[i]*(int32_t)b[j];
...
{
    // ----- for striding, schoolbook_8_wrap returns N8-degree polynomials ------
    uint32_t Z0[N8];
    uint32_t Z1[N8];
    uint32_t Z2[N8];
    ...
    memset(res, 0, (N16) * sizeof(uint32_t)); //Striding

    uint32_t A0[N8], A1[N8];
    uint32_t B0[N8], B1[N8];
    ...
    schoolbook_8_wrap(A0, B0, Z0);
    schoolbook_8_wrap(A1, B1, Z2);
    ...
    schoolbook_8_wrap(As, Bs, Z1);

    // ---- Striding Interpolation ----
    for (int i = 0; i < N8; i++) {
...
{
    // strided toom4_64 returns N64-degree polynomials as the product of two 64-degree input polynomials
    ...
    uint32_t w1[N16]={0}, w2[N16]={0}, w3[N16]={0},
             w4[N16]={0}, w5[N16]={0}, w6[N16]={0}, w7[N16]={0};

    memset(res, 0, N64*sizeof(uint32_t));

    uint32_t A0[N16], A1[N16], A2[N16], A3[N16];
    uint32_t B0[N16], B1[N16], B2[N16], B3[N16];
...
void toom_cook_4way (const uint32_t *a1, const uint32_t *b1, uint32_t *result) {
    ...
    // outputs a polynomial of length N (not 2N-1)
    uint32_t inv3 = 2863311531, inv9 = 954437177, inv15 = 4008636143;//Inverse with respect to 2^32

    uint32_t aw1[N64], aw2[N64], aw3[N64], aw4[N64], aw5[N64], aw6[N64], aw7[N64];
    uint32_t bw1[N64], bw2[N64], bw3[N64], bw4[N64], bw5[N64], bw6[N64], bw7[N64];

    // ----- Striding Implementation -----
    uint32_t w1[N64] = {0}, w2[N64] = {0}, w3[N64] = {0}, w4[N64] = {0},
                            w5[N64] = {0}, w6[N64] = {0}, w7[N64] = {0};
    // -----------------------------------

    uint32_t r0, r1, r2, r3, r4, r5, r6, r7;

    uint32_t A0[N64], A1[N64], A2[N64], A3[N64];
    uint32_t B0[N64], B1[N64], B2[N64], B3[N64];
