...
{
    /* uint32_t: 1024 bytes, always in L1, 8-wide AVX2 mullo */
    ...
    /* ------------------------------------------------------------------ */
    /* Inner loop: r[i+j] += OVERFLOWING_MUL(a[i], b[j])                 */
    /*                                                                     */
    /* For fixed i, j iterates 0..N-1 → r[i+j] is contiguous:           */
    /*   broadcast a[i] → _mm256_set1_epi32                               */
    /*   load b[j..j+7] → _mm256_loadu_si256                              */
    /*   load r[i+j..i+j+7] → _mm256_loadu_si256                         */
    /*   mullo + add → 8 OVERFLOWING_MULs in one instruction pair         */
    /* ------------------------------------------------------------------ */
    ...
    /* ------------------------------------------------------------------ */
    /* Negacyclic fold + mod-Q — fused single AVX2 pass                   */
    /*   c[k] = (r[k] + Q - r[k+N]) & (Q-1),  k = 0..N-2                */
    /*   c[N-1] = r[N-1] & (Q-1)              (no r[2N-1] term)          */
    /* ------------------------------------------------------------------ */
    __m256i vqmask = _mm256_set1_epi32((int)(Q - 1));
    __m256i vQ     = _mm256_set1_epi32((int)Q);
    uint32_t * restrict cc = c->coeffs;
 
    /* k = 0..N-9: 15 full AVX2 chunks of 8 (covers k=0..119 for N=128) */
    ...
    /* Scalar tail: k = N-8..N-2 (7 terms), then k = N-1 (no fold term) */
 ...
 *              output stream from SM3_XOF_136(seed|nonce)
 *
 * Arguments:   - poly *a: pointer to output polynomial
 ...
 *              sampling on output stream of SM3_XOF_136(seed|nonce)
 *
 * Arguments:   - poly *a: pointer to output polynomial
 *              - const unsigned char seed[]: byte array with seed of length
 *                                            SEEDBYTES + CRHBYTES
...
void polyt1_pack(unsigned char *r, const poly *a) {
  ...
  for(i = 0, j = 0; i < N; i += 8, j += 9) {
    r[j] = a->coeffs[i] & 0xff;
    r[j+1] = (a->coeffs[i] >> 8) | ((a->coeffs[i+1] & 0x7f) << 1);
    r[j+2] = (a->coeffs[i+1] >> 7) | ((a->coeffs[i+2] & 0x3f) << 2);
    r[j+3] = (a->coeffs[i+2] >> 6) | ((a->coeffs[i+3] & 0x1f) << 3);
    r[j+4] = (a->coeffs[i+3] >> 5) | ((a->coeffs[i+4] & 0xf) << 4);
    r[j+5] = (a->coeffs[i+4] >> 4) | ((a->coeffs[i+5] & 0x7) << 5);
    r[j+6] = (a->coeffs[i+5] >> 3) | ((a->coeffs[i+6] & 0x3) << 6);
    r[j+7] = (a->coeffs[i+6] >> 2) | ((a->coeffs[i+7] & 0x1) << 7);
...
void polyt1_unpack(poly *r, const unsigned char *a) {
  ...
  for(i = 0, j = 0; i < N; i += 8, j += 9) {
    r->coeffs[i] = a[j] | ((uint32_t)(a[j+1] & 0x1) << 8);
    r->coeffs[i+1] = (a[j+1] >> 1) | ((uint32_t)(a[j+2] & 0x3) << 7);
    r->coeffs[i+2] = (a[j+2] >> 2) | ((uint32_t)(a[j+3] & 0x7) << 6);
    r->coeffs[i+3] = (a[j+3] >> 3) | ((uint32_t)(a[j+4] & 0xf) << 5);
    r->coeffs[i+4] = (a[j+4] >> 4) | ((uint32_t)(a[j+5] & 0x1f) << 4);
    r->coeffs[i+5] = (a[j+5] >> 5) | ((uint32_t)(a[j+6] & 0x3f) << 3);
    r->coeffs[i+6] = (a[j+6] >> 6) | ((uint32_t)(a[j+7] & 0x7f) << 2);
    r->coeffs[i+7] = (a[j+7] >> 7) | ((uint32_t)a[j+8] << 1);
  }
}
...
void polyt0_unpack(poly *r, const unsigned char *a) {
  ...
  for(i = 0; i < N/8; ++i) {
    ...
    r->coeffs[8*i+0] = ((a[10*i+0]) | ((uint32_t)(a[10*i+1] & 0x3) << 8) );
    r->coeffs[8*i+1] = (((a[10*i+1] >>2) & 0x3F) | ((uint32_t)(a[10*i+2] & 0x0F) << 6) );

    r->coeffs[8*i+2] = (((a[10*i+2] >> 4) & 0xF) | ((uint32_t)(a[10*i+3] & 0x3F) <<4) );

    r->coeffs[8*i+3] = (((a[10*i+3]>>6) & 0x3) | ((uint32_t)(a[10*i+4] & 0xFF) <<2));
    r->coeffs[8*i+4] = ((a[10*i+5]) | ((uint32_t)(a[10*i+6] & 0x3) << 8));
    r->coeffs[8*i+5] = (((a[10*i+6] >>2) & 0x3F) | ((uint32_t)(a[10*i+7] & 0xF) << 6));
    r->coeffs[8*i+6] = (((a[10*i+7] >> 4) & 0xF) | ((uint32_t)(a[10*i+8] & 0x3F) <<4) );
    r->coeffs[8*i+7] = (((a[10*i+8]>>6) & 0x3) | ((uint32_t)(a[10*i+9] & 0xFF) <<2));
